...
/**
 * The annotation processor, which takes annotated Corfu objects and generates a class which can be
 * used by the runtime instead of requiring runtime instrumentation.
 *
 * <p>See README.md for details on how the annotation processor works and how to use it.
 *
 * <p>Created by mwei on 11/9/16.
 */
...
public class ObjectAnnotationProcessor extends AbstractProcessor {
  ...
  /**
   * Process each class, checking if the class is annotated and generating the appropriate proxy
   * class.
   *
    ...
    if (classElement.getAnnotation(CorfuObject.class) != null) {
      ...
      try {
        ...
        messager.printMessage(
            ...
            "Failed to process class " + classElement.getSimpleName() + " with IOException");
      }
    }
  ...
  public String getSmrFunctionName(ExecutableElement smrMethod) {
    ...
    return (mutator != null && !mutator.name().equals(""))
        ? mutator.name()
        : (mutatorAccessor != null && !mutatorAccessor.name().equals(""))
            ? mutatorAccessor.name()
            : smrMethod.getSimpleName().toString();
  }
  ...
  class SmrMethodInfo {
    ...
    public SmrMethodInfo(
        ExecutableElement method, TypeElement interfaceOverride, boolean doNotAdd) {
      this.method = method;
      ...
      } else if (mutatorAccessor != null
          && !mutatorAccessor.conflictParameterFunction().equals("")) {
    ...
    private boolean checkForConflictAnnotations(ExecutableElement method) {
      return method.getParameters().stream()
          .anyMatch(x -> x.getAnnotation(ConflictParameter.class) != null);
    }
  }
   ...
   */
  public void generateProxy(TypeElement classElement) throws IOException {
    ...
    ClassName proxyName =
        classElement.getNestingKind() == NestingKind.TOP_LEVEL
            ?
            ...
            ClassName.bestGuess(
                classElement.getEnclosingElement().getSimpleName()
                    + "$"
    ...
    TypeSpec.Builder typeSpecBuilder =
        TypeSpec.classBuilder(proxyName)
            .addTypeVariables(
                classElement.getTypeParameters().stream()
                    .map(TypeVariableName::get)
                    .collect(Collectors.toList()))
            .addSuperinterface(
                ParameterizedTypeName.get(ClassName.get(ICorfuSMR.class), originalName))
    ...
    typeSpecBuilder.addField(
        ParameterizedTypeName.get(ClassName.get(ICorfuSMRProxy.class), originalName),
        "proxy" + CORFUSMR_FIELD,
        Modifier.PUBLIC);
    typeSpecBuilder.addMethod(
        MethodSpec.methodBuilder("getCorfuSMRProxy")
            .addModifiers(Modifier.PUBLIC)
            .returns(ParameterizedTypeName.get(ClassName.get(ICorfuSMRProxy.class), originalName))
            .addStatement("return $L", "proxy" + CORFUSMR_FIELD)
            .build());
    typeSpecBuilder.addMethod(
        MethodSpec.methodBuilder("setCorfuSMRProxy")
            .addParameter(
                ParameterizedTypeName.get(ClassName.get(ICorfuSMRProxy.class), originalName),
                "proxy")
            .addModifiers(Modifier.PUBLIC)
    ...
    final Set<SmrMethodInfo> methodSet =
        classElement.getEnclosedElements().stream()
    ...
    do {
      ...
      superClassElement =
          ((TypeElement) ((DeclaredType) superClassElement.getSuperclass()).asElement());
      boolean samePackage =
          ClassName.get(superClassElement)
              .packageName()
              .equals(ClassName.get(classElement).packageName());
                        ...
                        y -> {
                          ...
                          if (y.method.getSimpleName().equals(x.getSimpleName())) {
                            if (y.method.getParameters().size() == x.getParameters().size()) {
                              ...
                              if (x.getParameters().stream()
                                  .anyMatch(p -> p.asType().getKind() == TypeKind.TYPEVAR)) {
                                return true;
                              } else {
              ...
              .forEach(
                  method -> {
                    ...
                    method.getModifiers().contains(Modifier.DEFAULT)
                        && methodSet.stream()
                            .noneMatch(x -> x.method.toString().equals(method.toString()))) {
                      methodSet.add(new SmrMethodInfo(method, ifaceElement, true));
                    }
                  });
        });
    ...
    methodSet.removeIf(
        x -> x.method.getSimpleName().toString().endsWith(ICorfuSMR.CORFUSMR_SUFFIX));
    ...
    methodSet.stream()
        ...
        .forEach(
            m -> {
              ...
              final MutatorAccessor mutatorAccessor =
                  smrMethod.getAnnotation(MutatorAccessor.class);
              final Mutator mutator = smrMethod.getAnnotation(Mutator.class);
              final TransactionalMethod transactional =
                  smrMethod.getAnnotation(TransactionalMethod.class);
              ...
              try {
                ...
                messager.printMessage(
                    Diagnostic.Kind.ERROR, "error trying to change methodspec" + e.getMessage());
              }
              ...
              if (m.conflictFunction != null) {
                ...
                addConflictFieldFromFunctionToMethod(
                    ms, conflictField, m.conflictFunction, smrMethod);
              ...
              if (mutatorAccessor != null) {
                ...
                ms.addStatement(
                    "$Lsuper.$L($L)",
                    smrMethod.getReturnType().getKind().equals(TypeKind.VOID) ? "" : "return ",
                ...
                ms.addStatement(
                    ...
                    smrMethod.getParameters().stream()
                        .map(VariableElement::getSimpleName)
                        .collect(Collectors.joining(", ")),
                    smrMethod.getReturnType().getKind().equals(TypeKind.VOID) ? "return null;" : "",
                    (m.hasConflictAnnotations ? conflictField : "null"));
              }
    ...
    typeSpecBuilder.addAnnotation(AnnotationSpec.builder(InstrumentedCorfuObject.class).build());
  ...
  private void addResetSet(
    ...
    FieldSpec resetSet =
        FieldSpec.builder(
                ...
                "resetSet" + CORFUSMR_FIELD,
                Modifier.FINAL,
                Modifier.PUBLIC)
            .initializer(
                "new $T()$L.build()",
                ParameterizedTypeName.get(
                    ClassName.get(ImmutableSet.Builder.class), ClassName.get(String.class)),
                resetString)
            .build();

    typeSpecBuilder.addField(resetSet);
    typeSpecBuilder.addMethod(
        MethodSpec.methodBuilder("getCorfuResetSet")
            .addModifiers(Modifier.PUBLIC)
            .returns(
                ParameterizedTypeName.get(ClassName.get(Set.class), ClassName.get(String.class)))
  ...
  private void addUpcallMap(
    ...
    FieldSpec upcallMap =
        FieldSpec.builder(
                ParameterizedTypeName.get(
                    ...
                    ParameterizedTypeName.get(
                        ClassName.get(ICorfuSMRUpcallTarget.class), originalName)),
                "upcallMap" + CORFUSMR_FIELD,
                Modifier.PUBLIC,
                Modifier.FINAL)
            .initializer(
                "new $T()$L.build()",
                ParameterizedTypeName.get(
                    ClassName.get(ImmutableMap.Builder.class),
                    ClassName.get(String.class),
                    ParameterizedTypeName.get(
                        ClassName.get(ICorfuSMRUpcallTarget.class), originalName)),
                upcallString)
    ...
    typeSpecBuilder.addField(upcallMap);

    typeSpecBuilder.addMethod(
        MethodSpec.methodBuilder("getCorfuSMRUpcallMap")
            .addModifiers(Modifier.PUBLIC)
            .returns(
                ParameterizedTypeName.get(
                    ClassName.get(Map.class),
  ...
  private void addUndoRecordMap(
        ...
        methodSet.stream()
            ...
            .map(
                x -> {
                  ...
                  String undoRecordFunction =
                      mutator == null
                          ? mutatorAccessor.undoRecordFunction()
                          : mutator.undoRecordFunction();

                  Optional<SmrMethodInfo> mi =
                      methodSet.stream()
                          .filter(
                              y -> y.method.getSimpleName().toString().equals(undoRecordFunction))
                          .findFirst();
                  ...
                  if (mi.get().method.getParameters().size()
                    ...
                    messager.printMessage(
                        ...
                        "undoRecord method "
                            + undoRecordFunction
                            + " contained the wrong number of parameters");
                  }

                  // Add the interface, if present.
                  if (mi.get().interfaceOverride != null) {
                    interfacesToAdd.add(
                        ParameterizedTypeName.get(mi.get().interfaceOverride.asType()));
                  }
                  String callingConvention =
                      mi.get().interfaceOverride == null
                          ? "this."
    ...
    FieldSpec undoRecordMap =
        FieldSpec.builder(
                ParameterizedTypeName.get(
                    ClassName.get(Map.class),
                    ClassName.get(String.class),
                    ParameterizedTypeName.get(
                        ClassName.get(IUndoRecordFunction.class), originalName)),
                "undoRecordMap" + CORFUSMR_FIELD,
                Modifier.PUBLIC,
                Modifier.FINAL)
            .initializer(
                "new $T()$L.build()",
    ...
    typeSpecBuilder.addMethod(
        MethodSpec.methodBuilder("getCorfuUndoRecordMap")
            .addModifiers(Modifier.PUBLIC)
            .returns(
                ParameterizedTypeName.get(
                    ClassName.get(Map.class),
  ...
  private void addUndoMap(
        ...
        methodSet.stream()
            ...
            .map(
                x -> {
                  ...
                  Mutator mutator = x.method.getAnnotation(Mutator.class);
                  String undoFunction =
                      mutator == null ? mutatorAccessor.undoFunction() : mutator.undoFunction();

                  Optional<SmrMethodInfo> mi =
                      methodSet.stream()
                          .filter(y -> y.method.getSimpleName().toString().equals(undoFunction))
                          .findFirst();
    ...
    FieldSpec undoMap =
        FieldSpec.builder(
                ParameterizedTypeName.get(
                    ClassName.get(Map.class),
                    ClassName.get(String.class),
                    ParameterizedTypeName.get(ClassName.get(IUndoFunction.class), originalName)),
                "undoMap" + CORFUSMR_FIELD,
                Modifier.PUBLIC,
                Modifier.FINAL)
            .initializer(
                "new $T()$L.build()",
    ...
    typeSpecBuilder.addMethod(
        MethodSpec.methodBuilder("getCorfuUndoMap")
            .addModifiers(Modifier.PUBLIC)
            .returns(
                ParameterizedTypeName.get(
                    ClassName.get(Map.class),
   ...
   */
  public void addConflictFieldToMethod(
      MethodSpec.Builder ms, String conflictField, ExecutableElement smrMethod) {
    ms.addStatement(
        "$T $L = new Object[]{$L}",
        Object[].class,
        conflictField,
        smrMethod.getParameters().stream()
            .filter(y -> y.getAnnotation(ConflictParameter.class) != null)
   ...
   * @param smrMethod The method element.
   */
  public void addConflictFieldFromFunctionToMethod(
      MethodSpec.Builder ms,
