...
# 	rm -f $seqcfg
# fi
# $CORFUDBBINDIR/cgenconfig-mem.sh --role $role --configfile $seqcfg --baseport $baseport --cnt $cnt
...
do
	...
	$CORFUDBBINDIR/cgenconfig-mem.sh --role $role --configfile $lgcfg --baseport $baseport --cnt $cnt --port $port
 	$CORFUDBBINDIR/crun.sh --configdir $TMPCFGDIR --unit $unit --cmd $cmd 
	#  rm -f $lgcfg
done
...
$CORFUDBBINDIR/cgenconfig-mem.sh --role $role --configfile $mastercfg --baseport $baseport --cnt $cnt
$CORFUDBBINDIR/crun.sh --configdir $TMPCFGDIR --unit $unit --cmd $cmd
