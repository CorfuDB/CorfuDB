...
public class Result<T, E extends RuntimeException> implements Supplier<T> {
  ...
  /**
   * Performs the given {@link Consumer} action with the value if the {@link Result} instance is a
   * valid result value.
   *
   ...
   *     function to the original internal value.
   */
  public <Q extends RuntimeException> Result<T, Q> mapError(
      Function<? super E, ? extends Q> function) {
    if (isError()) {
