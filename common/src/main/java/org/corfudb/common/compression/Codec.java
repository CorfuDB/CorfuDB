 ...
 *
 * <p>Created by Maithem on 11/6/19.
 */
public interface Codec {
  ...
  enum Type {
    ...
    /** The unique id of the code that will be used during serialization/deserialization */
    final int id;

    private Supplier<Codec> func;

    public static Map<Integer, Type> typeMap =
        Arrays.stream(Codec.Type.values())
  ...
  /**
   * Returns the compressed ByteBuffer as a ByteBuffer with the size of the uncompressed buffer as
   * the first 4-bytes only if input buffer has bytes. | 4 bytes | compressed buffer | uncompressed
   * length compressed bytes
   */
  ByteBuffer compress(ByteBuffer uncompressed);

  /** Strips the first 4-bytes and decompresses the remaining of the buffer into a ByteBuffer */
