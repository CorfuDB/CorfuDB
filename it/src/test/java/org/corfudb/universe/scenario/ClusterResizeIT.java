...
public class ClusterResizeIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Sequentially remove two nodes from cluster
   * 3) Verify layout and data path 4) Sequentially add two nodes back into cluster 5) Verify layout
   * and data path again
   */
  @Test(timeout = 300000)
  public void clusterResizeTest() {
    workflow(
        wf -> {
          ...
          List<CorfuServer> servers =
              Arrays.asList(corfuCluster.getServerByIndex(1), corfuCluster.getServerByIndex(2));
          ...
          // should remove two nodes from corfu cluster
          {
            ...
            for (CorfuServer candidate : servers) {
              corfuClient
                  ...
                  .getLayoutView()
                  .getRuntimeLayout()
                  .getBaseClient(candidate.getEndpoint())
                  .reset();
            }
