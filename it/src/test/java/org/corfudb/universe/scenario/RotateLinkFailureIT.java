...
public class RotateLinkFailureIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Create a link failure between node0 and
   * node1 3) Create a link failure between node1 and node2 and heal previous link failure 4) Create
   * a link failure between node2 and node0 and heal previous link failure 5) Reverse rotation
   * direction, create a link failure between node1 and node2 and heal previous link failure 6)
   * Verify layout and data path after each rotation 7) Recover cluster by removing all link
   * failures 8) Verify layout, cluster status and data path
   */
  @Test(timeout = 600000)
  public void rotateLinkFailureTest() {
    workflow(
        wf -> {
          ...
          log.info(
              "Wait for some time to ensure cluster stabilizes Server1 should stay "
          ...
          log.info(
              "Server0 and server2 has same number of link failure ie. 1, "
          ...
          log.info(
              "4th link failure rotation, reverse the rotating direction, "
