...
public class NodeUpAndPartitionedIT extends GenericIntegrationTest {
  ...
  /**
   * Test cluster behavior after an unresponsive node becomes available (up) and at the same time a
   * previously responsive node starts to have two link failures. One of which to a responsive node
   * and the other to an unresponsive. This tests asserts that regardless of equal number of
   * observed link failures for each of nodes in the responsive set towards the other responsive
   * nodes, the node which also has the most number of link failures to the unresponsive set
   * (potentially healed) will be taken out. In other word, it makes sure that we don't remove a
   * responsive node in a way that eliminates the possibility of future healing of unresponsive
   * nodes.
   *
   * <p>Steps taken in the test: 1) Deploy and bootstrap a three nodes cluster 2) Stop one node 3)
   * Create two link failures between a responsive node with smaller endpoint name and the rest of
   * the cluster AND restart the unresponsive node.
   *
   * <p>4) Verify that responsive node mentioned in step 3 becomes unresponsive 5) Verify that the
   * restarted unresponsive node in step 3 gets healed 6) Verify cluster status and data path
   */
  @Test(timeout = 300000)
    ...
    workflow(
        wf -> {
          ...
          CorfuCluster corfuCluster =
              wf.getUniverse().getGroup(wf.getFixture().data().getGroupParamByIndex(0).getName());

          CorfuClient corfuClient = corfuCluster.getLocalCorfuClient();

          CorfuTable<String, String> table =
              corfuClient.createDefaultCorfuTable(DEFAULT_STREAM_NAME);
          ...
          ClusterStatusReport clusterStatusReport =
              corfuClient.getManagementView().getClusterStatus();
