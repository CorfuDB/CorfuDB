...
public class NodeDownAndLinkFailureIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Stop one node 3) Create a link failure
   * between two nodes which results in a partial partition 4) Restart the stopped node 5) Verify
   * layout, cluster status and data path 6) Remove the link failure 7) Verify layout, cluster
   * status and data path again
   */
  @Test(timeout = 300000)
    ...
    workflow(
        wf -> {
          ...
          assertThat(corfuClient.getLayout().getUnresponsiveServers())
              .containsExactly(server2.getEndpoint());
          currEpoch++;
          ...
          // client
          // can still connect to two nodes, write to table so system down handler will not be
          // triggered.
          ...
          log.info(
              "Restart the stopped node, server0 and server1 still partitioned wait for "
          ...
          waitForLayoutChange(
              layout ->
                  layout
                      .getUnresponsiveServers()
                      .equals(Collections.singletonList(server1.getEndpoint())),
              corfuClient);
