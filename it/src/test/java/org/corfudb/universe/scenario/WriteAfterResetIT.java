...
public class WriteAfterResetIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy a single node cluster. 2) Connect a client's runtime to the node. 3) Write 100
   * addresses to the log unit. 4) Perform a reset. 5) Try writing record to the log unit, the write
   * should not go through the router, as the server is not bootstrapped. 6) Bootstrap a node with a
   * new layout. 7) Write a record to the log unit. 8) The write should go through, as the server is
   * now bootstrapped.
   */
                ...
                if (writeWentThrough) {
                  ...
                  ReadResponse response =
                      corfuClient2
                          ...
                          .read(address)
                          .join();
                  assertThat(new String(response.getAddresses().get(address).getData()))
                      .contains("Payload");
                } else {
