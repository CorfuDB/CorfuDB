...
public class NodeDownAndPartitionedIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Stop one node 3) Symmetrically partition
   * one node 4) Verify layout, cluster status and data path 5) Recover cluster by restart the
   * stopped node and fix partition 5) Verify layout, cluster status and data path
   */
  @Test(timeout = 300000)
  public void nodeDownAndPartitionTest() {
    workflow(
        wf -> {
          ...
          waitForLayoutChange(
              layout -> layout.getSegments().size() == 1 && layout.getAllServers().size() == 3,
              corfuClient);
