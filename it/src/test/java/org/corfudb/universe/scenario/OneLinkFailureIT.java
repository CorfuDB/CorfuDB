...
public class OneLinkFailureIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Create a link failure between two nodes
   * which results in a partial partition 3) Verify layout, cluster status and data path 4) Recover
   * cluster by removing the link failures 5) Verify layout, cluster status and data path again
   */
  @Test(timeout = 300000)
  public void oneLinkFailureTest() {
    workflow(
        wf -> {
          ...
          String serverToKick =
              Collections.max(Arrays.asList(server0.getEndpoint(), server2.getEndpoint()));
