...
public class TwoNodesDownIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Sequentially stop two nodes 3) Verify
   * layout, cluster status and data path 4) Recover cluster by sequentially restarting stopped
   * nodes 5) Verify layout, cluster status and data path again
   */
  @Test(timeout = 300000)
    ...
    workflow(
        wf -> {
          ...
          Map<String, ConnectivityStatus> connectivityStatusMap =
              clusterStatusReport.getClientServerConnectivityStatusMap();
          ...
          waitForLayoutChange(
              layout ->
                  layout.getEpoch() > initialLayout.getEpoch()
                      && layout.getUnresponsiveServers().size() == 0,
              corfuClient);
