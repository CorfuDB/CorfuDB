...
public class TwoLinksFailureIT extends GenericIntegrationTest {
   ...
   *
   * <p>1) Deploy and bootstrap a three nodes cluster 2) Create two link failure between one node
   * and the other two, which results in a partial partition 3) Verify layout, cluster status and
   * data path 4) Recover cluster by removing all link failures 5) Verify layout, cluster status and
   * data path again
   */
  @Test(timeout = 300000)
          ...
          waitForLayoutChange(
              layout ->
                  layout
                      ...
                      .equals(Collections.singletonList(server0.getEndpoint())),
              corfuClient);
