...
public class UniverseManager {
  ...
  @NonNull private final String corfuServerVersion;

  @NonNull private final String testName;

  @Default @Getter private final UniverseMode universeMode = UniverseMode.DOCKER;
  ...
  /**
   * Manages a testing workflow lifecycle. Provides api to: - customize the universe parameters -
   * initialize and deploy the clusters in the universe - get the universe fixture to setup the
   * initial parameters - shutdown the universe
   *
   * @param <T>
   */
  @Slf4j
  ...
  public static class UniverseWorkflow<T extends Fixture<UniverseParams>> {
    @NonNull private final T fixture;
    @NonNull private final UniverseMode universeMode;
    @NonNull private String corfuServerVersion;

    @NonNull private final String testName;

    @Getter private Universe universe;
    ...
    private UniverseWorkflow<T> initVmUniverse() {
      ...
      ApplianceManager manager = ApplianceManager.builder().universeParams(universeParams).build();

      VmUniverseFixture vmFixture = ClassUtils.cast(fixture);
      LoggingParams loggingParams = vmFixture.getLogging().testName(testName).build();
    ...
    private UniverseWorkflow<T> initDockerUniverse() {
      ...
      LoggingParams loggingParams = dockerFixture.getLogging().testName(testName).build();

      universe =
          DockerUniverse.builder()
    ...
    private UniverseWorkflow<T> initProcessUniverse() {
      ...
      LoggingParams loggingParams = processFixture.getLogging().testName(testName).build();

      // Assign universe variable before deploy prevents resources leaks
      universe =
          ProcessUniverse.builder()
