...
public class DockerCorfuServer extends AbstractCorfuServer<CorfuServerParams, UniverseParams> {
  ...
  @NonNull private final CorfuClusterParams<CorfuServerParams> clusterParams;
  ...
  public DockerCorfuServer(
      DockerClient docker,
      CorfuServerParams params,
      UniverseParams universeParams,
      CorfuClusterParams<CorfuServerParams> clusterParams,
  ...
  /**
   * This method attempts to gracefully stop the Corfu server. After timeout, it will kill the Corfu
   * server.
   *
  ...
  /**
   * Symmetrically disconnect the server from the cluster. This partitions the container from all
   * the others. The test runtime can still connect to this server.
   *
  ...
  public void disconnect() {
    ...
    clusterParams.getNodesParams().stream()
        ...
        .forEach(
            neighbourServer -> {
              try {
                ...
                String neighbourIp =
                    server.networkSettings().networks().values().asList().get(0).ipAddress();

                if (StringUtils.isEmpty(neighbourIp)) {
                  throw new NodeException(
                      "Empty ip address. Container: " + neighbourServer.getName());
                }

                // iptables -A INPUT -s $neighbourIp -j DROP
                dockerManager.execCommand(
                    params.getName(), "iptables", "-A", "INPUT", "-s", neighbourIp, "-j", "DROP");
                // iptables -A OUTPUT -d $neighbourIp -j DROP
                dockerManager.execCommand(
                    params.getName(), "iptables", "-A", "OUTPUT", "-d", neighbourIp, "-j", "DROP");
              } catch (DockerException | InterruptedException ex) {
                List<String> clusterNodes = clusterParams.getClusterNodes();
                throw new NodeException(
                    "Can't disconnect container: "
                        + params.getName()
                        + " from docker network. Corfu cluster: "
                        + clusterNodes,
                    ex);
              }
            });
  ...
  /**
   * Symmetrically disconnect a server from a list of other servers, which creates a partial
   * partition.
   *
  ...
  public void disconnect(List<CorfuServer> servers) {
    log.info(
        "Disconnecting the docker server: {} from specified servers: {}",
        params.getName(),
        servers);
        ...
        .forEach(
            neighbourServer -> {
              try {
                ...
                dockerManager.execCommand(params.getName(), IpTablesUtil.dropInput(neighbourIp));
                dockerManager.execCommand(params.getName(), IpTablesUtil.dropOutput(neighbourIp));
              } catch (DockerException | InterruptedException ex) {
                throw new NodeException(
                    "Can't disconnect container: "
                        + params.getName()
                        + " from server: "
                        + neighbourServer.getParams().getName(),
                    ex);
              }
            });
  }
  ...
  public void reconnect(List<CorfuServer> servers) {
    log.info(
        "Reconnecting the docker server: {} to specified servers: {}", params.getName(), servers);
    ...
    servers.stream()
        ...
        .forEach(
            neighbourServer -> {
              try {
                dockerManager.execCommand(
                    params.getName(), IpTablesUtil.revertDropInput(neighbourServer.getIpAddress()));
                dockerManager.execCommand(
                    params.getName(),
                    IpTablesUtil.revertDropOutput(neighbourServer.getIpAddress()));
              } catch (DockerException | InterruptedException ex) {
                throw new NodeException(
                    "Can't reconnect container: "
                        + params.getName()
                        + " to server: "
                        + neighbourServer.getParams().getName(),
                    ex);
  ...
  private String deployContainer() {
    ...
    String id;
    try {
      ListImagesParam corfuImageQuery = ListImagesParam.byName(params.getDockerImageNameFullName());
  ...
  private ContainerConfig buildContainerConfig() {
    // Bind ports
    List<String> ports =
        params.getPorts().stream().map(Objects::toString).collect(Collectors.toList());
    ...
    HostConfig hostConfig = hostConfigBuilder.privileged(true).portBindings(portBindings).build();
