...
public class DockerSupportServer<N extends SupportServerParams> implements SupportServer {
  ...
  private static final Map<NodeType, String> IMAGE_NAME =
      ImmutableMap.<NodeType, String>builder()
          .put(NodeType.METRICS_SERVER, "prom/prometheus")
          .put(NodeType.SHELL_NODE, "ubuntu")
          .build();

  private static final Map<NodeType, String> CMD =
      ImmutableMap.<NodeType, String>builder().put(NodeType.SHELL_NODE, "bash").build();

  @Default @Getter @NonNull
  private final String prometheusConfigPath = "/etc/prometheus/prometheus.yml";

  @Getter @NonNull protected final N params;

  @NonNull @Getter protected final UniverseParams universeParams;

  @NonNull private final DockerClient docker;

  @NonNull private final DockerManager dockerManager;

  @NonNull private final SupportClusterParams clusterParams;

  @NonNull @Default private final List<File> openedFiles = new ArrayList<>();

  @NonNull private final AtomicReference<String> ipAddress = new AtomicReference<>();
  ...
  private ContainerConfig buildContainerConfig() {
    ...
    HostConfig.Bind configurationFile =
  ...
  private String createConfiguration(Set<Integer> metricsPorts) {
    try {
      ...
      if (System.getProperty("os.name").compareToIgnoreCase(LINUX_OS) == 0) {
        corfuRuntimeIp =
            docker.inspectNetwork(universeParams.getNetworkName()).ipam().config().stream()
                .findFirst()
