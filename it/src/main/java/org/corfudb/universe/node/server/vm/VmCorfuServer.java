...
public class VmCorfuServer extends AbstractCorfuServer<VmCorfuServerParams, VmUniverseParams> {
  ...
  @NonNull @Getter private final VmManager vmManager;

  @NonNull private final IpAddress ipAddress;

  @Getter @NonNull private final RemoteOperationHelper remoteOperationHelper;

  @NonNull private final VmStress stress;

  @NonNull private final CorfuProcessManager processManager;

  @NonNull private final CorfuServerPath serverPath;

  @Builder
  public VmCorfuServer(
      VmCorfuServerParams params,
      VmManager vmManager,
      VmUniverseParams universeParams,
      VmStress stress,
      RemoteOperationHelper remoteOperationHelper,
  ...
  /**
   * Deploys a Corfu server on the VM as specified, including the following steps: a) Copy the corfu
   * jar file under the working directory to the VM b) Run that jar file using java on the VM
   */
  @Override
  public CorfuServer deploy() {
    ...
    remoteOperationHelper.copyFile(params.getInfrastructureJar(), serverPath.getServerJar());
  ...
  public void disconnect(List<CorfuServer> servers) {
    log.info(
        "Disconnecting the VM server: {} from the specified servers: {}",
        params.getName(),
        servers);
  ...
  public void start() {
    ...
    String cmd = String.format("sh -c '%s'", processManager.startCommand(getCommandLineParams()));
    executeCommand(cmd);
  }
  ...
  public void reconnect(List<CorfuServer> servers) {
    log.info("Reconnecting the VM server: {} to specified servers: {}", params.getName(), servers);
  ...
  /**
   * @param timeout a limit within which the method attempts to gracefully stop the {@link
   *     CorfuServer}.
   */
  @Override
  public void stop(Duration timeout) {
    ...
    try {
      ...
      String err =
          String.format(
              "Can't STOP corfu: %s. Process not found on vm: %s, ip: %s",
  ...
  public void kill() {
    ...
    try {
      ...
      String err =
          String.format(
              "Can't KILL corfu: %s. Process not found on vm: %s, ip: %s",
              params.getName(), params.getVmName(), ipAddress);
      throw new NodeException(err, e);
    }
  }
  ...
  /**
   * Remove corfu server application dir. AppDir is a directory that contains corfu-infrastructure
   * jar file and could have log files, stream-log files and so on, whatever used by the
   * application.
   */
  ...
  public void collectLogs() {
    ...
    try {
      remoteOperationHelper.downloadFile(
          corfuLogDir.resolve(params.getName() + ".log"), serverPath.getCorfuLogFile());
    } catch (Exception e) {
