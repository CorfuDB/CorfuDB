...
public class ProcessCorfuServer extends AbstractCorfuServer<CorfuServerParams, UniverseParams> {
  ...
  @NonNull @Default @Getter private final IpAddress ipAddress = LOCALHOST;
  ...
  public ProcessCorfuServer(
      @NonNull CorfuServerParams params,
      @NonNull UniverseParams universeParams,
  ...
  /**
   * Deploys a Corfu server on the target directory as specified, including the following steps: a)
   * Copy the corfu jar file under the working directory to the target directory b) Run that jar
   * file using java on the local machine
   */
  @Override
  public CorfuServer deploy() {
    ...
    commandHelper.copyFile(params.getInfrastructureJar(), serverPath.getServerJar());
    start();
    return this;
  }
  ...
  public void start() {
    executeCommand(
        Optional.of(serverPath.getCorfuDir()), processManager.startCommand(getCommandLineParams()));
  }
  ...
  public void kill() {
    ...
    try {
      ...
      String err =
          String.format(
              "Can't KILL corfu: %s. Process not found, ip: %s", params.getName(), ipAddress);
      throw new NodeException(err, e);
    }
  }
  ...
  public void collectLogs() {
    ...
    try {
      commandHelper.copyFile(
          serverPath.getCorfuLogFile(), corfuLogDir.resolve(params.getName() + ".log"));
    } catch (Exception e) {
