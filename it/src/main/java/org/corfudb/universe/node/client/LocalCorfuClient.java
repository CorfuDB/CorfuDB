...
/**
 * Provides Corfu client (utility class) used in the local machine (in current process) which is
 * basically a wrapper of CorfuRuntime.
 */
@Slf4j
public class LocalCorfuClient implements CorfuClient {
  private final CorfuRuntime runtime;

  @Getter private final ClientParams params;

  @Getter private final ImmutableSortedSet<String> serverEndpoints;
  ...
  public LocalCorfuClient(
    ...
    corfuRuntimeParams.layoutServers(layoutServers).systemDownHandler(this::systemDownHandler);
   ...
   *
   * @param timeout a limit within which the method attempts to gracefully stop the client (not used
   *     for a client).
   */
  @Override
