...
public class VmCorfuCluster extends AbstractCorfuCluster<VmCorfuServerParams, VmUniverseParams> {
  ...
  @Builder
  protected VmCorfuCluster(
      CorfuClusterParams<VmCorfuServerParams> corfuClusterParams,
  ...
  public void bootstrap() {
    ...
    log.info(
        "Bootstrap corfu cluster. Cluster: {}. layout: {}",
        params.getName(),
        layout.asJSONString());
  ...
  private Layout buildLayout() {
    ...
    List<String> servers =
        params.getNodesParams().stream()
            .map(this::getVmServerParams)
            .map(
                vmParams ->
                    vms.get(vmParams.getVmName()).getResolvedIpAddress() + ":" + vmParams.getPort())
