...
public class DockerCorfuCluster extends AbstractCorfuCluster<CorfuServerParams, UniverseParams> {
  ...
  @Builder
  public DockerCorfuCluster(
      DockerClient docker,
      CorfuClusterParams<CorfuServerParams> params,
  ...
  protected ImmutableSortedSet<String> getClusterLayoutServers() {
    List<String> servers =
        nodes().values().stream().map(CorfuServer::getEndpoint).collect(Collectors.toList());
  ...
  public void bootstrap() {
    ...
    log.info(
        "Bootstrap docker corfu cluster. Cluster: {}. layout: {}",
        params.getName(),
        layout.asJSONString());
