...
public abstract class AbstractSupportCluster
    extends AbstractCluster<
        SupportServer, SupportServerParams, SupportClusterParams, UniverseParams> {

  @Default
  private final ConcurrentNavigableMap<String, SupportServer> nodes = new ConcurrentSkipListMap<>();
  ...
  /**
   * Deploys a {@link Group}, including the following steps: a) Deploy the Corfu nodes b) Bootstrap
   * all the nodes to form a cluster
   *
  ...
  public AbstractSupportCluster deploy() {
    ...
    List<CompletableFuture<Node>> asyncMonDeployment =
    ...
    asyncMonDeployment.stream()
        ...
        .forEach(
            server -> log.debug("Corfu server was deployed: {}", server.getParams().getName()));
