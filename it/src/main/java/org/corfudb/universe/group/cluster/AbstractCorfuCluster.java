...
public abstract class AbstractCorfuCluster<P extends CorfuServerParams, U extends UniverseParams>
  ...
  @NonNull protected final LoggingParams loggingParams;

  public AbstractCorfuCluster(
      CorfuClusterParams<P> params, U universeParams, LoggingParams loggingParams) {
  ...
  public AbstractCorfuCluster<P, U> deploy() {
    ...
    List<CompletableFuture<Node>> asyncDeployment =
        nodes.values().stream().map(this::deployAsync).collect(Collectors.toList());
