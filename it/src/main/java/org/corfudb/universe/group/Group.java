...
/**
 * This provides an abstraction for a group of {@link Node}s that come together to provide a logical
 * service.
 *
 * <p>The following are the main functionalities provided by this class:
 *
 * <p>DEPLOY: deploys a {@link Group} representing a collection of {@link Node}-s using the provided
 * configuration in {@link GroupParams} STOP: stops a {@link Group} gracefully within the provided
 * timeout KILL: kills a {@link Group} immediately
 */
public interface Group<T extends Node, G extends Group.GroupParams> {
  ...
  /**
   * Stop the {@link Group} by stopping all individual {@link Node}-s of the group. Must happened
   * within the limit of provided timeout.
   *
  ...
  /**
   * Kill the {@link Group} immediately by killing all the {@link Node}-s of the group. Kill - means
   * stop or interrupt immediately
   */
  void kill();

  /**
   * Destroy the {@link Group} immediately by destroying all the {@link Node}-s of the group.
   * Destroy - means kill and clean up the node directory which contains the application itself and
   * could contain config files, db files and so on.
   */
  void destroy();
