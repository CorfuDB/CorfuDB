...
public interface CorfuCluster<T extends Node, G extends GroupParams> extends Cluster<T, G> {
  ...
  /**
   * Find a corfu server by index in the cluster: - get all corfu servers in the cluster - skip
   * particular number of servers in the cluster according to index offset - extract first server
   * from the list
   *
  ...
  /**
   * To find the first corfu server in a cluster: - get all corfu servers in the cluster - extract
