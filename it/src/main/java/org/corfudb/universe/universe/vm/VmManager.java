...
public class VmManager {
  ...
  @NonNull @Getter private final VmName vmName;

  /** Vsphere inventory navigator */
  @NonNull private final InventoryNavigator navigator;

  @NonNull private final VmUniverseParams universeParams;
                ...
                executeTask(
                    () -> {
                      ...
                      HostSystem hostSystem =
                          (HostSystem) navigator.searchManagedEntity(HOST_SYSTEM.resource, "host");
    ...
    return Result.of(
        () -> {
          try {
            VirtualMachine vm =
                (VirtualMachine)
                    navigator.searchManagedEntity(ManagedEntityType.VIRTUAL_MACHINE.typeName, name);
  ...
  public Result<TaskInfo, UniverseException> cloneVm() {
    ...
    return getVm(universeParams.getTemplateVMName())
        .flatMap(
            vmTemplate -> {
              ...
              try {
                ...
                String folderProp =
                    String.format("vm%d.%s", vmName.getIndex(), ResourceType.FOLDER.resource);
                ...
                return executeTask(
                    () -> vmTemplate.cloneVM_Task(folder, vmName.getName(), cloneSpec));
