...
/**
 * Fake DNS resolver which allows our tests to work well even though we use strange loopback IP
 * addresses (127.x.y.z) with no corresponding reverse DNS.
 *
 * <p>This overrides the reverse lookups for such IPs to return the same address in String form.
 *
 * <p>Without this class, reverse DNS lookups for such addresses often take 5 seconds to return,
 * causing timeouts and overall test slowness.
 *
 * <p>In the future this class might also be extended to test more interesting DNS-related
 * scenarios.
 */
public class FakeDns {
  ...
  /**
   * The NameService in all versions of Java has the same interface, so we can use the same
   * InvocationHandler as our proxy instance for both java.net.InetAddress$NameService and
  ...
  private class NameServiceListener implements InvocationHandler {
    ...
    private InetAddress[] lookupAllHostAddr(String host) throws UnknownHostException {
      ...
      try {
        Method method =
            fallbackNameService.getClass().getDeclaredMethod("lookupAllHostAddr", String.class);
        method.setAccessible(true);
    ...
    private String getHostByAddr(byte[] addr) throws UnknownHostException {
      ...
      try {
        Method method =
            fallbackNameService.getClass().getDeclaredMethod("getHostByAddr", byte[].class);
