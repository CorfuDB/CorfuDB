...
public abstract class AbstractUniverse<N extends NodeParams, P extends UniverseParams>
    implements Universe {
  @Getter @NonNull protected final P universeParams;
  @Getter @NonNull protected final UUID universeId;
    ...
    groupNames.forEach(
        groupName -> {
          try {
            ...
            if (group == null) {
              log.warn(
                  "Can't shutdown a group! The group doesn't exists in the universe: {}",
                  groupName);
              return;
            }
