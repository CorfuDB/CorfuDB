...
public class DockerUniverse extends AbstractUniverse<NodeParams, UniverseParams> {
  /**
   * Docker parameter --network=host doesn't work in mac machines, FakeDns is used to solve the
   * issue, it resolves a dns record (which is a node name) to loopback address always. See
   * Readme.md
   */
  ...
  @Builder
  public DockerUniverse(
      UniverseParams universeParams, DockerClient docker, LoggingParams loggingParams) {
  ...
  /**
   * Deploy a {@link Universe} according to provided parameter, docker client, docker network, and
   * other components. The instances of this class are immutable. In other word, when the state of
   * an instance is changed a new immutable instance is provided.
   *
    ...
    switch (groupParams.getType()) {
      case CORFU_CLUSTER:
        groupParams
            ...
            .forEach(
                node ->
                    FAKE_DNS.addForwardResolution(
                        node.getName(), InetAddress.getLoopbackAddress()));
  ...
  private class DockerNetwork {
    ...
    void setup() {
      ...
      NetworkConfig networkConfig =
          NetworkConfig.builder().checkDuplicate(true).attachable(true).name(networkName).build();
