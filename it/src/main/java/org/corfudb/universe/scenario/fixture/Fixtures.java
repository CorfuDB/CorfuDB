...
public interface Fixtures {
  ...
  class UniverseFixture implements Fixture<UniverseParams> {
    ...
    private final CorfuClusterParamsBuilder<CorfuServerParams> cluster =
        CorfuClusterParams.builder();
    ...
    private final SupportClusterParamsBuilder monitoringCluster = SupportClusterParams.builder();
    ...
    private final LoggingParamsBuilder logging = LoggingParams.builder().enabled(false);
    ...
    public UniverseParams data() {
      ...
      SupportServerParams monitoringServerParams =
          supportServer
      ...
      List<CorfuServerParams> serversParams = fixtureUtil.buildServers(clusterParams, server);
  ...
  class VmUniverseFixture implements Fixture<VmUniverseParams> {
    ...
    private final CorfuClusterParamsBuilder<VmCorfuServerParams> cluster =
        CorfuClusterParams.builder();
    ...
    @Setter private String vmPrefix = DEFAULT_VM_PREFIX;

    private Optional<VmUniverseParams> data = Optional.empty();

    private final FixtureUtilBuilder fixtureUtilBuilder = FixtureUtil.builder();

    private final Properties vmProperties = VmConfigPropertiesLoader.loadVmProperties().get();

    private final LoggingParamsBuilder logging = LoggingParams.builder().enabled(false);

    public VmUniverseFixture() {
      Properties credentials =
          VmConfigPropertiesLoader.loadVmCredentialsProperties().orElse(new Properties());
      ...
      VmCredentialsParams credentialParams =
          VmCredentialsParams.builder()
              .vSphereCredentials(vSphereCred)
              .vmCredentials(vmCred)
              .build();

      List<String> vSphereHost =
          Arrays.stream(vmProperties.getProperty("vsphere.host").split(","))
              .collect(Collectors.toList());

      universe =
          VmUniverseParams.builder()
              .vSphereUrl(vmProperties.getProperty("vsphere.url"))
              .vSphereHost(vSphereHost)
              .networkName(vmProperties.getProperty("vm.network"))
              .templateVMName(
                  vmProperties.getProperty("vm.template", "debian-buster-thin-provisioned"))
    ...
    public VmUniverseParams data() {
      ...
      FixtureUtil fixtureUtil = fixtureUtilBuilder.build();
      ImmutableList<VmCorfuServerParams> serversParams =
          fixtureUtil.buildVmServers(clusterParams, servers, vmPrefix);
      serversParams.forEach(clusterParams::add);

      ConcurrentMap<VmName, IpAddress> vmIpAddresses = new ConcurrentHashMap<>();
      for (int i = 0; i < clusterParams.getNumNodes(); i++) {
        VmName vmName = VmName.builder().name(vmPrefix + (i + 1)).index(i).build();
        vmIpAddresses.put(vmName, ANY_ADDRESS);
      }

      VmUniverseParams universeParams = universe.vmIpAddresses(vmIpAddresses).build();
