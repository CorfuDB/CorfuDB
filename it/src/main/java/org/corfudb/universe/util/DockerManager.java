...
public class DockerManager {
  ...
  public void destroy(String containerName) {
    ...
    try {
      ...
      throw new NodeException(
          "Can't destroy Corfu server. Already deleted. Container: " + containerName, ex);
    }
  }
  ...
  public void resume(String containerName) {
    ...
    try {
      ...
      if (!container.state().paused()) {
        log.warn(
            "The container `{}` is not paused, should pause before resuming", container.name());
        return;
      }
  ...
  public void execCommand(String containerName, String... command)
            ...
            new Thread(
                () -> {
                  try {
                    ...
                    log.debug(
                        "Corfu server shutdown hook. Can't kill container: {}", containerName);
                  }
                }));
  }
}
