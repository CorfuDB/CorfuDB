...
public class CheckpointTest extends AbstractObjectTest {
    ...
    public void delayedCkpointTest() throws Exception {
        ...
        CorfuRuntime rt = getARuntime();
        Map<String, Long> m2A = openMap(rt, streamNameA);
        // first, populate the map
        for (int i = 0; i < mapSize; i++) {
        ...
        t(1, () -> {
            ...
            long checkpointAddress = mcw1.appendCheckpoints(rt, author);
        ...
        t(2, () -> {
            CorfuRuntime newRt = getARuntime();

            Map<String, Long> localm2A = openMap(newRt, streamNameA);
