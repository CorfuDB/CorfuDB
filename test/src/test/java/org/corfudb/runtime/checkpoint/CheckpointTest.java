...
public class CheckpointTest extends AbstractObjectTest {
  ...
  Map<String, Long> openMap(CorfuRuntime rt, String mapName) {
    ...
    return (CorfuTable<String, Long>)
        instantiateCorfuObject(
            rt, new TypeToken<CorfuTable<String, Long>>() {}, mapName, serializer);
  }
  ...
  /**
   * Start a fresh runtime and instantiate the maps. This time the we check that the new map
   * instances contains all values
   *
   * @param mapSize
   ...
   *
   * <p>1. one populates the maps with mapSize items 2. one does a periodic checkpoint of the maps,
   * repeating ITERATIONS_VERY_LOW times 3. one repeatedly (LOW times) starts a fresh runtime, and
   * instantiates the maps. they should rebuild from the latest checkpoint (if available). this
   * thread performs some sanity checks on the map state
   *
   * <p>Finally, after all three threads finish, again we start a fresh runtime and instantiate the
   * maps. This time the we check that the new map instances contains all values
   *
   * @throws Exception
   */
  @Test
  public void periodicCkpointTest() throws Exception {
    ...
    // finally, after all three threads finish, again we start a fresh runtime and instantiate the
    // maps.
   ...
   *
   * <p>1. one thread performs ITERATIONS_VERY_LOW checkpoints 2. one thread repeats ITERATIONS_LOW
   * times starting a fresh runtime, and instantiating the maps. they should be empty.
   *
   * <p>Finally, after the two threads finish, again we start a fresh runtime and instantiate the
   * maps. Then verify they are empty.
   *
   * @throws Exception
   */
  @Test
  public void emptyCkpointTest() throws Exception {
    ...
    // Finally, after the two threads finish, again we start a fresh runtime and instantiate the
    // maps.
  ...
  /**
   * this test is similar to periodicCkpointTest(), but populating the maps is done BEFORE starting
   * the checkpoint/recovery threads.
   *
   * <p>First, the test builds two maps, m2A m2B, and populates them with mapSize items.
   *
   * <p>Then, it brings up two threads:
   *
   * <p>1. one does a periodic checkpoint of the maps, repeating ITERATIONS_VERY_LOW times 2. one
   * repeatedly (LOW times) starts a fresh runtime, and instantiates the maps. they should rebuild
   * from the latest checkpoint (if available). this thread checks that all values are present in
   * the maps
   *
   * <p>Finally, after all three threads finish, again we start a fresh runtime and instantiate the
   * maps. This time the we check that the new map instances contains all values
   ...
   *
   * <p>the test builds two maps, m2A m2B, and brings up three threads:
   *
   * <p>1. one populates the maps with mapSize items 2. one does a periodic checkpoint of the maps,
   * repeating ITERATIONS_VERY_LOW times, and immediately trims the log up to the checkpoint
   * position. 3. one repeats ITERATIONS_LOW starting a fresh runtime, and instantiating the maps.
   * they should rebuild from the latest checkpoint (if available). this thread performs some sanity
   * checks on the map state
   *
   * <p>Finally, after all three threads finish, again we start a fresh runtime and instantiate the
   * maps. This time the we check that the new map instances contains all values
   *
  ...
  public void periodicCkpointTrimTest() throws Exception {
    ...
    try {
      ...
      // thread 3: repeated ITERATION_LOW times starting a fresh runtime, and instantiating the
      // maps.
      // they should rebuild from the latest checkpoint (if available).
      // performs some sanity checks on the map state
      scheduleConcurrently(
  ...
  /**
   * This test verifies that a stream can be rebuilt from a checkpoint even when the prefix trim
   * message has not reached the sequencer but yet addresses have been actually trimmed from the
   * log.
   *
   * @throws Exception
   */
  ...
  /**
   * This test verifies that a client that recovers a map from checkpoint, but wants the map at a
   * snapshot -earlier- than the snapshot, will either get a transactionAbortException, or get the
   * right version of the map.
   *
   * <p>It works as follows. We build a map with one hundred entries [0, 1, 2, 3, ...., 99].
   *
   * <p>Note that, each entry is one put, so if a TX starts at snapshot at 77, it should see a map
   * with 77 items 0, 1, 2, ..., 76. We are going to verify that this works even if we checkpoint
   * the map, and trim a prefix, say of the first 50 put's.
   *
   * <p>First, we then take a checkpoint of the map.
   *
   * <p>Then, we prefix-trim the log up to position 50.
   *
   * <p>Now, we start a new runtime and instantiate this map. It should build the map from a
   * checkpoint.
   *
   * <p>Finally, we start a snapshot-TX at timestamp 77. We verify that the map state is [0, 1, 2,
   * 3, ..., 76].
   */
  @Test
        ...
        () -> {
          ...
          try {
            if (trimExceptionFlag.get() == false) {
              assertThat(localm2A.size()).isEqualTo(snapshotPosition);
              ...
              for (int i = snapshotPosition; i < mapSize; i++) {
                assertThat(localm2A.get(String.valueOf(i))).isEqualTo(null);
              }
            }
          } finally {
            rt.shutdown();
  ...
  /**
   * This test validates that trimming the address space on a non-existing address (-1) after data
   * is already present in the log, does not lead to sequencer trims.
   *
   * <p>Note: this comes along with the fact that trimming the bitmap on a negative value, gives the
   * cardinality of infinity (all entries in the map).
   */
  @Test
  public void testPrefixTrimOnNonAddress() throws Exception {
    ...
    try {
      StreamingMap<String, Long> testMap =
          rt.getObjectsView()
              .build()
  ...
  public void PersistedCorfuTableTests() {
    ...
    Supplier<StreamingMap<String, String>> writerMapSupplier =
        () ->
            new PersistedStreamingMap<>(
                Paths.get(path + tableId + "writer"),
                PersistedStreamingMap.getPersistedStreamingMapOptions(),
                Serializers.JSON,
                rt);

    CorfuTable<String, String> diskBackedMap =
        rt.getObjectsView()
            .build()
            .setTypeToken(new TypeToken<CorfuTable<String, String>>() {})
            .streamID(tableId)
    ...
    Supplier<StreamingMap<String, String>> readerMapSupplier =
        () ->
            new PersistedStreamingMap<>(
                Paths.get(path + tableId + "reader"),
                PersistedStreamingMap.getPersistedStreamingMapOptions(),
                Serializers.JSON,
                rt);

    CorfuTable<String, String> newDiskBackedMap =
        newRt
            .getObjectsView()
            .build()
            .setTypeToken(new TypeToken<CorfuTable<String, String>>() {})
    ...
    assertThat(
            Iterators.elementsEqual(
                newDiskBackedMap.entryStream().iterator(), diskBackedMap.entryStream().iterator()))
        .isTrue();
  }
}
