...
public class TestRule {
   ...
   *
   * @param matcher A function that takes a CorfuMsg and returns true if the message matches.
   */
    ...
    if (match(message)) {
      ...
      if (injectBefore != null && router instanceof IServerRouter)
        ((IServerRouter) router)
            .sendResponse(null, injectBefore.apply(message), injectBefore.apply(message));
    }
    return true;
  }
