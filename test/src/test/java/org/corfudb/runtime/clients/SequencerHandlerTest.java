...
public class SequencerHandlerTest extends AbstractClientTest {
  ...
  Set<AbstractServer> getServersForTest() {
    ...
    return new ImmutableSet.Builder<AbstractServer>().add(new SequencerServer(sc)).build();
  }
  ...
  Set<IClient> getClientsForTest() {
    ...
    client =
        new SequencerClient(router, 0L, UUID.fromString("00000000-0000-0000-0000-000000000000"));
    return new ImmutableSet.Builder<IClient>().add(sequencerHandler).add(new BaseHandler()).build();
  }

  @Before
  public void bootstrapSequencer() {
    client.bootstrap(0L, Collections.emptyMap(), 0L, false).join();
  }

  @Test
  public void requestSequencerMetrics() throws Exception {
  ...
  @Test
  public void canGetAToken() throws Exception {
  ...
  public void tokensAreIncrementing() throws Exception {
    ...
    assertThat(token2.getSequence()).isGreaterThan(token.getSequence());
  }
  ...
  public void perStreamTokensWork() throws Exception {
    ...
    long tokenA2 =
        client.nextToken(Collections.singletonList(streamA), 0).get().getStreamTail(streamA);
    assertThat(tokenA.getSequence()).isEqualTo(tokenA2);
    long tokenB =
        client.nextToken(Collections.singletonList(streamB), 0).get().getStreamTail(streamB);
    assertThat(tokenB).isNotEqualTo(tokenA2);
    Token tokenB2 = client.nextToken(Collections.singletonList(streamB), 1).get().getToken();
    long tokenB3 =
        client.nextToken(Collections.singletonList(streamB), 0).get().getStreamTail(streamB);
    assertThat(tokenB2.getSequence()).isEqualTo(tokenB3);
    long tokenA3 =
        client.nextToken(Collections.singletonList(streamA), 0).get().getStreamTail(streamA);
