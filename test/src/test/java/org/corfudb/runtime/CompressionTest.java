...
public class CompressionTest extends AbstractViewTest {
  ...
  // Static method that generates and returns test data (automatically test for all codec's
  // supported)
  @Parameterized.Parameters
  public static Collection<Codec.Type> input() {
    return Stream.of(Codec.Type.values()).collect(Collectors.toList());
  }
  ...
  private Object read(CorfuRuntime rt, long address) {
    ...
    // Read data
    return rt.getAddressSpaceView().read(address).getPayload(rt);
  }
  ...
  /**
   * This test evaluates writing and reading to a stream log with different available codecs. For
   * this test, we setup a single node Corfu, and write/read a single entry with the same runtime.
   */
  @Test
  ...
  /**
   * In this test, we validate that reading data from a runtime which is configured with a different
   * codec than that one set on the write path, will not fail.
   */
  @Test
  ...
  /**
   * Test that interleaved entries with different encoder/decoders are written correctly and read
