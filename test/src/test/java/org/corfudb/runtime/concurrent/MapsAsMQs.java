        ...
        scheduleConcurrently(t -> {
            ...
            for (int i = 0; i < numIterations; i++) {
                while (testMap1.get(1L) == null || testMap1.get(1L) != (long) i) {
                    log.debug(2 + "- wait for 1st trigger " + i);
                    Thread.sleep(busyDelay);
                }
                log.debug(2 + "- received 1st trigger " + i);
        ...
        scheduleConcurrently(t -> {
            ...
            for (int i = 0; i < numIterations; i++) {
                try {
                    ...
                    c1.await();
                    log.debug(3 + "- received 1st condition " + i);

                    // produce another tigger value
                    log.debug(3 + "- sending 2nd trigger " + i);
        ...
        scheduleConcurrently(t -> {
            ...
            for (int i = 0; i < numIterations; i++) {
                while (testMap1.get(2L) == null || testMap1.get(2L) != (long) i)
                    Thread.sleep(busyDelay);
                log.debug(4 + "- received 2nd trigger " + i);
                ...
                try {
                    ...
                    // 2nd producer signal
                    log.debug(4 + "- sending 2nd signal " + i);
                    c2.signal();
                } finally {
                    lock.unlock();
                }
