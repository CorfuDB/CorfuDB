...
public class StreamSeekAtomicityTest extends AbstractTransactionsTest {
  ...
  /**
   * This test verifies commit atomicity against concurrent -read- activity, which constantly causes
   * rollbacks and optimistic-rollbacks.
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * This test is similar to above, but with multiple maps. It verifies commit atomicity against
   * concurrent -read- activity, which constantly causes rollbacks and optimistic-rollbacks.
   *
   * @throws Exception
   */
  @Test
    ...
    scheduleConcurrently(
        t -> {
          ...
          for (txCnt = 0; txCnt < numIterations; txCnt++) {
            ...
            // generate optimistic mutation
            testMap1.put(1L, (long) txCnt);
            if (txCnt % 2 == 0) testMap2.put(1L, (long) txCnt);
          ...
          Assert.assertEquals((long) (txCnt - 1), (long) testMap1.get(1L));
          Assert.assertEquals(
              (long) ((txCnt - 1) % 2 == 0 ? (txCnt - 1) : txCnt - 2), (long) testMap2.get(1L));
        });
