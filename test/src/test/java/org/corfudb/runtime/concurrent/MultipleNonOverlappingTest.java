...
public class MultipleNonOverlappingTest extends AbstractTransactionsTest {
   ...
   * High level:
   *
   * <p>This test will create OBJECT_NUM of objects in an CorfuTable. Each object's sum will be
   * incremented by VAL until we reach FINAL_SUM. At the end of this test we:
   *
   * <p>1) Check that there are OBJECT_NUM number of objects in the CorfuTable. 2) Ensure that each
   * object's sum is equal to FINAL_SUM
   *
   * <p>Details (the mechanism by which we increment the values in each object):
   *
   * <p>1) We create THREAD_NUM number of threads 2) Each thread is given a non-overlapping range on
   * which to increment objects' sum. Each thread will increment it only once by VAL. 3) We spawn
   * all threads and we ensure that each object in the map is incremented only once. We wait for
   * them to finish. 4) Repeat the above steps FINAL_SUM number of times.
   */
  @Test
  ...
  public void testStress2() throws Exception {
    ...
    for (long i = 0; i < OBJECT_NUM; i++) {
      ...
      if (i % 2 == 0) Assert.assertEquals((long) testMap2.get(i), (long) FINAL_SUM2);
      else Assert.assertEquals((long) testMap2.get(i), (long) FINAL_SUM1);
    }
  }
  ...
  public class NonOverlappingWriter {
    ...
    private void simpleCreateImpl(long idx, long val) {
      ...
      if (!testMap1.containsKey(idx)) {
        if (expectedSum - 1 > 0)
          log.debug("OBJ FAIL {} doesn't exist expected={}", idx, expectedSum);
        ...
        if (value != (expectedSum - 1))
          log.debug("OBJ FAIL {} value={} expected={}", idx, value, expectedSum - 1);
      ...
      if (!testMap1.containsKey(idx)) {
        ...
        if (idx % 2 == 0) {
          if (value2 != (expectedSum / 2))
            log.debug("OBJ2 FAIL {} value={} expected={}", idx, value2, expectedSum / 2);
          if (expectedSum % 2 == 0) {
          ...
          if (value2 != (expectedSum - 1))
            log.debug("OBJ2 FAIL {} value={} expected={}", idx, value2, expectedSum - 1);
          log.debug("OBJ2 {} PUT {}+{}", idx, value2, val);
          testMap2.put(idx, value2 + val);
        }
      }
