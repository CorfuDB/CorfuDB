 ...
 *
 * <p>This test does the following: 1) Creates two threads. 2) Synchronizes them using a count-down
 * latch to ensure concurrent execution. 3) They both write to the same stream and the key. 4) One
 * of the threads is going to get aborted (TransactionAbortedException). 5) Ensure that the
 * exception correctly reports the offending TX ID, the stream ID and the key.
 */
public class TransactionAbortedTest extends AbstractTransactionContextTest {

  /**
   * In a write after write transaction, concurrent modifications with the same read timestamp
   * should abort.
   */
  @Override
  public void TXBegin() {
    ...
    t2(() -> {
          try {
            ...
            final ICorfuSMRProxyInternal proxyInternal =
                tae.getContext().getWriteSetInfo().getConflicts().keySet().stream()
                    .findFirst()
                    .get();
