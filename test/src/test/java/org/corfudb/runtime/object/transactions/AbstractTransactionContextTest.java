...
public abstract class AbstractTransactionContextTest extends AbstractTransactionsTest {
  ...
  void setupCounters() {
    ...
    for (int i = 0; i < numTasks; i++)
      sharedCounters.add(i, instantiateCorfuObject(CorfuSharedCounter.class, "test" + i));
  ...
  /**
   * Ensure that empty write sets are not written to the log. This test applies to all contexts
   * which is why it is in the abstract test.
   */
  @Test
  ...
  public void ensureUserTsIsInherited() {
    ...
    getRuntime().getObjectsView().TXBuild().snapshot(parentTs).build().begin();
    getRuntime().getObjectsView().TXBuild().build().begin();

    // Verify that the child inherits the user defined
    // snapshot from the parent
    assertThat(TransactionalContext.getCurrentContext().getSnapshotTimestamp()).isEqualTo(parentTs);
  ...
  public void ensureTsIsSetFromTail() {
    ...
    assertThat(TransactionalContext.getCurrentContext().getSnapshotTimestamp().getSequence())
        .isEqualTo(newTail);
    getRuntime().getObjectsView().TXEnd();
    assertThat(TransactionalContext.getCurrentContext().getSnapshotTimestamp().getSequence())
        .isEqualTo(newTail);
