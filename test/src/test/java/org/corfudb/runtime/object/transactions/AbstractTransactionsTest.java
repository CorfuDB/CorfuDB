...
public abstract class AbstractTransactionsTest extends AbstractObjectTest {
  ...
  /** Utility method to start a (default type) TX; must be overriden with the desired type of TX */
  ...
  protected void OptimisticTXBegin() {
    getRuntime().getObjectsView().TXBuild().type(TransactionType.OPTIMISTIC).build().begin();
  }
  ...
  protected void WWTXBegin() {
    getRuntime().getObjectsView().TXBuild().type(TransactionType.WRITE_AFTER_WRITE).build().begin();
  }
}
