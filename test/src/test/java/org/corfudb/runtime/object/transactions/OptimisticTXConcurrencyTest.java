...
public class OptimisticTXConcurrencyTest extends TXConflictScenariosTest {
  ...
  public void testNoWriteConflictSimpleOptimistic() throws Exception {
    ...
    assertThat(commitStatus.get(0) == COMMITVALUE || commitStatus.get(1) == COMMITVALUE).isTrue();
  }
  ...
  public void checkRollbackNested() throws Exception {
    ...
    t(
        1,
        () -> {
          assertThat(maps.get(0).get(key1)).isEqualTo(null);
          assertThat(maps.get(1).get(key1)).isEqualTo(null);
        });
  }

  /**
   * This test evaluates a case of false hash conflict, i.e., the case where given two different
   * keys (UUIDs) its hash codes conflict.
   */
  @Test
