...
public class CorfuSMRObjectProxyTest extends AbstractObjectTest {
  ...
  public void canOpenObjectWithTwoRuntimes() throws Exception {
    ...
    TestClass testClass = (TestClass) instantiateCorfuObject(new TypeToken<TestClass>() {}, "test");
    ...
    TestClass testClass2 =
        (TestClass) instantiateCorfuObject(runtime2, new TypeToken<TestClass>() {}, "test");
  ...
  public void canUseAnnotations() throws Exception {
    ...
    TestClassUsingAnnotation test2 =
        (TestClassUsingAnnotation) instantiateCorfuObject(TestClassUsingAnnotation.class, "test");
  ...
  public void canUseCustomSerializer() throws Exception {
    ...
    ISerializer customSerializer =
        new CustomSerializer((byte) (Serializers.SYSTEM_SERIALIZERS_COUNT + 1));
  ...
  /**
   * Once a SMR map is created with a serializer, we should not be able to change the serializer by
   * re-opening the map with a different serializer.
   *
   * @throws Exception
   */
  @Test
  public void doesNotResetSerializerIfMapAlreadyExists() throws Exception {
    ...
    ObjectsView.ObjectID mapId =
        new ObjectsView.ObjectID(CorfuRuntime.getStreamID("test"), CorfuTable.class);

    CorfuCompileProxy cp =
        ((CorfuCompileProxy)
            ((ICorfuSMR) r.getObjectsView().getObjectCache().get(mapId)).getCorfuSMRProxy());
