...
public class UndoTest extends AbstractTransactionsTest {
  ...
  @Test
  public void ckCorrectUndo() throws Exception {
    ...
    t(
        0,
        () -> {
          assertThat(testMap.get("z")).isEqualTo("z");
          testMap.put("a", "a");
        });
    ...
    t(
        ...
        () -> {
          assertThat(testMap.get("z")).isEqualTo("z");
          assertThat(testMap.get("y")).isEqualTo(null);
          assertThat(testMap.size()).isEqualTo(2);
        });
  }

  @Test
  public void canRollbackWithoutUndo() throws Exception {
    Map<String, String> testMap =
        getRuntime()
            .getObjectsView()
            .build()
   ...
   *
   * <p>An irreversible modification causes a total object-rebuild.
   *
   * <p>This test verifies that after one transaction with undoable operation is rolled- back, the
   * performance of further transactions is not hurt.
   *
   * @throws Exception
   */
  @Test
  public void canUndoAfterNoUndo() throws Exception {
    ...
    for (int i = 0; i < mapSize; i++) testMap.put(i, normalValue);
    ...
    t(
        ...
        () -> {
          ...
          assertThat(testMap.get(specialKey)).isEqualTo(normalValue);
        });
    ...
    t(
        ...
        () -> {
          ...
          testMap.clear();
          assertThat(testMap.size()).isEqualTo(0);
          assertThat(testMap.get(specialKey)).isEqualTo(null);
        });
    ...
    t(
        ...
        () -> {
          ...
          testStatus +=
              "reset rebuild time=" + String.format("%.0f", (float) (endTime - startTime)) + "ms";
        });
    ...
    t(
        ...
        () -> {
          ...
          assertThat(testMap.size()).isEqualTo(mapSize);

          getRuntime().getObjectsView().TXBegin();
          testMap.put(specialKey, specialValue);
          assertThat(testMap.get(specialKey)).isEqualTo(specialValue);
        });
    ...
    t(
        ...
        () -> {
          ...
          testStatus +=
              "undo rebuild time=" + String.format("%.0f", (float) (endTime - startTime)) + "ms";
        });
  }

  @Test
  public void ckRollbackToRightPlace() throws Exception {
    Map<Integer, String> testMap =
        getRuntime()
            .getObjectsView()
            .build()
  ...
  /**
   * In this test, transactions are started on two threads, 1, 2. Then two things happen:
   *
   * <p>1. some updates are committed then t1 resumes and should roll back these commits.
   *
   * <p>2. then t2 is resumed and makes optimistic updates, which should roll back
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * In this test, a variant of multi-stream interleaving is tested.
   *
   * <p>transactions are started on two threads, 1, 2. Then two things happen:
   *
   * <p>1. some updates are committed 2. then t2 is resumed and makes optimistic updates
   *
   * <p>then t1 resumes and should roll back both the optimistic updates adn these commits.
   *
   * @throws Exception
   */
  @Test
  ...
  protected void crossStream(ArrayList<Map> maps, String value) {
    ...
    WWTXBegin();
    for (Map m : maps) m.put(specialKey, value);
    TXEnd();

    // put separate updates on all streams before t1 starts
    for (Map m : maps) m.put(specialKey + 1, value);
  }
}
