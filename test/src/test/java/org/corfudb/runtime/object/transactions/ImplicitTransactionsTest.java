...
public class ImplicitTransactionsTest extends AbstractTransactionsTest {
  ...
  /**
   * Implicit transaction should not be nested. All implicit Tx end up in TXExecute, so here we
   * execute isInNestedTransaction in TXExecute to check that we are indeed not in a nested tx.
   *
   * @throws Exception
   */
  @Test
  public void implicitTransactionalMethodAreNotNested() throws Exception {
    TransactionalObject to =
        getDefaultRuntime()
            .getObjectsView()
            .build()
  ...
  /**
   * Implicit transaction should be in a transaction. If we are not in a transaction already, it
  ...
  /** Runtime exception thrown during "nested" transaction should abort the transaction. */
  @Test
