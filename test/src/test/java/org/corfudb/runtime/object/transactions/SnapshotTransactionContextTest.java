...
public class SnapshotTransactionContextTest extends AbstractTransactionContextTest {
  ...
  public void defaultSnapshotTest() {
    ...
    t2(
        () ->
            getRuntime().getObjectsView().TXBuild().type(TransactionType.SNAPSHOT).build().begin());
  ...
  /** Check if we can read a snapshot from the past, without concurrent modifications. */
  @Test
  ...
  /** Ensure that a snapshot remains stable, even with concurrent modifications. */
  @Test
  public void snapshotReadableWithConcurrentWrites() {
    ...
    t2(() -> get("k")).assertResult().isEqualTo("v3");
    t4(() -> put("k", "v4")); // TS = 4
    t2(() -> get("k")).assertResult().isEqualTo("v3");
    t2(this::TXEnd);
  }
