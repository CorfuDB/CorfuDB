...
public abstract class TXConflictScenariosTest extends AbstractTransactionContextTest {
   ...
   *
   * <p>there are 'numTasks' shared counters numbered 0..numTasks-1, and 'numTasks' tasks executing
   * by an interleaving engine by CONCURRENCY_SOME number of threads.
   *
   * <p>all counters are initialized to INITIAL.
   *
   * <p>Each task is specified as a state machine. The state machine starts a transaction. Then it
   * repeats twice modify, read own counter, read other counter. Specifically, - task j modifies
   * counter j to OVERWRITE_ONCE, - task j reads counter j (own), expecting to read OVERWRITE_ONCE,
   * - task j reads counter (j+1) mod n, and records the value it reads,
   *
   * <p>- then task j modifies counter j to OVERWRITE_TWICE, - task j reads counter j (own),
   * expecting to read OVERWRITE_TWICE, - task j reads counter (j+1) mod n, expecting to read the
   * same value as before,
   *
   * <p>Then all tasks try to commit their transactions.
   *
   * @throws Exception
   */
  void testOpacity(boolean testInterleaved) throws Exception {
    ...
    addTestStep(
        (task_num) -> {
          assertThat(sharedCounters.get(task_num).getValue()).isEqualTo(OVERWRITE_ONCE);
        });
    ...
    // SM step 7: each task  again verifies opacity, checking that it can read its own modified
    // value
    addTestStep(
        (task_num) -> {
          assertThat(sharedCounters.get(task_num).getValue()).isEqualTo(OVERWRITE_TWICE);
        });

    // SM step 8: all tasks try to commit their transaction.
    // Task k aborts if, and only if, counter k+1 was modified after it read it and transaction k+1
    // already committed.
    addTestStep(
        (task_num) -> {
          try {
    ...
    if (testInterleaved) scheduleInterleaved(PARAMETERS.CONCURRENCY_SOME, numTasks);
    else scheduleThreaded(PARAMETERS.CONCURRENCY_SOME, numTasks);
  }
   ...
   * <p>all counters are initialized to INITIAL.
   *
   * <p>Each task is specified as a state machine. The state machine starts a transaction. Within
   * each transaction, each task repeats twice modify, read own, read other.
   *
   * <p>Specifically, - task j modifies counter j to OVERWRITE_ONCE, - task j reads counter j (own),
   * expecting to read OVERWRITE_ONCE, - task j reads counter (j+1) mod n, expecting to read either
   * OVERWRITE_ONCE or INITIAL,
   *
   * <p>- then task j modifies counter (j+1) mod n to OVERWRITE_TWICE, - task j reads counter j+1
   * mod n (own), expecting to read OVERWRITE_TWICE, - task j reads counter j (the one it changed
   * before), expecting to read OVERWRITE_ONCE ,
   *
   * <p>Then all tasks try to commit their transactions.
   */
  void testRWConflicts(boolean testInterleaved) throws Exception {
    // populate numTasks and sharedCounters array
    setupCounters();

    assertThat(numTasks)
        .isGreaterThan(1); // don't change concurrency to less than 2, test will break
    ...
    // SM step 4: task k verifies opacity, checking that it can read its own modified value of
    // counter k
    addTestStep(
        (task_num) -> {
          assertThat(sharedCounters.get(task_num).getValue()).isEqualTo(OVERWRITE_ONCE);
    ...
    // SM step 6: task k again check opacity, reading its own modified value, this time of counter
    // k+1
    addTestStep(
        (task_num) -> {
          assertThat(sharedCounters.get((task_num + 1) % numTasks).getValue())
  ...
  public void finegrainedUpdatesDoNotConflict() throws Exception {
    ...
    Map<String, String> testMap2 =
        (Map<String, String>)
            instantiateCorfuObject(new TypeToken<CorfuTable<String, String>>() {}, "test stream");
  ...
  void getAbortTestSM() {
    ...
    addTestStep(
        (task_num) -> {
          ...
          testMap.put(Integer.toString(task_num), Integer.toString(task_num));
  ...
  public void concurrentAbortTest(boolean testInterleaved) throws Exception {
    ...
    // invoke the execution engine
    if (testInterleaved) scheduleInterleaved(numThreads, numTasks);
    else scheduleThreaded(numThreads, numTasks);

    int aborts = 0;
    for (int i = 0; i < numTasks; i++) if (commitStatus.get(i) != COMMITVALUE) aborts++;
