...
public class OptimisticTransactionContextTest extends AbstractTransactionContextTest {
        ...
        TestRule testRule = new TestRule()
                .matches(m -> {
                    if (m.getMsgType().equals(CorfuMsgType.WRITE) && retry[0] < rtSlowWriter.getParameters().getWriteRetry()) {
                        rtIntersect.getAddressSpaceView().write(new LogicalSequenceNumber(retry[0], 0), "hello world".getBytes());
                        retry[0]++;
                        return true;
                    } else {
                        return false;
    ...
    public void checkSlowWriterTxSucceedsOnSameDataOverwrite() {
        ...
        TestRule testRule = new TestRule()
                .matches(m -> {
                    if (m.getMsgType().equals(CorfuMsgType.WRITE)) {
                        rtPropagateWrite.getAddressSpaceView().write(new LogicalSequenceNumber(0, 0), "hello world".getBytes());
                        retry[0]++;
                        return true;
                    } else {
                        return false;
