...
public class CorfuSMRObjectConcurrencyTest extends AbstractObjectTest {
  ...
  public void testCorfuSharedCounterConcurrentReads() throws Exception {
    ...
    CorfuSharedCounter sharedCounter =
        (CorfuSharedCounter) instantiateCorfuObject(CorfuSharedCounter.class, "test");
    ...
    scheduleConcurrently(
        writeconcurrency,
        t -> {
          for (int i = 0; i < writerwork; i++) sharedCounter.setValue(t * writerwork + i);
        });
    scheduleConcurrently(
        ...
        t -> {
          ...
          for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            ...
            boolean assertflag =
                ((((lastread < writerwork && res < writerwork)
                            || (lastread >= writerwork && res >= writerwork))
                        && lastread <= res)
                    || ((lastread < writerwork && res >= writerwork)
                        || (lastread >= writerwork && res < writerwork)));
            assertThat(assertflag).isTrue();
          }
        });
