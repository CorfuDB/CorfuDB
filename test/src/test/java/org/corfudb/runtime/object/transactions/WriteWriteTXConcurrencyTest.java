...
public class WriteWriteTXConcurrencyTest extends TXConflictScenariosTest {
  ...
  public void simpleWWTest() {
    ...
    CorfuTable<String, Integer> map =
        (CorfuTable<String, Integer>)
            instantiateCorfuObject(new TypeToken<CorfuTable<String, Integer>>() {}, "A");
  ...
  public void testOpacityWW(boolean testInterleaved) throws Exception {
    ...
    for (int task_num = 0; task_num < numTasks; task_num++) {
      if (commitStatus.get(task_num) != COMMITVALUE) {
        ...
        assertThat(commitStatus.get((task_num + 1) % numTasks)).isEqualTo(COMMITVALUE);
      }
    }
  }

  /**
   * If task k aborts, then either task (k-1), or (k+1), or both, must have committed (wrapping
   * around for tasks n-1 and 0, respectively).
   */
  ...
  public void testNoWriteConflictSimpleWW() throws Exception {
    ...
    assertThat(commitStatus.get(0)).isEqualTo(COMMITVALUE);
    assertThat(commitStatus.get(1)).isEqualTo(COMMITVALUE);
  }

  /**
   * This test uses the concurrentAbortTest scenario. The test invokes numTasks tasks, each one
   * writes exclusively to a map entry, and reads a few entries arbitrarily at random.
   *
   * <p>Unlike optimistic TXs, in write-write conflict mode, there should be --no-- conflicts,
  ...
  public void testAbortWW(boolean testInterleaved) throws Exception {
    ...
    assertThat(aborts).isEqualTo(falseaborts);
  }
