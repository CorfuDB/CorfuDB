...
public class SequencerViewTest extends AbstractViewTest {
  ...
  public void canAcquireFirstToken() {
    ...
    assertThat(r.getSequencerView().next().getToken()).isEqualTo(new Token(0L, 0L));
  }
  ...
  public void canQueryMultipleStreams() {
    ...
    assertThat(r.getSequencerView().next(stream1).getToken()).isEqualTo(new Token(0l, 0l));
    assertThat(r.getSequencerView().next(stream2).getToken()).isEqualTo(new Token(0l, 1l));
  ...
  public void tokensAreIncrementing() {
    CorfuRuntime r = getDefaultRuntime();
    assertThat(r.getSequencerView().next().getToken()).isEqualTo(new Token(0L, 0L));
    assertThat(r.getSequencerView().next().getToken()).isEqualTo(new Token(0L, 1L));
  }
  ...
  public void checkTokenWorks() {
    CorfuRuntime r = getDefaultRuntime();
    assertThat(r.getSequencerView().next().getToken()).isEqualTo(new Token(0L, 0L));
    assertThat(r.getSequencerView().query().getToken()).isEqualTo(new Token(0L, 0L));
  }
  ...
  public void checkStreamTokensWork() {
    ...
    assertThat(r.getSequencerView().next(streamA).getToken()).isEqualTo(new Token(0L, 0L));
    assertThat(r.getSequencerView().query(streamA)).isEqualTo(0L);
    assertThat(r.getSequencerView().next(streamB).getToken()).isEqualTo(new Token(0L, 1L));
    assertThat(r.getSequencerView().query(streamB)).isEqualTo(1L);
    assertThat(r.getSequencerView().query(streamA)).isEqualTo(0L);
  }
  ...
  public void checkBackPointersWork() {
    ...
    assertThat(r.getSequencerView().query(new UUID[] {streamA}).getBackpointerMap()).isEmpty();
    assertThat(r.getSequencerView().next(streamB).getBackpointerMap())
        .containsEntry(streamB, Address.NON_EXIST);
    assertThat(r.getSequencerView().query(new UUID[] {streamB}).getBackpointerMap()).isEmpty();
    assertThat(r.getSequencerView().next(streamA).getBackpointerMap()).containsEntry(streamA, 0L);
    assertThat(r.getSequencerView().next(streamB).getBackpointerMap()).containsEntry(streamB, 1L);
  }

  /**
   * Check streamAddressSpace after an epoch is incremented. The call should be retried in case the
   * epoch is changed or cluster connectivity is affected.
   */
  @Test
  public void checkStreamAddressSpaceAcrossEpochs() throws Exception {
    ...
    controlRuntime
        ...
        .reconfigureSequencerServers(originalLayout, newLayout, false);
