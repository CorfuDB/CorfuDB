 ...
 *
 * <p>Created by WenbinZhu on 11/8/18.
 */
public class LogUnitSealTest extends AbstractViewTest {
  ...
  /**
   * Test that log unit server seals correctly. Check operations in the queue that come after the
   * seal operation, but stamped with an older epoch, are completed exceptionally with a
   * WrongEpochException.
   */
  @Test
  public void checkOperationWithOldEpochFailedAfterSeal() {
    ...
    assertThatThrownBy(
            () ->
                getLayoutServer(SERVERS.PORT_0)
                    .getServerContext()
                    .getServerRouter()
    ...
    getLayoutServer(SERVERS.PORT_0)
        .getServerContext()
        .getServerRouter()
        .setServerEpoch(layout.getEpoch());
