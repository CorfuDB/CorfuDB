 ...
 *
 * <p>Created by zlokhandwala on 5/24/17.
 */
public abstract class AbstractStreamViewTest extends AbstractViewTest {
  ...
  /**
   * tests navigating forward/backward on a stream, with intermittent appends to the stream.
   *
   * <p>in addition to correctness assertions, this test can be used for single-stepping with a
   * debugger and observing stream behavior.
   */
  @Test
  public void readQueueTest() {
    ...
    for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
      ...
      assertThat(new String(payLoad).equals(String.valueOf(i))).isTrue();
  ...
  /**
   * Test if seeking the stream after resetting and then calling previous returns the correct entry.
   */
  @Test
  public void seekSkipTest() {
    ...
    assertThat((byte[]) sv.previous().getPayload(runtime)).isEqualTo(ENTRY_0);
  }
  ...
  public void testStreamGC() throws Exception {
    ...
    assertThat(((AbstractQueuedStreamView) bpsvA).getContext().resolvedQueue)
        .hasSize(PARAMETERS.NUM_ITERATIONS_LOW);
    assertThat(((AbstractQueuedStreamView) bpsvB).getContext().resolvedQueue)
        .hasSize(PARAMETERS.NUM_ITERATIONS_LOW);
   ...
   *
   * <p>(1) Append 100 entries to stream. (2) Traverse stream to a given point (up to address 24
   * using the 'next' api) (3) Trim the stream at prefix 50. (4) Traverse the remaining of the
   * stream (up to 99)
   *
   * <p>TrimmedException should be thrown.
   */
  @Test
  ...
  /**
   * This test verifies that we can traverse a trimmed stream when the ignore trimmed flag is set.
   * Steps to reproduce this test are: (1) Append 100 entries to stream. (2) Traverse stream to a
   * given point (up to address 24 using the 'next' api) (3) Trim the stream at prefix 50. (4)
   * Traverse the remaining of the stream (up to 99)
   *
   * <p>TrimmedException should be ignored and step (4) should retrieve only 49 entries (as 25-50
   * are trimmed).
   */
  ...
  private IStreamView traverseStreamBeforeTrimMark(boolean ignoreTrimmed)
    ...
    CorfuRuntime runtime = getDefaultRuntime();
    StreamOptions options = StreamOptions.builder().ignoreTrimmed(ignoreTrimmed).build();
