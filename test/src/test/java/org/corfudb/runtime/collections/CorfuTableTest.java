...
public class CorfuTableTest extends AbstractViewTest {
  ...
  public void openingCorfuTableTwice() {
    CorfuTable<String, String> instance1 =
        getDefaultRuntime()
            .getObjectsView()
            .build()
    ...
    CorfuTable<String, String> instance2 =
        getDefaultRuntime()
            .getObjectsView()
            .build()
            .setTypeToken(new TypeToken<CorfuTable<String, String>>() {})
  ...
  public void canReadFromEachIndex() {
    ...
    assertThat(project(corfuTable.getByIndex(StringIndexer.BY_VALUE, "ab"))).containsExactly("ab");
  }

  /**
   * Verify that a lookup by index throws an exception, when the index has never been specified for
   * this CorfuTable.
   */
  ...
  /**
   * Verify that a lookup by index and filter throws an exception, when the index has never been
   * specified for this CorfuTable.
   */
  @Test(expected = IllegalArgumentException.class)
  @SuppressWarnings("unchecked")
  public void cannotLookupByIndexAndFilterWhenIndexNotSpecified() {
    ...
    corfuTable.getByIndexAndFilter(
        StringIndexer.BY_FIRST_LETTER, p -> p.getValue().contains("cd"), "a");
  }
  ...
  public void emptyIndexesReturnEmptyValues() {
    ...
    assertThat(corfuTable.getByIndex(StringIndexer.BY_FIRST_LETTER, "a")).isEmpty();

    assertThat(corfuTable.getByIndex(StringIndexer.BY_VALUE, "ab")).isEmpty();
  }

  /**
   * Ensure that issues that arise due to incorrect index function implementations are percolated
  ...
  /**
   * Ensure that issues that arise due to incorrect index function implementations are percolated
   * all the way to the client (TX flavour).
   */
  @Test
  @SuppressWarnings("unchecked")
  ...
  public void canReadWithoutIndexes() {
    ...
    assertThat(corfuTable)
        .containsExactly(
            MapEntry.entry("k1", "a"), MapEntry.entry("k2", "ab"), MapEntry.entry("k3", "b"));
  }
  ...
  /**
   * Ensure that {@link StreamingMap#entryStream()} always operates on a snapshot. If it does not,
   * this test will throw {@link ConcurrentModificationException}.
   */
  @Test
  ...
  public void canHandleHoleInTail() {
    ...
    // create a hole
    TokenResponse tokenResponse = getDefaultRuntime().getSequencerView().next(streamID);

    Token token = tokenResponse.getToken();

    getDefaultRuntime().getAddressSpaceView().write(tokenResponse, LogData.getHole(token));

    assertThat(getDefaultRuntime().getAddressSpaceView().read(token.getSequence()).isHole())
        .isTrue();
    ...
    assertThat(
            ((CorfuCompileProxy) ((ICorfuSMR) corfuTable).getCorfuSMRProxy())
                .getUnderlyingObject()
                .getSmrStream()
                .pos())
        .isEqualTo(3);
  ...
  /**
   * Ensure that if the values of the table contains any duplicates, APIs that tries to retrieve all
   * values can correctly return all values including duplicates.
   */
  @Test
  public void duplicateValues() {
    ...
    assertThat(corfuTable.entrySet())
