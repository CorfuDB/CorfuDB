...
public class SMRMultiLogunitTest extends AbstractViewTest {
  ...
  /** Single Thread. Test a read after write on CorfuTable. */
  @Test
  ...
  /**
   * Single Thread. Verifies a read after 2 writes on the same key. The writes are done across 2
   * different instantiations of the same CorfuTable.
   */
  @Test
  ...
  /** Single Thread. Verifies read after multiple writes (ONE_THOUSAND). */
  @Test
  ...
  /**
   * Multi Threaded. Verify reads after multiple transactional writes done concurrently (using 2
   * threads)
   *
  ...
  public void transactionalManyWritesThenRead() throws TransactionAbortedException {
    ...
    // concurrently run two conflicting transactions:  one or the other should succeed without
    // overlap
  ...
  /**
   * Multi Threaded. Verify reads after multiple non-transactional writes done concurrently (using 2
   * threads)
   */
  @Test
