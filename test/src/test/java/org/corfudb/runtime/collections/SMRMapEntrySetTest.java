...
public class SMRMapEntrySetTest extends AbstractTransactionsTest {
  ...
  public void manipulateSets() throws Exception {
    Map<Long, Long> testMap = instantiateCorfuObject(CorfuTable.class, "mapsettest");

    // populate the map
    for (long i = 0; i < (long) PARAMETERS.NUM_ITERATIONS_LOW; i++) testMap.put(i, i);

    assertThat(testMap.get(0L)).isEqualTo(0L);
    assertThat(testMap.size()).isEqualTo(PARAMETERS.NUM_ITERATIONS_LOW);
    ...
    assertThat(testMap.containsKey(0L)).isFalse();
    assertThat(testMap.size()).isEqualTo(PARAMETERS.NUM_ITERATIONS_LOW - 1);
  }
  ...
  public void manipulateSetsConcurrent() throws Exception {
    ...
    scheduleConcurrently(
        t -> {
          ...
          if (snapshotSize > 0) assertThat(keys.contains(0L)).isTrue();
