...
public class LayoutServerTest extends AbstractServerTest {
  ...
  /** Verifies that a server that is not yet bootstrap does not respond with a layout. */
  @Test
  ...
  /**
   * Verifies that a server responds with a layout that the server was bootstrapped with. There are
   * no layout changes between bootstrap and layout request.
   */
  @Test
  ...
  /**
   * Verifies that once a prepare with a rank has been accepted, any subsequent prepares with lower
   * ranks are rejected. Note: This is in the scope of same epoch.
   */
  @Test
  ...
  /**
   * Verifies that once a prepare with a rank has been accepted, any propose with a lower rank is
   * rejected. Note: This is in the scope of same epoch.
   */
  @Test
  ...
  /**
   * Verifies that once a proposal has been accepted, the same proposal is not accepted again. Note:
   * This is in the scope of same epoch.
   */
  @Test
  public void proposeRejectsAlreadyProposed() {
    ...
    Assertions.assertThat(sendPropose(epoch, LOW_RANK, layout, layout.getClusterId()).join())
        .isTrue();
  ...
  /**
   * Verifies all phases set epoch, prepare, propose, commit. Note: this is in the scope of a single
   * epoch.
   */
  @Test
  public void commitReturnsAck() {
    ...
    Assertions.assertThat(resp.getRank()).isEqualTo(-1);
    Assertions.assertThat(resp.getLayout()).isNull();
    Assertions.assertThat(sendPropose(newEpoch, HIGH_RANK, newLayout, layout.getClusterId()).join())
        .isTrue();
    Assertions.assertThat(sendCommitted(newEpoch, newLayout, layout.getClusterId()).join())
        .isTrue();
  }

  /**
   * Verifies that once set the epoch cannot regress. Note: it does not verify that epoch is a dense
  ...
  public void checkLayoutPersisted() throws Exception {
    ...
    Assertions.assertThat(sendPropose(NEW_EPOCH, 1, newLayout, layout.getClusterId()).join())
        .isTrue();
  ...
  /**
   * The test verifies that the data in accepted phase1 and phase2 messages is persisted to disk and
   * survives layout server restarts.
   *
   * @throws Exception
   */
  @Test
  public void checkPaxosPhasesPersisted() throws Exception {
    ...
    Assertions.assertThat(sendPropose(newEpoch, 1, newLayout, layout.getClusterId()).join())
        .isTrue();
    // shutdown this instance of server
    s2.shutdown();
  ...
  /**
   * Validates that the layout server accept or rejects incoming phase1 messages based on the last
   * persisted phase1 rank.
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * Validates that the layout server accept or rejects incoming phase2 messages based on the last
   * persisted phase1 and phase2 data. If persisted phase1 rank does not match the LAYOUT_PROPOSE
   * message then the server did not take part in the prepare phase. It should reject this message.
  ...
  public void checkMessagesValidatedAgainstPhase2PersistedData() throws Exception {
    ...
    assertThatThrownBy(
            () -> sendPropose(newEpoch, HIGH_RANK - 1, newLayout, layout.getClusterId()).join())
        .hasCauseExactlyInstanceOf(OutrankedException.class);
  ...
  /**
   * Validates that the layout server accept or rejects incoming phase1 and phase2 messages from
   * multiple clients based on current state {Phase1Rank [rank, clientID], Phase2Rank [rank,
   * clientID] } If LayoutServer has accepted a phase1 message from a client , it can only accept a
   * higher ranked phase1 message from another client. A phase2 message can only be accepted if the
   * last accepted phase1 message is from the same client and has the same rank.
   *
   * @throws Exception
   */
  @Test
  public void checkPhase1AndPhase2MessagesFromMultipleClients() throws Exception {
    ...
    assertThatThrownBy(
            () ->
                sendPrepare(
                        UUID.nameUUIDFromBytes("TEST_CLIENT_OTHER".getBytes()),
                        newEpoch,
                        HIGH_RANK,
                        layout.getClusterId())
                    .join())
    ...
    resp =
        sendPrepare(
            ...
            .join();
    assertThat(s1)
        .isPhase1Rank(new Rank(HIGH_RANK + 1, UUID.nameUUIDFromBytes("OTHER_CLIENT".getBytes())));
    ...
    // phase2 message from a different client than the one whose phase1 was last accepted is
    // rejected
    assertThatThrownBy(
            () -> sendPropose(newEpoch, HIGH_RANK + 1, newLayout, layout.getClusterId()).join())
        .hasCauseExactlyInstanceOf(OutrankedException.class);
    ...
    assertThat(s2)
        .isPhase1Rank(new Rank(HIGH_RANK + 1, UUID.nameUUIDFromBytes("OTHER_CLIENT".getBytes())));
    assertThat(s2)
        .isPhase2Rank(new Rank(HIGH_RANK + 1, UUID.nameUUIDFromBytes("OTHER_CLIENT".getBytes())));
    assertThat(s2).isProposedLayout(newLayout);
  ...
  /**
   * Make sure that Paxos decisions are not based on mutable state that can be changed out of band.
   */
  @Test
  public void testChangeEpochInPhase2ByBaseServer() {
    ...
    Mockito.doAnswer(invocation -> null)
        .when(router)
        .sendResponse(Mockito.any(), Mockito.any(), Mockito.any());
    ...
    Mockito.doAnswer(
        ...
        .when(spyLayoutServer)
        .setPhase2Data(Mockito.any(), Mockito.anyLong());
    ...
    LayoutPrepareRequest prepareRequest = new LayoutPrepareRequest(newLayout.getEpoch(), msgRank);
    ...
    LayoutProposeRequest proposeRequest =
        new LayoutProposeRequest(newLayout.getEpoch(), msgRank, newLayout);
    ...
    Assertions.assertThat(spyLayoutServer.getPhase2Data(newSealEpoch).isPresent()).isFalse();
    // Make sure no data has been accepted for the correct epoch.
    Assertions.assertThat(spyLayoutServer.getPhase2Data(newLayout.getEpoch()).isPresent()).isTrue();
  }

  private void commitReturnsAck(
      LayoutServer s1, Integer reboot, long baseEpoch, Layout bootstrappedLayout) {
    long newEpoch = baseEpoch + reboot;
    sendRequestWithClusterId(
            new CorfuPayloadMsg<>(CorfuMsgType.SEAL, newEpoch), bootstrappedLayout.getClusterId())
        .join();

    Layout layout = TestLayoutBuilder.single(SERVERS.PORT_0);
    layout.setEpoch(newEpoch);

    LayoutPrepareResponse prepareResp =
        sendPrepare(newEpoch, HIGH_RANK, bootstrappedLayout.getClusterId()).join();
    Assertions.assertThat(prepareResp.getRank()).isEqualTo(-1);
    Assertions.assertThat(prepareResp.getLayout()).isNull();

    Assertions.assertThat(
            sendPropose(newEpoch, HIGH_RANK, layout, bootstrappedLayout.getClusterId()).join())
        .isTrue();

    Assertions.assertThat(sendCommitted(newEpoch, layout, bootstrappedLayout.getClusterId()).join())
        .isTrue();
  ...
  private CompletableFuture<LayoutPrepareResponse> sendPrepare(
      long epoch, long rank, UUID clusterId) {
    return sendRequestWithClusterId(
        CorfuMsgType.LAYOUT_PREPARE.payloadMsg(new LayoutPrepareRequest(epoch, rank)), clusterId);
  }

  private CompletableFuture<Boolean> sendPropose(
      long epoch, long rank, Layout layout, UUID clusterId) {
    return sendRequestWithClusterId(
        CorfuMsgType.LAYOUT_PROPOSE.payloadMsg(new LayoutProposeRequest(epoch, rank, layout)),
        clusterId);
  }

  private CompletableFuture<Boolean> sendCommitted(long epoch, Layout layout, UUID clusterId) {
    return sendRequestWithClusterId(
        CorfuMsgType.LAYOUT_COMMITTED.payloadMsg(new LayoutCommittedRequest(epoch, layout)),
        clusterId);
  }

  private CompletableFuture<LayoutPrepareResponse> sendPrepare(
      UUID clientId, long epoch, long rank, UUID clusterId) {
    return sendRequestWithClusterId(
        clientId,
        CorfuMsgType.LAYOUT_PREPARE.payloadMsg(new LayoutPrepareRequest(epoch, rank)),
        clusterId);
  }

  private CompletableFuture<Boolean> sendPropose(
      UUID clientId, long epoch, long rank, Layout layout, UUID clusterId) {
    return sendRequestWithClusterId(
        clientId,
        CorfuMsgType.LAYOUT_PROPOSE.payloadMsg(new LayoutProposeRequest(epoch, rank, layout)),
        clusterId);
