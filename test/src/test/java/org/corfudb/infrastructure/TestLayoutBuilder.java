...
public class TestLayoutBuilder {
  ...
  @Getter @Setter UUID clusterId;

  @Getter @Setter long epoch = 0L;
  ...
  public Layout build() {
    List<Layout.LayoutSegment> segmentList =
        segments.stream().map(TestSegmentBuilder::build).collect(Collectors.toList());

    return new Layout(
        layoutServers, sequencerServers, segmentList, unresponsiveServers, epoch, clusterId);
  }
  ...
  public static class TestSegmentBuilder {
    ...
    @Setter Layout.ReplicationMode replicationMode = Layout.ReplicationMode.CHAIN_REPLICATION;

    @Setter long start = 0L;
    ...
    private Layout.LayoutSegment build() {
      List<Layout.LayoutStripe> allStripes =
          stripes.stream().map(TestStripeBuilder::build).collect(Collectors.toList());
