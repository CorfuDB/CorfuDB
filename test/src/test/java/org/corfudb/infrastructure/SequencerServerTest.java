...
public class SequencerServerTest extends AbstractServerTest {
  ...
  public void sequencerMetricsRequest() {
    CompletableFuture<SequencerMetrics> cFuture =
        sendRequest(CorfuMsgType.SEQUENCER_METRICS_REQUEST.msg());
    SequencerMetrics seqMetrics = cFuture.join();
    assertThat(seqMetrics.getSequencerStatus()).isEqualTo(SequencerMetrics.SequencerStatus.READY);
  }
  ...
  public void tokensAreIncreasing() {
    ...
    for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
      CompletableFuture<TokenResponse> future =
          sendRequest(
              new CorfuPayloadMsg<>(
                  CorfuMsgType.TOKEN_REQ, new TokenRequest(1L, Collections.emptyList())));
      Token thisToken = future.join().getToken();
      assertThat(thisToken.getSequence()).isGreaterThan(lastTokenValue);
  ...
  public void checkTokenPositionWorks() {
    for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
      ...
      CompletableFuture<TokenResponse> future1 =
          sendRequest(
              new CorfuPayloadMsg<>(
                  CorfuMsgType.TOKEN_REQ, new TokenRequest(1L, Collections.emptyList())));
      CompletableFuture<TokenResponse> future2 =
          sendRequest(
              new CorfuPayloadMsg<>(
                  CorfuMsgType.TOKEN_REQ, new TokenRequest(0L, Collections.emptyList())));

      Token thisToken = future1.join().getToken();
      Token checkToken = future2.join().getToken();

      assertThat(thisToken).isEqualTo(checkToken);
    }
  }
  ...
  public void perStreamCheckTokenPositionWorks() {
    ...
    for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
      ...
      assertThat(thisTokenA.getSequence()).isEqualTo(checkTokenA);
      ...
      assertThat(thisTokenB.getSequence()).isEqualTo(checkTokenB);
      ...
      assertThat(checkTokenA2).isEqualTo(checkTokenA);

      assertThat(thisTokenB.getSequence()).isGreaterThan(checkTokenA);
    }
  }
  ...
  public void checkBackpointersWork() {
    ...
    for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
      ...
      assertThat(thisTokenB.getSequence()).isEqualTo(checkTokenBValue);
      ...
      assertThat(thisTokenA.getSequence() + MULTI_TOKEN - 1).isEqualTo(checkTokenAValue);
  ...
  public void SequencerWillResetTails() throws Exception {
    ...
    CompletableFuture<TokenResponse> future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(1L, Collections.singletonList(streamA))));
    long tailA = future.join().getToken().getSequence();

    future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(1L, Collections.singletonList(streamB))));
    long tailB = future.join().getToken().getSequence();

    future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(1L, Collections.singletonList(streamC))));
    future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(1L, Collections.singletonList(streamC))));
    ...
    tailMap.put(
        streamA,
        new StreamAddressSpace(Address.NON_ADDRESS, Roaring64NavigableMap.bitmapOf(newTailA)));
    tailMap.put(
        streamB,
        new StreamAddressSpace(Address.NON_ADDRESS, Roaring64NavigableMap.bitmapOf(newTailB)));
    tailMap.put(
        streamC,
        new StreamAddressSpace(Address.NON_ADDRESS, Roaring64NavigableMap.bitmapOf(newTailC)));
    ...
    future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(0L, Collections.singletonList(streamA))));
    assertThat(future.join().getStreamTail(streamA)).isEqualTo(newTailA);

    future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(0L, Collections.singletonList(streamB))));
    ...
    future =
        sendRequest(
            new CorfuPayloadMsg<>(
                CorfuMsgType.TOKEN_REQ, new TokenRequest(0L, Collections.singletonList(streamC))));
   ...
   * sequencers. We assert that the failover sequencer should always receive a full bootstrap
   * message rather than an empty bootstrap message (without streamTailsMap)
   */
  @Test
  public void failoverSeqDoesNotRegressTokenValue() {
    ...
    CompletableFuture<Boolean> future1 =
        sendRequestWithEpoch(
            CorfuMsgType.BOOTSTRAP_SEQUENCER.payloadMsg(
                new SequencerRecoveryMsg(
                ...
                new SequencerRecoveryMsg(
                    ...
                    Collections.singletonMap(
                        ...
                        new StreamAddressSpace(
                            Address.NON_ADDRESS, Roaring64NavigableMap.bitmapOf(num))),
                    newEpoch,
                    false)),
            newEpoch);
    assertThat(future1.join()).isEqualTo(true);
    ...
    future =
        sendRequestWithEpoch(
            CorfuMsgType.TOKEN_REQ.payloadMsg(new TokenRequest(0L, Collections.emptyList())),
            newEpoch);
