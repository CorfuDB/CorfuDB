...
public abstract class AbstractServerTest extends AbstractCorfuTest {
  ...
  @Getter TestServerRouter router;

  @Getter TestClientRouter clientRouter;
  ...
  public <T> CompletableFuture<T> sendRequest(UUID clientId, CorfuMsg msg) {
    msg.setClientID(clientId).setRequestID(requestCounter.getAndIncrement()).setEpoch(0L);
  ...
  public <T> CompletableFuture<T> sendRequest(CorfuMsg msg) {
    msg.setClientID(testClientId).setRequestID(requestCounter.getAndIncrement()).setEpoch(0L);
    clientRouter.setClientID(testClientId);
    return clientRouter.sendMessageAndGetCompletable(msg);
  }

  public <T> CompletableFuture<T> sendRequestWithEpoch(CorfuMsg msg, long epoch) {
    msg.setClientID(testClientId).setRequestID(requestCounter.getAndIncrement()).setEpoch(epoch);
    clientRouter.setClientID(testClientId);
    return clientRouter.sendMessageAndGetCompletable(msg);
  }
  ...
  public <T> CompletableFuture<T> sendRequestWithClusterId(
      UUID clientId, CorfuMsg msg, UUID clusterId) {
  ...
  public <T> T getLastPayloadMessageAs(Class<T> type) {
    Assertions.assertThat(getLastMessage()).isInstanceOf(CorfuPayloadMsg.class);
  ...
  public void sendMessage(UUID clientId, CorfuMsg message) {
    message.setClientID(clientId).setRequestID(requestCounter.getAndIncrement()).setEpoch(0L);
