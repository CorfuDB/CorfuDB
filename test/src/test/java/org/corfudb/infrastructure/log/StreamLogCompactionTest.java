...
public class StreamLogCompactionTest extends AbstractCorfuTest {
  ...
  public void testCompaction() throws InterruptedException {
    ...
    StreamLogCompaction compaction =
        new StreamLogCompaction(
            streamLog, initialDelay, period, TimeUnit.MILLISECONDS, PARAMETERS.TIMEOUT_VERY_SHORT);

    // If metrics are enabled, set an expectation of two compactions more than current
    // compaction count
    final long expectedCompactionCounter =
        initialCompactionCounter + (MetricsUtils.isMetricsCollectionEnabled() ? 2 : 0);
  ...
  private long getCompactionCounter() {
    return metricRegistry.timer(StreamLogCompaction.STREAM_COMPACT_METRIC).getCount();
  }
}
