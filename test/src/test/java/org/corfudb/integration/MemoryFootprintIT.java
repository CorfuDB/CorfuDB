...
/**
 * An integration test to utilize memory assessment utilities. It can be used as an example for
 * implementing programmatic assessment of memory footprint for objects of interest. Follow the
 * instruction in {@link MetricsUtils} in order to enable collection and reporting of memory
 * footprint metrics on objects of interest.
 *
 * <p>Created by Sam Behnam on 6/15/18.
 */
@Slf4j
public class MemoryFootprintIT extends AbstractIT {
   ...
   * footprint assessment of the provided tool. This is done by putting key and value pairs to a
   * corfu table and then evaluating that this has the expected effects on the memory consumption of
   * the object being measured (i.e. the underlying corfu table)
   *
   * @throws Exception
   */
  @Test
  public void testCorfuTablePutMemoryFootprint() throws Exception {
    ...
    assertThat(sizeAfterPuts - initialSize).isGreaterThanOrEqualTo(count * entrySize);
    log.info("initialSize:{}, sizeAfterPuts:{}", initialSize, sizeAfterPuts);
  ...
  /**
   * This test utilizes the {@link MetricsUtils}'s memory measurement tools to test the memory
   * footprint assessment of the provided tool. This is done by removing key and value pairs on a
   * corfu table and then evaluating that this has the expected effects on the memory consumption of
   * the object being measured (i.e. the underlying corfu table)
   *
   * @throws Exception
  ...
  public void testCorfuTableRemoveMemoryFootprint() throws Exception {
    ...
    log.info("sizeAfterPuts:{}, sizeAfterRemoves:{}", sizeAfterPuts, sizeAfterRemoves);
