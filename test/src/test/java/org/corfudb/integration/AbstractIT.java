...
/** Integration tests. Created by zlokhandwala on 4/28/17. */
@Slf4j
public class AbstractIT extends AbstractCorfuTest {
  ...
  private static final String FORCE_KILL_ALL_CORFU_COMMAND =
  ...
  public static final String TEST_SEQUENCE_LOG_PATH =
      CORFU_LOG_PATH + File.separator + "testSequenceLog";
  ...
  public static Process runPersistentServer(String address, int port, boolean singleNode)
      throws IOException {
    return new CorfuServerRunner()
        .setHost(address)
        .setPort(port)
  ...
  public static CorfuRuntime createRuntime(String endpoint) {
    CorfuRuntime rt = new CorfuRuntime(endpoint).setCacheDisabled(true).connect();
    return rt;
  }
  ...
  public static CorfuRuntime createRuntimeWithCache(String endpoint) {
    CorfuRuntime rt = new CorfuRuntime(endpoint).setCacheDisabled(false).connect();
    return rt;
  }

  public static StreamingMap<String, Integer> createMap(CorfuRuntime rt, String streamName) {
    StreamingMap<String, Integer> map =
        rt.getObjectsView()
            .build()
    ...
    public void run() {
      new BufferedReader(new InputStreamReader(inputStream))
          ...
          .forEach(
              (x) -> {
                try {
                  Files.write(Paths.get(logfile), x.getBytes(), StandardOpenOption.APPEND);
  ...
  /**
   * This is a helper class for setting up the properties of a CorfuServer and creating an instance
   * of a Corfu Server accordingly.
   */
  @Getter
  @Setter
  ...
  public static class CorfuServerRunner {
    ...
    /**
     * Create a command line string according to the properties set for a Corfu Server Instance
     *
    ...
    public String getOptionsString() {
      ...
      command.append(" -d ").append(logLevel).append(" ").append(port);
      return command.toString();
    }

    /**
     * Creates a server with the options set according to the properties of this Corfu server
     * instance
     *
     * @return a {@link Process} running a Corfu server as it is setup through the properties of the
     *     instance on which this method is called.
     * @throws IOException
     */
    public Process runServer() throws IOException {
      final String serverConsoleLogPath =
          CORFU_LOG_PATH + File.separator + host + "_" + port + "_consolelog";
