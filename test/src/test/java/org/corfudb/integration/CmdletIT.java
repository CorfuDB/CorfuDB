...
/**
 * Tests for clojure cmdlets. TODO: Implement few tests with TLS options too. FIXME: Cmdlets logunit
 * read do not work if serverEpoch > 1
 *
 * <p>Created by zlokhandwala on 5/8/17.
 */
@Ignore
public class CmdletIT extends AbstractIT {
    ...
    return new Layout(
        ...
        Collections.singletonList(
            new Layout.LayoutSegment(
                ...
                -1L,
                Collections.singletonList(
                    new Layout.LayoutStripe(Collections.singletonList(ENDPOINT))))),
        Collections.EMPTY_LIST,
        0L,
        UUID.randomUUID());
  }

  public static String runCmdletGetOutput(String command) throws Exception {
  ...
  public void testCorfuLayoutCmdlet() throws Exception {
    ...
    assertThat(runCmdletGetOutput(command).contains(getSingleLayout().toString())).isTrue();
  ...
  public void testCorfuPingCmdlet() throws Exception {
    ...
    assertThat(runCmdletGetOutput(command).contains(expectedSubString)).isTrue();
    shutdownCorfuServer(corfuServerProcess);
  }

  /**
   * Testing corfu_layouts query TODO: Not testing corfu_layouts -c <endpoint> edit
   *
   * @throws Exception
   */
  @Test
  public void testCorfuLayoutsCmdlet() throws Exception {
    ...
    assertThat(
            runCmdletGetOutput(command).replace(" ", "").contains(getSingleLayout().asJSONString()))
        .isTrue();
    shutdownCorfuServer(corfuServerProcess);
  }
  ...
  public void testCorfuQueryCmdlet() throws Exception {
    ...
    final String expectedStartupArgs =
        new CorfuServerRunner()
            .setPort(PORT)
            .setLogPath(getCorfuServerLogPath(DEFAULT_HOST, PORT))
            .getOptionsString();
  ...
  public void testCorfuSequencerCmdlet() throws Exception {
    ...
    String commandNextToken =
        CORFU_PROJECT_DIR
            + "bin/corfu_sequencer -i "
            + streamA
            + " -c "
            + ENDPOINT
            + " next-token 3";
    ...
    String commandLatest =
        CORFU_PROJECT_DIR + "bin/corfu_sequencer -i " + streamA + " -c " + ENDPOINT + " latest";
  ...
  public void testCorfuAddressSpaceCmdlet() throws Exception {
    ...
    runCmdletGetOutput(commandAppend);
    String commandRead =
        CORFU_PROJECT_DIR + "bin/corfu_as -i " + streamA + " -c " + ENDPOINT + " read 0";
  ...
  public void testCorfuHandleFailuresCmdlet() throws Exception {
    ...
    final String expectedSubString =
        "Failure handler on " + ENDPOINT + " started.Initiation completed !";
    assertThat(runCmdletGetOutput(command).contains(expectedSubString)).isTrue();
    shutdownCorfuServer(corfuServerProcess);
  }
  ...
  public void testCorfuBootstrapCluster() throws Exception {
    corfuServerProcess = new CorfuServerRunner().setPort(PORT).setSingle(false).runServer();
    ...
    String command =
        CORFU_PROJECT_DIR + "bin/corfu_bootstrap_cluster -l " + layoutFile.getAbsolutePath();
  ...
  public void testCorfuManagementBootstrap() throws Exception {
    ...
    String command =
        CORFU_PROJECT_DIR
            + "bin/corfu_management_bootstrap -c "
            + ENDPOINT
            + " -l "
  ...
  public void testCorfuSMRObject() throws Exception {
    ...
    final String commandPut =
        CORFU_PROJECT_DIR
            ...
            + ENDPOINT
            + " "
            + CorfuTable.class.getCanonicalName()
            + " putIfAbsent x "
            + payload;
    runCmdletGetOutput(commandPut);

    final String commandGet =
        CORFU_PROJECT_DIR
            + "bin/corfu_smrobject"
            + " -i "
            + streamA
