...
public class StreamingIT extends AbstractIT {
   ...
   *
   * <p>This listener accumulates all updates streamed to it into a linked list that can then be
   * used to verify.
   */
  private class StreamListenerImpl implements StreamListener {
    ...
    @Getter private final LinkedList<CorfuStreamEntries> updates = new LinkedList<>();
   ...
   *
   * <p>The test updates a single table a few times and ensures that the listeners subscribed to the
   * streaming updates from the table get the updates.
   *
   * <p>The test verifies the following: 1. Streaming subscriptions with different timestamps get
   * the right set of updates. 2. The operation type, key, payload and metadata values are as
   * expected. 3. A listener does not get stream updates after it has been unsubscribed.
   */
  @Test
  public void testStreamingSingleTable() throws Exception {
    ...
    // Create a table
    Table<Uuid, Uuid, Uuid> n1t1 =
        store.openTable(
            "n1", "t1", Uuid.class, Uuid.class, Uuid.class, TableOptions.builder().build());
    ...
    store.subscribe(
        s1n1t1,
        "n1",
        Collections.singletonList(new TableSchema("t1", Uuid.class, Uuid.class, Uuid.class)),
        ts1);
    ...
    for (int i = 0; i < numUpdates; i++) {
      ...
      assertThat(entry.get(0).getOperation().equals(CorfuStreamEntry.OperationType.UPDATE))
          .isTrue();
   ...
   *
   * <p>The test updates two tables a few times and ensures that the listeners subscribed to the
   * streaming updates from the table get the updates.
   *
   * <p>The test verifies the following: 1. Streaming subscriptions with different timestamps get
   * the right set of updates. 2. The operation type, key, payload and metadata values are as
  ...
  public void testStreamingMultiTableStreams() throws Exception {
    ...
    Table<Uuid, Uuid, Uuid> n2t1 =
        store.openTable(
            "n2", "t1", Uuid.class, Uuid.class, Uuid.class, TableOptions.builder().build());
