...
public class CorfuStoreBrowserIT extends AbstractIT {
  ...
  /**
   * Create a table and add data to it. Verify that the browser tool is able to read its contents
   * accurately.
   *
   * @throws IOException
  ...
  public void browserTest()
    ...
    SampleSchema.Uuid uuidKey =
        SampleSchema.Uuid.newBuilder().setMsb(keyUuid).setLsb(keyUuid).build();
    SampleSchema.Uuid uuidVal =
        SampleSchema.Uuid.newBuilder().setMsb(valueUuid).setLsb(valueUuid).build();
    SampleSchema.Uuid metadata =
        SampleSchema.Uuid.newBuilder().setMsb(metadataUuid).setLsb(metadataUuid).build();
    ...
    for (Object obj : table.values()) {
      ...
      Assert.assertEquals(
          UnknownFieldSet.newBuilder().build(), record.getPayload().getUnknownFields());
    }
  ...
  /**
   * Create a table and add nested protobufs as data to it. Verify that the browser tool is able to
   * read the contents accurately.
   *
   * @throws IOException
   * @throws NoSuchMethodException
   * @throws IllegalAccessException
  ...
  public void nestedProtoTest()
    ...
    SampleSchema.Uuid uuidKey =
        SampleSchema.Uuid.newBuilder().setLsb(keyUuid).setMsb(keyUuid).build();
    SampleSchema.FirewallRule firewallRuleVal =
        SampleSchema.FirewallRule.newBuilder()
            .setRuleId(ruleIdVal)
            ...
            .build();
    SampleSchema.Uuid uuidMeta =
        SampleSchema.Uuid.newBuilder().setLsb(metaUuid).setMsb(metaUuid).build();
    TxBuilder tx = store.tx(namespace);
  ...
  /**
   * Create a table and add data to it. Verify that the browser tool is able to read the system
   * TableRegistry contents accurately.
   *
   * @throws IOException
   * @throws NoSuchMethodException
   * @throws IllegalAccessException
   ...
   */
  @Test
  public void browserRegistryTableTest()
      throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
    final String namespace = "namespace";
    final String tableName = "table";
    Process corfuServer = runSinglePersistentServer(corfuSingleNodeHost, corfuStringNodePort);
    ...
    Assert.assertEquals(
        2,
        browser.printTableInfo(
            TableRegistry.CORFU_SYSTEM_NAMESPACE, TableRegistry.REGISTRY_TABLE_NAME));
  }

  /**
   * Create a table and add data to it. Verify that the browser tool is able to read disk based
   * tables in disk based mode.
   *
   * @throws IOException
   * @throws NoSuchMethodException
   * @throws IllegalAccessException
   ...
   */
  @Test
  public void browserDiskBasedTableTest()
      throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
    final String namespace = "namespace";
    final String tableName = "table";
    runSinglePersistentServer(corfuSingleNodeHost, corfuStringNodePort);
