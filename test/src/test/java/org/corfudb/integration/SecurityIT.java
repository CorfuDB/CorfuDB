 ...
 *
 * <p>Created by Sam Behnam on 8/13/18.
 */
@Slf4j
public class SecurityIT extends AbstractIT {
   ...
   * each test by converting them from the values provided in CorfuDB.properties. Note that it will
   * throw {@link NumberFormatException} if the numerical properties provided in CorfuDB.properties
   * are not well-formed. If the CorfuDB.properties has invalid or non existent values for the
   * keystore or truststore, it will lead to throwing {@link IllegalArgumentException}.
   */
  @Before
   ...
   * provided in CorfuDB.properties. Corfu runtime configures TLS related parameters using {@link
   * CorfuRuntime}'s API and then asserts that operations on a CorfuTable is executed as Expected.
   *
   * @throws Exception
   */
  @Test
  public void testServerRuntimeTlsEnabledMethod() throws Exception {
    ...
    runtime =
        new CorfuRuntime(singleNodeEndpoint)
            .enableTls(
   ...
   * This test creates Corfu runtime and a single Corfu server according to the configuration
   * provided in CorfuDB.properties. Corfu runtime configures TLS related parameters using {@link
   * org.corfudb.runtime.CorfuRuntime.CorfuRuntimeParameters} and then asserts that operations on a
   * CorfuTable is executed as Expected.
   *
   * @throws Exception
   */
  @Test
  public void testServerRuntimeTlsEnabledByParameter() throws Exception {
    ...
    final CorfuRuntime.CorfuRuntimeParameters runtimeParameters =
  ...
  /**
   * Testing that configuring incorrect TLS parameters will lead to throwing {@link
   * UnrecoverableCorfuError} exception.
   *
   * @throws Exception
   */
