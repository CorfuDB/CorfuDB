...
/**
 * Simple performance test to insert data into corfu via regular table.put() and CorfuStore
 * protobufs
 */
public class CorfuStorePerfIT extends AbstractIT {
  ...
  public void corfuStorePerfComparisonTest() throws Exception {
    ...
    Process server1 = runServerWithQuota(DEFAULT_PORT, maxLogSize, true);
  ...
  private static final int randomPort = 8000;
  private static final int eventTime = 20000;
  private static final int randomFreq = 10;
  ...
  private void addObjectsToTable(CorfuRuntime rt, final int count) {
    ...
    for (int i = 0; i < count; i++) {
      event = new Event(i, "TestEvent" + i, randomPort, eventTime, randomFreq);
  ...
  private void addProtoToStore(CorfuRuntime rt, final int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      events.add(
          SampleSchema.EventInfo.newBuilder()
              .setId(i)
              .setName("event_" + i)
              .setEventTime(i)
              .build());
