...
public class LogReplicationAbstractIT extends AbstractIT {
    ...
    /**
     * Checkpoint a set of tables and trim
     *
     * @param rt Corfu runtime
     * @param toCheckpoint list of tables to checkpoint
     */
    public void checkpointAndTrim(CorfuRuntime rt, List<CorfuTable> toCheckpoint) {
    ...
    /**
     * Checkpoint and Trim a set of tables in addition to CorfuStore (UFO tables)
     *
     * @param rt runtime
     * @param tables list of additional tables to checkpoint
     **/
    public void checkpointAndTrimCorfuStore(CorfuRuntime rt, List<CorfuTable> tables) {

        TableRegistry tableRegistry = rt.getTableRegistry();
        ...
        // the serializer does not go back to the regular Protobuf Serializer
        ISerializer dynamicProtobufSerializer = new DynamicProtobufSerializer(rt);
        ...
        // First, checkpoint additional tables
        mcw.addAllMaps(tables);
        Token token = mcw.appendCheckpoints(rt, "checkpointer");
        trimToken = Token.min(trimToken, token);

        // Checkpoint the TableRegistry system table
        for (CorfuStoreMetadata.TableName tableName : tableRegistry.listTables(null)) {
            ...
            SMRObject.Builder<CorfuTable<CorfuDynamicKey, CorfuDynamicRecord>> corfuTableBuilder = rt.getObjectsView().build()
            ...
            token = mcw.appendCheckpoints(rt, "checkpointer");
            trimToken = Token.min(trimToken, token);
        }
    ...
    public void checkpointAndTrimStandby() {
        ...
        checkpointAndTrim(standbyCPRuntime, toCheckpoint);
    }
}