 ...
 */
public class CorfuHopStreamTemporaryHello {

    private static final Logger log = LoggerFactory.getLogger(CorfuHopStreamTemporaryHello.class);
        ...
        try (Stream s = new Stream(client, streamID))
        {
            try (Stream s2 = new Stream(client, streamID2))
            {
               ...
               if (!sresult.toString().equals("hello world from stream " + streamID2.toString()))
                ...
                log.info("Pulling stream  " + streamID2.toString() + " to stream " + streamID.toString() + " for 2 entries");
                s.pullStream(streamID2, 2);
