 ...
 */
public class CorfuHopStreamTemporaryMultiPayloadHello {

    private static final Logger log = LoggerFactory.getLogger(CorfuHopStreamTemporaryMultiPayloadHello.class);
        ...
        try (Stream s = new Stream(client, streamID))
        {
            try (Stream s2 = new Stream(client, streamID2))
            {
               ...
               if (!sresult.toString().equals("hello world from stream " + streamID3.toString()))
                ...
                s.pullStream(streams, "Merged stream entry!", 4);
/*
...
*/
       ...
       log.debug("Checking merged address equality");
       checkAddressEquality(s1_mergedAddress, s2_mergedAddress);
       checkAddressEquality(s1_mergedAddress, s3_mergedAddress);

       log.debug("Checking merged address order");
       checkAddresses(s1_mergedAddress, s1_secondAddress, s1_thirdAddress);
       checkAddresses(s2_mergedAddress, s3_secondAddress, s1_thirdAddress);
       checkAddresses(s3_mergedAddress, s3_firstAddress, s1_thirdAddress);
