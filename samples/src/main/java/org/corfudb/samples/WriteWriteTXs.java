 ...
 *
 * <p>By default, Corfu transaction wrapped between TXBegin()/TXEnd() guarantee serializabilty. The
 * other supported isolation levels are: - write-write: Check that modification by this transaction
 * do not overwrite concurrent modifications - snapshot: Always commits, only guarantees execution
 * against a consistent snapshot
 *
 * <p>All transactions execute against a consistent snapshot, but have different conflict resolution
 * criteria. By default, the snapshot time of a transaction is the time of its first object
 * accessor. Corfu supports explicitly designating snapshot time (a log offset, which is returned by
 * object mutators).
 *
 * <p>Below, the utility method {@link WriteWriteTXs ::TXBegin()} illustrates how to replace
 * TXBegin() with a transaction-builder indicating write-write isolation.
 *
 * <p>The utility method {@link WriteWriteTXs ::TXBegin(long snapTime)} illustrates how to
 * additionally set a transaction snapshot-time to a specific position.
 *
...
public class WriteWriteTXs extends BaseCorfuAppUtils {
  ...
  //     * In order to prevent a transaction from viewing stale snapshots, transactions touch the
  // entries they get().
