 ...
 *
 * <p>In the default (Optimistic) transaction isolation level, since it has no read-set, a
 * write-only transaction never needs to abort due to another transaction's commit.
 *
 * <p>The main reason to group mutator updates into a write-transaction is commit *atomicity*:
 * Either all of mutator updates are visible to an application or none.
 *
 * <p>This program illustrates the write atomicity concept with a simple transaction example.
 *
...
public class WriteOnlySimpleTransaction extends BaseCorfuAppUtils {
  ...
  static BaseCorfuAppUtils selfFactory() {
    return new WriteOnlySimpleTransaction();
  }
