...
/**
 * A simple program that makes use of {@link CorfuSharedCounter} and {@link CorfuCompoundObject}.
 */
public class SimpleUseCustomBaseCorfuObject extends BaseCorfuAppUtils {
  ...
  static BaseCorfuAppUtils selfFactory() {
    return new SimpleUseCustomBaseCorfuObject();
  }
  ...
  public void action() {
    CorfuSharedCounter cntr =
        instantiateCorfuObject(new TypeToken<CorfuSharedCounter>() {}, "CNTR");
    ...
    CorfuCompoundObject cmpnd =
        instantiateCorfuObject(new TypeToken<CorfuCompoundObject>() {}, "CMPND");

    cmpnd.set(cmpnd.new Inner("foo", "bar"), MAGIC_VALUE);
    System.out.println(
        "compound: "
            + "user="
            + cmpnd.getUser().getFirstName()
            + ","
            + cmpnd.getUser().getLastName()
            + " ID="
            + cmpnd.getID());
  }
}
