...
crashRecoverTest() {
  ...
  ./bin/corfuDBsingle.sh start > /dev/null 2>&1
  ./bin/corfuDBTestRuntime.sh CorfuDBTester -m http://localhost:8002/corfu -s $streamimpl -C $class -A crash -z $crashop -t $threads -n 20 -k 20 -r 0.25 $verbose $xdebug -p 9091 -i bnc/initfs_$size.ser -w bnc/wkldfs_$size.ser $stats > bnc/data/crash-$size-$class-t$threads-s$streamimpl.txt
  ./bin/corfuDBTestRuntime.sh CorfuDBTester -m http://localhost:8002/corfu -s $streamimpl -C $class -A recover -z $recoverop -t $threads -n 20 -k 20 -r 0.25 $verbose $xdebug -p 9091 -i bnc/initfs_$size.ser -w bnc/wkldfs_$size.ser $stats -L bnc/data/crash-$size-$class-t$threads-s$streamimpl.txt > bnc/data/recover-$size-$class-t$threads-s$streamimpl.txt
  ./bin/corfuDBsingle.sh stop > /dev/null 2>&1

  cat bnc/data/crash-$size-$class-t$threads-s$streamimpl.txt > tmp.txt
  cat bnc/data/recover-$size-$class-t$threads-s$streamimpl.txt >> tmp.txt
  echo $size-$class-t$threads-s$streamimpl >> bnc/data/tput-all.csv
  grep TPUT tmp.txt >> bnc/data/tput-all.csv
  grep TPUT tmp.txt > bnc/data/btree-tput-$size-$class-t$threads-s$streamimpl.csv
  grep FSREQLAT tmp.txt > bnc/data/btree-reqlat-$size-$class-t$threads-s$streamimpl.csv
  grep FSCMDLAT tmp.txt >> bnc/data/btree-reqlat-$size-$class-t$threads-s$streamimpl.csv
}
...
crashRecoverTest $class $size 20 21 -v -x -S $threads $streamimpl
