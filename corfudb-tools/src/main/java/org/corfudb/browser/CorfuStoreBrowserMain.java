...
/**
 * Main class for the CorfuStore Browser Tool. Command line options are documented in the USAGE
 * variable.
 *
 * <p>- Created by pmajmudar on 10/16/2019
 */
@Slf4j
public class CorfuStoreBrowserMain {
  ...
  public static void main(String[] args) {
    try {
      ...
      CorfuRuntime.CorfuRuntimeParameters.CorfuRuntimeParametersBuilder builder =
      ...
      if (tlsEnabled) {
        ...
        String truststore_password = opts.get("--truststore_password").toString();
        builder
            .tlsEnabled(tlsEnabled)
            .keyStore(keystore)
      ...
      String namespace =
          Optional.ofNullable(opts.get("--namespace")).map(n -> n.toString()).orElse(null);
      String tableName =
          Optional.ofNullable(opts.get("--tablename")).map(t -> t.toString()).orElse(null);
