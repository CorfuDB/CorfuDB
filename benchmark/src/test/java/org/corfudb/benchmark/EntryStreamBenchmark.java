...
public class EntryStreamBenchmark extends UniverseHook {
    ...
    public void entryStream(BenchmarkState state) {
        for (int txCount = 0; txCount < 10; txCount++) {
            TxnContext tx = state.store.txn(DEFAULT_STREAM_NAMESPACE);
            List<?> result =  tx.executeQuery(state.table, all -> !all.getPayload().getValue().isEmpty());
            tx.txAbort();
        }
    }
    ...
    @Warmup(iterations = 1)
    public void executeQuery(BenchmarkState state) throws InvocationTargetException, NoSuchMethodException, IllegalAccessException {
