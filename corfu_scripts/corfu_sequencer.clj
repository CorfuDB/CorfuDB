...
Usage:
  ...
  corfu_sequencer [-i <stream-id>] -c <config> [-e [-u <keystore> -f <keystore_password_file>] [-r <truststore> -w <truststore_password_file>] [-g -o <username_file> -j <password_file>]] next-logicalSequenceNumber <num-tokens>
Options:
...
(defn get-logicalSequenceNumber [stream, num-tokens]
...
(cond (.. localcmd (get "latest")) (get-logicalSequenceNumber stream 0)
  (.. localcmd (get "next-logicalSequenceNumber")) (get-logicalSequenceNumber stream (Integer/parseInt (.. localcmd (get "<num-tokens>"))))
