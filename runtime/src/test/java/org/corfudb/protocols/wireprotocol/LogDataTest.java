...
import org.corfudb.protocols.logprotocol.SMRGarbageInfo;
import org.corfudb.protocols.logprotocol.SMRLogEntry;
import org.corfudb.protocols.logprotocol.SMRRecord;
import org.corfudb.protocols.logprotocol.SMRRecordGarbageInfo;
...
public class LogDataTest {
    ...
    public void testGarbagePayloadSerializeDeserialize() {
        ...
        SMRRecordGarbageInfo recordGarbageInfo = new SMRRecordGarbageInfo(detectedAddress, smrEntrySize);
        SMRGarbageInfo garbageInfo = new SMRGarbageInfo();
        ...
        // TODO(Xin): runtime is not used in the test. Future patch will eliminate the dependency on runtime.
        SMRGarbageInfo deserializedSMREntryGarbageInfo =
                (SMRGarbageInfo) deserializedLogData.getPayload(null);


        Assert.assertEquals(detectedAddress,
                deserializedSMREntryGarbageInfo.getAllGarbageInfo().get(streamId).get(index).getMarkerAddress());
    }
}
