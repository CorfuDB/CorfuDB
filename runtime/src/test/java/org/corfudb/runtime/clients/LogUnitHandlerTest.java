...
public class LogUnitHandlerTest {
    ...
    /**
     * Test that the LogUnitHandler correctly throws DataOutrankedException upon DATA_OUTRANKED_ERROR.
     */
    @Test
    public void testDataOutrankedError() {
        ResponseMsg response = getResponseMsg(
                getBasicHeader(false, true),
                getDataOutrankedErrorMsg()
        );
        ...
        verify(mockClientRouter, never()).completeRequest(anyLong(), any());
        verify(mockClientRouter).completeExceptionally(
                eq(response.getHeader().getRequestId()), any(DataOutrankedException.class));
    }

    /**
     * Test that the LogUnitHandler correctly throws ValueAdoptedException upon VALUE_ADOPTED_ERROR.
     */
    @Test
    public void testValueAdoptedError() {
        ReadResponse rr = new ReadResponse();
        ResponseMsg response = getResponseMsg(
                getBasicHeader(false, true),
                getValueAdoptedErrorMsg(rr)
        );

        logUnitHandler.handleMessage(response, mockChannelHandlerContext);
        ArgumentCaptor<ValueAdoptedException> captor = ArgumentCaptor.forClass(ValueAdoptedException.class);
        // Verify that the correct request was completed exceptionally.
        verify(mockClientRouter, never()).completeRequest(anyLong(), any());
        verify(mockClientRouter).completeExceptionally(
                eq(response.getHeader().getRequestId()), captor.capture());
        assertEquals(rr, captor.getValue().getReadResponse());
    }
