...
public final class CorfuProtocolTxResolution {
    ...
    private static Set<UUID> getUUIDSet(List<RpcCommon.UuidMsg> streamIds) {
        return streamIds.stream().map(CorfuProtocolCommon::getUUID).collect(Collectors.toSet());
    }
    ...
    public static TxResolutionInfo getTxResolutionInfo(TxResolutionInfoMsg msg) {
        return new TxResolutionInfo(
                ...
                getUUIDSet(msg.getCreateSetList()),
                getUUIDSet(msg.getDeleteSetList())
        );
    }
}
