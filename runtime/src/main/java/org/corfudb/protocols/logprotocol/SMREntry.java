...
public class SMREntry extends LogEntry implements ISMRConsumable {
  ...
  @Getter private ISerializer serializerType;

  /** An undo record, which can be used to undo this method. */
  @Getter public transient Object undoRecord;

  /**
   * A flag indicating whether an undo record is present. Necessary because undo records may be
   * NULL.
   */
  @Getter public boolean undoable;

  /** The upcall result, if present. */
  @Getter public transient Object upcallResult;

  /** If there is an upcall result for this modification. */
  @Getter public transient boolean haveUpcallResult = false;
  ...
  /**
   * Given a buffer with the reader index pointing to a serialized SMREntry, this method will seek
   * the buffer's reader index to the end of the entry.
   */
