...
public class CheckpointEntry extends LogEntry {
  ...
  /** Type of entry. */
  @Getter CheckpointEntryType cpType;

  /**
   * Unique identifier for this checkpoint. All entries for the same checkpoint state must use the
   * same ID.
   */
  ...
  /**
   * Optional: SMREntry objects that contain SMR object state of the stream that we're
   * checkpointing. May be present in any CheckpointEntryType, but typically used by CONTINUATION
   * entries.
   */
  @Getter @Setter MultiSMREntry smrEntries;

  /**
   * Byte count of smrEntries in serialized form, zero if smrEntries.size() is zero or if value is
   * unknown.
   */
  @Getter int smrEntriesBytes = 0;
  ...
  /**
   * This function provides the remaining buffer. Child entries should initialize their contents
   * based on the buffer.
   *
   ...
   *
   * <p>NOTE: This method has a side-effect of updating the this.smrEntriesBytes field.
   *
