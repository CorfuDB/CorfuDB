...
public interface ICorfuPayload<T> {
  ...
  ConcurrentHashMap<Class<?>, PayloadConstructor<?>> constructorMap =
      new ConcurrentHashMap<>(
          ImmutableMap.<Class<?>, PayloadConstructor<?>>builder()
              ...
              .put(
                  DataRank.class,
                  x -> new DataRank(x.readLong(), new UUID(x.readLong(), x.readLong())))
              ...
              .put(
                  StreamAddressRange.class,
                  buffer ->
                      new StreamAddressRange(
                          new UUID(buffer.readLong(), buffer.readLong()),
                          buffer.readLong(),
                          buffer.readLong()))
              .put(
                  StreamAddressSpace.class,
                  buffer -> {
                    ...
                    try (ByteBufInputStream inputStream = new ByteBufInputStream(buffer)) {
                      ...
                      throw new SerializerException(
                          "Exception when attempting to " + "deserialize stream address space.",
                          ioe);
                    }
                  })
              .build());
  ...
  static <T> T fromBuffer(ByteBuf buf, Class<T> cls) {
    ...
    try {
      ...
      throw new RuntimeException(
          "CorfuPayloads must include a ByteBuf " + "constructor! for class: " + cls.toString());
    } catch (IllegalAccessException e) {
      throw new RuntimeException(e);
    }
  }
  ...
  static <T> T fromBuffer(ByteBuf buf, TypeToken<T> token) {
    ...
    if (rawType.isAssignableFrom(Set.class)) {
      return (T)
          setFromBuffer(buf, token.resolveType(Set.class.getTypeParameters()[0]).getRawType());
    }
  ...
  /**
   * A really simple flat map implementation. The first entry is the size of the map as an int, and
   * the next entries are each key followed by its value. Maps of maps are currently not supported.
   *
   * @param buf The buffer to deserialize.
  ...
  /**
   * A really simple flat set implementation. The first entry is the size of the set as an int, and
   * the next entries are each value.
   *
   * @param buf The buffer to deserialize.
   * @param valueClass The class of the values.
   * @param <V> The type of the values.
  ...
  /**
   * A really simple flat list implementation. The first entry is the size of the set as an int, and
   * the next entries are each value.
   *
   * @param <V> The type of the values.
   * @param buf The buffer to deserialize.
