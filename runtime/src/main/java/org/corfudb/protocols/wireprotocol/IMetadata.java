...
public interface IMetadata {
  ...
  default Set<UUID> getStreams() {
    return ((Map<UUID, Long>)
            getMetadataMap()
                .getOrDefault(LogUnitMetadataType.BACKPOINTER_MAP, Collections.emptyMap()))
        .keySet();
  }
  ...
  default Map<UUID, Long> getBackpointerMap() {
    return (Map<UUID, Long>)
        getMetadataMap().getOrDefault(LogUnitMetadataType.BACKPOINTER_MAP, Collections.EMPTY_MAP);
  }
  ...
  default Long getGlobalAddress() {
    return Optional.ofNullable((Long) getMetadataMap().get(LogUnitMetadataType.GLOBAL_ADDRESS))
  ...
  default Long getEpoch() {
    if (getMetadataMap() == null || getMetadataMap().get(LogUnitMetadataType.EPOCH) == null) {
      return Layout.INVALID_EPOCH;
    }
    return Optional.ofNullable((Long) getMetadataMap().get(LogUnitMetadataType.EPOCH))
        .orElse(Layout.INVALID_EPOCH);
  }
  ...
  default CheckpointEntry.CheckpointEntryType getCheckpointType() {
    return (CheckpointEntry.CheckpointEntryType)
        getMetadataMap().getOrDefault(LogUnitMetadataType.CHECKPOINT_TYPE, null);
  }
  ...
  default UUID getCheckpointId() {
    return (UUID) getMetadataMap().getOrDefault(LogUnitMetadataType.CHECKPOINT_ID, null);
  }
  ...
  default UUID getCheckpointedStreamId() {
    return (UUID) getMetadataMap().getOrDefault(LogUnitMetadataType.CHECKPOINTED_STREAM_ID, null);
  }
  ...
  default Long getCheckpointedStreamStartLogAddress() {
    return (Long)
        getMetadataMap()
            .getOrDefault(
                LogUnitMetadataType.CHECKPOINTED_STREAM_START_LOG_ADDRESS, Address.NO_BACKPOINTER);
  }
  ...
  default Codec.Type getPayloadCodecType() {
    return (Codec.Type)
        getMetadataMap().getOrDefault(LogUnitMetadataType.PAYLOAD_CODEC, Codec.Type.NONE);
  }
  ...
  enum LogUnitMetadataType implements ITypedEnum {
    ...
    final int type;
    @Getter final TypeToken<?> componentType;
