...
/**
 * A tail request can be specific to the following types: - Global log tail. - Tails for specific
 * streams (uniquely identified). - Tails for all available streams.
 *
 * <p>Whenever tails are requested for streams (specific or all), the global log tail is sent along
 * the response.
 */
@Data
@AllArgsConstructor
public class TailsRequest implements ICorfuPayload<TailsRequest> {
  ...
  public static final byte STREAMS_TAILS =
      1; /* Log tail is also provided when tails are requested for
