...
public class TxResolutionInfo implements ICorfuPayload<TxResolutionInfo> {
  ...
  @Getter @Setter Token snapshotTimestamp;

  /** A set of poisoned streams, which have a conflict against all updates. */
  @Getter final Map<UUID, Set<byte[]>> conflictSet;

  @Getter final Map<UUID, Set<byte[]>> writeConflictParams;
   ...
   *
   * <p>The first entry is a long, the snapshot timestamp. The second is an int, the size of the
   * map. Next, entries are serialized one by one, first the key, then each value, itself a set of
   * objects.
   *
  ...
  public void doSerialize(ByteBuf buf) {
    ...
    writeConflictParams.entrySet().stream()
        .forEach(
            x -> {
              ICorfuPayload.serialize(buf, x.getKey());
              ICorfuPayload.serialize(buf, x.getValue());
  ...
  public String toString() {
    return "TXINFO[" + Utils.toReadableId(TXid) + "](ts=" + snapshotTimestamp + ")";
  }
}
