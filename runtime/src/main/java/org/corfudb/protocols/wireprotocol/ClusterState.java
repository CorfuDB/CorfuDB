...
public class ClusterState {
   ...
   * For instance, three node cluster: {"a": {"endpoint": "a", "connectivity":{"a": true, "b": true,
   * "c": true}}} {"b": {"endpoint": "b", "connectivity":{"a": true, "b": true, "c": false}}} {"c":
   * {"endpoint": "c", "connectivity":{"a": true, "b": false, "c": true}}}
   */
  @Singular private final ImmutableMap<String, NodeState> nodes;
  ...
  /**
   * See if cluster is ready. If cluster contains at least one node with state NOT_READY the cluster
   * is not ready and cluster state can't be used to find failures.
   *
  ...
  public boolean isReady() {
    ...
    // if at least one node is not ready then entire cluster is not ready to provide correct
    // information
  ...
  public static ClusterState buildClusterState(
    ...
    Map<String, NodeState> graph =
        Arrays.stream(states)
            .collect(
                Collectors.toMap(
                    state -> state.getConnectivity().getEndpoint(), Function.identity()));
