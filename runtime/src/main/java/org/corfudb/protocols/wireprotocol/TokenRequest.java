 ...
 *
 * <p>There are four token request scenarios, designated by the relevant constants : 0. {@link
 * TokenRequest::TK_QUERY} : Query of the current log tail and of specific stream-tails. 1. {@link
 * TokenRequest::TK_RAW} : Ask for raw (global) log token(s). This extends the global log tail by
 * the requested # of tokens. 2. {@link TokenRequest::TK_MULTI_STREAM} : Ask for token(s) on
 * multiple streams. This extends both the global log tail, and each of the specified stream tails,
 * by the requested # of tokens. 3. {@link TokenRequest::TK_TX} : First, check transaction
 * resolution. If transaction can commit, then behave like {@link TokenRequest::TK_MULTI_STREAM}.
 */
@Data
@AllArgsConstructor
