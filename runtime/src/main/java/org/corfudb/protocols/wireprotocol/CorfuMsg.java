...
public class CorfuMsg {
  ...
  /**
   * Constructor which generates a message based only the message type. Typically used for
   * generating error messages, since sendmessage will populate the rest of the fields.
   *
  ...
  public static CorfuMsg deserialize(ByteBuf buffer) {
    ...
    if (marker != markerField) {
      throw new RuntimeException(
          "Attempt to deserialize a message which is not a CorfuMsg, "
              + "Marker = "
              + marker
              + " but expected 0xC0FC0FC0");
    }
