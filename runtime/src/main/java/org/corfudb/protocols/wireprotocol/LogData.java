...
public class LogData implements ICorfuPayload<LogData>, IMetadata, ILogData {
            ...
            if (hasPayloadCodec()) {
              ...
              serializedBuf =
                  Unpooled.wrappedBuffer(
                      getPayloadCodecType()
                          .getInstance()
                          .decompress(ByteBuffer.wrap(compressedArrayBuf)));
            }

            final Object actualValue;
            try {
              actualValue = Serializers.CORFU.deserialize(serializedBuf, runtime);
    ...
    if (object instanceof ByteBuf) {
      ...
      if (object instanceof CheckpointEntry) {
        ...
        setCheckpointedStreamStartLogAddress(
            Long.parseLong(cp.getDict().get(CheckpointEntry.CheckpointDictKey.START_LOG_ADDRESS)));
      }
    }
  }
  ...
  /**
   * LogData are considered equals if clientId and threadId are equal. Here, it means or both of
   * them are null or both of them are the same.
   *
   * @param o
   * @return
   */
  ...
  public boolean equals(Object o) {
    if (o == this) {
      ...
      if (compareTo(other) == 0) {
        boolean sameClientId =
            getClientId() == null
                ? other.getClientId() == null
                : getClientId().equals(other.getClientId());
        boolean sameThreadId =
            getThreadId() == null
                ? other.getThreadId() == null
