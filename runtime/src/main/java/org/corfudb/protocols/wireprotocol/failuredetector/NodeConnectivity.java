...
public class NodeConnectivity
    implements ICorfuPayload<NodeConnectivity>, Comparable<NodeConnectivity> {
  /** Node name */
  @Getter @NonNull private final String endpoint;

  @Getter @NonNull private final NodeConnectivityType type;

  /** Node connectivity information, contains connection status to other nodes */
  @Getter @NonNull private final ImmutableMap<String, ConnectionStatus> connectivity;
  ...
  public NodeConnectivity(ByteBuf buf) {
    ...
    ICorfuPayload.mapFromBuffer(buf, String.class, String.class)
  ...
  /**
   * Contains list of servers disconnected from this node. If the node A can't ping node B then node
   * B will be added to failedNodes list.
   */
  ...
  public ConnectionStatus getConnectionStatus(String node) {
    ...
    if (!connectivity.containsKey(node)) {
      String error =
          String.format(
              "%s: opposite node not found for %s. Connectivity: %s", endpoint, node, connectivity);
      throw new IllegalStateException(error);
    }
  ...
  public static NodeConnectivity connectivity(
      String endpoint, ImmutableMap<String, ConnectionStatus> connectivity) {
  ...
  public enum NodeConnectivityType {
    /**
     * Current node is not ready yet, missing information about current node state. Node is not
     * bootstrapped and/or didn't ping other nodes yet.
     */
    NOT_READY,
