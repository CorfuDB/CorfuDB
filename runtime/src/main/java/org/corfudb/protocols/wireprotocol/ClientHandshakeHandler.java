 ...
 *
 * <p>- Once the client connects to the server, it sends a handshake message that contains: its own
 * id and the (asserted) server's node id. - The server validates and replies with its node id and
 * current version of Corfu. - If validation is correct on both sides, message exchange is initiated
 * between client-server, otherwise, the handshake times out, and either server or client close the
 * connection.
 *
 * <p>Created by amartinezman on 12/8/17.
 */
@Slf4j
public class ClientHandshakeHandler extends ChannelDuplexHandler {
  ...
  /**
   * Creates a new ClientHandshakeHandler which will handle the handshake between the current client
   * and a remote server.
   *
  ...
  public void channelRead(ChannelHandlerContext ctx, Object m) throws Exception {
    ...
    try {
      ...
      log.info(
          "channelRead: Handshake Response received. Removing {} from pipeline.",
      ...
      log.warn(
          "channelRead: Non-handshake message received by handshake handler. "
    ...
    if (this.nodeId.equals(UUID.fromString("00000000-0000-0000-0000-000000000000"))) {
      ...
      log.debug(
          "channelRead: Client opened socket to server [{}] instead, connected to: [{}]",
          this.nodeId,
          serverId);
  ...
  public void channelActive(ChannelHandlerContext ctx) throws Exception {
    log.info(
        "channelActive: Outgoing connection established to: {}", ctx.channel().remoteAddress());

    // Write the handshake & add a timeout listener.
    CorfuMsg handshake =
        CorfuMsgType.HANDSHAKE_INITIATE.payloadMsg(new HandshakeMsg(this.clientId, this.nodeId));
    ...
    ctx.pipeline()
        .addBefore(ctx.name(), READ_TIMEOUT_HANDLER, new ReadTimeoutHandler(this.handshakeTimeout));
  }
