...
/**
 * Utilities for common options parsing and session configuration for encrypted & authenticated TLS
 * sessions.
 */
@Slf4j
public class TlsUtils {
  ...
  public static KeyStore openKeyStore(String keyStorePath, String password) throws SSLException {
    ...
    try {
      ...
      String errorMessage =
          "Error creating a key store object of algorithm [" + KeyStore.getDefaultType() + "].";
      log.error(errorMessage, e);
      throw new SSLException(errorMessage, e);
    } finally {
   ...
   * @return If file path is provided, returns the content, otherwise an empty string.
   * @throws SSLException Wraps the IOException.
   */
