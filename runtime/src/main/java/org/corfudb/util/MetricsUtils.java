...
public class MetricsUtils {
  ...
  private static final String PROPERTY_LOCAL_METRICS_COLLECTION = "corfu.local.metrics.collection";
  ...
  @Getter private static boolean metricsCollectionEnabled = false;
   ...
   *
   * <ul>
   *   <li>metricsCollectionEnabled: Boolean taken from jvm corfu.metrics.collection property to
   *       enable the metrics collection.
   *   <li>metricsJmxReportingEnabled: Boolean taken from jvm corfu.metrics.jmxreporting property to
   *       enable jmx reporting of metrics.
   *   <li>metricsJvmCollectionEnabled: Boolean taken from jvm corfu.metrics.jvm property for
   *       enabling reporting on jmv metrics such as garbage collection, threads, and memory
   *       consumption.
   *   <li>metricsLogAnalysisEnabled: Boolean taken from jvm corfu.metrics.log.analysis property for
   *       enabling reporting on logger statistics.
   *   <li>metricsLogInterval: Integer taken from jvm corfu.metrics.log.interval property for
   *       enabling and setting the intervals of reporting to log output (in seconds). A positive
   *       value indicates the reporting is enabled at provided intervals.
   *   <li>metricsCsvInterval: Integer taken from jvm corfu.metrics.csv.interval property for
   *       enabling and setting the intervals of reporting to csv (in seconds). A positive value
   *       indicates the reporting is enabled at provided intervals.
   *   <li>metricsCsvFolder: String taken from jvm corfu.metrics.csv.folder property for destination
   *       path of csv reporting.
   * </ul>
   *
   * <p>This method will be called to set the value of above-mentioned properties for collection and
   * reporting. For example using the following jvm flags will enable collection of corfu, jvm, and
   * log statistics and their reporting through logs, csv, and jmx.
   *
   * <p>{@code -Dcorfu.metrics.collection=True -Dcorfu.metrics.csv.interval=30
   * -Dcorfu.metrics.csv.folder=/tmp/csv5 -Dcorfu.metrics.jmxreporting=True
   * -Dcorfu.metrics.log.analysis=True -Dcorfu.metrics.jvm=True -Dcorfu.metrics.log.interval=60}
   */
  private static void loadVmProperties() {
    metricsLocalCollectionEnabled =
        Boolean.valueOf(System.getProperty(PROPERTY_LOCAL_METRICS_COLLECTION));

    metricsJmxReportingEnabled = Boolean.valueOf(System.getProperty(PROPERTY_JMX_REPORTING));
    metricsJvmCollectionEnabled =
        Boolean.valueOf(System.getProperty(PROPERTY_JVM_METRICS_COLLECTION));
   ...
   * @return a boolean representing whether metrics reporting has been already set up. earlier
   */
   ...
   * Reporting can be turned on and off via the system properties described in loadVmProperties()'s
   * docs. The report interval and report directory cannot be altered at runtime.
   *
  ...
  public static void metricsReportingSetup(
      @NonNull MetricRegistry metricRegistry, int prometheusMetricsPort) {
    ...
    metricsCollectionEnabled = true;
    log.info(
        "Metrics exporting via Prometheus has been enabled at port {}.", prometheusMetricsPort);
  }
  ...
  private static void setupCsvReporting(MetricRegistry metrics) {
    ...
    if (!directory.isDirectory() || !directory.exists() || !directory.canWrite()) {
      log.warn(
          "Provided CSV directory : {} doesn't exist, isn't a directory, or "
              + "not accessible for writes. Disabling CSV Reporting.",
          directory);
  ...
  /**
   * This method creates object size gauge and registers it to the metrics registry. The method
   * guarantees that no strong reference to the provided object will be retained. Hence it does not
   * prevent garbage collection if the client does not hold a strong reference to the object being
   * measured.
   *
   * @param metrics the metrics registry to which the size gauge will be registered.
   * @param object the object which its size will be measured with a corresponding gauge*
   * @return a gauge that provides an estimate of deep size for the provided object as long as there
   *     exist a strong reference to that object.
   */
  public static Gauge<Long> addMemoryMeasurerFor(@NonNull MetricRegistry metrics, Object object) {
    String simpleClassName = object == null ? "Object" : object.getClass().getSimpleName();

    return metrics.register(
        String.format("deep-sizeof.%s@%04x", simpleClassName, System.identityHashCode(object)),
        getSizeGauge(object));
  }

  /**
   * This method creates a gauge that returned the deep size estimation of an object while there is
   * a reference to it in the code. Otherwise it will return 0 as the object can be claimed by
   * Garbage collection. Note that this gauge guarantees that it will not hold any strong reference
   * to the object that it measures.
   *
   * @param object an object to which its size will be estimated using the return gauge
   * @return a gauge that provides an estimate of deep size for the provided object as long as there
   *     exist a strong reference to that object.
   */
  private static Gauge<Long> getSizeGauge(Object object) {
    ...
    return () -> {
      ...
      return (objectOfInterest != null) ? sizeOf.deepSizeOf(objectOfInterest) : 0L;
    };
  }

  /**
   * This method adds different properties of provided instance of {@link Cache} to the indicated
   * metrics registry. The added properties of cache are:
   *
   * <p>cache.object-counts: estimated size of cache. cache.evictions : number of cache evictions
   * cache.hit-rate : hit rate of cache. cache.hits : cache.misses :
   *
   * @param metrics
   * @param cache
   */
  public static void addCacheMeasurerFor(@NonNull MetricRegistry metrics, Cache cache) {
    try {
      metrics.register(
          "cache.object-counts." + cache.toString(), (Gauge<Long>) cache::estimatedSize);
      metrics.register(
          "cache.evictions." + cache.toString(), (Gauge<Long>) cache.stats()::evictionCount);
      metrics.register(
          "cache.hit-rate." + cache.toString(), (Gauge<Double>) cache.stats()::hitRate);
      metrics.register("cache.hits." + cache.toString(), (Gauge<Long>) cache.stats()::hitCount);
      metrics.register("cache.misses." + cache.toString(), (Gauge<Long>) cache.stats()::missCount);
    } catch (IllegalArgumentException e) {
      // Re-registering metrics during test runs, not a problem
    }
  }
