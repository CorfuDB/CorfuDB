...
public final class CFUtils {
  ...
  public static <
          T, A extends Throwable, B extends Throwable, C extends Throwable, D extends Throwable>
  ...
  public static <T, A extends Throwable, B extends Throwable, C extends Throwable>
      T getUninterruptibly(
          Future<T> future, Class<A> throwableA, Class<B> throwableB, Class<C> throwableC)
          throws A, B, C {
  ...
  public static <T, A extends Throwable, B extends Throwable> T getUninterruptibly(
    ...
    return getUninterruptibly(
        future, throwableA, throwableB, RuntimeException.class, RuntimeException.class);
  }

  public static <T, A extends Throwable> T getUninterruptibly(Future<T> future, Class<A> throwableA)
      throws A {
    return getUninterruptibly(
  ...
  /**
   * Generates a completable future which times out. inspired by NoBlogDefFound:
   ...
   * not, it is cancelled and completes exceptionally with TimeoutException. inspired by
   * NoBlogDefFound: www.nurkiewicz.com/2014/12/asynchronous-timeouts-with.html
   *
  ...
  public static <T> CompletableFuture<List<T>> sequence(List<CompletableFuture<T>> futures) {
    return allOf(futures)
        .thenCompose(
            empty -> {
              CompletableFuture<List<T>> aggregated =
                  CompletableFuture.completedFuture(new ArrayList<>());

              for (CompletableFuture<T> future : futures) {
                aggregated =
                    aggregated.thenCombine(
                        future,
                        (List<T> list, T value) -> {
                          list.add(value);
                          return list;
   ...
   * @return A new CompletableFuture that is completed normally when all of the given
   *     CompletableFutures complete. If at least one of the futures complete exceptionally, it
   ...
   */
  public static <T> CompletableFuture<T> runFutureAfter(
   ...
   */
  public static <A extends Throwable> void unwrap(Throwable throwable, Class<A> throwableA)
      throws A {
