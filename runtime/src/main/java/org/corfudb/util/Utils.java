...
public class Utils {
  ...
  /**
   * Convert a byte array to a hex string. Source: https://stackoverflow.com/questions/9655181/
  ...
  /**
   * Convert to byte string representation. from
  ...
  public static String convertToByteStringRepresentation(final long value) {
    ...
    for (int i = 0; i < dividers.length; i++) {
      ...
      if (value >= divider) {
        final double cresult = divider > 1 ? (double) value / (double) divider : (double) value;
  ...
  public static Token getTrimMark(RuntimeLayout runtimeLayout) {
    ...
    List<CompletableFuture<Long>> futures =
        layout.getAllLogServers().stream()
            .map(runtimeLayout::getLogUnitClient)
            .map(LogUnitClient::getTrimMark)
            .collect(Collectors.toList());

    long trimMark =
        futures.stream().map(CFUtils::getUninterruptibly).max(Comparator.naturalOrder()).get();
  ...
  public static void prefixTrim(RuntimeLayout runtimeLayout, Token address) {
    List<CompletableFuture<Void>> futures =
        runtimeLayout.getLayout().getAllLogServers().stream()
            .map(runtimeLayout::getLogUnitClient)
  ...
  public static long getCommittedTail(RuntimeLayout runtimeLayout) {
    ...
    return futures.stream().map(CFUtils::getUninterruptibly).max(Comparator.naturalOrder()).get();
  }
   ...
   */
  public static void updateCommittedTail(RuntimeLayout runtimeLayout, long newCommittedTail) {
   ...
   * unit in every stripe. QUORUM: Block on fetch of global log tail from a majority in every
   * stripe.
   *
   * @param runtimeLayout current RuntimeLayout
  ...
  /**
   * Given a set of request tails, we aggregate them and maintain the greatest address per stream
   * and the greatest tail over all responses.
   *
  ...
  static Map<UUID, StreamAddressSpace> aggregateStreamAddressMap(
    ...
    for (Map.Entry<UUID, StreamAddressSpace> stream : streamAddressSpaceMap.entrySet()) {
      if (aggregated.containsKey(stream.getKey())) {
        ...
        aggregated
            .get(stream.getKey())
            .setTrimMark(Math.max(currentTrimMark, stream.getValue().getTrimMark()));
      } else {
