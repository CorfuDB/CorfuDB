...
public class ExponentialBackoffRetry<
  ...
  @Getter @Setter private Duration backoffDuration = DEFAULT_BACKOFF_DURATION;

  /** Base to multiply. */
  @Getter @Setter private int base = DEFAULT_BASE;

  /**
   * Portion of the number to be randomized, between 0 and 1. 0 - no random portion, 1 - randomize
   * everything.
   */
  @Getter @Setter private float randomPortion = DEFAULT_RANDOM_PORTION;

  /** Additional fixed retry time in milliseconds for each retry. */
  @Getter @Setter private long extraWait = 0;

  /** Maximum duration at which the wait time needs to be capped. */
  @Getter @Setter private Duration maxRetryThreshold = Duration.ZERO;
