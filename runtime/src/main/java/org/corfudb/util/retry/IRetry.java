...
/**
 * The IRetry interface is used to support retry loops by using different strategies for the pause
 * between the retries. It is done by a builder, that needs the correct strategy and the lambda to
 * be executed. The code in the run function is responsible to handle the exceptions and to decide
 * whether a retry is needed or the exception should be thrown outside.
 *
...
public interface IRetry<
  ...
  static <R, Z extends IRetry>
      IRetry<RuntimeException, RuntimeException, RuntimeException, RuntimeException, R, Z> build(
          Class<Z> retryType,
          IRetryable<RuntimeException, RuntimeException, RuntimeException, RuntimeException, R>
              runFunction) {
    return build(
        retryType,
  ...
  static <T extends Exception, R, Z extends IRetry>
      IRetry<T, RuntimeException, RuntimeException, RuntimeException, R, Z> build(
          Class<Z> retryType,
  ...
  static <T extends Exception, U extends Exception, R, Z extends IRetry>
      IRetry<T, U, RuntimeException, RuntimeException, R, Z> build(
          Class<Z> retryType,
          Class<? extends T> firstExceptionType,
  ...
  static <T extends Exception, U extends Exception, V extends Exception, R, Z extends IRetry>
      IRetry<T, U, V, RuntimeException, R, Z> build(
          Class<Z> retryType,
          Class<? extends T> firstExceptionType,
          Class<? extends U> secondExceptionType,
  ...
  static <
    ...
    return (IRetry<T, U, V, W, R, Z>)
        retryType.getConstructor(IRetryable.class).newInstance(runFunction);
  }

  /** Run the retry. This function may never return. */
  default O run() throws E, F, G, H, InterruptedException {
    while (true) {
      try {
  ...
  /**
   * Configure settings for the underlying class. The consumer will be given access to the
   * underlying retry type to configure type-specific options.
   *
