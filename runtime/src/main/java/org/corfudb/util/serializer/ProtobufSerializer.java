 ...
 *
 * <p>To achieve this, this serializer requires a map of all seen class types or a classMap and
 * Google Protobuf 3's Any. Any type carries with it a typeUrl which helps identify the class
 * uniquely. This typeUrl is then used to index the classMap to retrieve the actual Protobuf message
 * while deserializing.
 */
@Slf4j
  ...
  public Object deserialize(ByteBuf b, CorfuRuntime rt) {
    ...
    try (ByteBufInputStream bbis = new ByteBufInputStream(b)) {
      ...
      if (!classMap.containsKey(payload.getTypeUrl())) {
        log.error(
            "Deserialization error: Encountered a log update for this class "
                + payload.getTypeUrl()
                + " but its corresponding class type cannot be found in in-memory type map."
                + " Dumping map..\n");
