...
public interface ISerializer {
  ...
  Map<Class<?>, Function<?, byte[]>> customHashingMap = new ConcurrentHashMap<>();
   ...
   * @return A function to generate a hash, or null, if no custom hasher was registered.
   */
  ...
  Map<Class<?>, Function<?, byte[]>> hashConversionMap =
      ImmutableMap.<Class<?>, Function<?, byte[]>>builder()
          .put(
              String.class,
              (String o) -> Utils.longToBigEndianByteArray(LongHashFunction.xx().hashChars(o)))
          .put(
              UUID.class,
              (UUID o) -> {
  ...
  /**
   * Given an object, generate a hash for it. This hash is used internally by Corfu for conflict
   * resolution.
   *
   * <p>The default implementation uses xxHash, a fast non-cryptographic hash algorithm on the
   * serialized payload.
   *
   * <p>It tries to be smart about some primitive types, not serializing them if possible to
  ...
  default byte[] hash(Object o) {
    ...
    Function<Object, byte[]> conversionFunc = (Function<Object, byte[]>) hashConversionMap.get(cls);
    if (conversionFunc != null) {
      // If we know how to convert this object quickly, do that.
      return conversionFunc.apply(o);
    } else if ((conversionFunc = (Function<Object, byte[]>) getCustomHasher(cls)) != null) {
