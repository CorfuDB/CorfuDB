...
public class ReflectionUtils {
  ...
  static final Map<String, Class> primitiveTypeMap =
  ...
  /**
   * Given the constructor arguments and the desired class type, find the closest matching
   * constructor. For example, if a wrapper class contains two constructors, Constructor(A) and
   * Constructor(B), and the constructor argument is of type B, where B inherits from A, instantiate
   * the object via Constructor(B) since it is closes with respect to type hierarchy.
   *
  ...
  public static <T> Object findMatchingConstructor(
    ...
    final List<Constructor> constructorCandidates =
    ...
    for (Constructor candidate : constructorCandidates) {
      ...
      matchingConstructors.put(resolutionList.stream().reduce(0, Integer::sum), candidate);
    }
   ...
   * @return 0 if if one Class cannot be assigned to a variable of another Class. Positive integer
   *     otherwise.
   */
  private static int maxDepth(Class parameterType, Class argumentType, int currentDepth) {
    if (!ClassUtils.isAssignable(argumentType, parameterType, true)) {
      ...
      return results.stream().max(Comparator.comparing(Integer::valueOf)).orElse(newDepth);
    }
  }
