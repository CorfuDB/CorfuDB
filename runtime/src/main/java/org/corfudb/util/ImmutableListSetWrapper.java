...
/**
 * An immutable set implementation that is backed by a list. This type is useful when we have lists
 * that we know conform to the set constraints, but creating the set is too expensive.
 */
public class ImmutableListSetWrapper<E> extends AbstractSet<E> implements Set<E> {
  ...
  public static <K, V> ImmutableListSetWrapper<Entry<K, V>> fromMap(@NonNull Map<K, V> original) {
    return new ImmutableListSetWrapper<>(
