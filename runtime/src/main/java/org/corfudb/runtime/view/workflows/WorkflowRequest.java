 ...
 *
 * <p>Created by Maithem on 1/19/18.
 */
@Slf4j
public abstract class WorkflowRequest {
  ...
  @NonNull protected String nodeForWorkflow;
  ...
  /**
   * Select an orchestrator on the node that responds to pings and is not on the same node affected
   * by the workflow. The layout might not reflect the state of the responsive servers, so we ping
   * the endpoint before we select it. An example of this would be a 3 node cluster, with two nodes
   * that die immediately, the layout won't have those two nodes as unresponsive because it can't
   * commit to a quorum.
   *
   * @return a management client that is connected to the selected orchestrator
   */
  protected Optional<ManagementClient> getOrchestrator() {
    ...
    List<String> availableLayoutServers =
    ...
    for (String endpoint : availableLayoutServers) {
      ...
      if (client.pingSync()) {
        ...
        managementClient =
            Optional.of(
                runtime.getLayoutView().getRuntimeLayout(layout).getManagementClient(endpoint));
        break;
      }
    }
   ...
   * @return true if the operation is reflected in the layout, otherwise return false
   */
  protected abstract boolean verifyRequest(Layout layout);

  /**
   * @param workflow the workflow id to poll
   * @param client a client that is connected to the orchestrator that is running the workflow
   ...
   * @throws TimeoutException if the workflow doesn't complete within the timeout period
   */
  private void waitForWorkflow(
      @NonNull UUID workflow,
      @NonNull ManagementClient client,
   ...
   *
   * <p>This method will succeed only if the workflow executed successfully, otherwise it can throw
   * a WorkflowResultUnknownException when the timeouts are exhausted and the expected side effect
   * cannot be verified.
   *
