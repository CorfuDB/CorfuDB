...
/**
 * Globally Unique Identity generator that returns ids with weak comparable ordering with minimal
 * distributed state sync. On startup increment a globally unique id via Corfu Object. Use this id
 * with locally incrementing values to return unique ids.
 *
 * <p>Created by Sundar Sridharan on 5/22/19.
 */
class CorfuGuid {
  ...
  @Getter private final long uniqueInstanceId;

  CorfuGuid(
      long UTCtimestamp, int driftCorrection, int resolutionCorrection, long uniqueInstanceId) {
  ...
  /**
   * Construct the CorfuGuid with the UTC + GloballyUniqueCounter + Corrections There are 2 types of
   * corrections necessary to ensure uniqueness: 1. UTC wall clock time can drift backwards - Drift
   * Adjust. 2. UTC resolution may be coarse if many threads try to generate a guid simultaneously -
   * Resolution Adjust.
   *
   * <p>Can tolerate up to 16 NTP time adjustments without global state sync. Can tolerate up to 65
   * seconds of time drifts without loss of ordering.
   * +---------------+--------------+---------------+-------------+-------------------+ | Timestamp
   * MSB | Drift Adjust | Timestamp LSB | Instance ID | Resolution Adjust |
   * +---------------+--------------+---------------+-------------+-------------------+ <----24
   * bits----><----4 bits---><----16 bits---><---16 bits--><-------4 bits----->
   *
  ...
  CorfuGuid(long encodedTs) {
    ...
    timestamp =
        (currentTimestamp & (~(TIMESTAMP_MSB_MASK | TIMESTAMP_LSB_MASK)))
            | timestampMSB
            | timestampLSB;
  ...
  public String toString() {
    return String.format(
        "%s: drift: %d resolution: %d instanceId %d",
...
public class CorfuGuidGenerator implements OrderedGuidGenerator {
  ...
  public static synchronized CorfuGuidGenerator getInstance(CorfuRuntime rt) {
  ...
  /**
   * Sync up with the distributed state using a Corfu Transaction Synchronous Network RPCs among
   * other overheads
   */
    ...
    while (true) {
      try {
        ...
        if (TransactionalContext.isInTransaction()) {
          txnType = TransactionalContext.getCurrentContext().getTransaction().getType();
        } else { // Pick default type as OPTIMISTIC
          txnType = TransactionType.OPTIMISTIC;
        }
        runtime.getObjectsView().TXBuild().type(txnType).build().begin();
  ...
  /**
   * 1. Get Current timestamp and compare it with previous timestamp -> if time has drifted
   * backwards bump up driftCorrection -> if driftCorrection has maxed out, bump up instance Id
   * (loss of ordering) -> if time is same as previous -> bump up resolutionCorrection -> if
   * resolutionCorrection has maxed out, bump up instance Id (possible loss of ordering)
   *
  ...
  private synchronized CorfuGuid getCorrectedTimestamp() {
    ...
    if (currentTimestamp < previousTimestamp) {
      ...
      if (driftCorrection > CorfuGuid.MAX_CORRECTION) {
        log.info(
            "updateInstanceId: Time went backward too many times "
