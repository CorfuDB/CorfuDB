 ...
 *
 * <p>A replication protocol exposes three public functions, which permit reading and writing to the
 * log.
 *
...
public interface IReplicationProtocol {
   ...
   *
   * <p>This function blocks until -a- write at the global address is committed to the log.
   *
   * <p>If the write which is committed to the log was this write, the function returns normally.
   *
   * <p>If the write which was committed to the log was not the result of this call, an
   * OverwriteException is thrown.
   *
   ...
   * @throws OverwriteException If a write was committed to the log and it was not the result of
   *     this call.
   */
   ...
   *
   * <p>This function only returns committed data. If the address given has not committed, the
   * implementation may either block until it is committed, or commit a hole filling entry to that
   * address.
   *
   ...
   *
   * <p>This method functions exactly like a read, except that it returns the result for multiple
   * addresses.
   *
   * <p>An implementation may optimize for this type of bulk request, but the default implementation
   ...
   *
   * <p>This function -may- return null if there was no entry committed at the given global address,
   * otherwise it returns committed data at the given global address. It does not attempt to hole
   * fill if there was no entry.
   *
   ...
   * @return the data that was committed at the given global address, or NULL, if there was no entry
   *     committed.
   */
