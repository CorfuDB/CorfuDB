...
public class ChainReplicationProtocol extends AbstractReplicationProtocol {
    ...
    try (ILogData.SerializationHandle sh = data.getSerializedForm()) {
      ...
      try {
        CFUtils.getUninterruptibly(
            runtimeLayout.getLogUnitClient(globalAddress, 0).write(sh.getSerialized()),
  ...
  public ILogData peek(RuntimeLayout runtimeLayout, long globalAddress) {
    ...
    ILogData peekResult =
        CFUtils.getUninterruptibly(
                runtimeLayout.getLogUnitClient(globalAddress, numUnits - 1).read(globalAddress))
            .getAddresses()
            .get(globalAddress);
   ...
   *
   * <p>- This method optimizes for the time to wait to hole fill in case empty addresses are
   * encountered. - If the waitForWrite flag is set to true, when an empty address is encountered,
   * it waits for one hole to be filled. All the rest empty addresses within the list are hole
   * filled directly and the reader does not wait. - In case the flag is set to false, none of the
   * reads wait for write completion and the empty addresses are hole filled right away.
   *
  ...
  @Override
  @Nonnull
  public Map<Long, ILogData> readAll(
    ...
    List<CompletableFuture<ReadResponse>> futures =
        serverAddressMap.entrySet().stream()
  ...
  public void commitAll(RuntimeLayout runtimeLayout, Collection<Long> addresses) {
    ...
    List<CompletableFuture<InspectAddressesResponse>> futures =
        serverAddressMap.entrySet().stream()
  ...
  private Map<LogUnitClient, List<Long>> groupAddressByLogUnit(
      RuntimeLayout runtimeLayout, Collection<Long> addresses) {
  ...
  private Map<Long, ILogData> waitOrHoleFill(
      RuntimeLayout runtimeLayout, Map<Long, LogData> readResult, boolean waitForWrite) {
  ...
  /**
   * Propagate a write down the chain, ignoring any overwrite errors. It is expected that the write
   * has already successfully completed at the head of the chain.
   *
   ...
   */
  private void propagate(RuntimeLayout runtimeLayout, long globalAddress, @Nullable ILogData data) {
    int numUnits = runtimeLayout.getLayout().getSegmentLength(globalAddress);

    for (int i = 1; i < numUnits; i++) {
      log.trace(
          "Propagate[{}]: chain {}/{}",
          Token.of(runtimeLayout.getLayout().getEpoch(), globalAddress),
          i + 1,
          numUnits);
      ...
      try {
        if (data != null) {
          CFUtils.getUninterruptibly(
              runtimeLayout.getLogUnitClient(globalAddress, i).write(data),
              OverwriteException.class);
        } else {
          Token token = new Token(runtimeLayout.getLayout().getEpoch(), globalAddress);
          LogData hole = LogData.getHole(token);
          CFUtils.getUninterruptibly(
              runtimeLayout.getLogUnitClient(globalAddress, i).write(hole),
              OverwriteException.class);
        }
      } catch (OverwriteException oe) {
  ...
  /**
   * Recover a failed write at the given global address, driving it to completion by invoking the
   * recovery protocol.
   *
   * <p>When this function returns the given globalAddress is guaranteed to contain a committed
   * value.
   *
   * <p>If there was no data previously written at the address, this function will throw a runtime
   * exception. The recovery protocol should -only- be invoked if we previously were overwritten.
   *
  ...
  private void recover(RuntimeLayout runtimeLayout, long globalAddress) {
    ...
    log.warn(
        "Recover[{}]: read chain head {}/{}",
        Token.of(runtimeLayout.getLayout().getEpoch(), globalAddress),
        1,
        numUnits);
    ILogData ld =
        CFUtils.getUninterruptibly(
                runtimeLayout.getLogUnitClient(globalAddress, 0).read(globalAddress))
            .getAddresses()
    ...
    for (int i = 1; i < numUnits; i++) {
      ...
      try {
        CFUtils.getUninterruptibly(
            runtimeLayout.getLogUnitClient(globalAddress, i).write(ld), OverwriteException.class);
  ...
  protected void holeFill(RuntimeLayout runtimeLayout, long globalAddress) {
    ...
    try {
      ...
      LogData hole = LogData.getHole(token);
      CFUtils.getUninterruptibly(
          runtimeLayout.getLogUnitClient(globalAddress, 0).write(hole), OverwriteException.class);
