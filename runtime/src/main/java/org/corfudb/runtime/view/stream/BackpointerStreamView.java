  ...
  protected boolean discoverAddressSpace(
    ...
    while (currentAddress > stopAddress && Address.isAddress(currentAddress)) {
      ...
      try {
        ...
        if (getReadOptions().isIgnoreTrim()) {
          log.warn(
              "followBackpointers: Ignoring trimmed exception for address[{}]," + " stream[{}]",
              currentAddress,
              id);
      ...
      if (d.containsStream(streamId)) {
        log.trace(
            "followBackpointers: address[{}] contains streamId[{}], apply filter",
            currentAddress,
            streamId);
        ...
        if (!checkpoint) {
          log.trace(
              "followBackpointers: Adding backpointer to address[{}] to queue", currentAddress);
      ...
      if (d.hasBackpointer(streamId)) {
        ...
        if (Address.isAddress(tmp) || tmp == Address.NON_EXIST) {
          ...
          if (!startingSingleStep) {
            ...
            log.info(
                "followBackpointers[{}]: Found backpointer for this stream at address {}."
                    + "Stop single step downgrade.",
                this,
                currentAddress);
      ...
      if (singleStep) {
        if (startingSingleStep) {
          ...
          log.info(
              "followBackpointers[{}]: Found hole at address {}. Starting single step downgrade.",
              this,
              currentAddress);
        }
        ...
        log.trace(
            "followBackpointers[{}]: downgrading to single step, found hole at {}",
            this,
            currentAddress);
