...
/**
 * This class provides a builder that takes an SMR object definition with some options, wraps it
 * with a proxy and returns a object reference consumable through the ObjectsView (i.e. becomes
 * replicated and transactional)
 *
 * <p>Created by mwei on 4/6/16.
 */
@Slf4j
@Getter
...
public class SMRObject<T extends ICorfuSMR<T>> {
  ...
  @NonNull private final CorfuRuntime runtime;
  ...
  @NonNull private final ISerializer serializer;

  @NonNull private final ObjectOpenOption option;

  @NonNull private final Object[] arguments;
  ...
  public static class Builder<T extends ICorfuSMR<T>> {
    ...
    @Getter private CorfuRuntime runtime;
    @Getter public UUID streamID;
    ...
    public T open() {
      ...
      try {
        log.info(
            "ObjectBuilder: open Corfu stream {} id {}",
            smrObject.getStreamName(),
                  ...
                  .computeIfAbsent(
                      ...
                      x -> {
                        try {
                          ...
                          // Get object serializer to check if we didn't attempt to set another
                          // serializer
                          // to an already existing map
                          ISerializer objectSerializer =
                              ((CorfuCompileProxy) ((ICorfuSMR) result).getCorfuSMRProxy())
                                  .getSerializer();

                          if (smrObject.getSerializer() != objectSerializer) {
                            log.warn(
                                "open: Attempt to open an existing object with a different"
        ...
        log.error(
            "Runtime instrumentation no longer supported and no compiled class found" + " for {}",
            type,
            ex);
        throw new UnrecoverableCorfuError(ex);
      }
