...
public class LayoutBuilder {
   ...
   *
   * @param endpoints a non null set of Strings representing endpoints to be added to the list of
   *     layout's unresponsive servers.
   * @return this builder
   */
   ...
   *
   * @param endpoints a non null set of endpoints to be removed from the layout's list of
  ...
  /**
   * Removes the Layout Server passed. If the layout does not include the endpoint no action will
   * take place.
   *
   * @param endpoint a non null string representing the Layout Server endpoint that needs to be
   *     removed
   * @return this builder
  ...
  public LayoutBuilder removeLayoutServer(@NonNull String endpoint) {
    ...
    if (layout.getLayoutServers().size() == 1
      ...
      log.warn(
          "Skipped removing layout server as {} is the last remaining layout servers.", endpoint);
  ...
  /**
   * Removes all the Layout Server endpoints present in the set passed. This method first checks
   * that removing the provided endpoints does not remove all the Layout Servers in the layout. In
   * case that at least one layout server remains after the removal. It proceeds with removing
   * Layout Server endpoints provided while silently dismissing any of provided endpoints which are
   * not included as one of the Layout Servers in the layout.
   *
   * @param endpoints a non null set of Strings representing Layout server endpoints to be removed
   *     from the layout.
   * @return this builder
   * @throws LayoutModificationException is thrown if removing the provided set of endpoints result
   *     in removing all the remaining Layout Servers in the layout.
   */
  public LayoutBuilder removeLayoutServers(@NonNull Set<String> endpoints) {
    ...
    if (endpoints.containsAll(layout.getLayoutServers())) {
      log.warn(
          "Skipped removing layout servers as remove request of {} would remove all "
              + "layout servers {}.",
          endpoints.toString(),
   ...
   *
   * @param endpoint a non null String representing the Layout Server endpoint to be added.
   * @return this builder
   */
   ...
   *
   * @param endpoint a non null String representing the Sequencer Server endpoint to be added.
   * @return this builder
   */
  ...
  /**
   * Adds a new log unit server. If the last segment's start address is equal to 'globalLogTail' +
   * 1, this segment does not have any new data, so it does not need to be split. Add a new log unit
   * endpoint to this segment's specified stripe index. Otherwise, split the latest open segment,
   * such that the the latest open segment (which has its end address as infinity) is now closed at
   * address 'globalLogTail' + 1 and a new segment is opened for all the future writes. This new
   * open segment starts at exclusive 'globalLogTail' + 1 and ends at infinity. This new segment
   * contains the new logunit server to be added in the specified stripe.
   *
   ...
   * @return this builder
   */
  public LayoutBuilder addLogunitServer(
      int stripeIndex, long globalLogTail, @NonNull String newLogunitEndpoint) {
    ...
    if (lastSegment.getStart() == globalLogTail + 1) {
      ImmutableList<String> logServers =
      ...
      LayoutSegment openSegment =
          new LayoutSegment(
              lastSegment.getReplicationMode(),
    ...
    List<String> logunitServerList =
        new ArrayList<>(lastSegment.getStripes().get(stripeIndex).getLogServers());
  ...
  /**
   * Merges the specified segment and the segment before this. No addition or removal of stripes are
   * allowed. Only 1 log unit server addition/removal allowed between segments.
   *
  ...
  public LayoutBuilder mergePreviousSegment(int segmentIndex) {
    ...
    List<LayoutSegment> layoutSegmentList = layout.getSegments();
    if (layoutSegmentList.get(segmentIndex).start != layoutSegmentList.get(segmentIndex - 1).end) {
    ...
    for (int i = 0; i < oldSegmentStripeList.size(); i++) {
      Set<String> oldSegmentStripe = new HashSet<>(oldSegmentStripeList.get(i).getLogServers());
      Set<String> newSegmentStripe = new HashSet<>(newSegmentStripeList.get(i).getLogServers());
    ...
    LayoutSegment mergedSegment =
        new LayoutSegment(
            layoutSegmentList.get(segmentIndex).getReplicationMode(),
  ...
  /**
   * Moves a responsive server to the top of the sequencer server list. If all have failed, throws
   * exception.
   *
   * @param endpoints a non null set of Strings representing failed endpoints
   * @return this builder
   * @throws LayoutModificationException is thrown if none of the sequencers in layout can be moved
   *     to the top of sequencer server list due to being unresponsive or a failed endpoint.
   */
  public LayoutBuilder assignResponsiveSequencerAsPrimary(@NonNull Set<String> endpoints) {
    ...
    log.warn(
        "Failover to a responsive sequencer server failed. Each of the layout "
  ...
  /**
   * Removes the Sequencer Server passed. If the layout does not include the endpoint no action will
   * take place.
   *
   * @param endpoint a non null string representing the sequencer server endpoint which needs to be
   *     removed
   * @return this builder
   * @throws LayoutModificationException is thrown if the provided endpoint is the last remaining
   *     Sequencer Server
  ...
  public LayoutBuilder removeSequencerServer(@NonNull String endpoint) {
    ...
    if (layout.getSequencers().size() == 1 && layout.getSequencers().get(0).equals(endpoint)) {
      log.warn("Skipped removing sequencer as {} is the last remaining sequencer.", endpoint);
  ...
  /**
   * Removes all the Sequencer Server endpoints present in the set passed. This method first checks
   * that removing the provided endpoints does not remove all the Sequencer Servers in the layout.
   * In case that at least one sequencer server remains after the removal. It proceeds with removing
   * Sequencer Server endpoints provided while silently dismissing any of provided endpoints which
   * are not included as one of the Sequencer Servers in the layout.
   *
   * @param endpoints a non null set of Strings representing sequencer server endpoints to be
   *     removed from the layout.
   * @return this builder
   * @throws LayoutModificationException is thrown if removing the provided set of endpoints result
   *     in removing all the remaining Sequencer Servers in the layout.
   */
  public LayoutBuilder removeSequencerServers(@NonNull Set<String> endpoints) {
    ...
    if (endpoints.containsAll(layout.getSequencers())) {
      log.warn(
          "Skipped removing sequencers as remove request of {} would remove all "
              + "sequencers {}.",
          endpoints.toString(),
   ...
   *
   * @param endpoint A non null String representing the endpoint to be removed
   * @param layoutStripe A non null stripe to remove the endpoint from
   * @param minReplicationFactor The least number of nodes needed to maintain redundancy
   */
  public void removeFromStripe(
      @NonNull String endpoint, @NonNull LayoutStripe layoutStripe, int minReplicationFactor) {
    ...
    for (LayoutSegment layoutSegment : layoutSegments) {
      for (LayoutStripe layoutStripe : layoutSegment.getStripes()) {
        int minReplicationFactor =
            layoutSegment.getReplicationMode().getMinReplicationFactor(tempLayout, layoutStripe);
   ...
   *
   * @param endpoints a non null set of Strings representing Log unit servers to be removed
   * @return this builder
   */
