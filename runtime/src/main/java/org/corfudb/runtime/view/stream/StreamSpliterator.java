...
/**
 * A spliterator for streams, which supports limiting the spliterator's scope to a maximum global
 * address.
 *
 * <p>This spliterator is guaranteed to never read PAST the maximum global address given. This is
 * necessary because reading from a stream MODIFIES the stream pointer, and streams provide the
 * guarantee that they will return entries in order exactly once.
 *
 * <p>Concurrent modification of the stream during iteration is NOT supported.
 *
...
public class StreamSpliterator extends Spliterators.AbstractSpliterator<ILogData> {
  ...
  public StreamSpliterator(IStreamView view, long maxGlobal) {
    super(maxGlobal - view.getCurrentGlobalPosition(), Spliterator.ORDERED | Spliterator.SORTED);
    streamView = view;
