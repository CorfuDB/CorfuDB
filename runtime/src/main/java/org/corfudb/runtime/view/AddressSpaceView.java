...
public class AddressSpaceView extends AbstractView {
  ...
  private static final long CACHE_KEY_SIZE = MetricsUtils.sizeOf.deepSizeOf(0L);
  private static final long DEFAULT_MAX_CACHE_ENTRIES = 5000;
  ...
  private void handleEviction(RemovalNotification<Long, ILogData> notification) {
    if (log.isTraceEnabled()) {
      log.trace(
          "handleEviction: evicting {} cause {}", notification.getKey(), notification.getCause());
    }
  }
   ...
   *
   * <p>There are [currently] three different scenarios: 1. The data was persisted to some log units
   * and we were able to recover it. 2. The data was not persisted and another client (or this
   * client) hole filled. In that case, we return an OverwriteException and let the upper layer
   * handle it. 3. The address we tried to write to was trimmed. In this case, there is no way to
   * know if the write went through or not. For sanity, we throw an OverwriteException and let the
   * above layer retry.
   *
   * @param address
   */
  private void validateStateOfWrittenEntry(long address, @Nonnull ILogData ld) {
    ...
    try {
      ...
      throw new UnrecoverableCorfuError(
          "We cannot determine state of an update because of a trim.");
    }
  ...
  /**
   * Write the given log data using a token, returning either when the write has been completed
   * successfully, or throwing an OverwriteException if another value has been adopted, or a
   * WrongEpochException if the token epoch is invalid.
   *
   * @param token The token to use for the write.
   * @param data The data to write.
   * @param cacheOption The caching behaviour for this write
   * @throws OverwriteException If the globalAddress given by the token has adopted another value.
  ...
  public void write(@Nonnull IToken token, @Nonnull Object data, @Nonnull CacheOption cacheOption) {
    ...
    layoutHelper(
        e -> {
          ...
          // Do the write
          try {
            l.getReplicationMode(token.getSequence()).getReplicationProtocol(runtime).write(e, ld);
  ...
  /**
   * Write the given log data and then add it to the address space cache (i.e. WRITE_THROUGH option)
   *
  ...
  /**
   * Directly read from the log, returning any committed value, or NULL, if no value has been
   * committed.
   *
   * @param address The address to read from.
   * @return Committed data stored in the log, or NULL, if no value has been committed.
   */
   ...
   */
  public @Nonnull ILogData read(long address, @NonNull ReadOptions options) {
   ...
   */
  public @Nonnull ILogData read(
      @NonNull Long nextRead, @NonNull NavigableSet<Long> addresses, @NonNull ReadOptions options) {
   ...
   *
   * <p>- If the waitForWrite flag is set to true, when an empty address is encountered, it waits
   * for one hole to be filled. All the rest empty addresses within the list are hole filled
   * directly and the reader does not wait. - In case the flag is set to false, none of the reads
   * wait for write completion and the empty addresses are hole filled right away.
   *
  ...
  /**
   * Attempts to insert a loaded value into the cache and return the cached value for a particular
   * key.
   */
  private ILogData cacheLoadAndGet(
      @NonNull Cache<Long, ILogData> cache,
      long address,
  ...
  public Map<Long, ILogData> read(Iterable<Long> addresses, @NonNull ReadOptions options) {
    ...
    final Set<Long> addressesToFetch =
        Sets.difference(Sets.newHashSet(addresses), cachedData.keySet());
    ...
    final Map<Long, ILogData> result =
        uncachedData.entrySet().stream()
            .collect(
                Collectors.toMap(
                    Entry::getKey,
                    entry ->
  ...
  /**
   * Get log address space, which includes: 1. Addresses belonging to each stream. 2. Log Tail.
   *
  ...
  /**
   * Commit the addresses in the range by first inspecting the addresses and if data does not exist
   * in log, hole fill the address. This is used by management agent for log consolidation.
   *
  ...
  public void commit(long start, long end) {
    ...
    ContiguousSet<Long> range =
        ContiguousSet.create(Range.closed(start, end), DiscreteDomain.longs());
   ...
   *
   * <p>At the end of a prefix trim, all addresses equal to or less than the address given will be
   * marked for trimming, which means that they may return either the original data, or a trimmed
   * exception.
   *
  ...
  public void prefixTrim(final Token address, boolean retry) {
    ...
    for (int x = 0; x < numRetries; x++) {
      try {
        ...
        // sequencer about a trim before actually trimming the log unit, we prevent a race
        // condition,
        // in which a client could attempt to read a trimmed address over and over again, as signal
        // has not reached the sequencer. The problem with this is that we have a retry limit of 2,
        // so
        ...
        if (e instanceof NetworkException || e.getCause() instanceof TimeoutException) {
          log.warn(
              "prefixTrim[{}]: encountered a network error on attempt {}/{}",
              address,
              x + 1,
              numRetries,
              e);
          ...
          log.warn(
              "prefixTrim[{}]: WrongEpochException, runtime is in epoch {}, while server "
  ...
  /**
   * Force compaction on an address space, which will force all log units to free space, and process
   * any outstanding trim requests.
   */
  public void gc() {
   ...
   *
   * <p>Does not validate log entries.
   *
  ...
  private Map<Long, ILogData> fetchAll(Iterable<Long> addresses, ReadOptions options) {
    ...
    final Iterable<List<Long>> batches =
        Iterables.partition(addresses, runtime.getParameters().getBulkReadSize());

    for (List<Long> batch : batches) {
      // Doesn't handle the case where some address have a different replication mode
      Map<Long, ILogData> batchResult =
          layoutHelper(
              layout ->
                  layout
                      .getLayout()
                      ...
                      .getReplicationProtocol(runtime)
                      .readAll(
                          layout, batch, options.isWaitForHole(), options.isServerCacheable()));
      // Sanity check for returned addresses
      if (batchResult.size() != batch.size()) {
        log.error(
            "fetchAll: Requested number of addresses not equal to the read result"
                + "from server, requested: {}, returned: {}",
            batch,
            batchResult.keySet());
        throw new IllegalStateException(
            "Requested number of addresses not equal to the read result");
      }
  ...
  /**
   * Checks whether a log entry is valid or not. If a read returns null, Empty, or trimmed an
  ...
  private boolean isLogDataValid(long address, ILogData logData, boolean throwException) {
    if (logData == null || logData.getType() == DataType.EMPTY) {
      throw new RuntimeException(
          "Unexpected return of empty data at address " + address + " on read");
    }
   ...
   */
  private @Nonnull ILogData fetch(final long address) {
    ILogData result =
        layoutHelper(
            e ->
                e.getLayout()
