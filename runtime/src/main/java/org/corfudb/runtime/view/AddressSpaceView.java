...
public class AddressSpaceView extends AbstractView {
    ...
    public AddressSpaceView(@Nonnull final CorfuRuntime runtime) {
        ...
        metricsRegistry.ifPresent(registry ->
                GuavaCacheMetrics.monitor(registry, readCache, "address_space.read_cache"));

        multiReadTimer = metricsRegistry
                .map(registry -> Timer.builder("address_space.read.latency")
                        .tag("type", "multi").register(registry));

        singleReadTimer = metricsRegistry
                .map(registry -> Timer.builder("address_space.read.latency")
                        .tag("type", "single").register(registry));

        writeTimer = metricsRegistry
                .map(registry -> Timer.builder("address_space.write.latency").register(registry));
