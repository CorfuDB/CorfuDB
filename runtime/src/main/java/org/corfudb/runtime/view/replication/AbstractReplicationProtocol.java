...
public abstract class AbstractReplicationProtocol implements IReplicationProtocol {
   ...
   *
   * @param holeFillPolicy The hole filling policy to be applied when a read returns uncommitted
   *     data.
   */
   ...
   * {@inheritDoc}
   *
   * <p>In the base implementation, we attempt to read data using the peek method. If data is
   * returned by peek, we use it. Otherwise, we invoke the hole filling protocol.
   */
  @Nonnull
  @Override
  public ILogData read(RuntimeLayout runtimeLayout, long globalAddress) {
    try {
      return holeFillPolicy.peekUntilHoleFillRequired(globalAddress, a -> peek(runtimeLayout, a));
  ...
  /**
   * Write a special hole filling entry using the given address. When this call returns, either the
   * hole filling write has been committed or the result of another client's write has committed
   * (you WILL need to -read- the address to find out which write was adopted).
   *
