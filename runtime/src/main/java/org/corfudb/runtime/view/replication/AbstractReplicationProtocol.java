...
public abstract class AbstractReplicationProtocol implements IReplicationProtocol {
    ...
    @Nonnull
    @Override
    public ILogData read(EpochedClient epochedClient, long globalAddress) {
        try {
            ...
            holeFill(epochedClient, globalAddress);
            return peek(epochedClient, globalAddress);
        }
    }
     ...
     */
    protected abstract void holeFill(EpochedClient epochedClient, long globalAddress);
}
