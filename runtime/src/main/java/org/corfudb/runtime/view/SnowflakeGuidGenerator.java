...
/**
 * Fast Globally Unique Identity (GUID) generator that returns compact ids having a notion of a
 * universal comparable ordering.
 *
...
public class SnowflakeGuidGenerator implements OrderedGuidGenerator {
  ...
  /**
   * Please pass something unique to this node. For example: UUID nodeId =
   * UUID.fromString("mac_address_of_machine"); SnowflakeGuidGenerator guidGenerator = new
   * SnowflakeGuidGenerator(nodeId.node());
   *
   ...
   * ______________________________________________ | timestamp lower 42 bits | Node Id | Sequence |
  ...
  public long nextLong() {
    ...
    return currentTime << NODE_SHIFT_LONG << SEQ_SHIFT | node << SEQ_SHIFT | counter;
  }

  /**
   * Fast best effort generation scheme for snowflake generation.
   * ____________________________________________________ | timestamp 8 bytes | Node Id | Sequence |
   * ------------------------- -------------------------- <------ 64 bits
   * --------><-10bits--><----54 bits---->
   *
