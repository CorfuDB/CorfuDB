...
/**
 * A hole filling policy which reads several times, waiting a static amount of time in between,
 * before requiring a hole fill.
 *
...
public class ReadWaitHoleFillPolicy implements IHoleFillPolicy {
  ...
  /** Duration after which no more read attempts are made and an address hole fill is attempted. */
    ...
    try {
      return IRetry.build(
              ...
              () -> {
                ...
                } else if (System.currentTimeMillis() - startTime.get()
                    >= holeFillThreshold.toMillis()) {
                  throw new RetryExhaustedException("Retries Exhausted.");
                }

                // Otherwise try again.
                log.debug(
                    "peekUntilHoleFillRequired: Attempted read at address {}, "
                        + "but data absent. Retrying.",
                    address);
                throw new RetryNeededException();
              })
          .setOptions(x -> x.setMaxRetryThreshold(retryWaitThreshold))
          .run();
