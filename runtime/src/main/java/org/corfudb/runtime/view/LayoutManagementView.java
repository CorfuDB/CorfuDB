 ...
 * <p>
 *
 * <p>Created by zlokhandwala on 11/1/17.
 */
@Slf4j
public class LayoutManagementView extends AbstractView {
  ...
  /**
   * Future which is reset every time a new task to bootstrap the sequencer is launched. This is to
   * avoid multiple bootstrap requests.
   */
  ...
  /**
   * On restart, if MANAGEMENT_LAYOUT exists in the local datastore. the Management Server attempts
   * to recover the cluster from that layout.
   *
  ...
  /**
   * Takes in the existing layout and a set of failed nodes. It first generates a new layout by
   * removing the failed nodes from the existing layout. It then seals the epoch to prevent any
   * client from accessing the stale layout. Finally we run paxos to update all servers with the new
   * layout.
   *
  ...
  public void handleFailure(
    ...
    Layout newLayout =
        failureHandlerPolicy.generateLayout(
            currentLayout, runtime, failedServers, Collections.emptySet());
    runLayoutReconfiguration(currentLayout, newLayout, false);
  }

  /**
   * Bootstraps the new node with the current layout. This action is invoked as a part of the add
   * node workflow which requires the new node to be added to the cluster to be bootstrapped with
   * the existing layout of this cluster. This bootstraps the Layout server with the existing
   * layout.
   *
  ...
  public void addNode(
    ...
    if (!currentLayout.getAllServers().contains(endpoint)) {
      ...
      if (isLogUnitServer) {
        layoutBuilder.addLogunitServer(
            logUnitStripeIndex,
      ...
      if (!runtime.getLayoutView().getLayout().equals(newLayout)) {
        throw new IllegalStateException(
            "This node's layout was not committed." + "Aborting add node workflow.");
      }
    } else {
    ...
    if (currentLayout.getUnresponsiveServers().contains(endpoint)) {
      ...
      CFUtils.getUninterruptibly(
          runtime
              ...
              .sealRemoteServer(currentLayout.getEpoch()));
      ...
      LayoutBuilder layoutBuilder = new LayoutBuilder(currentLayout);
      layoutBuilder.addLogunitServer(
          0, getLogTail(runtime.getLayoutView().getRuntimeLayout(currentLayout)), endpoint);
  ...
  /**
   * Attempts to merge all the segments of the layout starting from the first two. Once the
   * remaining segments can no longer be merged, attempts consensus on a new layout.
   *
   * @param currentLayout Current layout
  ...
  public void mergeSegments(Layout currentLayout) {
    ...
    Layout newLayout;
    Predicate<Layout> shouldMergeSegments =
        layout -> {
          if (layout.getSegments().size() > 1) {
            return Sets.difference(
                    layout.getSegments().get(1).getAllLogServers(),
                    layout.getSegments().get(0).getAllLogServers())
                .isEmpty();
          }
          return false;
        };
    ...
    if (shouldMergeSegments.test(tempLayout)) {
      ...
      while (shouldMergeSegments.test(tempLayout)) {
        ...
        tempLayout = layoutBuilder.mergePreviousSegment(1).build();
      }
   ...
   */
  public void removeNode(@Nonnull Layout currentLayout, @Nonnull String endpoint)
      throws OutrankedException {
    ...
    if (currentLayout.getAllServers().contains(endpoint)) {
      ...
      newLayout = currentLayout;
      log.info(
          "removeNode: Ignoring remove node on {} because it doesn't exist in {}",
          endpoint,
          currentLayout);
    }
  ...
  /**
   * Runs the layout reconfiguration process. Seals the layout. Runs paxos. During paxos if in
   * recovery mode, we increment rank until committed on same epoch. If not in recovery, we fail
   * with outrankedException. The new committed layout is then verified by invalidating the runtime.
   ...
   * @throws OutrankedException if consensus is outranked.
   */
  public void runLayoutReconfiguration(
      Layout currentLayout, Layout newLayout, final boolean forceSequencerRecovery)
      throws OutrankedException {
  ...
  private void attemptConsensus(Layout layout) throws OutrankedException {
    ...
    for (int x = 0; x < runtime.getParameters().getInvalidateRetry(); x++) {
      ...
      if (runtime.getLayoutView().getLayout().equals(layout)) {
        ...
        log.warn(
            "Runtime recovered with a different layout = {}", runtime.getLayoutView().getLayout());
      }
  ...
  /**
   * Reconfigures the sequencer. If the primary sequencer has changed in the new layout, the global
   * tail of the log units are queried and used to set the initial token of the new primary
   * sequencer.
   *
   ...
   */
  public void reconfigureSequencerServers(
      Layout originalLayout, Layout newLayout, boolean forceReconfigure) {
    ...
    if (acquiredLocked) {
      try {
        ...
        if (forceReconfigure
            || !originalLayout.getPrimarySequencer().equals(newLayout.getPrimarySequencer())) {

          StreamsAddressResponse streamsAddressesResponse =
              runtime.getAddressSpaceView().getLogAddressSpace();
        ...
        boolean sequencerBootstrapResult =
            CFUtils.getUninterruptibly(
                runtime
                    .getLayoutView()
  ...
  /**
   * Triggers a new task to bootstrap the sequencer for the specified layout. If there is already a
   * task in progress, this is a no-op.
   *
  ...
  public CompletableFuture<Boolean> asyncSequencerBootstrap(
    ...
    return sequencerRecoveryFuture.updateAndGet(
        sequencerRecovery -> {
