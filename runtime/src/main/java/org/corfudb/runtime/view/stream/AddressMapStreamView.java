 ...
 *
 * <p>A stream's address space is discovered by requesting to the sequencer, the map of all
 * addresses belonging to this stream.
 *
 * <p>Notice that, unlike the BackpointerStreamView, addresses are discovered in a single call to
 * the sequencer, therefore, entries are not read in advance. For this reason, when iterating over
 * the stream (getNextEntry) we perform batch reads, which significantly reduces the number of RPCs
 * to Log Units, as opposed to reading one entry at a time. Also, single stepping in the presence of
 * holes is no longer a need as address space is determined by the map provided by the sequencer.
 *
 * <p>Created by annym on 04/25/19.
 */
@Slf4j
public class AddressMapStreamView extends AbstractQueuedStreamView {
   ...
   */
  public AddressMapStreamView(
      final CorfuRuntime runtime, final UUID streamId, @Nonnull final StreamOptions options) {
    super(runtime, streamId, options);
  }

  public AddressMapStreamView(final CorfuRuntime runtime, final UUID streamId) {
      ...
      try {
        ...
        if (queue == getCurrentContext().readQueue) {
          ...
          if ((ld.containsStream(this.id) && ld.isData())
              || (ld.isHole()
                  && (ld.getBackpointerMap().isEmpty() || ld.containsStream(this.id)))) {
            addToResolvedQueue(getCurrentContext(), currentRead);
            readNext = false;
          } else {
            log.trace(
                "getNextEntry[{}]: the data for address {} does not belong to this stream. Skip.",
                this,
                currentRead);
        ...
        // we force trimmedExceptions to be thrown by lower layers, and handle ignoreTrimmed at this
        // layer.
        ...
        log.debug(
            "removeFromQueue[{}]: ignoring trimmed addresses {}", this, te.getTrimmedAddresses());
        ...
        // If a TrimmedException was caught, the requested address (nextRead) is trimmed (lower of
        // all),
  ...
  /**
   * Retrieve this stream's address map, i.e., a map of all addresses corresponding to this stream
   * between (stop address, start address] and return a boolean indicating if addresses were found
   * in this range.
   *
   ...
   * @param maxGlobal maximum Address until which we want to sync (is not necessarily equal to start
   *     address)
   * @return
   */
  @Override
  protected boolean discoverAddressSpace(
      final UUID streamId,
    ...
    if (Address.isAddress(startAddress) && (startAddress > stopAddress)) {
      ...
      StreamAddressSpace streamAddressSpace =
          getStreamAddressMap(startAddress, stopAddress, streamId);
  ...
  private void moveToReadQueue(
      final StreamAddressSpace streamAddressSpace,
      final NavigableSet<Long> queue,
      final long startAddress,
      final long stopAddress,
      final long maxGlobal) {
    ...
    if (isCheckpointCapable() && !isTrimCoveredByCheckpointOrLocalView(trimMark)) {
      if (getReadOptions().isIgnoreTrim()) {
        log.debug(
            "getStreamAddressMap[{}]: Ignoring trimmed exception for address[{}].",
            this,
        ...
        String message =
            String.format(
                ...
                startAddress,
                trimMark,
                getCurrentContext().getCheckpoint().startAddress,
                maxGlobal);
        throw new TrimmedException(message);
      }
    }
  ...
  private void processCheckpoint(
    ...
    Iterable<List<Long>> batches =
        Iterables.partition(
            checkpointAddresses, runtime.getParameters().getCheckpointReadBatchSize());
   ...
   * @return True, resolved checkpoint (reached end of valid checkpoint). False, otherwise.
   */
  private void processCheckpointBatchByEntry(List<Long> batch, Function<ILogData, Boolean> filter) {
  ...
  private StreamAddressSpace getStreamAddressMap(
      long startAddress, long stopAddress, UUID streamId) {
    // Case non-consecutive addresses
    if (startAddress - stopAddress > DIFF_CONSECUTIVE_ADDRESSES) {
      log.trace(
          "getStreamAddressMap[{}] get addresses from {} to {}", this, startAddress, stopAddress);
      // This step is an optimization.
      // Attempt to read the last entry and verify its backpointer,
      // if this address is already resolved locally do not request the stream map to the sequencer
      // as new
      ...
      if (isAddressToBackpointerResolved(startAddress, streamId)) {
        return new StreamAddressSpace(
            Address.NON_ADDRESS, Roaring64NavigableMap.bitmapOf(startAddress));
      }

      log.trace(
          "getStreamAddressMap[{}]: request stream address space between {} and {}.",
          streamId,
          startAddress,
          stopAddress);
      return runtime
    ...
    // Start and stop address are consecutive addresses, no need to request the address map for this
    // stream,
    // the only address to include is startAddress (stopAddress is already resolved - not included
    // in the lookup).
    return new StreamAddressSpace(
        Address.NON_ADDRESS, Roaring64NavigableMap.bitmapOf(startAddress));
  }
  ...
  private boolean isAddressToBackpointerResolved(long startAddress, UUID streamId) {
    ...
    try {
      ...
      if (getReadOptions().isIgnoreTrim()) {
        log.debug(
            "isAddressToBackpointerResolved[{}]: Ignoring trimmed exception for address[{}],"
                + " stream[{}]",
            this,
            startAddress,
            streamId);
        ...
        log.info(
            "getStreamAddressMap[{}]; Attempting to resolve backpointer for {} but address is"
                + " trimmed. Looking for a checkpoint.",
            this,
            startAddress);
        throw e;
      }
    ...
    if (d.hasBackpointer(streamId)) {
      ...
      log.trace(
          "getStreamAddressMap[{}]: backpointer for {} points to {}",
          streamId,
          startAddress,
          previousAddress);
      ...
      if ((Address.isAddress(previousAddress)
        ...
        log.trace(
            "getStreamAddressMap[{}]: backpointer {} is locally present, do not request "
                + "stream address map.",
            streamId,
            previousAddress);
        return true;
  ...
  /**
   * Verify that a trim is covered either by a loaded checkpoint or by the locally resolved
   * addresses.
   *
   * <p>Because address maps might have been trimmed, the trim mark is a 'marker' of addresses that
   * were removed from the map (historical) and that should be covered by a checkpoint.
   *
   * @param trimMark
  ...
  private boolean isTrimCoveredByCheckpointOrLocalView(long trimMark) {
    return isTrimResolvedLocally(trimMark) || isTrimCoveredByCheckpoint(trimMark);
  }
