...
/**
 * Factory for custom futures used by the quorum replication. Created by Konstantin Spirov on
 * 2/3/2017.
 */
@Slf4j
public class QuorumFuturesFactory {
  ...
  /**
   * Get a thread safe future that will complete only when (n/2)+1 futures complete or if there is
   * no hope (if n/2+1 futures are canceled or have conflicting value).
   *
   * <p>The future returned does not block explicitly, it aggregates the futures and delegates the
   * blocking.
   *
   * <p>In case of normal execution, any of the compete futures can be used to return the result. In
   * case of termination, the cancel flag will be updated and if any of the futures threw an
   * exception, ExecutionException will be thrown, otherwise the future will return null.
   *
   ...
   */
  static <R> CompositeFuture<R> getQuorumFuture(
      Comparator<R> comparator, CompletableFuture<R>... futures) {
    return new CompositeFuture<R>(comparator, (futures.length / 2) + 1, futures);
  }

  /**
   * Get a thread safe future that will complete only when n/2+1 futures complete or if there is no
   * hope (if n/2+1 futures are canceled or have conflicting value).
   *
   * <p>The future returned does not block explicitly, it aggregates the futures and delegates the
   * blocking.
   *
   ...
   *     immediately. All fail fast exceptions are directly propagated and thrown outside.
   * @return The composite future
   */
  public static <R> CompositeFuture<R> getQuorumFuture(
      Comparator<R> comparator, CompletableFuture<R>[] futures, Class... failFastThrowables) {
   ...
   * blocking.
   *
   * <p>In case if some future completes successfully its value will be returned. In case of
   * termination, the cancel flag will be updated and if any of the futures threw an exception,
   ...
   * @return The composite future
   */
  static <R> CompositeFuture<R> getFirstWinsFuture(
      Comparator<R> comparator, CompletableFuture<R>... futures) {
  ...
  public static class CompositeFuture<R> implements Future<R> {
    ...
    public R get(long timeout, TimeUnit unit)
      ...
      while (infinite || System.nanoTime() < until) {
        ...
        int greatestNumCompleteFutures =
            valuesSortedByCount.size() == 0 ? 0 : valuesSortedByCount.keySet().iterator().next();
    ...
    /**
     * @return true the future was canceled explicitly, or if the future was unable to reach quorum
     *     due to conflicts, canceled futures or futures that have thrown exception.
     */
    @Override
