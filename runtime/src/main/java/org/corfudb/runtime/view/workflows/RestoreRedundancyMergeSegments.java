...
/**
 * Sending a workflow request to restore all redundancies and merge all segments. Created by Zeeshan
 * on 2019-02-06.
 */
@Slf4j
public class RestoreRedundancyMergeSegments extends WorkflowRequest {
   ...
   * @return Workflow ID.
   * @throws TimeoutException thrown is orchestrator request was not acknowledged within the RPC
   *     timeout.
   */
  @Override
  protected UUID sendRequest(@NonNull ManagementClient managementClient) throws TimeoutException {
    ...
    log.info(
        "sendRequest: request to restore redundancy and merge segments for endpoint {} on"
            + " orchestrator {}:{}",
        nodeForWorkflow,
        managementClient.getRouter().getHost(),
  ...
  protected boolean verifyRequest(@NonNull Layout layout) {
    ...
    return layout.getAllServers().contains(nodeForWorkflow)
        && layout.getSegmentsForEndpoint(nodeForWorkflow).size() == layout.getSegments().size();
  }
  ...
  protected Optional<ManagementClient> getOrchestrator() {
    ...
    BaseClient baseClient =
        runtime.getLayoutView().getRuntimeLayout(layout).getBaseClient(nodeForWorkflow);
    if (baseClient.pingSync()) {
      log.info("getOrchestrator: orchestrator selected {}", nodeForWorkflow);
      return Optional.of(
          runtime.getLayoutView().getRuntimeLayout(layout).getManagementClient(nodeForWorkflow));
    } else {
