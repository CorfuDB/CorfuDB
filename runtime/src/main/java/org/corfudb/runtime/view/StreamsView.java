...
public class StreamsView extends AbstractView {
  ...
  /**
   * Checkpoint of streams have their own stream id derived from the stream id. We add the
   * checkpoint suffix to the original stream id.
   */
  public static final String CHECKPOINT_SUFFIX = "_cp";

  /**
   * This list should have references to all opened streams. The ViewsGarbageCollector will use this
   * list to clear trimmed addresses from streams. Since new streams can be opened during a runtime
   * garbage collection cycle, the collection has to be thread-safe.
   */
   ...
   * synchronization at a higher level, so the stream implementation doesn't have to be thread-safe.
   * Also, gc will not be called on unsafe streams therefore gc needs to be managed by the consumer.
   *
  ...
  /**
   * Create and return a new stream. This stream will automatically garbage collected by
  ...
  /**
   * Run garbage collection on all opened streams. Note that opened unsafe streams will be excluded
   * (because its unsafe for the garbage collector thread to operate on them while being used by a
   * different thread).
   */
  ...
  /**
   * Append to multiple streams simultaneously, possibly providing information on how to resolve
   * conflicts.
   *
  ...
  public long append(
    ...
    for (int x = 0; x < runtime.getParameters().getWriteRetry(); x++) {
      ...
      try {
        if (TransactionalContext.isInTransaction()) {
          ...
          log.debug(
              "append: Invoking {} preCommitListeners",
        ...
        log.warn(
            "append[{}]: Overwritten after {} retries, streams {}",
    ...
    log.error(
        "append[{}]: failed after {} retries, streams {}, write size {} bytes",
   ...
   */
  public long append(
      @Nonnull Object object, @Nullable TxResolutionInfo conflictInfo, @Nonnull UUID... streamIDs) {
