...
public class RuntimeLayout {
  ...
  @Getter private final CorfuRuntime runtime;
  ...
  /**
   * Attempts to move all servers in the system to the epoch of this layout. The seal however waits
   * only for a response from a quorum of layout servers (n/2 + 1), a quorum of log unit servers in
   * every stripe in case of QUORUM_REPLICATION or at least one log unit server in every stripe in
   * case of CHAIN_REPLICATION. The fault detector eventually corrects out of phase epochs by
   * resealing the servers.
   *
  ...
  public void sealMinServerSet() throws WrongEpochException, QuorumUnreachableException {
    log.debug(
        "Requested move of servers to new epoch {} servers are {}",
        layout.getEpoch(),
  ...
  /**
   * Sender Client Map. map(client type -> map(Endpoint -> senderClient)) This ensures that a client
   * for a particular endpoint stamped with the required epoch is created only once.
   */
  private final Map<Class<? extends IClient>, Map<String, IClient>> senderClientMap =
      new ConcurrentHashMap<>();

  /**
   * Updates the local map of clients. The epoch, client tuple is invalidated and overwritten when
   * there is an epoch mismatch. This ensures that a client for a particular endpoint stamped with
   * the required epoch is created only once.
   *
