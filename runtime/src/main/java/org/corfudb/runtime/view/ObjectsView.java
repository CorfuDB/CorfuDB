...
/** A view of the objects inside a Corfu instance. Created by mwei on 1/7/16. */
@Slf4j
public class ObjectsView extends AbstractView {
  ...
  @Getter @Setter boolean transactionLogging = false;

  @Getter Map<ObjectID, Object> objectCache = new ConcurrentHashMap<>();
  ...
  /**
   * Begins a transaction on the current thread. Automatically selects the correct transaction
   * strategy. Modifications to objects will not be visible to other threads or clients until TXEnd
   * is called.
   */
  ...
  /**
   * Aborts a transaction on the current thread. Modifications to objects in the current
   * transactional context will be discarded.
   */
  @SuppressWarnings({"checkstyle:methodname", "checkstyle:abbreviation"})
  public void TXAbort() {
    ...
    if (context == null) {
      ...
      TxResolutionInfo txInfo =
          new TxResolutionInfo(context.getTransactionID(), context.getSnapshotTimestamp());
   ...
   *
   * @return True, if called within a transactional context, False, otherwise.
   */
  @SuppressWarnings({"checkstyle:methodname", "checkstyle:abbreviation"})
  ...
  public long TXEnd() throws TransactionAbortedException {
    ...
    try (Timer.Context txCommitDuration =
      ...
      TxResolutionInfo txInfo = new TxResolutionInfo(context.getTransactionID(), snapshotTimestamp);
      ...
      TransactionAbortedException tae = new TransactionAbortedException(txInfo, cause, e, context);
      ...
      TxResolutionInfo txInfo =
          new TxResolutionInfo(context.getTransactionID(), Token.UNINITIALIZED);
      TransactionAbortedException tae =
          new TransactionAbortedException(txInfo, AbortCause.UNDEFINED, e, context);
      context.abortTransaction(tae);
  ...
  /**
   * Run garbage collection on all opened objects. Note that objects open with the NO_CACHE options
   * will not be gc'd
   */
  public void gc(long trimMark) {
    for (Object obj : getObjectCache().values()) {
      ((CorfuCompileProxy) ((ICorfuSMR) obj).getCorfuSMRProxy()).getUnderlyingObject().gc(trimMark);
    }
  }
