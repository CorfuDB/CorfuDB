 ...
 *
 * <p>Created by zlokhandwala on 3/10/17.
 */
@Slf4j
public class SealServersHelper {

  /**
   * Asynchronously seal all servers in layout by setting remote epochs. Different type of servers
   * can have their own logic of sealing, the base server dispatches sealing logic to each
   * individual server.
   *
   ...
   *     servers.
   */
  public static void waitForLayoutSeal(
      List<String> layoutServers, Map<String, CompletableFuture<Boolean>> completableFutureMap)
   ...
   * @param completableFutureMap A map of completableFutures for every remoteSetEpoch call.
   * @throws QuorumUnreachableException Thrown if responses not received from all the log unit
   *     servers.
   */
  public static void waitForChainSegmentSeal(
      Layout.LayoutSegment layoutSegment,
    ...
    for (Layout.LayoutStripe layoutStripe : layoutSegment.getStripes()) {
      ...
      try {
        success =
            CFUtils.getUninterruptibly(
                quorumFuture, TimeoutException.class, QuorumUnreachableException.class);
      ...
      if (success) {
        log.debug(
            "waitForChainSegmentSeal: Successfully waited at least one "
   ...
   *     servers.
   */
  public static void waitForQuorumSegmentSeal(
      Layout.LayoutSegment layoutSegment,
      Map<String, CompletableFuture<Boolean>> completableFutureMap)
      throws QuorumUnreachableException {
    for (Layout.LayoutStripe layoutStripe : layoutSegment.getStripes()) {
