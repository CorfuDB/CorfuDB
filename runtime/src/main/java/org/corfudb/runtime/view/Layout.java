...
/** This class represents the layout of a Corfu instance. Created by mwei on 12/8/15. */
@Data
public class Layout {

  /** Sorting layouts according to epochs in descending order */
  public static final Comparator<Layout> LAYOUT_COMPARATOR =
      Comparator.comparing(Layout::getEpoch).reversed();

  /** A Gson parser. */
  @Getter
  static final Gson parser =
      new GsonBuilder().registerTypeAdapter(Layout.class, new LayoutDeserializer()).create();
  /** A list of layout servers in the layout. */
  @Getter List<String> layoutServers;
  /** A list of sequencers in the layout. */
  @Getter List<String> sequencers;
  /** A list of the segments in the layout. */
  @Getter List<LayoutSegment> segments;
  /** A list of unresponsive nodes in the layout. */
  @Getter List<String> unresponsiveServers;
  /** The epoch of this layout. */
  @Getter @Setter long epoch;

  /** Invalid epoch value. Is used to fetch layout(epoch agnostic request) by the corfuRuntime. */
  public static final long INVALID_EPOCH = -1L;

  /** Invalid cluster id. It is used to fetch layout by the corfu runtime. */
  public static final UUID INVALID_CLUSTER_ID =
      UUID.fromString("00000000-0000-0000-0000-000000000000");

  /**
   * The unique Id for the Corfu cluster represented by this layout. Should remain consistent for
   * the lifetime of the layout. May be {@code null} in a legacy layout.
   */
  @Getter UUID clusterId;

  /**
   * Defensive constructor since we can create a Layout from a JSON file. JSON deserialize is forced
  ...
  public Layout(
      ...
      @NonNull List<String> unresponsiveServers,
      long epoch,
  ...
  public Set<String> getAllActiveServers() {
    ...
    segments.forEach(
        segment ->
            segment.getStripes().forEach(stripe -> activeServers.addAll(stripe.getLogServers())));
  ...
  /**
   * Get all the fully redundant log unit servers, i.e. log units that are present in all layout
   * segments.
   *
  ...
  /**
   * Return a list of segments which contain global addresses less than or equal to the given
   * address (known as the prefix).
   *
   * @param globalAddress The global address prefix to use.
   * @return A list of segments which contain addresses less than or equal to the global address.
   */
  public @Nonnull List<LayoutSegment> getPrefixSegments(long globalAddress) {
    return segments.stream().filter(p -> p.getEnd() <= globalAddress).collect(Collectors.toList());
  }
  ...
  public enum ReplicationMode {
    CHAIN_REPLICATION {
      @Override
      public void validateSegmentSeal(
          LayoutSegment layoutSegment, Map<String, CompletableFuture<Boolean>> completableFutureMap)
      ...
      public IReplicationProtocol getReplicationProtocol(CorfuRuntime r) {
        if (r.getParameters().isHoleFillingDisabled()) {
          ...
          return new ChainReplicationProtocol(
              new ReadWaitHoleFillPolicy(
                  r.getParameters().getHoleFillTimeout(),
    ...
    QUORUM_REPLICATION {
      ...
      public ClusterStatus getClusterHealthForSegment(
        ...
        for (LayoutStripe layoutStripe : layoutSegment.getStripes()) {
          List<String> responsiveLogServers = new ArrayList<>(layoutStripe.getLogServers());
    ...
    public abstract void validateSegmentSeal(
        LayoutSegment layoutSegment, Map<String, CompletableFuture<Boolean>> completableFutureMap)
        throws QuorumUnreachableException;

    /**
     * Compute the min replication factor for the log unit servers in the replication protocol for a
     * specific stripe.
     *
    ...
    public abstract IStreamView getUnsafeStreamView(
        CorfuRuntime r, UUID streamId, StreamOptions options);
     ...
     */
    public abstract ClusterStatus getClusterHealthForSegment(
        LayoutSegment layoutSegment, Set<String> responsiveNodes);
  }
  ...
  public static class LayoutSegment {
     ...
     *
     * <p>For example, [1...100], [101...200], [201...), where the last segment is active and open
     * ended.
     *
    ...
    public LayoutSegment(
        @NonNull ReplicationMode replicationMode,
        long start,
        long end,
