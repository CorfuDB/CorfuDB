    ...
    public void committed(long epoch, Layout layout, boolean force)
        ...
        for (CompletableFuture cf : commitList) {
            try {
                ...
                //todo(Maithem) fix these log statements
                log.warn("committed: Error while force committing", e);
            } catch (NoBootstrapException nbe) {
                log.warn("committed: Error while force committing", nbe);
            } catch (TimeoutException | NetworkException e) {
                log.warn("committed: Error while committing", e);
            }
        }
        log.debug("committed: Successful responses={}, responses={}", commitList.length, responses);
    }
