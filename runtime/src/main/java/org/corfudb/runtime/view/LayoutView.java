...
public class LayoutView extends AbstractView {
  ...
  /**
   * Retrieves the number of nodes needed to obtain a quorum. We define a quorum for the layout view
   * as n/2+1
   *
  ...
  /**
   * Drives the consensus protocol for persisting the new Layout. TODO currently the code can drive
   * only one Layout change. If it has to drive a previously incomplete round TODO it will drop it's
   * own set of changes. Need to revisit this. A change of layout proposal consists of a rank and
   * the desired layout.
   *
   * @param layout The layout to propose.
   * @param rank The rank for the proposed layout.
   * @throws QuorumUnreachableException Thrown if responses not received from a majority of layout
   *     servers.
  ...
  public void updateLayout(Layout layout, long rank)
    ...
    if (currentLayout.getEpoch() != epoch - 1) {
      log.error(
          "Runtime layout has epoch {} but expected {} to move to epoch {}",
    ...
    if (currentLayout.getClusterId() != null
      ...
      log.error(
          "updateLayout: Requested layout has cluster Id {} but expected {}",
          layout.getClusterId(),
          currentLayout.getClusterId());
  ...
  public Layout prepare(long epoch, long rank)
    ...
    CompletableFuture<LayoutPrepareResponse>[] prepareList =
        getLayout().getLayoutServers().stream()
            .map(
                x -> {
    ...
    while (true) {
      ...
      if (prepareList.length < getQuorumNumber()) {
        log.debug(
            "prepare: Quorum unreachable, remaining={}, required={}",
            prepareList,
            getQuorumNumber());
      ...
      try {
        CFUtils.getUninterruptibly(
            CompletableFuture.anyOf(prepareList),
      ...
      log.debug(
          "prepare: Successful responses={}, needed={}, timeouts={}, " + "wrongEpochRejected={}",
          acceptList.length,
          getQuorumNumber(),
          timeouts,
          wrongEpochRejected);
    ...
    List<LayoutPrepareResponse> list =
        Arrays.stream(acceptList).filter(x -> x.getLayout() != null).collect(Collectors.toList());
    if (list.isEmpty()) {
      return null;
    } else {
  ...
  public Layout propose(long epoch, long rank, Layout layout)
    ...
    CompletableFuture<Boolean>[] proposeList =
        getLayout().getLayoutServers().stream()
    ...
    while (true) {
      ...
      if (proposeList.length < getQuorumNumber()) {
        log.debug(
            "propose: Quorum unreachable, remaining={}, required={}",
            proposeList,
            getQuorumNumber());
      ...
      try {
        CFUtils.getUninterruptibly(
            CompletableFuture.anyOf(proposeList),
            OutrankedException.class,
            TimeoutException.class,
            NetworkException.class,
            WrongEpochException.class);
      ...
      log.debug(
          "propose: Successful responses={}, needed={}, timeouts={}, " + "wrongEpochRejected={}",
          count,
          getQuorumNumber(),
          timeouts,
          wrongEpochRejected);
  ...
  /**
   * Send committed layout to the old Layout servers and the new Layout Servers. TODO Current policy
   * is to send the committed layout once. Need to revisit this in order TODO to drive the new
   * layout to all the involved LayoutServers. TODO The new layout servers are not bootstrapped and
   * will reject committed messages. TODO Need to fix this.
   *
   * @throws WrongEpochException wrong epoch number.
   */
  ...
  /**
   * Send committed layout to the old Layout servers and the new Layout Servers. If force is true,
   * then the layout forced on all layout servers.
   */
  @SuppressWarnings("unchecked")
  public void committed(long epoch, Layout layout, boolean force) throws WrongEpochException {
    CompletableFuture<Boolean>[] commitList =
  ...
  /**
   * Bootstraps the layout server of the specified node. If already bootstrapped, it completes
   * silently.
   *
   ...
   */
  CompletableFuture<Boolean> bootstrapLayoutServer(
      @Nonnull String endpoint, @Nonnull Layout layout) {
    return getRuntimeLayout(layout)
