 ...
 *
 * <p>AbstractView requires a runtime, and provides a layoutHelper function.
 *
 * <p>The layoutHelper function is called whenever a view tries to access a layout. If the
 * layoutHelper catches an exception which is due to connection issues or an incorrect epoch, it
 * asks the runtime to invalidate that layout by reporting it to a layout server, and retries the
 * function.
 *
...
public abstract class AbstractView {
  ...
  public Layout getCurrentLayout() {
    while (true) {
      try {
        ...
        log.warn(
            "Error executing remote call, invalidating view and retrying in {}s",
            runtime.getParameters().getConnectionRetryRate(),
            ex);
  ...
  public <
          ...
          D extends RuntimeException>
      T layoutHelper(LayoutFunction<Layout, T, A, B, C, D> function) throws A, B, C, D {
    return layoutHelper(function, false);
  }

  /**
   * Maintains reference of the RuntimeLayout which gets invalidated if the AbstractView encounters
   * a newer layout. (with a greater epoch)
   */
  ...
  private Layout getLayoutUninterruptibly() {
    try {
      ...
      if (ex.getCause() instanceof RuntimeException) {
        log.error(
            "getLayoutUninterruptibly: Encountered unchecked exception. " + "Aborting layoutHelper",
            ex);
  ...
  /**
   * Helper function for view to retrieve layouts. This function will retry the given function
   * indefinitely, invalidating the view if there was a exception contacting the endpoint.
   *
   * <p>There is a flag to set if we want the caller to handle Runtime Exceptions. For some special
   * cases (like writes), we need to do a bit more work upon a Runtime Exception than just retry.
   *
  ...
  public <
          ...
          C extends RuntimeException,
          D extends RuntimeException>
      T layoutHelper(LayoutFunction<Layout, T, A, B, C, D> function, boolean rethrowAllExceptions)
          throws A, B, C, D {
    ...
    while (true) {
      ...
      try {
        ...
        if (re.getCause() instanceof TimeoutException) {
          log.warn(
              "Timeout executing remote call, invalidating view and retrying " + "in {}s",
              retryRate);
          ...
          log.warn(
              "Got a wrong epoch exception, updating epoch to {} and " + "invalidate view",
              we.getCorrectEpoch());
      ...
      log.info(
          "layoutHelper: Retried {} times, SystemDownHandlerTriggerLimit = {}",
  ...
  public interface LayoutFunction<
      V, R, A extends Throwable, B extends Throwable, C extends Throwable, D extends Throwable> {
