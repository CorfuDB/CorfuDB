...
class QuorumFutureFactory {
    ...
    /**
     * Get a thread safe future that will complete only when n/2+1 futures complete or n/2+1 futures are canceled.
     *
    ...
    /**
     * Get a thread safe future that will complete only when a single futures complete or n/2+1 futures are canceled.
     *
     * The future returned does not block explicitly, it aggregates the futures and delegates the blocking.
     *
    ...
    public static class CompositeFuture<R> implements Future<R> {
        ...
        /**
         * @return true if there was two successful results with different values, otherwise false
         */
