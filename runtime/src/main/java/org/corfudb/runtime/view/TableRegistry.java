...
/**
 * Table Registry manages the lifecycle of all the tables in the system. This is a wrapper over the
 * CorfuTable providing transactional operations and accepts only protobuf messages. It accepts a
 * primary key - which is a protobuf message. The payload is a CorfuRecord which comprises of 2
 * fields - Payload and Metadata. These are protobuf messages as well. The table creation registers
 * the schema used in the table which is further used for serialization. This schema is also used
 * for offline (without access to client protobuf files) browsing and editing.
 *
 * <p>Created by zlokhandwala on 2019-08-10.
 */
@Slf4j
public class TableRegistry {

  /**
   * System Table: To store the table schemas and other options. This information is used to view
   * and edit table using an offline tool without the dependency on the application for the schemas.
   */
  ...
  /**
   * Stores the schemas of the Key, Value and Metadata. A reference of this map is held by the
   * {@link ProtobufSerializer} to serialize and deserialize the objects.
   */
  private final ConcurrentMap<String, Class<? extends Message>> classMap;

  /**
   * Cache of tables allowing the user to fetch a table by fullyQualified table name without the
   * other options.
   */
  ...
  /**
   * This {@link CorfuTable} holds the schemas of the key, payload and metadata for every table
   * created.
   */
  @Getter
   ...
   */
  private <K extends Message, V extends Message, M extends Message> void registerTable(
    ...
    TableName tableNameKey =
        TableName.newBuilder().setNamespace(namespace).setTableName(tableName).build();
    ...
    // Schema validation to ensure that there is either proper modification of the schema across
    // open calls.
    ...
    int numRetries =
        3; // Since this is an internal transaction, retry a few times before giving up.
    long finalAddress = Address.NON_ADDRESS;
    while (numRetries-- > 0) {
      try {
        this.runtime.getObjectsView().TXBuild().type(TransactionType.OPTIMISTIC).build().begin();
        if (hasSchemaChanged) {
          this.registryTable.put(
              tableNameKey, new CorfuRecord<>(tableDescriptors, metadataBuilder.build()));
        } else {
          this.registryTable.putIfAbsent(
              tableNameKey, new CorfuRecord<>(tableDescriptors, metadataBuilder.build()));
        }
        ...
        log.info(
            "registerTable: commit failed. Will retry {} times. Cause {}", numRetries, txAbort);
        continue;
      } finally {
  ...
  /**
   * Inserts the current file descriptor and then performs a depth first search to insert its
   * depending file descriptors into the map in {@link TableDescriptors}.
   *
   * @param tableDescriptorsBuilder Builder instance.
   * @param rootFileDescriptor File descriptor to be added.
   */
  private void insertAllDependingFileDescriptorProtos(
      TableDescriptors.Builder tableDescriptorsBuilder, FileDescriptor rootFileDescriptor) {
    ...
    while (!fileDescriptorStack.isEmpty()) {
      ...
      if (tableDescriptorsBuilder
          .getFileDescriptorsMap()
          .containsKey(fileDescriptorProto.getName())) {
        continue;
      }

      // Add the fileDescriptorProto into the tableDescriptor map.
      tableDescriptorsBuilder.putFileDescriptors(
  ...
  /**
   * Gets the type Url of the protobuf descriptor. Used to identify the message during
   * serialization. Note: This is same as used in Any.proto.
   *
   ...
   * @throws IllegalAccessException If this is not a protobuf message.
   */
  public <K extends Message, V extends Message, M extends Message> Table<K, V, M> openTable(
  ...
  /**
   * Get an already opened table. Fetches the table from the cache given only the namespace and
   * table name. Throws a NoSuchElementException if table is not previously opened and not present
   * in cache.
   *
   * @param namespace Namespace of the table.
   * @param tableName Name of the table.
   ...
   * @return Table instance.
   */
  public <K extends Message, V extends Message, M extends Message> Table<K, V, M> getTable(
    ...
    if (!tableMap.containsKey(fullyQualifiedTableName)) {
      ...
      if (registryTable.containsKey(
        ...
        throw new IllegalArgumentException(
            "Please provide Key, Value & Metadata schemas to re-open"
                + " this existing table "
                + tableName
                + " in namespace "
                + namespace);
  ...
  /**
   * Only clears a table, DOES not delete its file descriptors from metadata. This is because if the
   * purpose of the delete is to upgrade from an old schema to a new schema Then we must first purge
   * all SMR entries of the current (old) format from corfu stream. It is the checkpointer which
   * actually purges via a trim operation. But to even get to the trim, it must be able to read the
   * table, which it can't do if the table's metadata (TableRegistry entry) is also removed here. So
   * what would be nice if we can place a marker indicating that this table is marked for deletion,
   * so that on the next Re-open we will actually force update the table registry entry. For now,
   * just force the openTable to always purge the prior entry assuming that the caller has done the
   * good work of 1. table.clear() using the old version 2. running checkpoint and trim 3. re-open
   * the table with new proto files.
   *
   * @param namespace Namespace of the table.
   * @param tableName Name of the table.
   */
  ...
  public Collection<TableName> listTables(@Nullable final String namespace) {
    return registryTable.keySet().stream()
        .filter(tableName -> namespace == null || tableName.getNamespace().equals(namespace))
        .collect(Collectors.toList());
  }

  /**
   * Gets the table descriptors for a particular fully qualified table name. This is used for
   * reconstructing a message when the schema is not available.
   *
