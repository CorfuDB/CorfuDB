...
/**
 * An abstract context stream view maintains contexts, which are used to implement copy-on-write
 * entries.
 *
 * <p>This implementation uses "contexts" to properly deal with copy-on-write streams. Every time a
 * stream is copied, a new context is created which redirects requests to the source stream for the
 * copy - each context contains its own queue and pointers. Implementers of fillReadQueue() and
 * readAndUpdatePointers should be careful to use the id of the context, rather than that of the
 * stream view itself.
 *
...
public abstract class AbstractContextStreamView<T extends AbstractStreamContext>
  ...
  /**
   * An ordered set of stream contexts, which store information about a stream copied via
   * copy-on-write entries. Streams which have never been copied have only a single context.
   */
   ...
   * @param contextFactory A function which generates a context, given the stream id and a maximum
   *     global address.
   */
  public AbstractContextStreamView(
      final CorfuRuntime runtime, final UUID id, final BiFunction<UUID, Long, T> contextFactory) {
    this.id = id;
  ...
  public final synchronized List<ILogData> remainingUpTo(long maxGlobal) {
    ...
    final List<ILogData> entries =
        getNextEntries(getCurrentContext(), maxGlobal, this::doesEntryUpdateContext);
    ...
    if (maxGlobal != Address.MAX) {
      ...
      // Update pointer from log data and then validate final position of the pointer against GC
      // trim mark.
   ...
   * @return True, if getNextEntry() may return an entry. False otherwise.
   */
   ...
   *
   * <p>This function is designed to implement a bulk read. In a bulk read, one of the entries may
   * cause the context to change - the implementation should check if the entry changes the context
   * and stop reading if this occurs, returning the entry that caused contextCheckFn to return true.
   *
  ...
  protected List<ILogData> getNextEntries(
      T context, long maxGlobal, Function<ILogData, Boolean> contextCheckFn) {
  ...
  protected boolean doesEntryUpdateContext(final ILogData data) {
    return data.hasBackpointer(getCurrentContext().id)
        && data.getBackpointer(getCurrentContext().id).equals(Address.COW_BACKPOINTER);
  }
  ...
  private void updatePointer(final ILogData data) {
    ...
    if ((data.isData() || data.isHole()) && !data.hasCheckpointMetadata()) {
      // Note: here we only set the global pointer and do not validate its position with respect to
      // the trim mark,
      // as the pointer is expected to be moving step by step (for instance when syncing a stream up
      // to maxGlobal)
      // The validation is deferred to these methods which call it in advance based on the expected
      // final position
      // of the pointer.
   ...
   *
   * <p>If it does, add it to the context stack. Otherwise, pop the context.
   *
   * <p>It is important that this method be called in order, since it updates the global pointer and
   * can change the global pointer.
   *
   ...
   *
   * <p>Should never throw a NoSuchElement exception because streamContexts should always at least
   * have one element.
   */
