...
public class SequencerView extends AbstractView {
    ...
    private void setupTimers() {
        ...
        queryRateTimer = metricsRegistry
                .map(registry ->
                        Timer.builder("sequencer.query").register(registry)
                );

        nextRateTimer = metricsRegistry
                .map(registry ->
                        Timer.builder("sequencer.next").register(registry)
                );

        resolutionRateTimer = metricsRegistry
                .map(registry -> Timer.builder("sequencer.tx_resolution").register(registry)
                );
    }
