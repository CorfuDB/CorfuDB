...
public class SequencerView extends AbstractView {
  ...
  private void setupTimers() {
    sequencerQuery = metricRegistry.timer(CorfuComponent.CLIENT_SEQUENCER + "query");
    sequencerTrimCache = metricRegistry.timer(CorfuComponent.CLIENT_SEQUENCER + "trim-cache");
    sequencerNextOneStream =
        metricRegistry.timer(CorfuComponent.CLIENT_SEQUENCER + "particular-next");
    sequencerNextMultipleStream =
        metricRegistry.timer(CorfuComponent.CLIENT_SEQUENCER + "multiple-next");
    sequencerDeprecatedNextOneStream =
        metricRegistry.timer(CorfuComponent.CLIENT_SEQUENCER + "deprecated-particular-next");
    sequencerDeprecatedNextMultipleStream =
        metricRegistry.timer(CorfuComponent.CLIENT_SEQUENCER + "deprecated-multiple-next");
  }
        ...
        return layoutHelper(
            e ->
                CFUtils.getUninterruptibly(
                    e.getPrimarySequencerClient().nextToken(Collections.emptyList(), 0)));
      } else {
        return layoutHelper(
            e ->
                CFUtils.getUninterruptibly(
                    e.getPrimarySequencerClient().nextToken(Arrays.asList(streamIds), 0)));
      }
    }
  }
  ...
  public long query(UUID streamId) {
    try (Timer.Context context = MetricsUtils.getConditionalContext(sequencerQuery)) {
      return layoutHelper(
              e ->
                  CFUtils.getUninterruptibly(
                      e.getPrimarySequencerClient().nextToken(Arrays.asList(streamId), 0)))
          .getStreamTail(streamId);
    }
  }
  ...
  public TokenResponse next(UUID... streamIds) {
    try (Timer.Context context = MetricsUtils.getConditionalContext(sequencerNextOneStream)) {
      return layoutHelper(
          e ->
              CFUtils.getUninterruptibly(
                  e.getPrimarySequencerClient().nextToken(Arrays.asList(streamIds), 1)));
    }
  }
   ...
   * @return address space composed of the trim mark and collection of all addresses belonging to
   *     this stream.
   */
  public StreamAddressSpace getStreamAddressSpace(StreamAddressRange streamsAddressesRange) {
    return getStreamsAddressSpace(Arrays.asList(streamsAddressesRange))
        .get(streamsAddressesRange.getStreamID());
  }
   ...
   */
  public Map<UUID, StreamAddressSpace> getStreamsAddressSpace(
    ...
    try (Timer.Context context = MetricsUtils.getConditionalContext(sequencerNextOneStream)) {
      StreamsAddressResponse streamsAddressResponse =
          layoutHelper(
              e ->
                  CFUtils.getUninterruptibly(
                      e.getPrimarySequencerClient().getStreamsAddressSpace(streamsAddressesRange)));
   ...
   * each stream (if streamIDs is not empty). The token returned is the global address as previously
   * defined, namely, max global address across all the streams.
   *
   * @param streamIDs The stream IDs to retrieve from.
  ...
  public TokenResponse nextToken(Set<UUID> streamIDs, int numTokens) {
    try (Timer.Context context =
        MetricsUtils.getConditionalContext(sequencerDeprecatedNextOneStream)) {
      return layoutHelper(
          e ->
              CFUtils.getUninterruptibly(
                  e.getPrimarySequencerClient()
  ...
  public TokenResponse nextToken(
      Set<UUID> streamIDs, int numTokens, TxResolutionInfo conflictInfo) {
    try (Timer.Context context =
        MetricsUtils.getConditionalContext(sequencerDeprecatedNextMultipleStream)) {
      return layoutHelper(
          e ->
              CFUtils.getUninterruptibly(
                  e.getPrimarySequencerClient()
