 ...
 *
 * <p>Given the snapshot (log address) given by the TxBuilder, access all objects within the same
 * snapshot during the course of this transactional context.
 *
...
public class SnapshotTransactionalContext extends AbstractTransactionalContext {
  ...
  @Getter private Set<ICorfuSMRProxyInternal> modifiedProxies = ImmutableSet.of();
  ...
  public <R, T extends ICorfuSMR<T>> R access(
    ...
    return proxy
        ...
        .access(
            o ->
                o.getVersionUnsafe() == getSnapshotTimestamp().getSequence()
