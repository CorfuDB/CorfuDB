 ...
 *
 * <p>(1) Read-your-own Writes: Reads in a transaction are guaranteed to observe a write in the same
 * transaction, if a write happens before the read.
 *
 * <p>(2) Opacity: Read in a transaction observe the state of the system ("snapshot") as of the time
 * of the first read which occurs in the transaction ("first read timestamp"), except in case (1)
 * above where they observe the own transaction's writes.
 *
 * <p>(3) Atomicity: Writes in a transaction are guaranteed to commit atomically, and commit if and
 * only if none of the objects which were read (the "read set") were modified between the first read
...
public class OptimisticTransactionalContext extends AbstractTransactionalContext {
  ...
  @Getter private final Set<ICorfuSMRProxyInternal> modifiedProxies = new HashSet<>();
   ...
   * snapshot version. If this succeeds, we invoke the corfu-object access method, and un-grab the
   * read-lock.
   *
   * <p>2. Otherwise, we grab a write-lock on the proxy and bring it to the correct version - Inside
   * proxy.setAsOptimisticStream, if there are currently optimistic updates on the proxy, we roll
   * them back. Then, we set this transactional context as the proxy's new optimistic context. -
   * Then, inside proxy.syncObjectUnsafe, depending on the proxy version, we may need to undo or
   * redo committed changes, or apply forward committed changes.
   *
   * <p>{@inheritDoc}
   */
  @Override
  public <R, T extends ICorfuSMR<T>> R access(
    ...
    return proxy
        ...
        .access(
            o -> {
              ...
              // remembered it.
              long streamReadPosition =
                  getKnownStreamPosition().getOrDefault(proxy.getStreamID(), ts);

              return ((stream == null || stream.isStreamCurrentContextThreadCurrentContext())
                  && (stream != null
                          && getWriteSetEntrySize(proxy.getStreamID()) == stream.pos() + 1
            ...
            o -> {
              ...
              syncWithRetryUnsafe(o, getSnapshotTimestamp(), proxy, o::setUncommittedChanges);
  ...
  /**
   * if a Corfu object's method is an Accessor-Mutator, then although the mutation is delayed, it
   * needs to obtain the result by invoking getUpcallResult() on the optimistic stream.
   *
   * <p>This is similar to the second stage of access(), accept working on the optimistic stream
   * instead of the underlying stream.- grabs the write-lock on the proxy. - uses
   * proxy.setAsOptimisticStream in order to set itself as the proxy optimistic context, including
   * rolling-back current optimistic changes, if any. - uses proxy.syncObjectUnsafe to bring the
   * proxy to the desired version, which includes applying optimistic updates of the current
   * transactional context.
   *
   * <p>{@inheritDoc}
   */
  @Override
  public <T extends ICorfuSMR<T>> Object getUpcallResult(
        ...
        .update(
            o -> {
              ...
              throw new RuntimeException(
                  "Tried to get upcall during a transaction but"
                      + " we don't have it even after an optimistic sync (asked for "
                      + timestamp
                      + " we have 0-"
                      + (getWriteSetEntryList(proxy.getStreamID()).size() - 1)
                      + ")");
            });
  }

  /**
   * Logs an update. In the case of an optimistic transaction, this update is logged to the write
   * set for the transaction.
   *
   * <p>Return the "address" of the update; used for retrieving results from operations via
   * getUpcallResult.
   *
  ...
  /**
   * Commit the transaction. If it is the last transaction in the stack, append it to the log,
   * otherwise merge it into a nested transaction.
   *
  ...
  public long getConflictSetAndCommit(ConflictSetInfo conflictSet) {
    ...
    Set<UUID> affectedStreamsIds =
        new HashSet<>(getWriteSetInfo().getWriteSet().getEntryMap().keySet());
