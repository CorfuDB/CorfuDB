...
public class Transaction {
  ...
  @Default final TransactionType type = TransactionType.OPTIMISTIC;
  ;

  /** For snapshot transactions, the address the snapshot will start at. */
  @Default final Token snapshot = Token.UNINITIALIZED;
  ;
  ...
  private void verify() {
    ...
    if (parentCtx != null) {
      ...
      if (type != parentCtx.getTransaction().getType()) {
        String msg =
            String.format(
                "Attempting to nest transactions with a "
      ...
      if (!this.snapshot.equals(Token.UNINITIALIZED) && !this.snapshot.equals(parentTimestamp)) {
        String msg =
            String.format(
                "Attempting to nest transactions with"
    ...
    //            ILogData pos =
    // getRuntime().getAddressSpaceView().peek(this.snapshot.getSequence());
    //            if (pos == null || !pos.getToken().equals(this.snapshot)) {
    //                String msg = String.format("User Defined snapshot(ts=%s) is invalid",
    // this.snapshot);
    //                throw new IllegalArgumentException(msg);
    //            }
    //        }
