...
/**
 * In the Corfu runtime, on top of a stream, an SMR object layer implements objects whose history of
 * updates are backed by a stream.
 *
 * <p>
 *
 * <p>This class implements the methods that an in-memory corfu-object proxy carries in order to by
 * in sync with a stream.
 *
 * <p>
 *
 * <p>We refer to the program's object as the -corfu object-, and to the internal object
 * implementation as the -proxy-.
 *
 * <p>
 *
 * <p>If a Corfu object's method is an Accessor, it invokes the proxy's access() method.
 *
 * <p>
 *
 * <p>If a Corfu object's method is a Mutator or Accessor-Mutator, it invokes the proxy's
 * logUpdate() method.
 *
 * <p>
 *
 * <p>Finally, if a Corfu object's method is an Accessor-Mutator, it obtains a result by invoking
 * getUpcallResult().
 *
 * <p>
 *
...
public class CorfuCompileProxy<T extends ICorfuSMR<T>> implements ICorfuSMRProxyInternal<T> {
  ...
  /**
   * The underlying object. This object stores the actual state as well as the version of the
   * object. It also provides locks to access the object safely from a multi-threaded context.
   */
  @Getter VersionLockedObject<T> underlyingObject;

  /** The CorfuRuntime. This allows us to interact with the Corfu log. */
  ...
  /**
   * The type of the underlying object. We use this to instantiate new instances of the underlying
   * object.
   */
  final Class<T> type;

  /** The serializer SMR entries will use to serialize their arguments. */
  @Getter ISerializer serializer;
  ...
  private <R> R accessInner(ICorfuSMRAccess<R, T> accessMethod, Object[] conflictObject) {
    if (TransactionalContext.isInTransaction()) {
      try {
        return TransactionalContext.getCurrentContext().access(this, accessMethod, conflictObject);
      } catch (Exception e) {
    ...
    for (int x = 0; x < rt.getParameters().getTrimRetry(); x++) {
      ...
      final long timestamp = rt.getSequencerView().query(getStreamID());
  ...
  public long logUpdate(
      ...
      final boolean keepUpcallResult,
  ...
  private <R> R getUpcallResultInner(long timestamp, Object[] conflictObject) {
    ...
    for (int x = 0; x < rt.getParameters().getTrimRetry(); x++) {
      try {
        return underlyingObject.update(
            o -> {
        ...
        log.info(
            "getUpcallResultInner: Encountered trimmed address space "
                + "while accessing version {} of stream {} on attempt {}",
            timestamp,
            getStreamID(),
            x);
  ...
  private <R> R TXExecuteInner(Supplier<R> txFunction) {
    ...
    while (true) {
      try {
        ...
        if (retries == 1) {
          MetricsUtils.incConditionalCounter(counterTxnRetry1, 1);
        }
  ...
  public long getVersion() {
    return access(o -> underlyingObject.getVersionUnsafe(), null);
  }
  ...
  private T getNewInstance() {
    try {
      T ret = (T) ReflectionUtils.findMatchingConstructor(type.getDeclaredConstructors(), args);
  ...
  private void abortTransaction(Exception e) {
    ...
    final TransactionAbortedException tae =
