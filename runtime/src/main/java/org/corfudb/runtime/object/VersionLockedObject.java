 ...
 *
 * <p>Users of the VersionLockedObject cannot access the versioned object directly, rather they use
 * the access() and update() methods to read and manipulate the object.
 *
 * <p>access() and update() allow the user to provide functions to execute under locks. These
 * functions execute various "unsafe" methods provided by this object which inspect and manipulate
 * the object state.
 *
...
public class VersionLockedObject<T extends ICorfuSMR<T>> {
  ...
  /**
   * A list of upcalls pending in the system. The proxy keeps this set so it can remember to save
   * the upcalls for pending requests.
   */
  @Getter private final Set<Long> pendingUpcalls;
  ...
  @Getter private final Map<Long, Object> upcallResults;

  /**
   * A lock, which controls access to modifications to the object. Any access to unsafe methods
   * should obtain the lock.
   */
   ...
   */
  public VersionLockedObject(
      Supplier<T> newObjectFn, StreamViewSMRAdapter smrStream, ICorfuSMR<T> wrapperObject) {
  ...
  /**
   * Access the internal state of the object, trying first to optimistically access the object, then
   * obtaining a write lock the optimistic access fails.
   *
   * <p>If the directAccessCheckFunction returns true, then we execute the accessFunction without
   * running updateFunction. If false, we execute the updateFunction to allow the user to modify the
   * state of the object before calling accessFunction.
   *
   * <p>directAccessCheckFunction is executed under an optimistic read lock. Read-only unsafe
   * operations are permitted.
   *
   * <p>updateFunction is executed under a write lock. Both read and write unsafe operations are
   * permitted.
   *
   * <p>accessFunction is accessed either under a read lock or write lock depending on whether an
   * update was necessary or not.
   *
   * @param directAccessCheckFunction A function which returns True if the object can be accessed
   *     without being updated.
   * @param updateFunction A function which is executed when direct access is not allowed and the
   *     object must be updated.
   * @param accessFunction A function which allows the user to directly access the object while
   *     locked in the state enforced by either the directAccessCheckFunction or updateFunction.
   * @param <R> The type of the access function return.
   * @return Returns the access function.
   */
  public <R> R access(
      Function<VersionLockedObject<T>, Boolean> directAccessCheckFunction,
    ...
    if (ts != 0) {
      try (Timer.Context optimistReadDuration = VloMetricsHelper.getOptimisticReadContext()) {
        ...
        log.warn(
            "Access [{}] Direct (optimistic-read) exception, upgrading lock. Exception ", this, e);
      }
    }
  ...
  /**
   * Roll the object back to the supplied version if possible. This function may roll back to a
   * point prior to the requested version. Otherwise, throws a NoRollbackException.
   *
   * <p>Unsafe, requires that the caller has acquired a write lock.
   *
   * @param timestamp The version to rollback to.
   * @throws NoRollbackException If the object cannot be rolled back to the supplied version.
   */
  ...
  /**
   * Move the pointer for this object (effectively, forcefuly change the version of this object
   * without playing any updates).
   *
  ...
  /**
   * Bring the object to the requested version, rolling back or syncing the object from the log if
   * necessary to reach the requested version.
   *
   ...
   *
   * <p>If the Optimistic stream doesn't reflect the current transaction context, we create the
   * correct WriteSetSMRStream and pick the latest context as the current context.
   */
  public void setUncommittedChanges() {
    WriteSetSMRStream stream = getOptimisticStreamUnsafe();
    if (stream == null || !stream.isStreamCurrentContextThreadCurrentContext()) {
  ...
  /**
   * Set the optimistic stream for this thread, rolling back any previous threads if they were
   * present.
   *
  ...
  /**
   * Get the version of this object. This corresponds to the position of the pointer into the SMR
   * stream.
   *
   ...
   *
   * <p>The format of this string is [type]@[version][+] (where + is the optimistic flag)
   *
  ...
  /**
   * Given a stream, return the context under which the underlying object should be updated.
   *
  ...
  private void applyUndoRecordUnsafe(SMREntry entry, ISMRStream stream) {
    ...
    if (undoFunction != null) {
      undoFunction.doUndo(
          object.getContext(context), entry.getUndoRecord(), entry.getSMRArguments());
      return;
  ...
  /**
   * Given a timestamp, return the context under which the underlying object should be updated.
   *
  ...
  private Object applyUpdateUnsafe(SMREntry entry, long timestamp) {
    ...
    if (!entry.isUndoable() || !Address.isAddress(entry.getGlobalAddress())) {
      ...
      IUndoRecordFunction<T> undoRecordTarget = undoRecordFunctionMap.get(entry.getSMRMethod());
      // If there was no previously calculated undo entry
      if (undoRecordTarget != null) {
        // Calculate the undo record.
        entry.setUndoRecord(
            undoRecordTarget.getUndoRecord(object.getContext(context), entry.getSMRArguments()));
  ...
  /**
   * Roll back the given stream by applying undo records in reverse order from the current stream
   * position until rollbackVersion.
   *
   * @param stream The stream of SMR updates to apply in reverse order.
   * @param rollbackVersion The version to stop roll back at.
   * @throws NoRollbackException If an entry in the stream did not contain undo information.
   */
    ...
    while (stream.pos() > rollbackVersion) {
      ...
      if (entries.stream().allMatch(SMREntry::isUndoable)) {
        ...
        if (log.isTraceEnabled()) {
          log.trace(
              "rollbackStreamUnsafe: one or more stream entries in address @{} are not undoable. "
                  + "Undoable entries: {}/{}",
              stream.pos(),
  ...
  /**
   * Sync this stream by playing updates forward in the stream until the given timestamp. If
   * Address.MAX is given, updates will be applied until the current tail of the stream. If
   * Address.OPTIMISTIC is given, updates will be applied to the end of the stream, and upcall
   * results will be stored in the resulting entries.
   *
   * <p>When the stream is trimmed, this exception is passed up to the caller, unless the timestamp
   * was Address.MAX, in which the entire object is reset and re-try the sync, which should pick up
   * any checkpoint that was inserted.
   *
  ...
  protected void syncStreamUnsafe(ISMRStream stream, long timestamp) {
    log.trace(
        "Sync[{}] {}", this, (timestamp == Address.OPTIMISTIC) ? "Optimistic" : "to " + timestamp);
            ...
            entry -> {
              try {
                ...
                if (timestamp == Address.OPTIMISTIC) {
                  ...
                  log.debug("Sync[{}] Upcall Result {}", this, entry.getGlobalAddress());
                  upcallResults.put(
                      entry.getGlobalAddress(), res == null ? NullValue.NULL_VALUE : res);
  ...
  private static class VloMetricsHelper {
    ...
    private static final String VLO_OPTIMISTIC_READ =
        CorfuComponent.OBJECT.toString() + "vlo.optimistic-read";
    private static final String VLO_UPDATED_OBJECT_READ =
        CorfuComponent.OBJECT.toString() + "vlo.updated-object-read";
