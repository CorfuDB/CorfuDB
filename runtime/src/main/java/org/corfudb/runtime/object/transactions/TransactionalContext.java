...
/**
 * A class which allows access to transactional contexts, which manage transactions. The static
 * methods of this class provide access to the thread's transaction stack, which is a stack of
 * transaction contexts active for a particular thread.
 *
...
public class TransactionalContext {
  ...
  /** A thread local stack containing all transaction contexts for a given thread. */
  private static final ThreadLocal<Deque<AbstractTransactionalContext>> threadTransactionStack =
      ThreadLocal.withInitial(LinkedList<AbstractTransactionalContext>::new);
  ...
  /**
   * Callback context for objects that need special processing with TokenResponse That is returned
   * after conflict resolution is successful but before write happens. This can be used to capture
   * the commit address of the object into the object itself.
   */
