...
public abstract class AbstractVersionedObject<S extends SnapshotGenerator<S>> {
     ...
     */
    protected final ISMRStream smrStream;
     ...
     */
    protected abstract void syncStreamUnsafe(long timestamp);

    /**
     * Return the MVOCache associated with this object, if present.
     * Otherwise, this method throws an UnsupportedOperationException.
     */
    public abstract MVOCache<S> getMvoCache();
     ...
     */
    protected void applyUpdateUnsafe(@Nonnull SMREntry updateEntry) {
        if (log.isTraceEnabled()) {
