...
/**
 * StreamViewSMRAdapter wraps a stream and implements the ISMRStream API over it.
 *
 * <p>This is a relatively thin wrapper. For example, an underlying stream returns from current() a
 * LogData entry. StreamViewSMRAdapter verifies that the entry contains data (otherwise, return
 * null), and that the payload is of type ISMRConsumable (otherwise, again return null). Since the
 * underlying log supports multi-stream entries, it collects and returns the SMREntries related to
 * the current stream.
 *
...
public class StreamViewSMRAdapter implements ISMRStream {
  ...
  /**
   * Returns the list of SMREntries positioned at the current global log. It returns null of no data
   * is null.
   *
   ...
   *
   * @return Returns the list of SMREntries positioned at the previous log position of this stream.
   */
  public List<SMREntry> previous() {
    ILogData data = streamView.previous();

    while (Address.isAddress(streamView.getCurrentGlobalPosition()) && data != null) {
      if (data.getType() == DataType.DATA && data.getPayload(runtime) instanceof ISMRConsumable) {
        return ((ISMRConsumable) data.getPayload(runtime)).getSMRUpdates(streamView.getId());
      }
