...
public class TransactionAbortedException extends RuntimeException {
  ...
  @Getter AbortCause abortCause;

  @Getter TxResolutionInfo txResolutionInfo;

  /** The key that triggered the abort. */
  @Getter byte[] conflictKey;

  /** The stream ID at which the conflict occurred. */
  @Getter UUID conflictStream;

  /** The address at which the conflict occurred. */
  @Getter Long offendingAddress;

  @Getter Throwable cause;

  @Getter AbstractTransactionalContext context;
  ...
  public TransactionAbortedException(
    ...
    this(
        txResolutionInfo, conflictKey, conflictStream, offendingAddress, abortCause, null, context);
  }
  ...
  public TransactionAbortedException(
    ...
    super(
        "TX ABORT "
            + " | Snapshot Time = "
            + txResolutionInfo.getSnapshotTimestamp()
            + " | Failed Transaction ID = "
            + txResolutionInfo.getTXid()
            + " | Offending Address = "
            + offendingAddress
            + " | Conflict Key = "
            + Utils.bytesToHex(conflictKey)
            + " | Conflict Stream = "
            + conflictStream
            + " | Cause = "
            + abortCause
            + " | Time = "
            + (context == null ? "Unknown" : System.currentTimeMillis() - context.getStartTime())
            + " ms"
