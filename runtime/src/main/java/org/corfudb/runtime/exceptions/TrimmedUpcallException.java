...
/**
 * This exception is thrown when a client attempts to resolve an upcall, but the address is trimmed
 * before the upcall result can be resolved.
 *
...
public class TrimmedUpcallException extends TrimmedException {
  ...
  public TrimmedUpcallException(long address) {
    super(
        "Attempted to get upcall result @"
            + address
