...
/**
 * A wrapper class that makes a access to the RocksIterator safe: the rocks library does some checks
 * but they are enforced via assert, but assert checking is disabled on many jvms by default, hence
 * the explicit checking.
 *
 * <p>Created by Maithem on 1/24/20.
 */
public class WrappedRocksIterator implements RocksIteratorInterface {
  ...
  /**
   * A helper method that is called before accessing the iterator. This is necessary to make sure
   * that the iterator hasn't been closed.
   */
