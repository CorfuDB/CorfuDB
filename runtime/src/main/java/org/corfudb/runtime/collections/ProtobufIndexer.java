...
/**
 * This layer implements the ProtobufIndexer uses the special FieldOptions that the application can
 * place on its protobuf definitions (like secondary_key) and create secondary indexes callbacks
 * over CorfuTable based on that.
 *
 * <p>Created by hisundar on 2019-08-12.
 */
public class ProtobufIndexer implements Index.Registry<Message, CorfuRecord<Message, Message>> {

  private final HashMap<
          String, Index.Spec<Message, CorfuRecord<Message, Message>, ? extends Comparable<?>>>
      indices = new HashMap<>();
  ...
  private <T extends Comparable<T>>
      Index.Spec<Message, CorfuRecord<Message, Message>, ? extends Comparable<?>> getIndex(
          String indexName, FieldDescriptor fieldDescriptor) {
  ...
  @Override
  public Optional<Index.Spec<Message, CorfuRecord<Message, Message>, ? extends Comparable<?>>> get(
      Index.Name name) {
    return Optional.ofNullable(name).map(indexName -> indices.get(indexName));
  }

  @Override
  public Iterator<Index.Spec<Message, CorfuRecord<Message, Message>, ? extends Comparable<?>>>
      iterator() {
