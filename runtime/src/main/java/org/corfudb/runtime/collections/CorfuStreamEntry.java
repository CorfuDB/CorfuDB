 ...
 *     <p>NOTE: Ensure that the above protobuf generated classes are accessible to your JVM!
 *     <p>Created by hisundar on 2019-10-18
 */
@Slf4j
public class CorfuStreamEntry<K extends Message, V extends Message, M extends Message> {
  ...
  @Getter private final OperationType operation;
  /** Version number of the layout at the time of this entry. */
  @Getter private final long epoch;
  /** Stream address of this entry */
  @Getter private final long address;

  public CorfuStreamEntry(
      K key, V payload, M metadata, long epoch, long address, OperationType operation) {
    this.key = key;
  ...
  public static <K extends Message, V extends Message, M extends Message>
      CorfuStreamEntry<K, V, M> fromSMREntry(
          SMREntry entry,
          @Nonnull final long epoch,
    ...
    log.trace(
        "fromSRMEntry: Table {} streamer got SMR {}", keyClass.getName(), entry.getSMRMethod());
    switch (entry.getSMRMethod()) {
      ...
      default:
        throw new RuntimeException(
            "SMRMethod " + entry.getSMRMethod() + " cannot be translated to CorfuStreamEntry");
    }
    // TODO[sneginhal]: Need a way to differentiate between update and create.
    Object[] args = entry.getSMRArguments();

    log.trace(
        "Converting SMREntry at address {} to CorfuStreamEntry: Operation: {} Length of arguments:"
            + " {}. ({} -> {}, {})",
        address,
        operationType,
        args.length,
