 ...
 *
 * <p>The primary interface to the CorfuTable is a Map, where the keys must be unique and each key
 * is mapped to exactly one value. Null values are not permitted.
 *
 * <p>The CorfuTable also supports an unlimited number of secondary indexes, which the user provides
 * at construction time as an enum which implements the IndexSpecification interface. An index
 * specification consists of a IndexFunction, which returns a set of secondary keys (indexes) the
 * value should be mapped to. Secondary indexes are many-to-many: values can be mapped to multiple
 * indexes, and indexes can be mapped to multiples values. Each IndexSpecification also specifies a
 * projection function, which specifies a transformation that can be done on a retrieval on the
 * index. A common projection is to emit only the values.
 *
...
public class CorfuTable<K, V> implements ICorfuTable<K, V>, ICorfuSMR<CorfuTable<K, V>> {
  ...
  // Accessor/Mutator threads can interleave in a way that create a deadlock because they can create
  // a
  // circular dependency between the VersionLockedObject(VLO) lock and the common forkjoin thread
  // pool. In order
  // to break the dependency, parallel stream operations have to execute on a separate pool that
  // applications
  // cant use. For example, if there are 4 accessor threads all using the common forkjoin pool, one
  // of the threads
  // can acquire the VLO lock and cause the other 3 threads to wait, but after acquiring the VLO
  // lock, the thread
  // gets block on parallel stream, because the pool is exhausted with threads that are trying to
  // acquire the VLO
  ...
  /**
   * The main constructor that generates a table with a given implementation of the {@link
   * StreamingMap} along with {@link Index.Registry} and {@link VersionPolicy} specification.
   */
  public CorfuTable(
      Index.Registry<K, V> indices,
    ...
    this.optimisticTable =
        new CorfuTable<>(
            this.mainMap.getOptimisticMap(), this.indexSpec, this.secondaryIndexes, null);
  ...
  /**
   * Generate a table with a given implementation for the {@link StreamingMap} and {@link
   * Index.Registry}.
   */
  public CorfuTable(
      Index.Registry<K, V> indices, Supplier<ContextAwareMap<K, V>> streamingMapSupplier) {
   ...
   */
  public CorfuTable(
      Supplier<ContextAwareMap<K, V>> streamingMapSupplier, VersionPolicy versionPolicy) {
  ...
  public CorfuTable(Index.Registry<K, V> indexRegistry) {
    this(
        indexRegistry,
        () -> new StreamingMapDecorator<>(new HashMap<>()),
  ...
  @Accessor
  public @Nonnull <I extends Comparable<I>> Collection<Entry<K, V>> getByIndex(
      @Nonnull Index.Name indexName, I indexKey) {
    ...
    if (secondaryIndexes.containsKey(secondaryIndex)
      ...
      return res == null ? Collections.emptySet() : new HashSet<>(res.entrySet());
    }

    // If index is not specified, the lookup by index API must fail.
    log.error(
        "CorfuTable: secondary index "
            + secondaryIndex
   ...
   */
  @Accessor
  public @Nonnull <I extends Comparable<I>> Collection<Map.Entry<K, V>> getByIndexAndFilter(
      @Nonnull Index.Name indexName,
      @Nonnull Predicate<? super Entry<K, V>> entryPredicate,
      I indexKey) {
    ...
    log.error(
        "CorfuTable: secondary index "
            + secondaryIndex
            + " does not exist for this table, cannot complete the get by index and filter.");
    throw new IllegalArgumentException("Secondary Index " + secondaryIndex + " is not defined.");
  }
  ...
  Object[] putAllConflictFunction(Map<? extends K, ? extends V> m) {
    return m.keySet().stream().map(Object::hashCode).toArray(Object[]::new);
  }
  ...
  /**
   * Generate an undo record for putAll, given the previous state of the map and the parameters to
   * the putAll call.
   *
   * @param previousState The previous state of the map
   * @param m The map from the putAll call
   * @return An undo record, which for a putAll is all the previous entries in the map.
   */
  @DontInstrument
  @SuppressWarnings("unchecked")
  Map<K, V> undoPutAllRecord(CorfuTable<K, V> previousState, Map<? extends K, ? extends V> m) {
  ...
  /**
   * Undo a remove, given the current state of the map, an undo record and the arguments to the
   * remove command to undo.
   *
   * @param table The state of the map after the put to undo
   * @param undoRecord The undo record generated by undoRemoveRecord
   */
  @DontInstrument
  void undoPutAll(CorfuTable<K, V> table, Map<K, V> undoRecord, Map<? extends K, ? extends V> m) {
    undoRecord
        .entrySet()
        .forEach(
            e -> {
  ...
  public Collection<Map.Entry<K, V>> scanAndFilterByEntry(
    ...
    try (Stream<Map.Entry<K, V>> entries = mainMap.unsafeEntryStream()) {
      return pool.submit(
              () -> entries.filter(entryPredicate).collect(Collectors.toCollection(ArrayList::new)))
          .join();
    }
  }
  ...
  /**
   * Makes a shallow copy of the map (i.e. all map entries), but not the key/values (i.e only the
   * mappings are copied).
   */
  @Override
  @Accessor
  ...
  /**
   * Present the content of a {@link CorfuTable} via the {@link Stream} interface. Because the
   * stream can point to other resources managed off-heap, its necessary to explicitly close it
   * after consumption.
   *
  ...
  protected void unmapSecondaryIndexes(K key, V value) {
    ...
    try {
      ...
      log.error(
          "Received an exception while computing the index. "
              + "This is most likely an issue with the client's indexing function. {}",
          e);
