...
public class Index {
  ...
  /**
   * Denotes a function that supplies the unique name of an index registered to {@link CorfuTable}.
   */
  @FunctionalInterface
  public interface Name extends Supplier<String> {}

  /**
   * Denotes a function that takes as input the key and value of an {@link CorfuTable} record, and
   * computes the associated index value for the record.
   *
  ...
  /**
   * Descriptor of named indexing function entry. The indexing function can be single indexer {@link
   * Function} mapping a value to single secondary index value, or a multi indexer {@link Function}
  ...
  public static class Spec<K, V, I extends Comparable<?>> {
    ...
    public Spec(Name name, Function<K, V, I> indexFunction) {
      this.name = name;
      this.indexFunction = (k, v) -> Collections.singletonList(indexFunction.apply(k, v));
    }
   ...
   */
  public interface Registry<K, V> extends Iterable<Spec<K, V, ? extends Comparable<?>>> {

    Registry<?, ?> EMPTY =
        new Registry<Object, Object>() {
          @Override
