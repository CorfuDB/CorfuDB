...
/**
 * A concrete implementation of {@link StreamingMap} that is capable of storing data off-heap. The
 * location for the off-heap data is provided by {@link File} dataPath, while the resource policy
 * (memory and storage limits) are defined in {@link Options}.
 *
 * @param <K> key type
 * @param <V> value type
 */
...
public class PersistedStreamingMap<K, V> implements ContextAwareMap<K, V> {
   ...
   *
   * <p>For a set of options that dictate RocksDB memory usage can be found here:
   * https://github.com/facebook/rocksdb/wiki/Memory-usage-in-RocksDB
   *
   * <p>Block Cache: Which can be set via Options::setTableFormatConfig. Out of box, RocksDB will
   * use LRU-based block cache implementation with 8MB capacity. Index/Filter: Is a function of the
   * block cache. Generally it inflates the block cache by about 50%. The exact number can be
   * retrieved via "rocksdb.estimate-table-readers-mem" property. Write Buffer: Also known as
   * memtable is defined by the ColumnFamilyOptions option. The default is 64 MB.
   */
  ...
  public PersistedStreamingMap(
      @NonNull Path dataPath,
  ...
  public boolean containsKey(@NonNull Object key) {
    ...
    try {
      byte[] value =
          rocksDb.get(keyPayload.array(), keyPayload.arrayOffset(), keyPayload.readableBytes());
      return value != null;
   ...
   * {@inheritDoc}
   *
   * <p>Please use {@link StreamingMap#entryStream()}.
   */
  @Override
  ...
  public V put(@NonNull K key, @NonNull V value) {
    ...
    final boolean keyExists =
        rocksDb.keyMayExist(
            keyPayload.array(),
            keyPayload.arrayOffset(),
            keyPayload.readableBytes(),
            new Holder<>());
    if (!keyExists) {
    ...
    try {
      rocksDb.put(
          ...
          keyPayload.readableBytes(),
          valuePayload.array(),
          valuePayload.arrayOffset(),
          valuePayload.readableBytes());
    } catch (RocksDBException ex) {
      throw new UnrecoverableCorfuError(ex);
    } finally {
      keyPayload.release();
  ...
  public V remove(@NonNull Object key) {
    ...
    try {
      V value = get(key);
      if (value != null) {
        rocksDb.delete(keyPayload.array(), keyPayload.arrayOffset(), keyPayload.readableBytes());
  ...
  public Stream<Entry<K, V>> entryStream() {
    final RocksDbEntryIterator<K, V> entryIterator =
        new RocksDbEntryIterator<>(rocksDb, serializer);
    Stream<Entry<K, V>> resStream =
