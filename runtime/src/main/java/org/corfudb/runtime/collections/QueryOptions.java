...
public class QueryOptions<K extends Message, V extends Message, M extends Message, R> {
  ...
  public static class QueryOptionsBuilder<
      K extends Message, V extends Message, M extends Message, R> {
    ...
    public QueryOptionsBuilder<K, V, M, R> setProjection(
        Function<CorfuStoreEntry<K, V, M>, R> projection) {
      this.projection = projection;
      return this;
    }
    ...
    public QueryOptions<K, V, M, R> build() {
      return new QueryOptions<>(timestamp, distinct, comparator, projection);
    }
  }
}
