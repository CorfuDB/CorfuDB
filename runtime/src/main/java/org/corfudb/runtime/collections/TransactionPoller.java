...
public class TransactionPoller implements Runnable {
   ...
   *
   * <p>This is the list of StreamingSubscriptionContexts sorted by the last read addresses.
   */
  private final List<StreamingSubscriptionContext> streamContexts;

  /** A reference to the transaction stream */
  @Getter private final IStreamView txnStream;
  ...
  public TransactionPoller(
      @Nonnull CorfuRuntime runtime, @Nonnull List<StreamingSubscriptionContext> streams) {
    ...
    StreamOptions options = StreamOptions.builder().cacheEntries(false).build();

    txnStream = runtime.getStreamsView().getUnsafe(ObjectsView.TRANSACTION_STREAM_ID, options);
  }
  ...
  private void pollTxStream() {
    ...
    log.trace(
        "txStream current global position after seeking {}, hasNext {}",
        txnStream.getCurrentGlobalPosition(),
        txnStream.hasNext());
