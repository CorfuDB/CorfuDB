 ...
 *
 * <p>Key APIs exposed are: o-> TxBuilder() for CRUD operations o-> getTimestamp() for database
 * snapshots o-> table lifecycle management
 *
 * <p>By itself it is a lightweight layer and only carries the CorfuRuntime and therefore can be
 * instantiated many times with the same runtime.
 *
 * <p>Created by zlokhandwala on 2019-08-02.
 */
public class CorfuStore {
  ...
  /**
   * Creates and registers a table. A table needs to be registered before it is used.
   *
  ...
  public <K extends Message, V extends Message, M extends Message> Table<K, V, M> openTable(
    ...
    return runtime
        .getTableRegistry()
        .openTable(namespace, tableName, kClass, vClass, mClass, tableOptions);
  }

  /**
   * Fetches an existing table. This table should have been registered with this instance of the
   * Corfu runtime.
   *
   * @param namespace Namespace of the table.
   * @param tableName Table name.
  ...
  @Nonnull
  public <K extends Message, V extends Message, M extends Message> Table<K, V, M> openTable(
      @Nonnull final String namespace, @Nonnull final String tableName) {
  ...
  /**
   * Lists all the tables in a particular namespace. Lists all the tables in the database if
   * namespace is null.
   *
  ...
  /**
   * Start appending mutations to a transaction. The transaction does not begin until a commit is
   * invoked. On a commit the latest available snapshot will be used to resolve the transaction.
   *
  ...
  public TxBuilder tx(@Nonnull final String namespace) {
    return new TxBuilder(this.runtime.getObjectsView(), this.runtime.getTableRegistry(), namespace);
  }
  ...
  public Query query(@Nonnull final String namespace) {
    return new Query(this.runtime.getTableRegistry(), this.runtime.getObjectsView(), namespace);
  }

  /**
   * Subscribe to a specific a table in a namespace or the entire namespace. Objects returned will
   * honor transactional boundaries
   *
   ...
   * @param timestamp - If specified, all stream updates from this timestamp will be returned - if
   *     null, only future updates will be returned.
   */
  public <K extends Message, V extends Message, M extends Message> void subscribe(
      @Nonnull StreamListener streamListener,
      @Nonnull String namespace,
      @Nonnull List<TableSchema> tablesOfInterest,
      @Nullable Timestamp timestamp) {
    txnStreamingManager.subscribe(
        streamListener,
        namespace,
        tablesOfInterest,
  ...
  /**
   * Gracefully shutdown a streamer. Once this call returns no further stream updates will be
   * returned.
   *
   * @param streamListener - callback context.
   */
