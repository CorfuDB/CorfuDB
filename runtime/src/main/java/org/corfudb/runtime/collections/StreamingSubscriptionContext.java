 ...
 *
 * <p>Created by sneginhal on 10/22/2019.
 */
@Slf4j
public class StreamingSubscriptionContext {
  ...
  /** StreamListener. */
  @Getter private final StreamListener listener;
   ...
   * Tables of interest.
   *
   * <p>Map of (Stream Id - TableSchema)
   */
  @Getter
  private final Map<UUID, TableSchema<? extends Message, ? extends Message, ? extends Message>>
      tablesOfInterest;
  ...
  @Getter private long lastReadAddress;
  ...
  @Getter @Setter private volatile Throwable pollerException = null;

  /**
   * Is the StreamingSubscriptionContext ready to be reaped.
   *
   * <p>After an exception has been delivered to the client, the StreamingSubscriptionContext is no
   * longer active and can be reaped.
   */
  @Getter private volatile boolean readyToBeReaped = false;
  ...
  private static final long thresholdForLongRunningCallsInNano = 60L * 1_000_000_000;
   ...
   */
  public StreamingSubscriptionContext(
  ...
  public boolean enqueueStreamEntry(long lastAddress, ILogData logData) {
    ...
    if (logDataStreams.isEmpty()) {
      log.trace(
          "Entry at address {} does not contain any streams of interest to listener {}",
    ...
    synchronized (this) {
      ...
      if (lastReadAddress != lastAddress) {
        log.error(
            "Incorrect address given to enqueueStreamEntry. Given {} Expected {}",
            lastAddress,
            lastReadAddress);
        return false;
      }
      ...
      CorfuStreamEntries update =
          new CorfuStreamEntries(
              multiObjSMREntry.getEntryMap().entrySet().stream()
                  .filter(e -> tablesOfInterest.containsKey(e.getKey()))
                  .collect(
                      Collectors.toMap(
                          e -> tablesOfInterest.get(e.getKey()),
                          e ->
                              e.getValue().getUpdates().stream()
                                  .map(
                                      smrEntry ->
                                          CorfuStreamEntry.fromSMREntry(
                                              smrEntry,
                                              epoch,
  ...
  public void notifyClient(int numUpdatesToDeliver) {
    if (notificationInProgress.compareAndSet(false, true)) {
      ...
      try {
        ...
        do {
          ...
          if (pollerException != null) {
            ...
            if (!readyToBeReaped) {
              log.warn(
                  "Encountered {} when streaming updates on namespace {}, " + "{} for {}",
                  pollerException,
                  namespace,
                  listener.toString());
              listener.onError(pollerException);
              log.trace("Throwable {} delivered to {}", pollerException, listener.toString());
        ...
        log.error(
            "Aborting client notification for {} due to an exception {}", listener.toString(), t);
      } finally {
        log.trace(
            "End of notification for {}, delivered {} updates",
            listener.toString(),
            updatesDelivered);
      ...
      log.trace(
          "Another notification thread is already servicing this "
              + "StreamingSubscriptionContext {}",
          listener.toString());
    }
  }
