        ...
        try (Timer.Context context = MetricsUtils.getConditionalContext(appendCheckpointsTimer)) {
            for (ICorfuSMR<T> map : maps) {
                ...
                ISerializer serializer = ((CorfuCompileProxy) map.getCorfuSMRProxy())
                        .getSerializer();
                CheckpointWriter<T> cpw = new CheckpointWriter<>(
                        rt, streamId, author, ClassUtils.cast(map), serializer
                );
                ...
                while (retry < numRetries) {
                    try {
                        ...
                        log.info("Epoch changed to {} during append checkpoint snapshot " +
                                        "resolution. Sequencer failover can lead to potential" +
                                        " epoch regression, retry {}/{}", wee.getCorrectEpoch(),
                                retry, numRetries
                        );
                        retry++;
                        if (retry == numRetries) {
                            String msg = String.format("Epochs changed during checkpoint cycle, " +
                                    "over more than %s times. Potential sequencer regressions " +
                                    "can lead to data loss. Aborting.", numRetries);
                ...
                if (minSnapshot == Token.UNINITIALIZED) {
                    ...
                    // Given that the snapshot returned by appendCheckpoint is a global
                    // snapshot that shouldn't regress.
                    String msg = String.format("Potential epoch regression. " +
                            "Subsequent checkpoint returned a greater" +
                            "snapshot %s than previous %s.", minCPSnapshot, minSnapshot);
                    throw new IllegalStateException(msg);
                }
            }
        } finally {
