...
public class MultiCheckpointWriter<T extends StreamingMap> {
  @Getter private List<ICorfuSMR<T>> maps = new ArrayList<>();
  ...
  private static final String MULTI_CHECKPOINT_TIMER_NAME =
      CorfuComponent.GARBAGE_COLLECTION + "append-several-checkpoints";
  ...
  /**
   * Checkpoint multiple CorfuTables. Since this method is Map specific then the keys are unique and
  ...
  public Token appendCheckpoints(CorfuRuntime rt, String author) {
    ...
    try (Timer.Context context = MetricsUtils.getConditionalContext(appendCheckpointsTimer)) {
      for (ICorfuSMR<T> map : maps) {
        ...
        ISerializer serializer = ((CorfuCompileProxy) map.getCorfuSMRProxy()).getSerializer();
        ...
        while (retry < numRetries) {
          try {
            ...
            log.info(
                "Epoch changed to {} during append checkpoint snapshot resolution. Sequencer"
                    + " failover can lead to potential epoch regression, retry {}/{}",
                wee.getCorrectEpoch(),
                retry,
                numRetries);
            retry++;
            if (retry == numRetries) {
              String msg =
                  String.format(
                      "Epochs changed during checkpoint cycle, over more than %s times. Potential"
                          + " sequencer regressions can lead to data loss. Aborting.",
                      numRetries);
        ...
        if (minSnapshot == Token.UNINITIALIZED) {
          ...
          // Given that the snapshot returned by appendCheckpoint is a global snapshot that
          // shouldn't regress.
          String msg =
              String.format(
                  "Potential epoch regression. Subsequent checkpoint returned a greater"
                      + "snapshot {} than previous {}.",
                  minCPSnapshot,
                  minSnapshot);
          throw new IllegalStateException(msg);
        }
      ...
      log.trace(
          "appendCheckpoints: finished, author '{}' at min globalAddress {}", author, minSnapshot);
      rt.getObjectsView().TXEnd();
    }
    final long cpStop = System.currentTimeMillis();

    log.info(
        "appendCheckpoints: took {} ms to append {} checkpoints", cpStop - cpStart, maps.size());
    return minSnapshot;
  }
}
