...
/**
 * Utility to Reboot a server which includes reset or restart Reset wipes out all existing Corfu
 * data while restart does not.
 *
...
public class RebootUtil {
  ...
  /**
   * Resets the given server. Attempts to reset a server finite number of times. Note: reset will
   * wipe out all existing Corfu data. If the retries are exhausted, the utility throws the
   ...
   * @param clusterId Optional cluster Id. If None is provided, then the default one will be used to
  ...
  /**
   * Restarts the given server. Attempts to restart a server finite number of times. Note: restart
   * will NOT wipe out any existing data. If the retries are exhausted, the utility throws the
   * responsible exception.
   *
   * @param endpoint endpoint of the server to reset
   * @param retries Number of retries to bootstrap each node before giving up.
  ...
  private static void reboot(
    ...
    try (NettyClientRouter router =
        new NettyClientRouter(NodeLocator.parseString(endpoint), corfuRuntimeParameters)) {
      router.addClient(new BaseHandler());
      BaseClient baseClient =
          new BaseClient(router, Layout.INVALID_EPOCH, clusterId.orElse(DEFAULT_CLUSTER_ID));
