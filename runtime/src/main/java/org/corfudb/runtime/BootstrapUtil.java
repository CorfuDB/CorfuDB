...
public class BootstrapUtil {
  ...
  /**
   * Bootstraps the given layout. Attempts to bootstrap each node finite number of times. If the
   * retries are exhausted, the utility throws the responsible exception.
   *
  ...
  public static void bootstrap(
      @NonNull Layout layout, int retries, @NonNull Duration retryDuration) {
  ...
  private static void bootstrapLayoutServer(IClientRouter router, Layout layout)
    ...
    try {
      CFUtils.getUninterruptibly(
          layoutClient.bootstrapLayout(layout), AlreadyBootstrappedException.class);
    } catch (AlreadyBootstrappedException abe) {
      if (!layoutClient.getLayout().get().equals(layout)) {
        log.error(
            "BootstrapUtil: Layout Server {}:{} already bootstrapped with different " + "layout.",
            router.getHost(),
            router.getPort());
        throw abe;
      }
  ...
  private static void bootstrapManagementServer(IClientRouter router, Layout layout)
    ...
    try {
      CFUtils.getUninterruptibly(
          managementClient.bootstrapManagement(layout), AlreadyBootstrappedException.class);
    } catch (AlreadyBootstrappedException abe) {
      if (!managementClient.getLayout().get().equals(layout)) {
        log.error(
            "BootstrapUtil: Management Server {}:{} already bootstrapped with "
                + "different layout.",
            router.getHost(),
            router.getPort());
        throw abe;
  ...
  public static void bootstrap(
    ...
    for (String server : layout.getAllServers()) {
      ...
      try (NettyClientRouter router =
          new NettyClientRouter(NodeLocator.parseString(server), corfuRuntimeParameters)) {
        router
