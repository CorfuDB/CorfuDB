...
public class NodeRouterPool {
   ...
   */
  @Getter @Setter private Function<String, IClientRouter> createRouterFunction;
  ...
  /**
   * Fetches a router from the pool if already present. Else creates a new router using the provided
   * function and adds it to the pool.
   *
   * @param endpoint Endpoint to connect the router.
   * @return IClientRouter.
   */
  public IClientRouter getRouter(NodeLocator endpoint) {
    return nodeRouters.computeIfAbsent(
        endpoint, s -> createRouterFunction.apply(s.toEndpointUrl()));
  }
