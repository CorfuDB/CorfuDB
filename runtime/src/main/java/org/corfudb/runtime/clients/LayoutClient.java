 ...
 *
 * <p>The layout client is used by layout servers to initiate a Paxos-based protocol for determining
 * the next layout. Created by mwei on 12/9/15.
 */
public class LayoutClient extends AbstractClient {
   ...
   * @return A completable future which will return TRUE if the bootstrap was successful, false
   *     otherwise.
   */
   ...
   * @return True, if the prepare was successful. Otherwise, the completableFuture completes
   *     exceptionally with OutrankedException.
   */
   ...
   *
   * @param epoch epoch for which the paxos rounds are being run
   * @param rank The rank to use for the propose. It should be the same rank from a successful
   *     prepare (phase 1).
   * @param layout The layout to install for phase 2.
   * @return True, if the propose was successful. Otherwise, the completableFuture completes
   *     exceptionally with OutrankedException.
   */
  ...
  public CompletableFuture<Boolean> force(@Nonnull Layout layout) {
    return sendMessageWithFuture(
        CorfuMsgType.LAYOUT_COMMITTED.payloadMsg(
            new LayoutCommittedRequest(true, layout.getEpoch(), layout)));
  }
}
