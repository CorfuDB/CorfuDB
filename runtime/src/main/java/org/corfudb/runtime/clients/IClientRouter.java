...
/**
 * This is an interface in which all client routers must implement. Client routers are classes which
 * talk to server routers. Clients are registered on client routers using the addClient() interface,
 * and can be retrieved using the getClient() interface.
 *
...
public interface IClientRouter {
  ...
  <T> CompletableFuture<T> sendMessageAndGetCompletable(
      ChannelHandlerContext ctx, CorfuMsg message);

  /**
   * Send a message using the router channel handler and get a completable future to be fulfilled by
   * the reply.
   *
   * @param message The message to send.
   * @param <T> The type of completable to return.
