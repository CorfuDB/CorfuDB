 ...
 *
 * <p>Failure Detection: This client allows a client to trigger failures handlers with relevant
 * failures.
 *
...
public class ManagementClient extends AbstractClient {
  ...
  public CompletableFuture<Boolean> handleFailure(long detectorEpoch, Set<String> failedNodes) {
    return sendMessageWithFuture(
        CorfuMsgType.MANAGEMENT_FAILURE_DETECTED.payloadMsg(
  ...
  public CompletableFuture<Boolean> handleHealing(long detectorEpoch, Set<String> healedNodes) {
    return sendMessageWithFuture(
        CorfuMsgType.MANAGEMENT_HEALING_DETECTED.payloadMsg(
            new DetectorMsg(detectorEpoch, Collections.emptySet(), healedNodes)));
  }
  ...
  public CreateWorkflowResponse addNodeRequest(@Nonnull String endpoint) throws TimeoutException {
    ...
    CompletableFuture<OrchestratorResponse> resp =
        sendMessageWithFuture(CorfuMsgType.ORCHESTRATOR_REQUEST.payloadMsg(req));
    return (CreateWorkflowResponse)
        CFUtils.getUninterruptibly(resp, TimeoutException.class).getResponse();
  }
   ...
   * @throws TimeoutException when the rpc times out
   */
  public CreateWorkflowResponse healNodeRequest(
    ...
    OrchestratorMsg req =
        new OrchestratorMsg(
            new HealNodeRequest(
                endpoint, isLayoutServer, isSequencerServer, isLogUnitServer, stripeIndex));
    CompletableFuture<OrchestratorResponse> resp =
        sendMessageWithFuture(CorfuMsgType.ORCHESTRATOR_REQUEST.payloadMsg(req));
    return (CreateWorkflowResponse)
        CFUtils.getUninterruptibly(resp, TimeoutException.class).getResponse();
