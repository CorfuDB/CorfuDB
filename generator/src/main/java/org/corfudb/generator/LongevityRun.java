...
public class LongevityRun {
  ...
  public static void main(String[] args) {
    ...
    Option corfuEndpoint = new Option("c", CORFU_ENDPOINT, true, "corfu server to connect to");
    Option checkPointFlag = new Option("cp", CHECKPOINT, false, "enable checkpoint");
    ...
    try {
      ...
      if (!timeUnitValue.equals("m") && !timeUnitValue.equals("s") && !timeUnitValue.equals("h")) {
    ...
    String configurationString =
        cmd.hasOption(CORFU_ENDPOINT) ? cmd.getOptionValue(CORFU_ENDPOINT) : "localhost:9000";

    boolean checkPoint = cmd.hasOption(CHECKPOINT) ? true : false;
