...
public class State {
  ...
  @Getter final Streams streams;

  @Getter final Keys keys;

  @Getter final OperationCount operationCount;

  @Getter final Operations operations;

  @Getter final CorfuRuntime runtime;

  @Getter final Map<UUID, CorfuTable> maps;

  @Getter @Setter volatile long lastSuccessfulReadOperationTimestamp = -1;

  @Getter @Setter volatile long lastSuccessfulWriteOperationTimestamp = -1;

  @Getter volatile Token trimMark = Token.UNINITIALIZED;
  ...
  public void startSnapshotTx() {
    runtime.getObjectsView().TXBuild().type(TransactionType.SNAPSHOT).build().begin();
  }
