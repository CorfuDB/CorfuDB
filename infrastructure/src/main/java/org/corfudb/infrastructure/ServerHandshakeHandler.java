...
/**
 * The ServerHandshakeHandler waits for the handshake message, validates and sends a response to the
 * client. This reply contains its node id and current version of Corfu.
 *
 * <p>Created by amartinezman on 12/11/17.
 */
@Slf4j
public class ServerHandshakeHandler extends ChannelDuplexHandler {
  ...
  /**
   * Creates a new ServerHandshakeHandler which will handle the handshake--initiated by a client on
   * the server side.
   *
  ...
  public void channelRead(ChannelHandlerContext ctx, Object m) throws Exception {
    ...
    try {
      ...
      log.debug(
          "channelRead: Handshake Message received. Removing {} from pipeline.",
      ...
      if (this.state.completed()) {
        ...
        try {
          ...
          log.error("channelRead: Message received by Server is not a valid " + "CorfuMsg type.");
        }
      }
      return;
    }
    ...
    if (serverId.equals(UUID.fromString("00000000-0000-0000-0000-000000000000"))) {
      ...
      log.error(
          "channelRead: Invalid handshake: this is {} and client is trying to connect to {}",
          this.nodeId,
          serverId);
    ...
    log.debug(
        "channelRead: Sending handshake response: Node Id: {} Corfu Version: {}",
        this.nodeId,
        this.corfuVersion);

    CorfuMsg handshakeResponse =
        CorfuMsgType.HANDSHAKE_RESPONSE.payloadMsg(
            new HandshakeResponse(this.nodeId, this.corfuVersion));
  ...
  public void channelActive(final ChannelHandlerContext ctx) throws Exception {
    log.info(
        "channelActive: Incoming connection established from: {} Start Read Timeout.",
  ...
  public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
    ...
    if (cause instanceof ReadTimeoutException) {
      ...
      log.error(
          "Client authentication failed for remote address {}", ctx.channel().remoteAddress());
    } else {
