...
/**
 * A definition of a workflow that adds a new node to the cluster. This workflow has almost no retry
 * logic, therefore errors can result in its failure and the the client is responsible to ensure
 * that it has completed by restarting the workflow.
 *
 * @author Maithem
 */
@NotThreadSafe
...
public class AddNodeWorkflow implements IWorkflow {
  ...
  /**
   * Bootstrap the new node to be added to the cluster, or ignore bootstrap if it's already
   * bootstrapped.
   */
  ...
  /**
   * This action adds a new node to the layout. If it is also added as a logunit server, then in
   * addition to adding the node the address space segment is split at the tail determined during
   * the layout modification.
   */
  private class AddNodeToLayout extends Action {
    ...
    public void impl(@Nonnull CorfuRuntime runtime) throws Exception {
      ...
      runtime
          ...
          .addNode(currentLayout, request.getEndpoint(), true, true, true, false, 0);
