 ...
 * missing. It transfers multiple segments at once and opens the segments that have been
 * transferred.
 */
@Slf4j
@Getter
...
public class RestoreRedundancyMergeSegments extends Action {
  ...
  @Getter @NonNull private final String currentNode;

  @Getter @NonNull private final RedundancyCalculator redundancyCalculator;

  @Default private final int retryBase = 3;

  @Default private final Duration backoffDuration = Duration.ofSeconds(10);

  @Default private final Duration extraWait = Duration.ofMillis(20);

  @Default private final float randomPart = 0.5f;

  @Default private final int restoreRetries = 3;

  /** A data class that stores both the layout and the transferred segments. */
  @AllArgsConstructor
  private static class LayoutTransferSegments {
    @Getter @NonNull private final Layout layout;

    @Getter @NonNull private final ImmutableList<TransferSegment> transferSegments;
  }
   ...
   */
  private LayoutTransferSegments performStateTransfer(
      CorfuRuntime runtime, StateTransferManager transferManager) throws InterruptedException {
    // Settings for the retries
    Consumer<ExponentialBackoffRetry> retrySettings =
        settings -> {
    ...
    return IRetry.build(
            ...
            () -> {
              try {
                ...
                log.info(
                    "State transfer on {}: Layout before transfer: {}", currentNode, currentLayout);
                ...
                layoutTransferSegments.getTransferSegments().stream()
                ...
                log.warn(
                    "performStateTransfer: Retrying: {} times after exception: {}.",
                    retries.get(),
                    re);
  ...
  private void mergeSegments(
    ...
    BaseClient baseClient =
        layoutView.getRuntimeLayout(layoutTransferSegments.getLayout()).getBaseClient(currentNode);
    ...
    if (transferredSegments.isEmpty()) {
      log.info(
          "State transfer on: {}: No transfer occurred, " + "try merging the segments.",
          currentNode);
    ...
    // State transfer happened.
      ...
      Layout newLayout =
          redundancyCalculator.updateLayoutAfterRedundancyRestoration(
      ...
      // If the segments can't be merged, just propose a new layout.
        ...
        layoutManagementView.runLayoutReconfiguration(currentLayout, newLayout, false);
      }
    }
  }

  /**
   * Perform a state transfer on a current node, if needed, and then propose a new layout based on a
   * transfer result. If a state transfer was not needed, try merging the segments of a current
   * layout and then proposing it.
   *
  ...
  private Layout restore(CorfuRuntime runtime, StateTransferManager transferManager)
    ...
    mergeSegments(
        runtime.getLayoutView(), runtime.getLayoutManagementView(), layoutTransferSegments);
    runtime.invalidateLayout();
    return runtime.getLayoutView().getLayout();
  }

  /**
   * Perform a state transfer for the current node. Return all the segments that are transferred.
  ...
  private List<TransferSegment> getTransferSegments(
    ...
    ImmutableList<TransferSegmentRange> transferSegmentRanges =
        redundancyCalculator.prepareTransferWorkload(preTransferList, committedTail);
    ...
    ImmutableList<TransferSegment> transferList =
        transferManager.handleTransfer(transferSegmentRanges);

    log.info("getTransferSegments: Transfer segments after transfer: {}", transferList);
    // Get all the transfers that failed.
    List<TransferSegment> failedList =
        transferList.stream()
    ...
    Optional<TransferSegmentException> transferSegmentFailure =
        failedList.stream().findFirst().flatMap(ts -> ts.getStatus().getCauseOfFailure());

    transferSegmentFailure.ifPresent(
        failure -> {
          throw failure;
        });
  ...
  /**
   * Tries to retrieve a committed tail. If this fails, returns an empty option. State transfer will
   * still finish but all the addresses will be transferred via replication protocol.
   *
  ...
  private Optional<Long> tryGetCommittedTail(RuntimeLayout runtimeLayout) {
    for (int i = 0; i < retryBase; i++) {
      try {
        ...
        log.warn(
            "tryGetCommittedTail: encountered a network exception. Retry {}/{}", i, retryBase, ne);
      } catch (WrongEpochException wee) {
        log.error(
            "tryGetCommittedTail: encountered a wrong epoch exception."
        ...
        if (re.getCause() instanceof TimeoutException) {
          log.warn(
              "tryGetCommittedTail: encountered a time out exception. Retry {}/{}",
              i,
              retryBase,
              re);
        } else {
          throw new IllegalStateException(
              "Encountered an unexpected exception " + "when trying to get the committed tail.",
              re);
        }
      }
    }
    log.warn(
        "tryGetCommittedTail: failed to retrieve the committed tail after the retries. "
            + "State transfer will proceed via replication protocol.");
    return Optional.empty();
  }

  /**
   * Sets the trim mark on this endpoint's log unit, performs a prefix trim and then compaction. If
   * the exceptions are: NetworkException, WrongEpochException or TimeoutException, propagate to the
   * caller. If it's some other exception, wrap it into IllegalStateException and propagate to the
   * caller.
   *
    ...
    for (int i = 0; i < retryBase; i++) {
      try {
        ...
        if (re.getCause() instanceof TimeoutException) {
          ...
          throw new IllegalStateException(
              "Encountered an unexpected exception " + "when trimming the log.", re);
        }
      }
    }
  ...
  /**
   * Send the last transferred address as the committed tail to the target log unit. This is
   * required to prevent loss of committed tail after state transfer finishes and then all other log
   * units failed and auto commit service is paused.
   */
    ...
    for (int i = 0; i < retryBase; i++) {
      try {
        CFUtils.getUninterruptibly(
            logUnitClient.updateCommittedTail(committedTail),
        ...
        log.error(
            "transferCommittedTail: encountered network issue "
                + "when transferring a new committed tail.");
      }
    }
    log.warn(
        "transferCommittedTail: failed to transfer the latest committed tail of {}.",
        committedTail);
  }
  ...
  public void impl(@Nonnull CorfuRuntime runtime) throws Exception {
    ...
    ProtocolBatchProcessor protocolBatchProcessor =
        ProtocolBatchProcessor.builder()
    ...
    CommittedBatchProcessor committedBatchProcessor =
        CommittedBatchProcessor.builder()
