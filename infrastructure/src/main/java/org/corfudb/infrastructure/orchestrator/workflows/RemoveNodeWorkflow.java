...
/**
 * A workflow definition that removes a node from the cluster, if it exists. This workflow will fail
 * if the remove results in a cluster that is not redundant.
 *
 * @author Maithem
 */
@Slf4j
public class RemoveNodeWorkflow implements IWorkflow {
  ...
  class RemoveNode extends Action {
    ...
    public void impl(@Nonnull CorfuRuntime runtime) throws Exception {
      ...
      runtime.getLayoutManagementView().removeNode(layout, request.getEndpoint());
    }
  }
}
