 ...
 * the endpoint from the current layout, seals the system and then sends a force layout request to
 * all the endpoints in the new layout.
 *
 * <p>Created by Maithem on 12/13/17.
 */
@Slf4j
public class ForceRemoveWorkflow implements IWorkflow {
  ...
  @Getter final List<Action> actions;
  ...
  /**
   * Remove the endpoint from the current layout bypassing consensus and layout constraints, sealing
   * the new cluster and then send a force layout to all the endpoints in the new layout.
   */
