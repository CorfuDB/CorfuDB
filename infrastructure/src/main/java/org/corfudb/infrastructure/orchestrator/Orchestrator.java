...
/**
 * The orchestrator is a stateless service that runs on all management servers and its purpose is to
 * execute workflows. A workflow defines multiple smaller actions that must run in order that is
 * specified by Workflow.getActions() to achieve a bigger goal. For example, growing the cluster.
 * Initiated through RPC, the orchestrator will create a workflow instance and attempt to execute
 * all its actions.
 *
 * <p>Created by Maithem on 10/25/17.
 */
@Slf4j
public class Orchestrator {
  ...
  public Orchestrator(
      @Nonnull SingletonResource<CorfuRuntime> runtime, @Nonnull ServerContext serverContext) {
  ...
  public void handle(
    ...
    switch (orchReq.getRequest().getType()) {
      ...
      case RESTORE_REDUNDANCY_MERGE_SEGMENTS:
        workflow =
            new RestoreRedundancyMergeSegmentsWorkflow(
   ...
   * Query a workflow id.
   *
   * <p>Queries a workflow id and returns true if this orchestrator is still executing the workflow,
  ...
  void query(CorfuPayloadMsg<OrchestratorMsg> msg, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    r.sendResponse(
        ctx, msg, CorfuMsgType.ORCHESTRATOR_RESPONSE.payloadMsg(new OrchestratorResponse(resp)));
  }

  /**
   * Run a workflow on this orchestrator, if there is an existing workflow that is executing on the
   * same endpoint, then just return the corresponding workflow id. Dispatch is the only place where
   * workflows are executed based on reading activeWorkflows and therefore needs to be synchronized
  ...
  synchronized void dispatch(
    ...
    if (id != null) {
      ...
      OrchestratorResponse resp = new OrchestratorResponse(new CreateWorkflowResponse(id));
      r.sendResponse(ctx, msg, CorfuMsgType.ORCHESTRATOR_RESPONSE.payloadMsg(resp));
      return;
    } else {
      ...
      OrchestratorResponse resp =
          new OrchestratorResponse(new CreateWorkflowResponse(workflow.getId()));
      r.sendResponse(ctx, msg, CorfuMsgType.ORCHESTRATOR_RESPONSE.payloadMsg(resp));
    }
  }

  /**
   * Run a particular workflow, which entails executing all its defined actions
   *
  ...
  void run(@Nonnull IWorkflow workflow, int actionRetry) {
    ...
    try {
      ...
      List<NodeLocator> servers =
          currLayout.getAllActiveServers().stream()
      ...
      for (Action action : workflow.getActions()) {
        ...
        log.info(
            "run: finished action {} for workflow {} in {} ms",
            action.getName(),
            workflow.getId(),
        ...
        if (action.getStatus() != ActionStatus.COMPLETED) {
          log.error(
              "run: Failed to execute action {} for workflow {}, status {}, ",
              action.getName(),
              workflow.getId(),
              action.getStatus());
          return;
      ...
      log.info(
          "run: Completed workflow {} in {} ms", workflow.getId(), workflowEnd - workflowStart);
    } catch (Exception e) {
