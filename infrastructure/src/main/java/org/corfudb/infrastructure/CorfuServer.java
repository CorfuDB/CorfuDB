...
public class CorfuServer {
    ...
    private static final String USAGE =
            "Corfu Server, the server for the Corfu Infrastructure.\n"
                    ...
                    + " --segment-garbage-ratio-threshold=<segment_garbage_ratio_threshold>        "
                    + "              Threshold of the segment garbage ratio for that segment to be"
                    + " considered for compaction. A segment will be selected for compaction if "
                    + " either this threshold or --segment-garbage-size-threshold is exceeded"
                    + " [default: 0.5].\n"
                    + " --segment-garbage-size-threshold=<segment_garbage_size_threshold>          "
                    + "              Threshold of the segment garbage size in MB for that segment"
                    + " to be considered for compaction. A segment will be selected for compaction"
                    + " if either this threshold or --segment-garbage-ratio-threshold is exceeded"
                    + " [default: 100.0].\n"
                    + " --total-garbage-size-threshold=<total_garbage_size_threshold>              "
                    + "              Threshold of the total garbage size in MB that will trigger "
                    + " a force compaction (all segments could be selected for compaction)"
                    + " if exceeded [default: 4096.0].\n"
                    + " --agent                                                                    "
