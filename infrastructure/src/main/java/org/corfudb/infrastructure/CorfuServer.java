...
public class CorfuServer {
  /**
   * This string defines the command line arguments, in the docopt DSL (see http://docopt.org) for
   * the executable. It also serves as the documentation for the executable.
   *
   * <p>Unfortunately, Java doesn't support multi-line string literals, so you must concatenate
   * strings and terminate with newlines.
   *
   * <p>Note that the java implementation of docopt has a strange requirement that each option must
  ...
  private static final String USAGE =
      "Corfu Server, the server for the Corfu Infrastructure.\n"
          + "\n"
          + "Usage:\n"
          + "\tcorfu_server (-l <path>|-m) [-nsNA] [-a <address>|-q <interface-name>] [-c <ratio>]"
          + " [-d <level>] [-p <seconds>] [--layout-server-threads=<layout_server_threads>]"
          + " [--base-server-threads=<base_server_threads>] "
          + "[--log-size-quota-percentage=<max_log_size_percentage>][--logunit-threads=<logunit_threads>]"
          + " [--management-server-threads=<management_server_threads>][-e [-u <keystore> -f"
          + " <keystore_password_file>] [-r <truststore> -w <truststore_password_file>] [-b] [-g"
          + " -o <username_file> -j <password_file>] [-k <seqcache>] [-T <threads>] [-B <size>]"
          + " [-i <channel-implementation>] [-H <seconds>] [-I <cluster-id>] [-x <ciphers>] [-z"
          + " <tls-protocols>]] [--metrics] [--metrics-port <metrics_port>][-P <prefix>] [-R"
          + " <retention>] [--agent] <port>\n"
          + "\n"
          + "Options:\n"
          ...
          + " -I <cluster-id>, --cluster-id=<cluster-id>              For a single node"
          + " configuration the cluster id to use in UUID,              base64 format, or auto to"
          + " randomly generate [default: auto].\n"
          + " -T <threads>, --Threads=<threads>                                                   "
          + "   Number of corfu server worker threads, or 0 to use 2x the               number of"
          + " available processors [default: 0].\n"
          + " -P <prefix> --Prefix=<prefix>              The prefix to use for threads (useful for"
          + " debugging multiple              servers) [default: ].                               "
          + "                                                         The server will be"
          + " bootstrapped with a simple one-unit layout.\n"
          + " -a <address>, --address=<address>                                                   "
          + "   IP address for the server router to bind to and to advertise to external"
          + " clients.\n"
          + " -q <interface-name>, --network-interface=<interface-name>                           "
          + "   The name of the network interface.\n"
          + " -i <channel-implementation>, --implementation <channel-implementation>              "
          + "   The type of channel to use (auto, nio, epoll, kqueue)[default: nio].\n"
          + " -m, --memory                                                                        "
          + "   Run the unit in-memory (non-persistent).\n"
          + "                                                                                     "
          + "   Data will be lost when the server exits!\n"
          + " -c <ratio>, --cache-heap-ratio=<ratio>                                              "
          + "   The ratio of jvm max heap size we will use for the the in-memory cache to serve"
          + " requests from -\n"
          + "                                                                                     "
          ...
          + "   Maximum number of system reconfigurations (i.e. layouts)    retained for debugging"
          + " purposes [default: 1000].\n"
          ...
          + " -f <keystore_password_file>, --keystore-password-file=<keystore_password_file>      "
          ...
          + " -w <truststore_password_file>, --truststore-password-file=<truststore_password_file>"
          ...
          + "   Path to the file containing the username for SASL Plain Text Authentication.\n"
          + " -j <password_file>, --sasl-plain-text-password-file=<password_file>                 "
          + "   Path to the file containing the password for SASL Plain Text Authentication.\n"
          ...
          + "               If this limit is exceeded               write requests will be"
          + " rejected [default: 100.0].\n"
          + "                                                                                   "
          ...
          + "                                                                          "
          + " --logunit-threads=<logunit_threads>                                Number of threads"
          + " dedicated for the logunit server.\n"
          + "                                                                           --agent   "
  ...
  private static void startServer(String[] args) {
    ...
    if (Integer.parseInt((String) opts.get("--metadata-retention")) < 1) {
      throw new IllegalArgumentException(
          "Max number of metadata files to retain must be greater than 0.");
    }
  ...
  /**
   * Setup logback logger - pick the correct logging level before outputting error messages - add
   * serverEndpoint information
   *
  ...
  /**
   * Print an object to the console, followed by a newline. Call this method instead of calling
   * System.out.println().
   *
  ...
  private static void printStartupMsg(Map<String, Object> opts) {
    ...
    final String dataLocation =
        (Boolean) opts.get("--memory") ? "MEMORY mode" : opts.get("--log-path").toString();
