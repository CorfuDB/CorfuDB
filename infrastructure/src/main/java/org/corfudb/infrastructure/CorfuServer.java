...
public class CorfuServer {
    ...
    private static final String USAGE =
            "Corfu Server, the server for the Corfu Infrastructure.\n"
                    + "\n"
                    + "Usage:\n"
                    + "\tcorfu_server (-l <path>|-m) [-nsDN] [-a <address>|-q <interface-name>] "
                    + "[-t <token>] [-c <ratio>] [-d <level>] "
                    ...
                    + "[--max-open-stream-segments=<max_open_stream_segments>] "
                    + "[--max-open-garbage-segments=<max_open_garbage_segments>] "
                    + "[--compaction-policy=<compaction_policy>] "
                    + "[--compaction-initial-delay=<compaction_initial_delay>] "
                    + "[--compaction-period=<compaction_period>] "
                    + "[--compaction-worker-threads=<compaction_worker_threads>] "
                    + "[--max-segments-for-compaction=<max_segments_compaction>] "
                    + "[--protected-segments=<protected-segments>] "
                    + "[--segment-garbage-ratio-threshold=<segment_garbage_ratio_threshold>] "
                    + "[--segment-garbage-size-threshold=<segment_garbage_size_threshold>] "
                    + "[--total-garbage-size-threshold=<total_garbage_size_threshold>] "
                    + "[-e [-u <keystore> -f <keystore_password_file>] "
                    + "[-r <truststore> -w <truststore_password_file>] "
                    ...
                    + "              The prefix to use for threads."
                    + " (useful for debugging multiple servers) [default: ].\n"
                    ...
                    + " in-memory cache to serve requests from - (e.g. ratio = 0.5 means the "
                    + " cache size will be 0.5 * jvm max heap size) [default: 0.5].\n"
                    + " -D, --no-compaction                                                        "
                    + "              Disable server log compaction.\n"
                    ...
                    + "              The read/write batch size used for data transfer operations"
                    + " [default: 100].\n"
                    ...
                    + "              The type (i.e. LZ4, ZSTD or None) of compression the log unit"
                    + " will use to compress the log entries. [default: ZSTD].\n"
                    ...
                    + " --keystore-password-file=<keystore_password_file>           Path to the"
                    + " file containing the key store password.\n"
                    ...
                    + " If this limit is exceeded, write requests will be rejected until compaction"
                    + " happens and used size goes below this limit [default: 100.0].\n"
                    + " --max-open-stream-segments=<max_open_stream_segments>                      "
                    + "              Maximum opened stream log segments on log unit server."
                    + " Setting this limits the server memory consumption [default: 100].\n"
                    + " --max-open-garbage-segments=<max_open_garbage_segments>                    "
                    + "              Maximum opened garbage log segments on log unit server."
                    + " Setting this limits the server memory consumption [default: 50].\n"
                    + " --compaction-policy=<compaction_policy>                                    "
                    + "              The policy to use for compaction: (SNAPSHOT_LENGTH_FIRST,"
                    + " GARBAGE_SIZE_FIRST) [default: SNAPSHOT_LENGTH_FIRST].\n"
                    + " --compaction-initial-delay=<compaction_initial_delay>                      "
                    + "              The initial delay in minutes for the periodical log compaction"
                    + " task, after which the first compaction cycle starts. [default: 15].\n"
                    + " --compaction-period=<compaction_period>                                    "
                    + "              The period between the execution of two compaction tasks."
                    + " [default: 15].\n"
                    + " --compaction-worker-threads=<compaction_worker_threads>                    "
                    + "              Number of compaction task worker threads, or 0 to use the"
                    + " number of available processors [default: 4].\n"
                    + " --max-segments-for-compaction=<max_segments_compaction>                    "
                    + "              Maximum of log segments to compact in one compaction cycle."
                    + " [default: 24].\n"
                    + " --protected-segments=<protected-segments>                                  "
                    + "              Number of newest segments that will not be selected "
                    + " for compaction, should at least be 1 [default: 2].\n"
                    + " --segment-garbage-ratio-threshold=<segment_garbage_ratio_threshold>        "
                    + "              Threshold of the segment garbage ratio (approximate) for that"
                    + " segment to be considered for compaction. A segment will be selected for"
                    + " compaction if either this threshold or --segment-garbage-size-threshold"
                    + " is exceeded [default: 0.3].\n"
                    + " --segment-garbage-size-threshold=<segment_garbage_size_threshold>          "
                    + "              Threshold of the segment garbage size in MB (approximate)"
                    + " for that segment to be considered for compaction. A segment will"
                    + " be selected for compaction if either this threshold"
                    + " or --segment-garbage-ratio-threshold is exceeded [default: 100.0].\n"
                    + " --total-garbage-size-threshold=<total_garbage_size_threshold>              "
                    + "              Threshold of the total garbage size in MB (approximate) that"
                    + " will trigger a force compaction (all segments could be selected for"
                    + " compaction) if exceeded [default: 4096.0].\n"
                    + " --agent                                                                    "
                    + "              Run with byteman agent to enable runtime code injection.\n"
                    + " --metrics                                                                  "
