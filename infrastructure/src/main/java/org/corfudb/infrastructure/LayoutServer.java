 ...
 *
 * <p>For replication and high availability, the layout server implements a basic Paxos protocol.
 * The layout server functions as a Paxos acceptor, and accepts proposals from clients consisting of
 * a rank and desired layout. The protocol consists of three rounds:
 *
 * <p>1) Prepare(rank) - Clients first contact each server with a rank. If the server responds with
 * ACK, the server promises not to accept any requests with a rank lower than the given rank. If the
 * server responds with LAYOUT_PREPARE_REJECT, the server informs the client of the current high
 * rank and the request is rejected.
 *
 * <p>2) Propose(rank,layout) - Clients then contact each server with the previously prepared rank
 * and the desired layout. If no other client has sent a prepare with a higher rank, the layout is
 * persisted, and the server begins serving that layout to other clients. If the server responds
 * with LAYOUT_PROPOSE_REJECT, either another client has sent a prepare with a higher rank, or this
 * was a propose of a previously accepted rank.
 *
 * <p>3) Committed(rank, layout) - Clients then send a hint to each layout server that a new rank
 * has been accepted by a quorum of servers.
 *
...
public class LayoutServer extends AbstractServer {
  ...
  @NonNull private final PaxosDataStore paxosDataStore;
  ...
  public LayoutServer(@Nonnull ServerContext serverContext) {
    ...
    this.paxosDataStore = PaxosDataStore.builder().dataStore(serverContext.getDataStore()).build();
  ...
  public synchronized void handleMessageLayoutRequest(
    ...
    if (msgEpoch <= serverEpoch) {
      ...
      log.warn(
          "handleMessageLayoutRequest: Message Epoch {} ahead of Server epoch {}",
          msgEpoch,
          serverEpoch);
    }
  }
  ...
  public synchronized void handleMessageLayoutBootstrap(
    ...
    if (getCurrentLayout() == null) {
      ...
      log.info("handleMessageLayoutBootstrap: Bootstrap with new layout={}, {}", layout, msg);

      if (layout.getClusterId() == null) {
        log.warn("handleMessageLayoutBootstrap: The layout does {} not have a clusterId", layout);
      ...
      log.warn(
          "handleMessageLayoutBootstrap: Got a request to bootstrap a server which is "
  ...
  public synchronized void handleMessageLayoutPrepare(
    ...
    if (payloadEpoch != serverEpoch) {
      r.sendResponse(ctx, msg, new CorfuPayloadMsg<>(CorfuMsgType.WRONG_EPOCH, serverEpoch));
      log.trace(
          "handleMessageLayoutPrepare: Incoming message with wrong epoch, got {}, "
              + "expected {}, message was: {}",
          msg.getPayload().getEpoch(),
          serverEpoch,
          msg);
      return;
    }
    ...
    if (phase1Rank != null && prepareRank.lessThanEqualTo(phase1Rank)) {
      log.debug(
          "handleMessageLayoutPrepare: Rejected phase 1 prepare of rank={}, " + "phase1Rank={}",
          prepareRank,
          phase1Rank);
      r.sendResponse(
          ctx,
          msg,
          CorfuMsgType.LAYOUT_PREPARE_REJECT.payloadMsg(
              new LayoutPrepareResponse(phase1Rank.getRank(), proposedLayout)));
    } else {
      // Return the layout with the highest rank proposed before.
      Rank highestProposedRank =
          proposedLayout == null ? new Rank(-1L, msg.getClientID()) : getPhase2Rank(payloadEpoch);
      setPhase1Rank(prepareRank, payloadEpoch);
      log.debug("handleMessageLayoutPrepare: New phase 1 rank={}", prepareRank);
      r.sendResponse(
          ctx,
          msg,
          CorfuMsgType.LAYOUT_PREPARE_ACK.payloadMsg(
              new LayoutPrepareResponse(highestProposedRank.getRank(), proposedLayout)));
    }
  }

  /**
   * Accepts a proposal for which it had accepted in the prepare phase. A minor optimization is to
   * reject any duplicate propose messages.
   *
  ...
  public synchronized void handleMessageLayoutPropose(
    ...
    if (payloadEpoch != serverEpoch) {
      r.sendResponse(ctx, msg, new CorfuPayloadMsg<>(CorfuMsgType.WRONG_EPOCH, serverEpoch));
      log.trace(
          "handleMessageLayoutPropose: Incoming message with wrong epoch, got {}, "
              + "expected {}, message was: {}",
          payloadEpoch,
          serverEpoch,
          msg);
    ...
    if (phase1Rank == null) {
      log.debug(
          "handleMessageLayoutPropose: Rejected phase 2 propose of rank={}, " + "phase1Rank=none",
          proposeRank);
      r.sendResponse(
          ctx, msg, CorfuMsgType.LAYOUT_PROPOSE_REJECT.payloadMsg(new LayoutProposeResponse(-1)));
      return;
    }
    ...
    if (!proposeRank.equals(phase1Rank)) {
      log.debug(
          "handleMessageLayoutPropose: Rejected phase 2 propose of rank={}, " + "phase1Rank={}",
          proposeRank,
          phase1Rank);
      r.sendResponse(
          ctx,
          msg,
          CorfuMsgType.LAYOUT_PROPOSE_REJECT.payloadMsg(
              new LayoutProposeResponse(phase1Rank.getRank())));
      return;
    }
    ...
    if (proposeLayout.getEpoch() != payloadEpoch) {
      log.debug(
          "Phase II error: layout {} and payload {} epoch should be the same",
          proposeLayout.getEpoch(),
          payloadEpoch);
      r.sendResponse(
          ctx,
          msg,
    ...
    if (proposeRank.equals(phase2Rank)) {
      log.debug(
          "handleMessageLayoutPropose: Rejected phase 2 propose of rank={}, " + "phase2Rank={}",
          proposeRank,
          phase2Rank);
      r.sendResponse(
          ctx,
          msg,
          CorfuMsgType.LAYOUT_PROPOSE_REJECT.payloadMsg(
              new LayoutProposeResponse(phase2Rank.getRank())));
      return;
    }

    log.debug(
        "handleMessageLayoutPropose: New phase 2 rank={}, layout={}", proposeRank, proposeLayout);
    setPhase2Data(new Phase2Data(proposeRank, proposeLayout), payloadEpoch);
    r.sendResponse(ctx, msg, new CorfuMsg(CorfuMsgType.ACK));
  }

  /**
   * Force layout enables the server to bypass consensus and accept a new layout.
   *
   * @param msg corfu message containing LAYOUT_FORCE
   * @param ctx netty ChannelHandlerContext
   * @param r server router
   */
  private synchronized void forceLayout(
    ...
    if (payloadEpoch != serverEpoch) {
      ...
      r.sendResponse(ctx, msg, new CorfuMsg(CorfuMsgType.NACK));
      log.warn(
          "forceLayout: Trying to force a layout with an old epoch. Layout {}, "
              + "current epoch {}",
          payloadEpoch,
          serverEpoch);
      return;
  ...
  /**
   * Accepts any committed layouts for the current epoch or newer epochs. As part of the accept, the
   * server changes it's current layout and epoch.
   *
  ...
  public Rank getPhase1Rank(long epoch) {
    return paxosDataStore.getPhase1Rank(epoch).orElse(null);
  }
  ...
  public Rank getPhase2Rank(long epoch) {
    return paxosDataStore.getPhase2Data(epoch).map(Phase2Data::getRank).orElse(null);
  }
  ...
  public Layout getProposedLayout(long epoch) {
    return paxosDataStore.getPhase2Data(epoch).map(Phase2Data::getLayout).orElse(null);
  }
