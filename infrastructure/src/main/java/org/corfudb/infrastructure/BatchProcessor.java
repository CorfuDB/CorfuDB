...
/**
 * This class manages access for operations that need ordering while executing against the backing
 * storage.
 */
@Slf4j
public class BatchProcessor implements AutoCloseable {
   ...
   * BatchWriterOperation arriving after the sealEpoch with an epoch less than the sealEpoch is
   * completed exceptionally with a WrongEpochException. This is persisted in the ServerContext by
   * the LogUnitServer to withstand restarts.
   */
      ...
      while (true) {
        ...
        if (currOp == null) {
          ...
          break;
            ...
            && currOp.getMsg().getPriorityLevel() != PriorityLevel.HIGH) {
          currOp
              .getFutureResult()
              .completeExceptionally(
                  new QuotaExceededException(
                      "Quota of " + streamLog.quotaLimitInBytes() + " bytes"));
          ...
          log.info(
              "batchWriteProcessor: updating from {} to {}", sealEpoch, currOp.getMsg().getEpoch());
          ...
          log.warn(
              "batchWriteProcessor: wrong epoch on {} msg, seal epoch is {}, and msg epoch is {}",
              currOp.getType(),
              sealEpoch,
              currOp.getMsg().getEpoch());
          ...
          try {
            switch (currOp.getType()) {
              ...
              case WRITE:
                ...
                streamLog.append((LogData) write.getData());
                break;
              case RANGE_WRITE:
            ...
            log.error(
                "Stream log error. Batch [queue size={}]. StreamLog: [trim mark: {}].",
                operationsQueue.size(),
                streamLog.getTrimMark(),
                e);
  ...
  public void close() {
    ...
    try {
      processorService.awaitTermination(
          ServerContext.SHUTDOWN_TIMER.toMillis(), TimeUnit.MILLISECONDS);
