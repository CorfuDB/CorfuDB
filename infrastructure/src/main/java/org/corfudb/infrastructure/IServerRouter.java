...
public interface IServerRouter {
  ...
  default void sendWrongEpochMessage(CorfuMsg msg, ChannelHandlerContext ctx) {
    ...
    log.trace(
        "Incoming message with wrong epoch, got {}, expected {}, message was: {}",
        msg.getEpoch(),
        getServerEpoch(),
        msg);
  }
   ...
   */
  default void sendWrongClusterIdMessage(
      CorfuMsg msg, ChannelHandlerContext ctx, UUID currentClusterID) {
    sendResponse(
        ctx,
        msg,
        new CorfuPayloadMsg<>(
            CorfuMsgType.WRONG_CLUSTER_ID,
            new WrongClusterMsg(currentClusterID, msg.getClusterID())));
    log.trace(
        "Incoming message with a wrong cluster ID, got {}, expected {}, message was {}",
        msg.getClusterID(),
        currentClusterID,
        msg);
  }
  ...
  /**
   * Validate the epoch of a CorfuMsg, and send a WRONG_EPOCH response if the server is in the wrong
   * epoch. Ignored if the message type is reset (which is valid in any epoch).
   *
  ...
  default boolean clusterIdIsValid(CorfuMsg msg, ChannelHandlerContext ctx, Layout currentLayout) {
    ...
    boolean clusterIdsMatch = msg.getClusterID().equals(currentClusterID);
  ...
  /**
   * Validate the incoming message. The message is valid if: 1) The flag ignoreEpoch is set to true
   * or it's set to false, and the epoch is valid for all the messages. 2) Also if the flag
   * ignoreClusterId is set to false, a. The current layout server should be bootstrapped and b. the
   * message's cluster ID should be equal to the bootstrapped layout's cluster ID.
   *
   * @param msg The incoming message.
   * @param ctx The context of the channel handler.
