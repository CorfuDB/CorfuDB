...
/**
 * Sequencer server cache. Contains transaction conflict-resolution data structures.
 *
 * <p>The cache map maps conflict keys (stream id + key) to versions (long), illustrated below:
 * Conflict Key | ck1 | ck2 | ck3 | ck4 Version | v1 | v1 | v2 | v3 Consider the case where we need
 * to insert a new conflict key (ck), but the cache is full, we need to evict the oldest conflict
 * keys. In the example above, we can't just evict ck1, we need to evict all keys that map to v1, so
 * we need to evict ck1 and ck2, this eviction policy is FIFO on the version number. The simple FIFO
 * approach in Caffein etc doesn't work here, as it may evict ck1, but not ck2. Notice that we also
 * can't evict ck3 before the keys for v1, that's because it will create holes in the resolution
 * window and can lead to incorrect resolutions.
 *
 * <p>We use priority queue as a sliding window on the versions, where a version can map to multiple
 * keys, so we also need to maintain the beginning of the window which is the maxConflictWildcard
 * variable.
 *
 * <p>SequencerServerCache achieves consistency by using single threaded cache. It's done by
 * following code: `.executor(Runnable::run)`
 */
@NotThreadSafe
@Slf4j
public class SequencerServerCache {
  /**
   * TX conflict-resolution information: a cache of recent conflict keys and their latest global-log
   * position.
   */
  ...
  @Getter private final int cacheSize; // the max number of entries in SequencerServerCache

  /**
   * A "wildcard" representing the maximal update timestamp of all the conflict keys which were
   * evicted from the cache
   */
  @Getter private long maxConflictWildcard;

  /**
   * maxConflictNewSequencer represents the max update timestamp of all the conflict keys which were
   * evicted from the cache by the time this server is elected the primary sequencer. This means
   * that any snapshot timestamp below this actual threshold would abort due to NEW_SEQUENCER cause.
   */
  @Getter private long maxConflictNewSequencer;

  /**
   * It is used to calculate the size of ServerCache. Each entry relates two pointers used by
   * HashMap, one pointer in PriorityQueue.
   */
  ...
  /**
   * The cache limited by size. For a synchronous cache we are using a same-thread executor
   * (Runnable::run) https://github.com/ben-manes/caffeine/issues/90
   *
  ...
  public SequencerServerCache(int cacheSize, long maxConflictNewSequencer) {
    ...
    cacheEntries =
        new PriorityQueue(
            cacheSize, Comparator.comparingLong(a -> ((ConflictTxStream) a).txVersion));
  ...
  /**
   * The memory space used by the entries and also the space used by priority queue and hashmap to
   * store the pointers
   *
  ...
  public long byteSize() {
    log.debug(
        "the cache has {} entries,  the object size used {}, calculated by beepSize {}",
        size(),
        CONFLICTTXSTREAM_OBJ_SIZE,
  ...
  public boolean put(ConflictTxStream conflictStream) {
    ...
    if (val > conflictStream.txVersion) {
      log.error(
          "For key {} the new entry address {} is smaller than the entry "
              + "address {} in cache. There is a sequencer regression.",
          conflictStream,
          conflictStream.txVersion,
          conflictKeys.get(conflictStream));
      return false;
    }
  ...
  public static class ConflictTxStream {
    ...
    @Getter private final UUID streamId;
    @Getter private final byte[] conflictParam;

    @EqualsAndHashCode.Exclude public final long txVersion;
