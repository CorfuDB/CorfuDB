...
/**
 * A class used to compute the transfer segments, as well as the layout after the redundancy
 * restoration.
 */
@AllArgsConstructor
public class RedundancyCalculator {
  ...
  /**
   * Given a current layout and a restored transfer segment, create a new layout, that contains a
   * restored node.
   *
                ...
                layoutSegment -> {
                  if (layoutSegment.getEnd() == transferSegment.getEndAddress() + 1L) {
                    ...
                    return new LayoutSegment(
                        layoutSegment.getReplicationMode(),
                        layoutSegment.getStart(),
                        layoutSegment.getEnd(),
                        newStripes);
                  } else {
                    return new LayoutSegment(
  ...
  /**
   * Returns true if a given server can restore redundancy for this layout. A particular server can
   * restore redundancy for this layout if the segment is split and a set difference of log servers
   * between any two adjacent segments contains this node. This ensures that the absence of a node
   * in a layout is detected in any multiple split segment scenarios which results in the current
   * node being absent closer to the beginning or to the end of a layout segment list.
   *
    ...
    if (layout.getSegments().size() == 1) {
      ...
      return IntStream.range(1, layout.getSegments().size())
          ...
          .reduce(
              ...
              (nodePresentBefore, nodePresentNow) -> nodePresentBefore || nodePresentNow);
    }
  }

  /**
   * Returns true if any server can merge the segments of this layout. Any server can merge the
   * segments of this layout if the segment is split and a set difference of log servers between the
   * first two adjacent segments is empty.
   *
   * @param layout Current layout.
  ...
  public static boolean canMergeSegments(Layout layout) {
    if (layout.getSegments().size() == 1) {
      ...
      return Sets.difference(
              ...
              layout.getSegments().get(firstSegmentIndex).getAllLogServers())
          .isEmpty();
    }
  }

  /**
   * Returns true if any layout restoration action is needed. A restoration action is needed when
   * any server can merge the segments or a current server can restore a layout redundancy.
   *
   * @param layout Current layout.
   * @param server Current server.
  ...
  /**
   * Given a layout, and a global trim mark, creates an initial list of non-empty and bounded
   * transfer segments.
   *
   * @param layout A current layout.
        ...
        .map(
            segment -> {
              ...
              if (segmentContainsServer(segment, getServer())) {
                ...
                TransferSegmentStatus notTransferred =
                    TransferSegmentStatus.builder().segmentState(NOT_TRANSFERRED).build();
  ...
  /**
   * Creates the list of transfer segment ranges given the transfer segment list and the committed
   * tail. The ranges are consumed by the state transfer manager in order to execute the state
   * transfer.
   *
  ...
  public ImmutableList<TransferSegmentRange> prepareTransferWorkload(
      ImmutableList<TransferSegment> transferSegmentList, Optional<Long> committedTail) {
