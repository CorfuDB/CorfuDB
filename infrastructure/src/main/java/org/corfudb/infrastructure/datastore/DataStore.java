 ...
 * The key is determined as (prefix + "_" + key). The cache here serves mostly for easily managed
 * synchronization of in-memory/file. Some on-disk files will be deleted when their number exceed
 * user specified limit.
 *
 * <p>If 'opts' either has '--memory=true' or a log-path for storing files is not provided, the
 * store is just an in memory cache.
 *
...
public class DataStore implements KvDataStore {
  ...
  @Getter private final Cache<String, Object> cache;
  private final String logDirPath;

  @Getter private final long dsCacheSize = 1_000; // size bound for in-memory cache for dataStore
  ...
  public DataStore(@Nonnull Map<String, Object> opts, @Nonnull Consumer<String> cleanupTask) {
    ...
    if ((opts.containsKey("--memory") && (Boolean) opts.get("--memory"))
        || !opts.containsKey("--log-path")) {
  ...
  /**
   * obtain a {@link LoadingCache}. The cache is backed up by file-per-key under {@link
   * DataStore::logDirPath}. The cache size is bounded by {@link DataStore::dsCacheSize}.
   *
        ...
        .writer(
            new CacheWriter<String, Object>() {
              ...
              public synchronized void delete(
                  @Nonnull String key, @Nullable Object value, @Nonnull RemovalCause cause) {
                try {
  ...
  /**
   * Since the cache can't maintain key->null mappings, this enum is a place holder for null to
   * allow keys to map to null.
   */
    ...
    Object val =
        cache.get(
            ...
            k -> {
              ...
              // were empty. This is required to prevent loading an empty key more than once, which
              // is expensive.
