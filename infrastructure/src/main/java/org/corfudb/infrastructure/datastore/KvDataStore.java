...
/**
 * Key Value data store abstraction that provides persistence for variables that need retain values
 * across node restarts or need to be accessed by multiple modules/threads.
 *
 * <p>The key value store is partitioned by prefix (namespace/table). All values being stored under
 * a prefix should be of a single Type or Class T.
 *
...
public interface KvDataStore {
  ...
  /**
   * Key-value meta information class, provides all the information for saving and getting data from
   * a data store
   *
   * @param <T> data type
   */
