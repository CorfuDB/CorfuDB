...
public class CorfuInterClusterReplicationServer implements Runnable {
    ...
    private static final String USAGE =
            "Corfu Log Replication Server, the server for replication across clusters.\n"
                    ...
                    + "[--log-replication-server-threads=<log_replication_server_threads>]"
                    ...
                    + "[-k <seqcache>] [-T <threads>] [--replication_threads=<replication_threads>] "
                    + "[--runtime_threads=<runtime_threads>] [--ack_reader_threads=<ack_reader_threads>] "
                    + "[--initial_log_entry_read_backoff_time=<initial_log_entry_read_backoff_time>] "
                    + "[--log_entry_read_backoff_increment=<log_entry_read_backoff_interval>] "
                    + "[--log_entry_read_max_backoff=<log_entry_read_max_backoff>] "
                    + "[--snapshot_read_timeout=<snapshot_read_timeout>] [-B <size>] "
                    + "[-i <channel-implementation>] [-H <seconds>] [-I <cluster-id>] [-x <ciphers>] "
                    + " [-z <tls-protocols>]] [--disable-cert-expiry-check-file=<file_path>] [--metrics] "
                    + "[--corfu-port-for-lr=<corfu-port-for-lr>] [-P <prefix>] [-R <retention>] <port>\n\n"
                    + "Options:\n"
                    ...
                    + " --log-replication-server-threads=<log_replication_server_threads>        "
                    + "              Number of threads dedicated for log replication message handlers [default: 1]. \n"
                    ...
                    + "             Number of threads in the replication workers thread pool [default: 2] \n"
                    + " --runtime_threads=<runtime_threads>                                      "
                    + "             Number of threads in the runtime workers thread pool [default: 2] \n"
                    + " --ack_reader_threads=<ack_reader_threads>                                "
                    + "             Number of threads in the ack readers thread pool [default: 2] \n"
                    + " --initial_log_entry_read_backoff_time=<initial_log_entry_read_backoff_time>"
                    + "             Initial time(in ms) for which the Log Entry Reader should backoff if no data to "
                    + "             replicate found [default: 100] \n                            "
                    + " --log_entry_read_backoff_increment=<log_entry_read_backoff_increment>    "
                    + "             Increment(in ms) in the backoff time every time Log Entry Reader does not find "
                    + "             data to send [default: 50] \n                                "
                    + " --log_entry_read_max_backoff=<log_entry_read_backoff_increment>          "
                    + "             Max time(in ms) for which the Log Entry Reader will backoff if no data to "
                    + "             replicate is found [default: 200] \n                         "
                    + " --snapshot_read_timeout=<--snapshot_read_timeout>                        "
                    + "             Time(in ms) for which RoutingQueues Snapshot Reader waits for data from the "
                    + "             application before making a new snapshot sync request.  Default is 2mins "
                    + "             [default: 120000] \n                                         "
                    + " -h, --help                                                               "
                    + "              Show this screen\n"
                    + " --version                                                                "
