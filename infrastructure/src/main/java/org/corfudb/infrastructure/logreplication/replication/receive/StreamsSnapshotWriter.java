...
public class StreamsSnapshotWriter implements SnapshotWriter {
    ...
    private final static String SHADOW_STREAM_NAME_SUFFIX = "_shadow";
    ...
    private void clearTables() {
        ...
        if (topologyConfigId != persistSiteConfigID || srcGlobalSnapshot != persistSnapStart ||
            ...
            log.warn("Skip current topologyConfigId " + topologyConfigId + " srcGlobalSnapshot " + srcGlobalSnapshot + " currentSeqNum " + recvSeq +
    ...
    private void processOpaqueEntry(List<SMREntry> smrEntries, Long currentSeqNum, UUID dstUUID) {
        ...
        if (topologyConfigId != persistConfigID || srcGlobalSnapshot != persistSnapStart || currentSeqNum != (persitSeqNum + 1)) {
            log.warn("Skip current topologyConfigId " + topologyConfigId + " srcGlobalSnapshot " + srcGlobalSnapshot + " currentSeqNum " + currentSeqNum +
                    " persistedMetadata " + logReplicationMetadataManager.getTopologyConfigId() + " startSnapshot " + logReplicationMetadataManager.getLastSnapStartTimestamp() +
                    " lastSnapSeqNum " + logReplicationMetadataManager.getLastSnapSeqNum());
            return;
        }
