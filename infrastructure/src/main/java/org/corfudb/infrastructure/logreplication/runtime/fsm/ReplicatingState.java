...
public class ReplicatingState implements LogReplicationRuntimeState {
        ...
        switch (event.getType()) {
            case ON_CONNECTION_DOWN:
                ...
                if (fsm.getRemoteLeaderNodeId().isPresent() && fsm.getRemoteLeaderNodeId().get().equals(nodeIdDown)) {
                    log.warn("Connection to remote leader id={} is down. Attempt to reconnect ONLY if connection starter." +
                            "Session {}", nodeIdDown, fsm.getSession());
                ...
                log.debug("Connection lost to non-leader node {}. Session {}", nodeIdDown, fsm.getSession());
            ...
            case REMOTE_LEADER_LOSS:
                if (fsm.getRemoteLeaderNodeId().get().equals(event.getNodeId())) {
                    log.warn("Remote node {} lost leadership, stop replication & discover new leader for session {}.", event.getNodeId(),
                            fsm.getSession());
                    fsm.resetRemoteLeaderNodeId();
            ...
            case LOCAL_LEADER_LOSS:
                ...
                return null;
            default: {
                log.warn("Unexpected communication event {} when in init state. Session {}", event.getType(), fsm.getSession());
    ...
    public void onEntry(LogReplicationRuntimeState from) {
        ...
        switch (replicationEvent.getType()) {
            ...
            case LOG_ENTRY_SYNC_REQUEST:
                ...
                break;
            default:
                log.info("Invalid Negotiation result. Re-trigger discovery for session {}.", fsm.getSession());
                break;
        }
    }
    ...
    public void onExit(LogReplicationRuntimeState to) {
        log.debug("Transition to {} from replicating state for session {}", to.getType(), fsm.getSession());
        switch (to.getType()) {
            case STOPPED:
                log.debug("onExit :: transition to stopped state for session {}", fsm.getSession());
                replicationSourceManager.shutdown();
                break;
            case VERIFYING_REMOTE_LEADER:
            case WAITING_FOR_CONNECTIVITY:
                log.debug("onExit :: transition to {} state for session {}", to.getType(), fsm.getSession());
                replicationSourceManager.stopLogReplication();
                break;
            default:
                break;
