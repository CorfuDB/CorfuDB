...
public class InSnapshotSyncState implements LogReplicationState {
    ...
    public LogReplicationState processEvent(LogReplicationEvent event) throws IllegalTransitionException {
        switch (event.getType()) {
            case SNAPSHOT_SYNC_REQUEST:
                ...
                setTransitionEventId(event.getMetadata().getRequestId());
            ...
            case SNAPSHOT_SYNC_CONTINUE:
                ...
                if (fsm.isValidTransition(transitionEventId, event.getMetadata().getRequestId())) {
                    log.debug("InSnapshotSync[{}] :: Continuation of snapshot sync for {}", transitionEventId,
                            event.getMetadata().getRequestId());
                    return this;
                } else {
                    log.warn("Unexpected snapshot sync continue for snapshot_sync ID {} when in snapshot_sync ID {}.",
                            event.getMetadata().getRequestId(), transitionEventId);
                }
                return this;
            case SNAPSHOT_TRANSFER_COMPLETE:
                if (fsm.isValidTransition(transitionEventId, event.getMetadata().getRequestId())) {
                    log.info("Snapshot Sync transfer is complete for {}", event.getMetadata().getRequestId());
                ...
                log.warn("Sync Transfer Complete for eventId {}, but running snapshot sync for {}",
                        event.getMetadata().getRequestId(), transitionEventId);
                return this;
            case SYNC_CANCEL:
    ...
    public void onEntry(LogReplicationState from) {
        try {
            log.info("Shama, the from has req iD {}, curr has requestID {}", from.getTransitionEventId(), this.transitionEventId);
            log.info("Shama...if same, then we need to reset the snapshotSnedr.startSnapshotSync()");
