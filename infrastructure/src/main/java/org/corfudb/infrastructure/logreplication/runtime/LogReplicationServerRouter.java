...
/**
 * This class is shared by all the connection receiver routers.
 * Since there can be only 1 server per endpoint, this class creates the server, sets the server in the routers which
 * is per session.
 */
@Slf4j
public class LogReplicationServerRouter implements IServerRouter {
    ...
    public LogReplicationServerRouter(Map<Class, AbstractServer> serverMap, ServerContext serverContext,
        ...
        setServerAdapterInRouters(sessionToSourceServer, sessionToSinkServer);
    }

    /**
     * Set the server adapter in routers where it is not yet set.
     * @param sessionToSourceRouter
     * @param sessionToSinkRouter
     */
    public void setServerAdapterInRouters(Map<LogReplicationSession, LogReplicationSourceServerRouter> sessionToSourceRouter,
                                          Map<LogReplicationSession, LogReplicationSinkServerRouter> sessionToSinkRouter) {
        serverAdapter.updateRouters(sessionToSourceRouter, sessionToSinkRouter);
        // set the server adapter in routers.
        if(!sessionToSourceRouter.isEmpty()) {
            sessionToSourceRouter.values().stream().filter(router -> router.serverChannelAdapter == null).forEach(router ->
