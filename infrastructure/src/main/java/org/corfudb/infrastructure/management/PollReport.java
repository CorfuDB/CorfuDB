...
/**
 * Poll Report generated by the detectors that poll to detect failed or healed nodes. This is
 * consumed and analyzed by the Management Server to detect changes in the cluster and take
 * appropriate action. Created by zlokhandwala on 3/21/17.
 */
@Builder
@Slf4j
@Getter
...
public class PollReport {
  ...
  @Default private final long pollEpoch = Layout.INVALID_EPOCH;

  /** A map of nodes answered with {@link WrongEpochException}. */
  @Default private final ImmutableMap<String, Long> wrongEpochs = ImmutableMap.of();

  /** Current cluster state, collected by a failure detector */
  @NonNull private final ClusterState clusterState;

  /** List of responsive servers that successfully responded to our ping */
  @NonNull private final ImmutableList<String> pingResponsiveServers;

  /** Time spent on collecting this report */
  @NonNull private final Duration elapsedTime;
  ...
  /**
   * Contains list of servers successfully connected with current node. It doesn't contain nodes
   * answered with WrongEpochException.
   */
  ...
  /**
   * Check to see if the current epoch/register has been decided/filled. We do this by comparing the
   * epoch of the latest committed layout and the epoch associated with the {@link
   * WrongEpochException}.
   *
  ...
  public Optional<Long> getLayoutSlotUnFilled(@NonNull Layout latestCommittedLayout) {
    log.trace(
        "getLayoutSlotUnFilled. Wrong epochs: {}, latest layout epoch: {}",
        wrongEpochs,
        latestCommittedLayout.getEpoch());

    long maxOutOfPhaseEpoch =
        wrongEpochs.values().stream().max(Long::compare).orElse(latestCommittedLayout.getEpoch());
