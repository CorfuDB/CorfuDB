...
/**
 * This class is an implementation of {@link ClusterType} where the ideal state of the Corfu cluster
 * is a fully connected network (i.e. complete graph) in which there is an active link amongst all
 * nodes in the cluster. Failed and Healed nodes are recommended so that the cluster remains fully
 * connected.
 *
 * <p>Created by Sam Behnam on 10/27/18.
 */
@Slf4j
public class CompleteGraphAdvisor implements ClusterAdvisor {
  ...
  /**
   * Provides list of servers from a given layout(epoch) that this implementation of COMPLETE_GRAPH
   * algorithm has determined as failed. The implementation of the algorithm in this method is an
   * approach by executing the following steps:
   *
   * <p>The failed node is the recommendation of this strategy which their removal from cluster will
   * lead to a fully connected cluster.
   *
   * <p>To find a failed node: - get decision maker - find a failed node. Check if decision maker is
   * not equal to failed node and failed node is not in unresponsive servers already.
   *
   * @param clusterState represents the state of connectivity amongst the Corfu cluster nodes from a
   *     node's perspective.
  ...
  public Optional<NodeRank> failedServer(ClusterState clusterState) {
    ...
    if (!decisionMaker.is(localEndpoint)) {
      String message =
          "The node can't be a decision maker, skip operation. Decision maker node is: {}";
  ...
  /**
   * Provide a server considered to have healed in the Corfu cluster according to the COMPLETE_GRAPH
   * implementation of algorithm for {@link ClusterType}.
   *
   * <p>The node can heal only itself. The node responsible only for itself, can't heal other nodes.
   * It simplifies healing algorithm and guaranties that if node became available it mark itself as
   * a responsible node in the layout. It helps us to simplify analysis/debugging process and brings
   * simple and reliable algorithm for healing process.
   *
   * @param clusterState represents the state of connectivity amongst the Corfu cluster nodes from a
   *     node's perspective.
   * @return a {@link List} of servers considered as healed according to the underlying {@link
   *     ClusterType}.
   */
  @Override
