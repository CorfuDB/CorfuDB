...
/**
 * Collects information about cluster state. Provides list of servers in the cluster with higher
 * epochs. Builds the {@link ClusterState} and local {@link NodeState}.
 *
 * <p>The collector navigates via list of requests represented as CompletableFuture-s and collecting
...
public class ClusterStateCollector {
  ...
  @NonNull private final Map<String, CompletableFuture<NodeState>> clusterState;
  ...
  public ClusterState collectClusterState(
      long epoch, ImmutableList<String> unresponsiveNodes, SequencerMetrics sequencerMetrics) {
  ...
  private NodeState collectLocalNodeState(long epoch, SequencerMetrics sequencerMetrics) {
    ...
    NodeConnectivity localConnectivity =
        NodeConnectivity.connectivity(localEndpoint, ImmutableMap.copyOf(localNodeConnections));
