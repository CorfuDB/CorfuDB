...
/**
 * The ReconfigurationEventHandler handles the trigger provided by any source or policy detecting a
 * failure or healing in the cluster. It performs the following functions: Cluster recovery:
 * Recovers the cluster on trigger at startup. Handle failures: Handles any failures in the layout
 * based on the desired policy. Handle healing: Handles healing of unresponsive marked nodes which
 * are now responsive. Handle mergeSegments: Handles merging of multiple segments.
 *
...
public class ReconfigurationEventHandler {
  ...
  @Getter @Setter private static int workflowRetries = 3;
   ...
   */
  public static boolean handleFailure(
      @Nonnull IReconfigurationHandlerPolicy failureHandlerPolicy,
  ...
  /**
   * Estimate a timeout to complete stateTransfer depending on the number of addresses to be
   * transferred, the RPC timeout of the corfu runtime and the bulk size of the transfer. TODO:
   * Resize the bulk size in state transfer in case of timeouts or failures.
   *
  ...
  /**
   * Takes in the existing layout and a set of healed nodes. The LayoutManagementView launches a
   * workflow to heal the specified node. This call is blocked until the workflow is completed or
   * aborted. The node is healed and added back to the layout with all components enabled i.e., with
   * its layout, sequencer and log unit server enabled and as an active participant in the data
   * replication view.
   *
   * @param runtime Connected corfu runtime instance
   ...
   */
  public static boolean handleHealing(
    ...
    try {
      for (String healedServer : healedServers) {
        ...
        log.info(
            "handleHealing: Workflow to heal {} timeout set to {} ms",
            healedServer,
            workflowTimeout);
   ...
   * This task sends an orchestrator request to the current endpoint orchestrator to run this
   * workflow. The task is run asynchronously by the failure detector if it detects that: 1. The
   * current node is not present in the unresponsive list. 2. The current node requires state
   * transfer or the segments of the layout can be merged. This is a blocking task and will complete
   * if the workflow completes with a success or a failure.
   *
   ...
   */
  public static boolean handleMergeSegments(
    ...
    try {
      Duration workflowTimeout = getStateTransferTimeoutEstimate(runtime);
      log.info(
          "handleMergeSegments: Workflow to merge segments for layout {} timeout set to {} ms",
          layout,
          workflowTimeout);

      runtime
          .getManagementView()
          .mergeSegments(currentEndpoint, workflowRetries, workflowTimeout, retryQueryTimeout);
      return true;
    } catch (Exception e) {
