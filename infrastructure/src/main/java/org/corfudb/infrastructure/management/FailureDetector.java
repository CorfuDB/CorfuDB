...
/**
 * FailureDetector polls all the "responsive members" in the layout. Responsive members: All
 * endpoints that are responding to heartbeats. This list can be derived by excluding
 * unresponsiveServers from all endpoints. For every poll method call invoked, it starts a polling
 * round and generates a poll report. Each polling round comprises of "failureThreshold" number of
 * iterations. - We asynchronously poll every known responsive member in the layout. - Poll result
 * aggregation. - If we complete an iteration without detecting failures, we end the round
 * successfully. The management Server ensures only one instance of this class and hence this is NOT
 * thread safe. Created by zlokhandwala on 11/29/17.
 */
@Slf4j
public class FailureDetector implements IDetector {

  /** Number of iterations to execute to detect a failure in a round. */
  @Getter @Setter private int failureThreshold = 3;

  @NonNull private final String localEndpoint;

  @NonNull @Default @Setter
  ...
  /**
   * Executes the policy once. Checks for changes in the layout. Then polls all the servers
   * generates pollReport.
   *
  ...
  public PollReport poll(
      ...
      @NonNull SequencerMetrics sequencerMetrics) {
   ...
   * responsive nodes in the layout and also collects their {@link NodeState}-s to provide {@link
   * ClusterState}. Failure detector collect a number of {@link PollReport}-s equals to
   * failureThreshold number then aggregates final {@link PollReport}. Aggregation step: - go
   * through all reports - aggregate all wrong epochs from all intermediate poll reports then remove
   * all responsive nodes from the list. If wrongEpochsAggregated variable still contains wrong
   * epochs then it will be corrected by {@link RemoteMonitoringService} - Aggregate connected and
   * failed nodes from all reports - Timeouts tuning - provide a poll report based on latest
   * pollIteration report and aggregated values
   *
  ...
  PollReport pollRound(
      ...
      Map<String, IClientRouter> router,
    ...
    for (int iteration = 0; iteration < failureThreshold; iteration++) {
      ...
      if (!currReport.getFailedNodes().isEmpty()) {
        ...
        tuneRoutersResponseTimeout(router, allReachableNodes, networkStretcher.getCurrentPeriod());
      }
    ...
    Set<String> allConnectedNodes =
        Sets.union(connectedNodesAggregated, wrongEpochsAggregated.keySet());

    tunePollReportTimeouts(router, failedNodesAggregated, allConnectedNodes);

    List<ClusterState> clusterStates =
        reports.stream().map(PollReport::getClusterState).collect(Collectors.toList());

    ClusterStateAggregator aggregator =
        ClusterStateAggregator.builder()
    ...
    Duration totalElapsedTime =
        reports.stream().map(PollReport::getElapsedTime).reduce(Duration.ZERO, Duration::plus);
  ...
  /**
   * We can try to scale back the network latency time after every poll round. If there are no
   * failures, after a few rounds our polling period converges back to initPeriodDuration.
   *
  ...
  private void tunePollReportTimeouts(
      Map<String, IClientRouter> clientRouters,
  ...
  /**
   * Poll iteration step, provides a {@link PollReport} composed from pings and {@link NodeState}-s
   * collected by this node from the cluster. Algorithm: - ping all nodes - collect all node states
   * - collect wrong epochs - collect connected/failed nodes - calculate if current layout slot is
   * unfilled - build poll report
   *
  ...
  private PollReport pollIteration(
    ...
    ClusterStateCollector clusterCollector =
        ClusterStateCollector.builder()
            .localEndpoint(localEndpoint)
    ...
    ClusterState clusterState =
        clusterCollector.collectClusterState(epoch, layoutUnresponsiveNodes, sequencerMetrics);
  ...
  /**
   * Poll all members servers once asynchronously and store their futures in pollCompletableFutures.
   *
  ...
  private Map<String, CompletableFuture<NodeState>> pollAsync(
    ...
    allServers.forEach(
        s -> {
          try {
            clusterState.put(
                s,
                new ManagementClient(clientRouters.get(s), epoch, clusterId)
    ...
    // Possible exceptions are held by their CompletableFutures. They will be handled in
    // pollIteration method
    try {
  ...
  /**
   * Set the timeoutResponse for all the routers connected to the given endpoints with the given
   * value.
   *
