...
/**
 * NetworkStretcher increases or decreases the timeout intervals for polling based on whether the
 * poll was unsuccessful or successful respectively.
 */
@Builder
public class NetworkStretcher {
  /**
   * Max duration for the responseTimeouts of the routers. In the worst case scenario or in case of
   * failed servers, their response timeouts will be set to a maximum value of maxPeriod.
   */
  @Getter @Default private final Duration maxPeriod = Duration.ofSeconds(5);

  @Getter @Default private final Duration minPeriod = Duration.ofSeconds(2);

  /** Response timeout for every router. */
  @Getter @Default private Duration currentPeriod = Duration.ofSeconds(2);

  /** Poll interval between iterations in a pollRound */
  @Default private final Duration initialPollInterval = Duration.ofSeconds(1);

  /**
   * Increments in which the period moves towards the maxPeriod in every failed iteration provided.
   */
  @Default private final Duration periodDelta = Duration.ofSeconds(1);
