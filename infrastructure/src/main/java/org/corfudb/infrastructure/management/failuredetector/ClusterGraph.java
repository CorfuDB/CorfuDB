...
/**
 * Graph representation of a cluster state. For instance, asymmetric graph, there is an asymmetric
 * failure between nodes `a` and `b`. {@link ConnectionStatus} could be OK/FAILED: {a -> {{a: OK},
 * {b: FAILED}, {c: OK}}} {b -> {{a: FAILED}, {b: OK}, {c: FAILED}}} {c -> {{a: OK}, {b: false}, {c:
 * OK}}}
 *
 * <p>This class represents a cluster as a graph, it allows to: - find fully connected node: node
 * that connected to all other nodes exclude unresponsive nodes. - find a decision maker: a node
 * which should decide to add failed node to the unresponsive list. The decision maker is an
 * optimization technique, needed to reduce parallel updates of the cluster state. Still, each node
 * can choose its own decision maker. See {@link this#getDecisionMaker()} for details - find a node
 * which should be healed and excluded from the unresponsive list - transform all asymmetric
 * failures to a symmetric ones. If there is an asymmetric failure (node B can connect to node B,
 * but node B can not connect to node A) between two nodes then make it symmetric which means
 * neither node A nor B can connect to each other.
 */
@Builder
@ToString
@Slf4j
public class ClusterGraph {
  ...
  @NonNull private final ImmutableMap<String, NodeConnectivity> graph;

  @NonNull private final String localNode;

  @NonNull private final ImmutableList<String> unresponsiveNodes;
  /**
   * Transform a cluster state to the cluster graph. ClusterState contains some extra information,
   * cluster graph is a pure representation of a graph of nodes. To be able to work efficiently with
   * cluster state to find failed and healed nodes the cluster state should be transformed to
   * cluster graph
   *
  ...
  public static ClusterGraph toClusterGraph(ClusterState cluster) {
    Map<String, NodeConnectivity> graph =
  ...
  /**
   * Convert a cluster graph which could have asymmetric failures to a graph with symmetric failures
   * between nodes. See {@link ConnectionStatus} for OK/FAILED For instance: {a: [{a: OK, b:
   * FAILED}]} {b: [{a: OK, b: OK}]} Node A believes that node B is disconnected Node B believes
   * that node A is connected. The graph will be changed to: {a: [{a: OK, b: FAILED}]} {b: [{a:
   * FAILED, b: OK}]} Node B is not connected to node A anymore.
   *
              ...
              node.getConnectivity()
                  ...
                  .forEach(
                      adjNodeName -> {
                        ...
                        // If current node is not the local node and another node is unavailable we
                        // don't change
                        ...
                        ConnectionStatus oppositeNodeConnection =
                            getConnectionStatus(adjNode, node);

                        // Symmetric failure - connection successful only if both nodes connected
                        // status is true
                        // in the other case - make the failure symmetric
                        ConnectionStatus status = ConnectionStatus.OK;
                        if (EnumSet.of(nodeConnection, oppositeNodeConnection)
                            .contains(ConnectionStatus.FAILED)) {
                          status = ConnectionStatus.FAILED;
                        }
                        newConnectivity.put(adjNodeName, status);
                      });

              NodeConnectivity symmetricConnectivity =
  ...
  /**
   * Get a decision maker node to detect a failure. It must have: - highest number of successful
   * connections in the graph. There could be many nodes with same number of successful connections,
   * then the decision maker will be a node with smallest name.
   *
   * <p>The decision maker is an optimization technique, needed to reduce parallel updates of the
   * cluster state. Still, each node can choose its own decision maker. Note: it's not required to
   * choose a particular one for entire cluster to add a node to unresponsive list. In a partitioned
   * scenario, a minority side can choose to have a decision maker just that it's decisions will not
   * be used as it does not have consensus
   *
   * <p>The decision maker always exists. There is always at least the local node, it always has at
   * least one successful connection to itself. We also have two additional checks to prevent all
   * possible incorrect ways. Decision maker not found if: - ClusterGraph is empty, which is an
   * invalid state. - the decision maker doesn't have connections, which is also impossible.
   *
  ...
  /**
   * Find failed node in a graph. Collect all node ranks in a graph and choose the node with
   * smallest number of successful connections and with highest name (sorted alphabetically)
   *
   * @return failed node
   */
  public Optional<NodeRank> findFailedNode() {
    ...
    boolean isFailedNodeFullyConnected =
        fullyConnected.map(fcNode -> fcNode.equals(last)).orElse(false);
  ...
  public Optional<NodeRank> findFullyConnectedNode(String endpoint) {
    ...
    switch (node.getType()) {
      ...
      case CONNECTED:
        ...
        NodeRank rank = new NodeRank(endpoint, getNodeConnectivity(endpoint).getConnected());
   ...
   * {b: [{a: OK}, {b: FAILED}]} getConnectionStatus("a", "b") -> FAILED getConnectionStatus("b",
   * "a") -> OK
   *
  ...
  private ConnectionStatus getConnectionStatus(
      NodeConnectivity sourceNode, NodeConnectivity targetNode) {

    if (sourceNode == null || targetNode == null) {
      String errMsg =
          "Source or target node is null. Source: " + sourceNode + ", target: " + targetNode;
  ...
  public static class ClusterGraphHelper {
    ...
    public static ClusterGraph cluster(
        String localNode, ImmutableList<String> unresponsiveNodes, NodeConnectivity... nodes) {
      Map<String, NodeConnectivity> graph =
          Arrays.stream(nodes)
              .collect(Collectors.toMap(NodeConnectivity::getEndpoint, Function.identity()));
