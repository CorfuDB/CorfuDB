...
public class BaseServer extends AbstractServer {
  ...
  public BaseServer(@Nonnull ServerContext context) {
    ...
    executor =
        Executors.newFixedThreadPool(
            serverContext.getBaseServerThreadCount(),
  ...
  /**
   * Respond to a keep alive message. Note: this message ignores epoch.
   *
   * @param msg The incoming message
   * @param ctx The channel context
   * @param r The server router.
  ...
  private void getVersion(CorfuMsg msg, ChannelHandlerContext ctx, IServerRouter r) {
    VersionInfo vi =
        new VersionInfo(serverContext.getServerConfig(), serverContext.getNodeIdBase64());
    r.sendResponse(ctx, msg, new JSONPayloadMsg<>(vi, CorfuMsgType.VERSION_RESPONSE));
  }

  /**
   * Respond to a epoch change message. This method also executes sealing logic on each individual
   * server type.
   *
   * @param msg The incoming message
   * @param ctx The channel context
  ...
  public synchronized void handleMessageSetEpoch(
      @NonNull CorfuPayloadMsg<Long> msg, ChannelHandlerContext ctx, @NonNull IServerRouter r) {
    try {
      ...
      log.info(
          "handleMessageSetEpoch: Received SEAL from (clientId={}:{}), moving to new epoch {}",
          msg.getClientID(),
          remoteHostAddress,
          epoch);
      ...
      log.debug(
          "handleMessageSetEpoch: Rejected SEAL current={}, requested={}",
          e.getCorrectEpoch(),
          msg.getPayload());
      r.sendResponse(
          ctx, msg, new CorfuPayloadMsg<>(CorfuMsgType.WRONG_EPOCH, e.getCorrectEpoch()));
    }
  }

  /**
   * Reset the JVM. This mechanism leverages that corfu_server runs in a bash script which monitors
   * the exit code of Corfu. If the exit code is 100, then it resets the server and DELETES ALL
   * EXISTING DATA.
   *
   * @param msg The incoming message
   * @param ctx The channel context
  ...
  /**
   * Restart the JVM. This mechanism leverages that corfu_server runs in a bash script which
   * monitors the exit code of Corfu. If the exit code is 200, then it restarts the server.
   *
   * @param msg The incoming message
   * @param ctx The channel context
   * @param r The server router.
