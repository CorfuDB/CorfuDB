...
public class StateTransferManager {
  ...
  @Getter @NonNull private final LogUnitClient logUnitClient;

  /** A size of one batch of transfer. */
  @Getter @NonNull private final int batchSize;

  /** A processor that parallelizes the transfer workload. */
  @Getter @NonNull private final ParallelTransferProcessor parallelTransferProcessor;

  /** A processor that transfers the workload via a replication protocol. */
  @Getter @NonNull private final BasicTransferProcessor basicTransferProcessor;
  ...
  ImmutableList<Long> getUnknownAddressesInRange(long rangeStart, long rangeEnd) {
    Set<Long> knownAddresses =
        CFUtils.getUninterruptibly(logUnitClient.requestKnownAddresses(rangeStart, rangeEnd))
            .getKnownAddresses();
  ...
  TransferSegmentRangeSingle getUnknownAddressesInRangeForRange(TransferSegmentRangeSingle range) {
    ...
    return range.toBuilder().unknownAddressesInRange(unknownAddressesInRange).build();
  }

  /**
   * Transform the given range into a stream of batch requests.
   *
   * @param range A transfer segment range that contains unknown addresses and maybe available log
   *     unit servers.
  ...
  /**
   * Go over all the single transfer segment ranges, filter them by the provided type, get all the
   * unknown addresses in range, and then turn the unknown addresses in range into the transfer
   * batch request stream - ready to be consumed by the appropriate state transfer processor.
   *
   ...
   */
  List<Stream<TransferBatchRequest>> createBatchWorkload(
      List<TransferSegmentRangeSingle> ranges, StateTransferType typeOfTransfer) {
  ...
  /**
   * Transform all the transferred ranges back into the transfer segments. This data is used later
  ...
  ImmutableList<TransferSegment> toSegments(List<TransferSegmentRange> transferRanges) {
    return transferRanges.stream()
        .map(TransferSegmentRange::toTransferSegment)
        .collect(ImmutableList.toImmutableList());
  }
  ...
  private CompletableFuture<TransferProcessorResult> runCommittedStateTransfer(
    ...
    if (consistentBatchStreamList.size() != parallelismFactorForEachRange.size()) {
      throw new IllegalArgumentException(
          "The lists of streams and par factors" + " should be equal.");
    }
    for (int i = 0; i < parallelismFactorForEachRange.size(); i++) {
      int parFactor = parallelismFactorForEachRange.get(i);
      Stream<TransferBatchRequest> transferBatchRequestStream = consistentBatchStreamList.get(i);
      TransferProcessorResult parallelTransferResult =
          parallelTransferProcessor.runStateTransfer(transferBatchRequestStream, parFactor).join();
  ...
  /**
   * Performs the state transfer for the current non-transferred transfer segments and also updates
   * their state as a result.
   *
  ...
  public ImmutableList<TransferSegment> handleTransfer(
    ...
    Stream<TransferBatchRequest> protocolBatchStream =
        createBatchWorkload(singleNotTransferredRanges, PROTOCOL_READ).stream()
    ...
    List<Integer> parallelismFactorForEachRange =
        singleNotTransferredRanges.stream()
                ...
                range -> {
                  if (range.getAvailableServers().isPresent()) {
                    ...
                    throw new IllegalStateException(
                        "No available servers " + "for consistent transfer.");
                  }
                })
    ...
    TransferProcessorResult result =
        runCommittedStateTransfer(consistentBatchStreamList, parallelismFactorForEachRange)
            .thenCompose(
                res -> {
    ...
    if (result.getTransferState() == TRANSFER_SUCCEEDED) {
      newTransferSegmentStatus =
          TransferSegmentStatus.builder()
              .segmentState(TRANSFERRED)
              .causeOfFailure(Optional.empty())
              .build();
    } else {
      newTransferSegmentStatus =
          TransferSegmentStatus.builder()
              .segmentState(FAILED)
              .causeOfFailure(result.getCauseOfFailure())
              .build();
    }

    // Update the status of the not transferred segment ranges.
    ImmutableList<TransferSegmentRange> transferredSegmentRanges =
        updateNotTransferredSegmentRangeStatus(newTransferSegmentStatus, beforeTransferRanges);
