...
/**
 * A container object that holds log tail offsets and the global log tail that has been seen. Note
 * that holes don't belong to any stream therefore the globalTail needs to be tracked separately.
 *
...
public class LogMetadata {
  ...
  @Getter private volatile long globalTail;

  @Getter private final Map<UUID, StreamAddressSpace> streamsAddressSpaceMap;

  @Getter private final Map<UUID, Long> streamTails;
  ...
  /**
   * Updates relevant info of a stream's space, concretely: 1. Stream's tail, i.e., the last
   * observed address for the stream. 2. Stream's address space, i.e., space of all observed updates
   * for the stream.
   *
  ...
  private void updateStreamSpace(UUID streamId, long entryAddress) {
    ...
    // Update stream address space (used for sequencer recovery), add this entry as a valid address
    // for this stream.
    streamsAddressSpaceMap.compute(
        streamId,
        (id, addressSpace) -> {
  ...
  /**
   * Update's relevant info of a stream's space from a checkpoint, concretely: 1. Stream tail for
   * those stream's that have all updates within a checkpoint. 2. Stream trim mark, i.e., last
   * observed address for a stream subsumed by a checkpoint.
   *
   * @param entry log entry
   * @param initialize true, if called on log unit initialization (full scan) false, otherwise.
   */
          ...
          streamsAddressSpaceMap.compute(
              ...
              (id, addressSpace) -> {
                if (addressSpace == null) {
                  ...
                  // this stream yet. We can initialize the trim mark to the last observed update by
                  // the
                  // checkpoint. If further entries are observed they will be added to the address
                  // space.
                  return new StreamAddressSpace(lastUpdateToStream, new Roaring64NavigableMap());
                }
                // We will hold the maximum of these observed updates as the stream trim mark
                // (highest
                // checkpointed address), as this guarantees data is available in a checkpoint (safe
                // trim mark).
  ...
  public void prefixTrim(long address) {
    ...
    for (Map.Entry<UUID, StreamAddressSpace> streamAddressMap : streamsAddressSpaceMap.entrySet()) {
      log.trace(
          "prefixTrim: trim address space for stream {} up to trim mark {}",
          streamAddressMap.getKey(),
          address);
