        ...
        () -> {
          while (iterator.hasNext()) {
            ...
            if (result.getStatus() == TransferBatchResponse.TransferStatus.FAILED) {
              String errorMessage = "Failed to transfer: " + result.getTransferBatchRequest();
              TransferSegmentException transferSegmentException =
                  result
                      .getCauseOfFailure()
                      .map(ex -> new TransferSegmentException(errorMessage, ex))
