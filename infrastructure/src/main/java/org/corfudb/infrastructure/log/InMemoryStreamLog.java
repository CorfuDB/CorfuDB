...
/**
 * This class implements the StreamLog interface using a Java hash map. The stream log is only
 * stored in-memory and not persisted. This should only be used for testing. Created by maithem on
 * 7/21/16.
 */
@Slf4j
public class InMemoryStreamLog implements StreamLog, StreamLogWithRankedAddressSpace {
  ...
  private void throwLogUnitExceptionsIfNecessary(long address, LogData entry) {
    if (entry.getRank() == null) {
      ...
      log.trace(
          "throwLogUnitExceptionsIfNecessary: overwritten exception for address {}, cause: {}",
          address,
          overwriteCause);
  ...
  /**
   * Returns the known addresses in this Log Unit in the specified consecutive range of addresses.
   *
