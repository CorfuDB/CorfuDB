...
public class StreamLogFiles implements StreamLog, StreamLogWithRankedAddressSpace {
  ...
  private static ByteBuffer getByteBuffer(Metadata metadata, AbstractMessage message) {
    ByteBuffer buf =
        ByteBuffer.allocate(metadata.getSerializedSize() + message.getSerializedSize());
  ...
  private void verifyLogs() {
    ...
    for (File file : files) {
      ...
      if (header.getVersion() != VERSION) {
        String msg =
            String.format(
                "Log version %s for %s should match the LogUnit log version %s",
      ...
      if (verify && !header.getVerifyChecksum()) {
        String msg =
            String.format(
                "Log file %s not generated with check sums, can't verify!", file.getAbsoluteFile());
        throw new IllegalStateException(msg);
      }
    }
  }
  ...
  private LogData getLogData(LogEntry entry) {
    ...
    LogData logData =
        new LogData(
            org.corfudb.protocols.wireprotocol.DataType.typeMap.get(
                (byte) entry.getDataType().getNumber()),
    ...
    if (entry.hasCheckpointEntryType()) {
      logData.setCheckpointType(
          CheckpointEntry.CheckpointEntryType.typeMap.get(
              (byte) entry.getCheckpointEntryType().ordinal()));
      ...
      logData.setCheckpointedStreamStartLogAddress(entry.getCheckpointedStreamStartLogAddress());
    }
  ...
  /**
   * Parse the metadata field. This method should only be called when a metadata field is expected.
   *
  ...
  private Metadata parseMetadata(FileChannel fileChannel, String segmentFile) throws IOException {
    ...
    if (actualMetaDataSize < METADATA_SIZE) {
      log.error(
          "Meta data has wrong size. Actual size: {}, expected: {}",
          actualMetaDataSize,
          METADATA_SIZE);
      return null;
    }
    ...
    try {
      ...
      String errorMessage =
          getDataCorruptionErrorMessage("Can't parse metadata", fileChannel, segmentFile);
      throw new DataCorruptionException(errorMessage, e);
    }

    if (metadata.getLengthChecksum() != Checksum.getChecksum(metadata.getLength())) {
      String errorMessage =
          getDataCorruptionErrorMessage(
              "Metadata: invalid length checksum", fileChannel, segmentFile);
  ...
  private String getDataCorruptionErrorMessage(
    ...
    return message
        ...
        + ". Global tail: "
        + logMetadata.getGlobalTail()
        + ". Tail segment: "
        + dataStore.getTailSegment()
        + ". Stream tails size: "
        + logMetadata.getStreamTails().size();
  }
   ...
   * @throws IOException IO exception
   */
  private ByteBuffer getPayloadForMetadata(FileChannel fileChannel, Metadata metadata)
      throws IOException {
  ...
  /**
   * Parse the logfile header, or create it, or recreate it if it was partially written.
   *
  ...
  private LogHeader parseHeader(FileChannel channel, String segmentFile) throws IOException {
    ...
    if (Checksum.getChecksum(buffer.array()) != metadata.getPayloadChecksum()) {
      String errorMessage =
          getDataCorruptionErrorMessage("Invalid metadata checksum", channel, segmentFile);
      throw new DataCorruptionException(errorMessage);
    }
    ...
    try {
      ...
      String errorMessage = getDataCorruptionErrorMessage("Invalid header", channel, segmentFile);
      throw new DataCorruptionException(errorMessage, e);
    }
  ...
  private LogEntry parseEntry(FileChannel channel, Metadata metadata, String fileName)
    ...
    if (verify && metadata.getPayloadChecksum() != Checksum.getChecksum(buffer.array())) {
      String errorMessage =
          getDataCorruptionErrorMessage(
              "Checksum mismatch detected while trying to read file", channel, fileName);
      throw new DataCorruptionException(errorMessage);
    }
    ...
    try {
      ...
      String errorMessage = getDataCorruptionErrorMessage("Invalid entry", channel, fileName);
      throw new DataCorruptionException(errorMessage, e);
    }
    return entry;
  }
  ...
  private void readAddressSpace(SegmentHandle segment) throws IOException {
    ...
    while (fileChannel.size() - fileChannel.position() > 0) {
      ...
      AddressMetaData addressMetadata =
          new AddressMetaData(
              metadata.getPayloadChecksum(), metadata.getLength(), channelOffset + METADATA_SIZE);
  ...
  private LogData readRecord(SegmentHandle segment, long address) throws IOException {
    ...
    try {
      ...
      String errorMessage =
          getDataCorruptionErrorMessage("Invalid entry", fileChannel, segment.getFileName());
      throw new DataCorruptionException(errorMessage, e);
    }
  }
  ...
  private FileChannel getChannel(String filePath, boolean readOnly) throws IOException {
    if (readOnly) {
      ...
      return FileChannel.open(
          FileSystems.getDefault().getPath(filePath), EnumSet.of(StandardOpenOption.READ));
    }
  ...
  /**
   * Gets the file channel for a particular address, creating it if is not present in the map.
   *
  ...
  private LogEntry getLogEntry(long address, LogData entry) {
    ...
    if (entry.getClientId() != null && entry.getThreadId() != null) {
      logEntryBuilder.setClientIdMostSignificant(entry.getClientId().getMostSignificantBits());
      logEntryBuilder.setClientIdLeastSignificant(entry.getClientId().getLeastSignificantBits());
      logEntryBuilder.setThreadId(entry.getThreadId());
    }

    if (entry.hasCheckpointMetadata()) {
      logEntryBuilder.setCheckpointEntryType(
          Types.CheckpointEntryType.forNumber(entry.getCheckpointType().ordinal()));
  ...
  private Optional<Types.DataRank> createProtobufsDataRank(IMetadata entry) {
    ...
    Types.DataRank result =
        Types.DataRank.newBuilder()
  ...
  private IMetadata.DataRank createDataRank(LogEntry entity) {
    ...
    return new IMetadata.DataRank(
        rank.getRank(), new UUID(rank.getUuidMostSignificant(), rank.getUuidLeastSignificant()));
  }
   ...
   * @throws IOException IO exception
   */
  private Map<Long, AddressMetaData> writeRecords(SegmentHandle segment, List<LogData> entries)
      throws IOException {
    ...
    try (MultiReadWriteLock.AutoCloseableLock ignored =
      ...
      for (int ind = 0; ind < entryBuffs.size(); ind++) {
        long channelOffset =
            segment.getWriteChannel().position() + allRecordsBuf.position() + METADATA_SIZE;
  ...
  /**
   * Attempts to write a buffer to a file channel, if write fails with an IOException then the
   * channel pointer is moved back to its original position before the write
   *
  ...
  /**
   * Pre-process a range of entries to be written. This includes removing trimmed entries and
   * advancing the trim mark appropriately.
   *
  ...
  /**
   * This method verifies that a range of entries doesn't span more than two segments and that the
   * log addresses are ordered sequentially.
   *
  ...
  public void append(List<LogData> range) {
    ...
    Set<Long> pendingWrites =
        range.stream().map(ILogData::getGlobalAddress).collect(Collectors.toSet());

    // See if the provided range overlaps with any of the previously written entries.
    Set<Long> segOneOverlap =
        Sets.intersection(pendingWrites, firstSh.getKnownAddresses().keySet());
    Set<Long> segTwoOverlap = Sets.intersection(pendingWrites, lastSh.getKnownAddresses().keySet());
    ...
    try {
      ...
      log.error(
          "Disk_write[{}-{}]: Exception", first.getGlobalAddress(), last.getGlobalAddress(), e);
  ...
  private void closeSegmentHandlers(long endSegment) {
    for (SegmentHandle sh : writeChannels.values()) {
      ...
      if (sh.getRefCount() != 0) {
        log.warn(
            "closeSegmentHandlers: Segment {} is trimmed, but refCount is {}, attempting to trim"
                + " anyways",
            sh.getSegment(),
            sh.getRefCount());
      }
  ...
  private void deleteFilesMatchingFilter(FileFilter fileFilter) {
    ...
    log.info(
        "deleteFilesMatchingFilter: completed, deleted {} files, freed {} bytes",
        numFiles,
        freedBytes);
  }
