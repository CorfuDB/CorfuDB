 ...
 *
 * <p>Keeps stream log related meta information: startingAddress and tailSegment. Provides access to
 * the stream log related meta information.
 */
@Slf4j
public class StreamLogDataStore {
  ...
  private static final KvRecord<Long> TAIL_SEGMENT_RECORD =
      new KvRecord<>(TAIL_SEGMENT_PREFIX, TAIL_SEGMENT_KEY, Long.class);

  private static final KvRecord<Long> STARTING_ADDRESS_RECORD =
      new KvRecord<>(STARTING_ADDRESS_PREFIX, STARTING_ADDRESS_KEY, Long.class);

  private static final KvRecord<Long> COMMITTED_TAIL_RECORD =
      new KvRecord<>(COMMITTED_TAIL_PREFIX, COMMITTED_TAIL_KEY, Long.class);

  private static final long ZERO_ADDRESS = 0L;

  @NonNull private final KvDataStore dataStore;
   ...
   *
   * <p>WARNING: this method is not thread safe, it is supposed to be called by operations executed
   * in batch processor to achieve thread safety.
   *
  ...
  public void updateStartingAddress(long newStartingAddress) {
    if (startingAddress.get() >= newStartingAddress) {
      log.trace(
          "New starting address {} less than or equals to the old one. Ignore", newStartingAddress);
      return;
    }
   ...
   *
   * <p>This method is synchronized to prevent concurrent updates that could cause the cached
   * committed tail and the disk value inconsistent. Note that in this case atomic operation could
   * not help. The alternative is to do the update through batch processor which is single threaded,
   * but doing that could be less efficient.
   *
  ...
  public synchronized void updateCommittedTail(long newCommittedTail) {
    if (committedTail.get() >= newCommittedTail) {
      log.trace(
          "New committed tail {} less than or equals to the old one. Ignore", newCommittedTail);
      return;
    }
