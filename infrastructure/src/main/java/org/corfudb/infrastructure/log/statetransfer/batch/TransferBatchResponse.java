...
/**
 * A result of a transfer of a batch of addresses. If completed successfully, contains a status
 * SUCCEEDED, a {@link #transferBatchRequest} that contains a list of addresses transferred, and an
 * optional destination server. If completed exceptionally, contains a status FAILED, and a cause of
 * failure, a {@link #transferBatchRequest} that contains an empty list, and an optional destination
 * server.
 */
...
public class TransferBatchResponse {
  ...
  /** A success status. */
  @Default private final TransferStatus status = SUCCEEDED;
