...
public abstract class AbstractLogSegment implements
    ...
    AddressMetaData writeRecord(long address, LogData logData,
        ...
        LogEntry logEntry = getLogEntry(address, logData, logParams.compressionCodec);
    ...
    Map<Long, AddressMetaData> writeRecords(List<LogData> entries,
        ...
        for (LogData curr : entries) {
            LogEntry logEntry = getLogEntry(curr.getGlobalAddress(), curr, logParams.compressionCodec);
            Metadata metadata = getMetadata(logEntry);
