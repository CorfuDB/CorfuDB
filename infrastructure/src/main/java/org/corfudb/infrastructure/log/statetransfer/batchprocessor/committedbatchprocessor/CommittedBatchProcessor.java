...
/**
 * A transferBatchRequest processor that transfers committed addresses one transferBatchRequest at a
 * time via reading directly from the provided log unit servers in a random order. A committed
 * address is the one that belongs to the part of an address space that has been previously
 * committed by the auto-commit service or some previously run state transfer.
 */
@Slf4j
@Builder
@Getter
public class CommittedBatchProcessor implements StateTransferBatchProcessor {
  ...
  @Default private final int maxReadRetries = 3;

  @Default private final Duration readSleepDuration = Duration.ofMillis(300);
  @Default private final int maxWriteRetries = 3;
  @Default private final Duration writeSleepDuration = Duration.ofMillis(300);
  ...
  /**
   * An iterator over a provided transfer batch request. It iterates over the available destination
   * nodes in the random order producing the TransferBatchRequestForNode requests. This allows to
   * distribute the workload among the multiple consecutive transfer batch requests destined for
   * this batch processor somewhat evenly.
   */
  public static class RandomNodeIterator implements Iterator<TransferBatchRequestForNode> {
    ...
    public RandomNodeIterator(TransferBatchRequest request) {
      ...
      ArrayList<String> nodes =
          new ArrayList<>(request.getDestinationNodes().orElse(ImmutableList.of()));
  ...
  /**
   * Given a random node iterator created from the transfer batch request, try perform a state
   * transfer by reading the data from the randomly picked log unit and writing it to the current
   * log unit server. If the read failed for a node, pick the next available one randomly and retry.
   * If the transfer failed for some other reason (e.g. during the writes) or if there are no more
   * nodes left to distribute the workload over, fail the transfer with an exception.
   *
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          while (nodeIterator.hasNext()) {
            ...
            LogUnitClient logUnitClientToCurrentNode = runtimeLayout.getLogUnitClient(currentNode);
            try {
              ReadBatch readBatch =
                  readRecords(addresses, Optional.of(destinationNode), logUnitClientToTargetNode);
              return writeRecords(
                  readBatch, logUnitClientToCurrentNode, maxWriteRetries, writeSleepDuration);
            } catch (ReadBatchException re) {
              log.warn(
                  "Read exception for node {} occurred. "
                      + "Retry transfer for the next available node.",
                  destinationNode,
                  re);
            } catch (Exception e) {
  ...
  public CompletableFuture<TransferBatchResponse> transfer(
    ...
    return tryTransferForRandomNodes(new RandomNodeIterator(transferBatchRequest))
        .exceptionally(
            error ->
                TransferBatchResponse.builder()
                    .transferBatchRequest(transferBatchRequest)
                    .status(FAILED)
                    .causeOfFailure(
                        Optional.of(
                            new StateTransferBatchProcessorException(
   ...
   */
  public ReadBatch readRecords(
      List<Long> addresses, Optional<String> destNode, LogUnitClient client) {
    for (int i = 0; i < maxReadRetries; i++) {
      try {
        ReadResponse response = CFUtils.getUninterruptibly(client.readAll(addresses));
        Map<Long, ILogData> records = new HashMap<>(response.getAddresses());
        ReadBatch readBatch = checkReadRecords(addresses, records, destNode);
        if (readBatch.getStatus() == ReadBatch.ReadStatus.FAILED) {
          throw new IllegalStateException(
              "Some addresses failed to transfer: " + readBatch.getFailedAddresses());
        }
        return readBatch;
