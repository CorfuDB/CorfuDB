...
public interface StateTransferBatchProcessor {
   ...
   */
  default Set<Long> getNonTransferredAddresses(
      LogUnitClient client, ImmutableList<Long> addressesToTransfer) throws TimeoutException {
    ...
    Set<Long> knownAddresses =
        CFUtils.getUninterruptibly(
                client.requestKnownAddresses(start, end),
  ...
  /**
   * Create a read batch from the addresses that failed to be written to the current log unit
   * server.
   *
   * @param dataToTransfer A list of log data that was supposed to be written to the current log
   *     unit server entirely.
   * @param nonWrittenAddresses A list of addresses that are not present in the current log unit
   *     server.
   * @return A new read batch.
   */
  default ReadBatch createReadBatchFromNotTransferredAddresses(
      List<LogData> dataToTransfer, Set<Long> nonWrittenAddresses) {
    if (nonWrittenAddresses.isEmpty()) {
      throw new IllegalArgumentException(
          "nonWrittenAddresses should be not empty " + "to create a new read batch.");
    }
    Comparator<LogData> addressComparator = Comparator.comparing(IMetadata::getGlobalAddress);
    // Create a new read batch with the missing data..
    ImmutableList<LogData> nonWrittenData =
        dataToTransfer.stream()
   ...
   */
  default TransferBatchResponse tryWrite(
      ReadBatch readBatch, LogUnitClient logUnitClient, List<Long> allAddressesToTransfer)
      throws TimeoutException {
    List<LogData> remainingDataToWrite = readBatch.getData();

    CFUtils.getUninterruptibly(
        logUnitClient.writeRange(remainingDataToWrite),
        OverwriteException.class,
        TimeoutException.class,
        NetworkException.class);

    return TransferBatchResponse.builder()
        .transferBatchRequest(
            new TransferBatchRequest(
                allAddressesToTransfer, readBatch.getDestinationNode().map(ImmutableList::of)))
        .status(SUCCEEDED)
        .build();
  }

  /**
   * Appends records to the stream log. If a log throws an exception during an append, wait and
   * retry.
   *
   * @param readBatch The non-empty batch of log entries as well as an optional node they were read
   *     from.
  ...
  default TransferBatchResponse writeRecords(
      ...
      int writeRetriesAllowed,
      Duration writeSleepDuration) {
        ...
        if (lastException.isPresent()) {
          ...
          if (notWrittenAddresses.isEmpty()) {
            return TransferBatchResponse.builder()
                .transferBatchRequest(
                    new TransferBatchRequest(
                        addressesToTransfer, readBatch.getDestinationNode().map(ImmutableList::of)))
                .status(SUCCEEDED)
                .build();
          }
          // Create a new ReadBatch from the notWrittenAddresses and retry the workflow.
          readBatch =
              createReadBatchFromNotTransferredAddresses(dataToTransfer, notWrittenAddresses);
        }
        return tryWrite(readBatch, logUnitClient, addressesToTransfer);
      } catch (OverwriteException | TimeoutException | NetworkException e) {
        log.warn("writeRecords: tryWrite failed. Retrying {}/{}.", i, writeRetriesAllowed, e);
    ...
    throw lastException.map(RetryExhaustedException::new).orElse(new RetryExhaustedException());
  }
   ...
   */
  @VisibleForTesting
  default ReadBatch checkReadRecords(
      List<Long> addresses, Map<Long, ILogData> readResult, Optional<String> destinationNode) {
    List<Long> transferredAddresses =
        addresses.stream().filter(readResult::containsKey).collect(Collectors.toList());
    if (transferredAddresses.equals(addresses)) {
      List<LogData> logData =
          readResult.entrySet().stream()
              .sorted(Map.Entry.comparingByKey())
              .map(entry -> (LogData) entry.getValue())
