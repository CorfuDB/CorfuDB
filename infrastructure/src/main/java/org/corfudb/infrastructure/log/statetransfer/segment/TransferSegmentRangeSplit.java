...
public class TransferSegmentRangeSplit implements TransferSegmentRange {
  ...
  public TransferSegmentRange updateStatus(TransferSegmentStatus newStatus) {
    TransferSegmentRangeSingle first = splitSegments.first.toBuilder().status(newStatus).build();
    TransferSegmentRangeSingle second = splitSegments.second.toBuilder().status(newStatus).build();
    return this.toBuilder().splitSegments(new Tuple<>(first, second)).build();
  }
    ...
    public void verify() {
      ...
      if (firstRange.getEndAddress() + 1 != secondRange.getStartAddress()) {
        throw new IllegalStateException(
            String.format(
                "End address of a first: %s and "
                    + "start address of a second: %s "
                    + "are not consecutive.",
                firstRange.getEndAddress(), secondRange.getStartAddress()));
      }
      if (!firstRange.getStatus().equals(secondRange.getStatus())) {
        throw new IllegalStateException(
            String.format(
                "Status of a first: %s and " + "status of a second: %s " + "are not the same.",
                firstRange.getStatus(), secondRange.getStatus()));
      }

      if (firstRange.getTypeOfTransfer() != CONSISTENT_READ) {
        throw new IllegalStateException("First range should be of a type " + "consistent read.");
      }

      if (secondRange.getTypeOfTransfer() != PROTOCOL_READ) {
        throw new IllegalStateException(
            "Second range type should be " + "of a type protocol read.");
      }
    }
