...
public class TransferSegmentRangeSingle implements TransferSegmentRange {
  ...
  @NonNull private final StateTransferType typeOfTransfer;
  /**
   * If the typeOfTransfer field is COMMITTED_READ, it's the list of servers to read committed data
   * from. Otherwise, the field is empty.
   */
  @NonNull private final Optional<ImmutableList<String>> availableServers;

  /**
   * The list of the actual non-transferred addresses in the range on this node's log unit server.
   */
  @NonNull private final ImmutableList<Long> unknownAddressesInRange;

  /** The transfer status. */
  @NonNull private final TransferSegmentStatus status;
  ...
  public static class TransferSegmentRangeSingleBuilder {
    ...
    public void verify() {
      if (startAddress < 0L || endAddress < 0L) {
        throw new IllegalStateException(
            String.format(
                "Start: %s or end: %s " + "can not be negative.", startAddress, endAddress));
      }
      if (startAddress > endAddress) {
        throw new IllegalStateException(
            String.format(
                "Start: %s can not be " + "greater than end: %s.", startAddress, endAddress));
      }
    ...
    public TransferSegmentRangeSingle build() {
      verify();
      return new TransferSegmentRangeSingle(
          startAddress,
          endAddress,
          typeOfTransfer,
          availableServers,
