...
public interface StreamLogWithRankedAddressSpace extends StreamLog {
  ...
  /**
   * Check whether the data can be appended to a given log address. Note that it is not permitted
   * multiple threads to access the same log address concurrently through this method, this method
   * does not lock or synchronize. This method needs
   *
   * @param address log address of append
   * @param newEntry log entry to append
   * @throws DataOutrankedException if the log entry cannot be assigned to this log address as there
   *     is a data with higher rank
   * @throws ValueAdoptedException if the new message is a proposal during the two phase recovery
   *     write and there is an existing data at this log address already.
   * @throw OverwriteException if the new data is with rank 0 (not from recovery write). This can
   *     happen only if there is a bug in the client implementation.
   */
