...
/**
 * A transferBatchRequest processor that transfers non-committed addresses one transferBatchRequest
 * at a time via a replication protocol. A non-committed address is the one that belongs to the part
 * of an address space that is inconsistent.
 */
@Slf4j
@Builder
public class ProtocolBatchProcessor implements StateTransferBatchProcessor {
  ...
  private final ReadOptions readOptions =
      ReadOptions.builder().waitForHole(true).clientCacheable(false).serverCacheable(false).build();

  @Getter private final LogUnitClient logUnitClient;

  @Getter private final AddressSpaceView addressSpaceView;
  ...
  public CompletableFuture<TransferBatchResponse> transfer(
    ...
    return readRecords(transferBatchRequest)
        .thenApply(
            records -> writeRecords(records, logUnitClient, maxWriteRetries, writeSleepDuration))
        .exceptionally(
            error ->
  ...
  /**
   * Reads data entries by utilizing the replication protocol. If there are errors, retry. If the
   * wrong epoch exception occurs, fail immediately, so that we can retry the workflow earlier.
   *
            ...
            try {
              Map<Long, ILogData> records =
                  addressSpaceView.simpleProtocolRead(
                      transferBatchRequest.getAddresses(), readOptions);
              ReadBatch batch =
                  checkReadRecords(transferBatchRequest.getAddresses(), records, Optional.empty());
              if (batch.getStatus() == ReadBatch.ReadStatus.FAILED) {
                throw new IllegalStateException(
                    "Some addresses failed to transfer: " + batch.getFailedAddresses());
              }
              return batch;
