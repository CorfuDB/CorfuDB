...
/**
 * A transfer processor that performs state transfer by distributing and parallelizing the workload
 * among the source log unit servers of each segment.
 */
@Slf4j
public class ParallelTransferProcessor {
  ...
  private CompletableFuture<Void> handleNonTransferException(
      CompletableFuture<Void> futures, Throwable ex) {
    CompletableFuture<Void> failedFuture = new CompletableFuture<>();
    failedFuture.completeExceptionally(ex);
    return CFUtils.allOfOrTerminateExceptionally(futures, failedFuture);
  }

  private CompletableFuture<Void> handleBatchRequest(
      TransferBatchRequest request, CompletableFuture<Void> allFutures, Semaphore semaphore)
  ...
  public CompletableFuture<TransferProcessorResult> runStateTransfer(
    ...
    if (!iterator.hasNext()) {
      return CompletableFuture.completedFuture(
          TransferProcessorResult.builder().transferState(TRANSFER_SUCCEEDED).build());
    }

    CompletableFuture<Void> allFutures = CompletableFuture.completedFuture(null);
    if (parFactor <= 0) {
      allFutures =
          handleNonTransferException(
              allFutures, new IllegalArgumentException("Number of nodes should be > 0"));
    } else {
