...
public class RecoveryHandler {
     ...
     *
     * @param layout Layout to use to recover
     * @return True if the cluster was recovered, False otherwise
     */

    private static boolean attemptRecovery(@NonNull CorfuRuntime corfuRuntime,
                                           @NonNull Layout layout) {
        try {
            corfuRuntime.getLayoutManagementView().recoverCluster(layout);
        } catch (Exception e) {
     ...
     */
    private static boolean runRecoveryReconfiguration(Layout layout, CorfuRuntime corfuRuntime) {
        if (layout == null) {
            log.error("Management layout is null. Cannot recover.");
            return false;
        }
        Layout localRecoveryLayout = new Layout(layout);
        boolean recoveryReconfigurationResult = attemptRecovery(corfuRuntime, layout);
