 ...
 *
 * <ul>
 *   <li>Contains the common node level {@link DataStore}
 *   <li>Responsible for Server level EPOCH
 *   <li>Should contain common services/utilities that the different Servers in a node require.
 * </ul>
 *
 * <p>Note: It is created in {@link CorfuServer} and then passed to all the servers including {@link
 * NettyServerRouter}.
 *
...
public class ServerContext implements AutoCloseable {
  ...
  private static final KvRecord<Layout> CURR_LAYOUT_RECORD =
      KvRecord.of(PREFIX_LAYOUT, KEY_LAYOUT, Layout.class);

  private static final KvRecord<Long> SERVER_EPOCH_RECORD =
      KvRecord.of(PREFIX_EPOCH, KEY_EPOCH, Long.class);

  private static final KvRecord<Long> SEQUENCER_RECORD =
      KvRecord.of(KEY_SEQUENCER, PREFIX_SEQUENCER_EPOCH, Long.class);

  private static final KvRecord<Layout> MANAGEMENT_LAYOUT_RECORD =
      KvRecord.of(PREFIX_MANAGEMENT, MANAGEMENT_LAYOUT, Layout.class);

  private static final KvRecord<Long> LOG_UNIT_WATERMARK_RECORD =
      KvRecord.of(PREFIX_LOGUNIT, EPOCH_WATER_MARK, Long.class);

  /** various duration constants. */
  public static final Duration SHUTDOWN_TIMER = Duration.ofSeconds(5);

  @Getter private final Map<String, Object> serverConfig;

  @Getter private final DataStore dataStore;

  @Getter @Setter private IServerRouter serverRouter;

  @Getter @Setter private IReconfigurationHandlerPolicy failureHandlerPolicy;

  @Getter private final EventLoopGroup clientGroup;
  ...
  @Getter private final String localEndpoint;
  ...
  public ServerContext(Map<String, Object> serverConfig) {
    ...
    nodeLocator =
        NodeLocator.parseString(serverConfig.get("--address") + ":" + serverConfig.get("<port>"));
  ...
  /**
   * Cleanup the DataStore files with names that are prefixes of the specified fileName when so that
   * the number of these files don't exceed the user-defined retention limit. Cleanup is always done
   * on files with lower epochs.
   */
                  ...
                  .sorted(
                      Comparator.comparingInt(
                          file -> {
                            ...
                            return matcher.find(prefix.length())
  ...
  /**
   * Install a single node layout if and only if no layout is currently installed. Synchronized, so
  ...
  public Layout getNewSingleNodeLayout() {
    ...
    String localAddress =
        getServerConfig().get("--address") + ":" + getServerConfig().get("<port>");
    return new Layout(
        ...
        Collections.singletonList(
            new LayoutSegment(
                ...
                -1L,
                Collections.singletonList(
                    new Layout.LayoutStripe(Collections.singletonList(localAddress))))),
        0L,
        clusterId);
  }
  ...
  public void setLayoutInHistory(Layout layout) {
    KvRecord<Layout> currLayoutRecord =
        KvRecord.of(PREFIX_LAYOUTS, String.valueOf(layout.getEpoch()), Layout.class);
    dataStore.put(currLayoutRecord, layout);
  }

  /**
   * Persists the sequencer epoch. This is set only by the SequencerServer in the resetServer. No
   * lock required as it relies on the resetServer lock.
   *
  ...
  /**
   * Save detected failure in a history. History represents all cluster state changes. Disabled by
   * default.
   *
  ...
  public synchronized void saveFailureDetectorMetrics(FailureDetectorMetrics detector) {
    boolean enabled =
        Boolean.parseBoolean(System.getProperty("corfu.failuredetector", Boolean.FALSE.toString()));
    if (!enabled) {
      return;
    }

    KvRecord<FailureDetectorMetrics> fdRecord =
        KvRecord.of(
  ...
  /**
   * Get latest change in a cluster state saved in data store. Or provide default value if history
   * is disabled.
   *
  ...
  public FailureDetectorMetrics getFailureDetectorMetrics() {
    ...
    return Optional.ofNullable(dataStore.get(fdRecord))
  ...
  private EventLoopGroup getNewBossGroup() {
    final ThreadFactory threadFactory =
        new ThreadFactoryBuilder().setNameFormat(getThreadPrefix() + "accept-%d").build();
    EventLoopGroup group = getChannelImplementation().getGenerator().generate(1, threadFactory);
  ...
  private @Nonnull EventLoopGroup getNewWorkerGroup() {
    final ThreadFactory threadFactory =
        new ThreadFactoryBuilder().setNameFormat(getThreadPrefix() + "worker-%d").build();

    final int requestedThreads = Integer.parseInt(getServerConfig(String.class, "--Threads"));
    final int numThreads =
        requestedThreads == 0 ? Runtime.getRuntime().availableProcessors() * 2 : requestedThreads;
    EventLoopGroup group =
        getChannelImplementation().getGenerator().generate(numThreads, threadFactory);

    log.info(
        "getWorkerGroup: Type {} with {} threads", group.getClass().getSimpleName(), numThreads);
    return group;
  }
  ...
  private @Nonnull EventLoopGroup getNewClientGroup() {
    final ThreadFactory threadFactory =
        new ThreadFactoryBuilder().setNameFormat(getThreadPrefix() + "client-%d").build();
    ...
    log.info(
        "getClientGroup: Type {} with {} threads", group.getClass().getSimpleName(), numThreads);
    return group;
  }
   ...
   * {@inheritDoc}
   *
   * <p>Cleans up and releases all resources (such as thread pools and files) opened by this {@link
   * ServerContext}.
   */
  @Override
  public void close() {
