...
public class ServerContext implements AutoCloseable {
     ...
     * To support backward compatibility for some tests
     *
     * @param serverConfig map of configuration strings to objects
     */
    public ServerContext(Map<String, Object> serverConfig) {
        ServerConfiguration conf;
        if (serverConfig.containsKey("--config-file") && serverConfig.get("--config-file") != null) {
            conf = ServerConfiguration.getServerConfigFromFile((String) serverConfig.get("--config-file"));
        } else {
            conf = ServerConfiguration.getServerConfigFromMap(serverConfig, CLIArgumentOptionsMappingUtil.getOptionsToPropertiesMapping());
        }
