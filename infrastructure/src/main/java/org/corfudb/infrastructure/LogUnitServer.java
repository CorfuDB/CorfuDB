...
import static org.corfudb.protocols.CorfuProtocolServerErrors.getDataOutrankedErrorMsg;
...
import static org.corfudb.protocols.CorfuProtocolServerErrors.getValueAdoptedErrorMsg;
...
import static org.corfudb.protocols.service.CorfuProtocolMessage.getHighPriorityHeaderMsg;
...
public class LogUnitServer extends AbstractServer {
    ...
    public LogUnitServer(ServerContext serverContext, LogUnitServerInitializer serverInitializer) {
        ...
        if (config.isMemoryMode()) {
            ...
            streamLog = serverInitializer.buildStreamLog();
        } else {
    ...
    private void handleCompactLogRequest(RequestMsg req, ChannelHandlerContext ctx, IServerRouter r) {
        ...
        // Note: we reuse the request header as the ignore_cluster_id and
        // ignore_epoch fields are the same in both cases.
        r.sendResponse(getResponseMsg(req.getHeader(), getCompactResponseMsg()), ctx);
    }
    ...
    private void handleFlushCacheRequest(RequestMsg req, ChannelHandlerContext ctx, IServerRouter r) {
        ...
        // Note: we reuse the request header as the ignore_cluster_id and
        // ignore_epoch fields are the same in both cases.
        r.sendResponse(getResponseMsg(req.getHeader(), getFlushCacheResponseMsg()), ctx);
    }
