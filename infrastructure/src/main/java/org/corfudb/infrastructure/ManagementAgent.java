...
/**
 * Instantiates and performs failure detection and handling asynchronously, as well as the auto
 * commit service.
 *
 * <p>Failure Detector: Executes detection policy to detect failed and healed nodes. It then checks
 * for status of the nodes. If there are failed or healed nodes to be addressed, this then triggers
 * the respective handler which then responds to these reconfiguration changes based on a policy.
 *
...
public class ManagementAgent {
  ...
  /** LocalMonitoringService to poll local server metrics: Sequencer ready/not ready state. */
  ...
  @Getter private final RemoteMonitoringService remoteMonitoringService;

  /** AutoCommitService to periodically commit the unwritten addresses. */
  @Getter private final AutoCommitService autoCommitService;

  /**
   * Checks and restores if a layout is present in the local datastore to recover from. Spawns an
   * initialization task which recovers if required, and start monitoring services.
   *
  ...
  ManagementAgent(
    ...
    this.localMonitoringService =
        new LocalMonitoringService(serverContext, runtimeSingletonResource);
    ...
    this.remoteMonitoringService =
        new RemoteMonitoringService(
            serverContext,
    ...
    this.initializationTaskThread =
        new Thread(this::initializationTask, "initializationTaskThread");
  ...
  /**
   * Initialization task. This task is blocked until the management server is bootstrapped and has a
   * connected runtime. Performs recovery if required. Initiates the monitoring services which
   * performs failure detection and healing detection tasks.
   */
  private void initializationTask() {
    ...
    try {
      ...
      log.debug(
          "initializationTask: Initialization task interrupted without "
    ...
    while (!shutdown && !recovered) {
      try {
        boolean recoveredSuccessfully =
            runRecoveryReconfiguration(serverContext.copyManagementLayout(), getCorfuRuntime());
        ...
        log.error(
            "initializationTask: Recovery failed {} times. Retrying in {}s.",
            ++recoveryAttempts,
