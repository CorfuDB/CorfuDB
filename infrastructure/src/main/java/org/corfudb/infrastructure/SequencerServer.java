 ...
 *
 * <p>The sequencer server maintains the current tail of the log, the current tail of every stream,
 * and a cache of timestamps of updates on recent conflict-parameters.
 *
 * <p>A token request can be of several sub-types, which are defined in {@link TokenRequest}:
 *
 * <p>{@link TokenRequest::TK_QUERY} - used for only querying the current tail of the log and/or the
 * tails of specific streams
 *
 * <p>{@link TokenRequest::TK_RAW} - reserved for getting a "raw" token in the global log
 *
 * <p>{@link TokenRequest::TK_MULTI_STREAM} - used for logging across one or more streams
 *
 * <p>{@link TokenRequest::TK_TX} - used for reserving an address for transaction commit.
 *
 * <p>The transaction commit is the most sophisticated functaionality of the sequencer. The
 * sequencer reserves an address for the transaction only on if it knows that it can commit.
 *
 * <p>The TK_TX request contains a conflict-set and a write-set. The sequencer checks the
 * conflict-set against the stream-tails and against the conflict-parameters timestamp cache it
 * maintains. If the transaction commits, the sequencer updates the tails of all the streams and the
 * cache of conflict parameters.
 *
...
public class SequencerServer extends AbstractServer {
  ...
  @Getter private final ServerContext serverContext;

  /**
   * - {@link SequencerServer::globalLogTail}: global log first available position (initially, 0).
   */
  @Getter private long globalLogTail = Address.getMinAddress();

  private long trimMark = Address.NON_ADDRESS;

  /**
   * - {@link SequencerServer::streamTailToGlobalTailMap}: per streams map to last issued global-log
   * position. used for backpointers.
   */
  private Map<UUID, Long> streamTailToGlobalTailMap = new HashMap<>();

  /**
   * Per streams map and their corresponding address space (an address space is defined by the
   * stream's addresses and its latest trim mark)
   */
  ...
  @Getter private SequencerServerCache cache;

  @Getter @Setter private volatile long sequencerEpoch = Layout.INVALID_EPOCH;

  /**
   * The lower bound of the consecutive epoch range that this sequencer observes as the primary
   * sequencer. i.e. this sequencer has been the primary sequencer for all the consecutive epochs
   * from this epoch to {@link this#sequencerEpoch}
   */
  @Getter private long epochRangeLowerBound = Layout.INVALID_EPOCH;
  ...
  public boolean isServerReadyToHandleMsg(CorfuMsg msg) {
    ...
    if ((sequencerEpoch != serverContext.getServerEpoch())
      ...
      log.warn(
          "Rejecting msg at sequencer : sequencerStateEpoch:{}, serverEpoch:{}, " + "msg:{}",
          sequencerEpoch,
  ...
  private void setUpTimerNameCache() {
    ...
    timerNameCache.put(
        TokenRequest.TK_MULTI_STREAM, CorfuComponent.INFRA_SEQUENCER + "multi-stream-token");
    timerNameCache.put(TokenRequest.TK_TX, CorfuComponent.INFRA_SEQUENCER + "tx-token");
  }

  /**
   * Checks if an epoch is within a consecutive closed range [{@link this#epochRangeLowerBound},
   * {@link this#sequencerEpoch}].
   *
   * <p>This sequencer serves as the primary sequencer for all the consecutive epochs in this range.
   *
  ...
  /**
   * If the request submits a timestamp (a global offset) that is less than one of the global
   * offsets of a streams specified in the request, then abort; otherwise commit.
   *
   * @param txInfo info provided by corfuRuntime for conflict resolution: - timestamp : the snapshot
   *     (global) offset that this TX reads - conflictSet: conflict set of the txn. if any
   *     conflict-param (or stream, if empty) in this set has a later timestamp than the snapshot,
   *     abort
  ...
  private TxResolutionResponse txnCanCommit(TxResolutionInfo txInfo) {
    ...
    if (!isEpochInRange(txSnapshotEpoch)) {
      log.debug(
          "ABORT[{}] snapshot-ts[{}] current epoch[{}] lower bound[{}]",
          txInfo,
          txSnapshotTimestamp,
          sequencerEpoch,
          epochRangeLowerBound);
    ...
    for (Map.Entry<UUID, Set<byte[]>> conflictStream : txInfo.getConflictSet().entrySet()) {
      ...
      for (byte[] conflictParam : conflictParamSet) {
        ...
        Long keyAddress =
            cache.get(
                new ConflictTxStream(conflictStream.getKey(), conflictParam, Address.NON_ADDRESS));
        ...
        if (keyAddress > txSnapshotTimestamp.getSequence()) {
          ...
          return new TxResolutionResponse(
              TokenType.TX_ABORT_CONFLICT, keyAddress, conflictParam, conflictStream.getKey());
        }
        ...
        if (txSnapshotTimestamp.getSequence() < maxConflictNewSequencer) {
          log.debug(
              "ABORT[{}] snapshot-ts[{}] WILDCARD New Sequencer ts=[{}]",
              txInfo,
              txSnapshotTimestamp,
              maxConflictNewSequencer);
          return new TxResolutionResponse(TokenType.TX_ABORT_NEWSEQ);
        }
   ...
   *
   * <p>This returns information about the tail of the log and/or streams without
   * changing/allocating anything.
   *
   * @param msg corfu message containing token query
   * @param ctx netty ChannelHandlerContext
   * @param r server router
   */
  private void handleTokenQuery(
      CorfuPayloadMsg<TokenRequest> msg, ChannelHandlerContext ctx, IServerRouter r) {
  ...
  public void resetServer(
      CorfuPayloadMsg<SequencerRecoveryMsg> msg, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    // NOT_READY sequencer.
    final boolean bootstrapWithoutTailsUpdate = msg.getPayload().getBootstrapWithoutTailsUpdate();
    ...
    if (bootstrapWithoutTailsUpdate
      ...
      log.warn(
          "Cannot update existing sequencer. Require full bootstrap. SequencerEpoch : {}, MsgEpoch"
              + " : {}",
          sequencerEpoch,
          bootstrapMsgEpoch);
    ...
    if (serverContext.getSequencerEpoch() >= bootstrapMsgEpoch) {
      log.info(
          "Sequencer already bootstrapped at epoch {}. Discarding bootstrap request with epoch {}",
          sequencerEpoch,
          bootstrapMsgEpoch);
    ...
    if (!bootstrapWithoutTailsUpdate) {
      ...
      for (Map.Entry<UUID, StreamAddressSpace> streamAddressSpace : addressSpaceMap.entrySet()) {
        ...
        log.trace(
            "On Sequencer reset, tail for stream {} set to {}",
            streamAddressSpace.getKey(),
            streamTail);
      ...
      for (Map.Entry<UUID, StreamAddressSpace> streamAddressSpace :
          this.streamsAddressMap.entrySet()) {
        log.info(
            "Stream[{}] set to last trimmed address {} and {} addresses in the range [{}-{}], "
                + "on sequencer reset.",
        ...
        if (log.isTraceEnabled()) {
          log.trace(
              "Stream[{}] address map on sequencer reset: {}",
              Utils.toReadableId(streamAddressSpace.getKey()),
    ...
    log.info(
        "Sequencer reset with token = {}, size {} streamTailToGlobalTailMap = {}, sequencerEpoch ="
            + " {}",
        globalLogTail,
        streamTailToGlobalTailMap.size(),
  ...
  public void handleMetricsRequest(CorfuMsg msg, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    r.sendResponse(
        ctx, msg, new CorfuPayloadMsg<>(CorfuMsgType.SEQUENCER_METRICS_RESPONSE, sequencerMetrics));
  }
  ...
  /**
   * Return a timer based on the type of request. It will take the name from the cache initialized
   * at construction of the sequencer server to avoid String concatenation.
   *
  ...
  private Timer getTimer(byte reqType) {
    final String timerName =
        timerNameCache.getOrDefault(reqType, CorfuComponent.INFRA_SEQUENCER + "unknown");
    return ServerContext.getMetrics().timer(timerName);
  }

  /**
   * this method serves log-tokens for a raw log implementation. it simply extends the global log
   * tail and returns the global-log token
   *
   * @param msg corfu message containing raw token
   * @param ctx netty ChannelHandlerContext
   * @param r server router
   */
  private void handleRawToken(
      CorfuPayloadMsg<TokenRequest> msg, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    r.sendResponse(
        ctx,
        msg,
        CorfuMsgType.TOKEN_RES.payloadMsg(new TokenResponse(token, Collections.emptyMap())));
  }

  /**
   * this method serves token-requests for transaction-commit entries.
   *
   * <p>it checks if the transaction can commit. - if the transaction must abort, then a 'error
   * token' containing an Address.ABORTED address is returned. - if the transaction may commit, then
   * a normal allocation of log position(s) is pursued.
   *
   * @param msg corfu message containing transaction token
   * @param ctx netty ChannelHandlerContext
   * @param r server router
   */
  private void handleTxToken(
      CorfuPayloadMsg<TokenRequest> msg, ChannelHandlerContext ctx, IServerRouter r) {
    final TokenRequest req = msg.getPayload();
  ...
  /**
   * this method does the actual allocation of log addresses, it also maintains stream-tails,
   * returns a map of stream-tails for backpointers, and maintains a conflict-parameters map.
   *
  ...
  private void handleAllocation(
    ...
    for (UUID id : req.getStreams()) {
      ...
      streamTailToGlobalTailMap.compute(
          id,
          (k, v) -> {
            if (v == null) {
      ...
      // step 3. add allocated addresses to each stream's address map (to keep track of all updates
      // to this stream)
      streamsAddressMap.compute(
          id,
          (streamId, addressMap) -> {
            if (addressMap == null) {
    ...
    r.sendResponse(
        ctx,
        msg,
        CorfuMsgType.TOKEN_RES.payloadMsg(new TokenResponse(token, backPointerMap.build())));
  }

  /**
   * This method handles the request of streams addresses.
   *
   * <p>The request of address spaces can be of two types: - For specific streams (and specific
   * ranges for each stream). - For all streams (complete range).
   *
   * <p>The response contains the requested streams address maps and the global log tail.
   */
  @ServerHandler(type = CorfuMsgType.STREAMS_ADDRESS_REQUEST)
  private void handleStreamsAddressRequest(
      CorfuPayloadMsg<StreamsAddressRequest> msg, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    log.trace(
        "handleStreamsAddressRequest: return address space for streams [{}]",
        streamsAddressMap.keySet());
    r.sendResponse(
        ctx,
        msg,
        CorfuMsgType.STREAMS_ADDRESS_RESPONSE.payloadMsg(
   ...
   */
  private Map<UUID, StreamAddressSpace> getStreamsAddresses(
    ...
    for (StreamAddressRange streamAddressRange : addressRanges) {
      ...
      if (streamsAddressMap.containsKey(streamId)) {
        ...
        requestedAddressSpaces.put(
            streamId,
            new StreamAddressSpace(streamsAddressMap.get(streamId).getTrimMark(), addressMap));
      } else {
        log.warn(
            "handleStreamsAddressRequest: address space map is not present for stream {}. "
                + "Verify this is a valid stream.",
            streamId);
      }
    }
  ...
  public static class Config {
    ...
    @Default private final int cacheSize = DEFAULT_CACHE_SIZE;

    public static Config parse(Map<String, Object> opts) {
      int cacheSize =
          (int)
              (opts.containsKey("--sequencer-cache-size")
                  ? Integer.parseInt((String) opts.get("--sequencer-cache-size"))
                  : DEFAULT_CACHE_SIZE);
      return Config.builder().cacheSize(cacheSize).build();
    }
  }
}
