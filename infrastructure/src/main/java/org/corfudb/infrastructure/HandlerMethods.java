...
/**
 * This class implements message handlers for {@link AbstractServer} implementations. Servers define
 * methods with signatures which follow the signature of the {@link HandlerMethod} functional
 * interface, and generate a handler using the {@link this#generateHandler(MethodHandles.Lookup,
 * AbstractServer)} method.
 *
 * <p>For maximum performance, make the handlers static whenever possible. Handlers should be as
 * short as possible (not block), since handler threads come from a shared pool used by all servers.
 * Blocking operations should be offloaded to I/O threads.
 *
...
public class HandlerMethods {
  ...
  /**
   * A functional interface for server message handlers. Server message handlers should be fast and
   * not block. If a handler blocks for an extended period of time, it will exhaust the server's
   * thread pool. I/O and other long operations should be handled on another thread.
   */
  @FunctionalInterface
  interface HandlerMethod<T extends CorfuMsg> {
    void handle(@Nonnull T corfuMsg, @Nonnull ChannelHandlerContext ctx, @Nonnull IServerRouter r);
  }
  ...
  public void handle(CorfuMsg message, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    try {
      ...
      log.error("handle: Unhandled exception processing {} message", message.getMsgType(), ex);
      r.sendResponse(
          ctx, message, CorfuMsgType.ERROR_SERVER_EXCEPTION.payloadMsg(new ExceptionMsg(ex)));
    }
  }
   ...
   */
  public static HandlerMethods generateHandler(
      @Nonnull final MethodHandles.Lookup caller, @Nonnull final AbstractServer server) {
  ...
  /**
   * Takes a method annotated with the {@link org.corfudb.infrastructure.ServerHandler} annotation,
   * converts it into a lambda, and registers it in the {@code handlerMap}.
   *
   * @param caller The context that is being used. Call MethodHandles.lookup() to obtain.
   * @param server The object that implements the server.
   * @param method The method to be registered. Must be annotated with the {@link ServerHandler}
   *     annotation.
   */
  @SuppressWarnings("unchecked")
  private void registerMethod(
    ...
    if (!method.getParameterTypes()[0].isAssignableFrom(
        annotation.type().messageType.getRawType())) {
    ...
    if (handlerMap.containsKey(annotation.type())) {
      throw new UnrecoverableCorfuError(
          "HandlerMethod for " + annotation.type() + " already registered!");
    }
    ...
    try {
      ...
      if (Modifier.isStatic(method.getModifiers())) {
        ...
        h =
            (HandlerMethod<CorfuMsg>)
                LambdaMetafactory.metafactory(
                        caller,
                        "handle",
                        MethodType.methodType(HandlerMethod.class),
                        mt,
                        mh,
                        mh.type())
                    .getTarget()
        ...
        h =
            (HandlerMethod<CorfuMsg>)
                LambdaMetafactory.metafactory(
                        ...
                        mtGeneric.dropParameterTypes(0, 1),
                        mh,
                        mh.type().dropParameterTypes(0, 1))
                    .getTarget()
                    .bindTo(server)
                    .invoke();
      }
  ...
  /**
   * Generate a conditional handler, which instruments the handler with metrics if configured as
   * enabled and checks whether the server is shutdown and ready.
   *
   ...
   * @return A new {@link HandlerMethod} which conditionally executes the handler based on
   *     preconditions (whether the server is shutdown/ready).
   */
  private HandlerMethod<CorfuMsg> generateConditionalHandler(
      @Nonnull final CorfuMsgType type, @Nonnull final HandlerMethod<CorfuMsg> handler) {
  ...
  private Timer getTimer(@Nonnull CorfuMsgType type) {
    timerNameCache.computeIfAbsent(
        type, aType -> (CorfuComponent.INFRA_MSG_HANDLER + aType.name().toLowerCase()));
