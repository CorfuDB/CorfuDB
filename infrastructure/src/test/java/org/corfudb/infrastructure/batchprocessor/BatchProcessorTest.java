...
public class BatchProcessorTest {
    ...
    public void testTailsQueryStreamsTail() {
        List<UUID> streamIds = Collections.singletonList(UUID.randomUUID());
        RequestMsg request = getRequestMsg(
                getBasicHeader(false, false),
                getTailRequestMsg(streamIds)
        );

        when(mockStreamLog.getTails(streamIds)).thenReturn(new TailsResponse(0L));
        Object ret = batchProcessor.addTask(BatchWriterOperation.Type.TAILS_QUERY, request).join();
        verify(mockStreamLog).getTails(streamIds);
        assertThat(ret).isInstanceOf(TailsResponse.class);
    }
