...
public class LogUnitServerTest {
    ...
    /**
     * A helper method that creates a simple RANK_ONLY LogData object with default values.
     *
     * @param address LogData's global address
     * @param rank    LogData's rank
     * @return        the corresponding LogData
     */
    private LogData getRankOnlyLogData(long address, long rank) {
        ByteBuf b = Unpooled.buffer();
        Serializers.CORFU.serialize(PAYLOAD_DATA.getBytes(), b);
        LogData ld = new LogData(DataType.RANK_ONLY, b);
        ld.setGlobalAddress(address);
        ld.setEpoch(1L);
        ld.setRank(new IMetadata.DataRank(rank));
        return ld;
    }
    ...
    /**
     * Test that the LogUnitServer correctly handles a WRITE_LOG request
     * when there is already data, with a higher rank, at the specified
     * address. This is mocked by completing the future exceptionally.
     */
    @Test
    public void testHandleWriteLogOutranked() {
        RequestMsg request = getRequestMsg(
                getBasicHeader(false, false),
                getWriteLogRequestMsg(getRankOnlyLogData(1L, 1L))
        );

        // Prepare a future that can be used by the caller.
        CompletableFuture cf = new CompletableFuture<>();
        cf.completeExceptionally(new DataOutrankedException());
        when(mBatchProcessor.addTask(BatchWriterOperation.Type.WRITE, request)).thenReturn(cf);
        ...
        assertTrue(compareBaseHeaderFields(request.getHeader(), response.getHeader()));
        assertTrue(response.getPayload().hasServerError());
        assertTrue(response.getPayload().getServerError().hasDataOutrankedError());
    }
    ...
    /**
     * Test that the LogUnitServer correctly handles a RANGE_WRITE_LOG
     * request when there is already data, with a higher rank, at one
     * of the specified address. This is mocked by completing the future
     * exceptionally.
     */
    @Test
    public void testHandleRangeWriteLogOutranked() {
        RequestMsg request = getRequestMsg(
                getBasicHeader(false, false),
                getRangeWriteLogRequestMsg(Collections.singletonList(getRankOnlyLogData(1L, 1L)))
        );

        // Prepare a future that can be used by the caller.
        CompletableFuture cf = new CompletableFuture<>();
        cf.completeExceptionally(new DataOutrankedException());
        when(mBatchProcessor.addTask(BatchWriterOperation.Type.RANGE_WRITE, request)).thenReturn(cf);
