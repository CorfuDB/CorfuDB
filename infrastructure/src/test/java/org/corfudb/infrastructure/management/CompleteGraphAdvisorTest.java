...
public class CompleteGraphAdvisorTest {
  ...
  /**
   * By definition, a fully connected node can not be added to the unresponsive list. Failed
   * connection(s) between unresponsive and fully connected node(s) can't be used to determine if a
   * fully connected node is failed.
   */
  @Test
  ...
  /**
   * B node believes that everyone disconnected, but actually it's B disconnected. Make a decision
   * to exclude C.
   */
  @Test
  ...
  /**
   * Asymmetric partition where: - B and C nodes disconnected from each other - decision maker is
   * node A - node A makes node C unresponsive
   */
  @Test
  public void
      testFailureDetectionForThreeNodes_asymmetricPartition_b_c_disconnectedFromEachOther() {
    ...
    ClusterState nodeAClusterState =
        buildClusterState(
            "a",
            ImmutableList.of(),
            nodeState("a", epoch, OK, OK, OK),
    ...
    ClusterState nodeBClusterState =
        buildClusterState(
            "b",
            ImmutableList.of(),
            nodeState("a", epoch, OK, OK, OK),
            nodeState("b", epoch, OK, OK, FAILED),
            NodeState.getUnavailableNodeState("c"));
    ClusterState nodeCClusterState =
        buildClusterState(
            "c",
            ImmutableList.of(),
            nodeState("a", epoch, OK, OK, OK),
