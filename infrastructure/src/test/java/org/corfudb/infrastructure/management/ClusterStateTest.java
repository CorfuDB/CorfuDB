...
public class ClusterStateTest {
  ...
  public void isReady() {
    ...
    ClusterState invalidClusterState =
    ...
    ClusterState validClusterState =
        ClusterState.buildClusterState(
            localEndpoint,
            ImmutableList.of(),
            nodeState("a", epoch1, OK),
  ...
  /**
   * Ensure that {@link ClusterState#getPingResponsiveNodes()} returns all nodes that have responded
   * to our ping, despite the face that all nodes are marked as unresponsive by the current layout
   * and one of the nodes responded with WrongEpochException.
   */
  @Test
