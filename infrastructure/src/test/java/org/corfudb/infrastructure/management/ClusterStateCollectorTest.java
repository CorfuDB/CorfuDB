...
public class ClusterStateCollectorTest {
  ...
  /**
   * Checks the aggregated cluster state. Cluster state is: - node A is a local node - node A
   * connected to node B, - node C - replied with wrong epoch exception The test contains following
   * scenarios: - node A: check that node A is connected to all other nodes - node B: connected to A
  ...
  public void testAggregatedState() {
    ...
    ClusterState predefinedCluster =
    ...
    CompletableFuture<NodeState> nodeAStateCf =
        CompletableFuture.completedFuture(predefinedCluster.getNode("a").get());

    CompletableFuture<NodeState> nodeBStateCf =
        CompletableFuture.completedFuture(predefinedCluster.getNode("b").get());
    ...
    ClusterStateCollector collector =
    ...
    ClusterState clusterState =
        collector.collectClusterState(1, ImmutableList.of(), SequencerMetrics.UNKNOWN);
  ...
  public void testWrongEpochs() {
    ...
    CompletableFuture<NodeState> nodeAState =
        CompletableFuture.completedFuture(NodeState.builder().build());
