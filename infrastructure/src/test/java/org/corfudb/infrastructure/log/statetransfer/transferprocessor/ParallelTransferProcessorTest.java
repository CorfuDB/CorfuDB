...
class ParallelTransferProcessorTest {
  ...
  private final Stream<TransferBatchRequest> createStream(
      int numBatches, Optional<ImmutableList<String>> nodes) {
    return IntStream.range(0, numBatches)
        .boxed()
        .map(x -> TransferBatchRequest.builder().destinationNodes(nodes).build());
  }
  ...
  void testRunEmptyStream() {
    ...
    TransferProcessorResult res = transferProcessor.runStateTransfer(emptyBatchStream, 1).join();
  ...
  void testRunSingleTransferError() {
    ...
    FaultyBatchProcessor faultyBatchProcessor = new FaultyBatchProcessor(1);
    ParallelTransferProcessor transferProcessor =
  ...
  void testRunSingleTransferSuccess() {
    SuccessfulBatchProcessor batchProcessor = new SuccessfulBatchProcessor();
    Stream<TransferBatchRequest> stream = createStream(1, Optional.empty());
  ...
  void testRunParallelTransferSuccess() {
    ...
    SuccessfulBatchProcessor batchProcessor = new SuccessfulBatchProcessor(Optional.of(300L));
  ...
  void testRunParallelTransferFailure() {
    ...
    FaultyBatchProcessor batchProcessor = new FaultyBatchProcessor(9, Optional.of(300L));
            ...
            .map(
                x ->
                    TransferBatchRequest.builder()
                        .destinationNodes(Optional.of(ImmutableList.of("node1", "node2", "node3")))
                        .build());
    ParallelTransferProcessor transferProcessor = new ParallelTransferProcessor(batchProcessor);
    CompletableFuture<TransferProcessorResult> result =
  ...
  void testRunParallelTransferShortCircuitsOnFailure() {
    ...
    FaultyBatchProcessor batchProcessor = new FaultyBatchProcessor(13, Optional.of(2000L));
    Stream<TransferBatchRequest> stream =
