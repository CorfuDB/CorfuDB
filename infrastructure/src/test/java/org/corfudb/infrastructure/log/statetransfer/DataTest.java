...
public class DataTest {
  ...
  public Map<Long, ILogData> createStubMap(List<LogData> data) {
    return data.stream()
        .map(rec -> new AbstractMap.SimpleEntry<>(rec.getGlobalAddress(), (ILogData) rec))
        .collect(
  ...
  public Map<Long, ILogData> createStubMapFromLongs(List<Long> data) {
    return data.stream()
        .map(add -> new AbstractMap.SimpleEntry<>(add, createStubRecord(add)))
        .collect(
            Collectors.toMap(AbstractMap.SimpleEntry::getKey, AbstractMap.SimpleEntry::getValue));
  }
  ...
  public List<LogData> getRecordsFromStubMap(Map<Long, ILogData> stubMap) {
    return Ordering.natural()
        .sortedCopy(stubMap.values().stream().map(x -> (LogData) x).collect(Collectors.toList()));
  }
}
