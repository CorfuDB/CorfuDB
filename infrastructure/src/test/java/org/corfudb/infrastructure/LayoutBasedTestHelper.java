...
public class LayoutBasedTestHelper {
  ...
  public Layout createNonPresentLayout() {
    ...
    Layout.LayoutSegment segment1 =
        new Layout.LayoutSegment(CHAIN_REPLICATION, 0L, 2L, Collections.singletonList(stripe1));

    Layout.LayoutSegment segment2 =
        new Layout.LayoutSegment(CHAIN_REPLICATION, 2L, 4L, Collections.singletonList(stripe2));

    Layout.LayoutSegment segment3 =
        new Layout.LayoutSegment(CHAIN_REPLICATION, 4L, -1L, Collections.singletonList(stripe3));
  ...
  public Layout createPresentLayout() {
    ...
    return createTestLayout(Arrays.asList(layout.getSegment(0L), segment2, layout.getSegment(4L)));
  }

  public MockedSegment transformToMock(TransferSegment segment) {
    return new MockedSegment(
        segment.getStartAddress(), segment.getEndAddress(), segment.getStatus());
  }

  public ImmutableList<MockedSegment> transformListToMock(List<TransferSegment> segments) {
    return ImmutableList.copyOf(
        segments.stream().map(this::transformToMock).collect(Collectors.toList()));
  }
}
