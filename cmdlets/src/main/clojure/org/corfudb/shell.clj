      ...
      (.. opts (get "--enable-tls"))
        (def *o (new NettyClientRouter
          ...
          (-> (CorfuRuntime$CorfuRuntimeParameters/builder)
              ...
              (.tsPasswordFile (.. opts (get "--truststore-password-file")))
              (.saslPlainTextEnabled (.. opts (get "--enable-sasl-plain-text-auth")))
              (.usernameFile (.. opts (get "--sasl-plain-text-username-file")))
              (.passwordFile (.. opts (get "--sasl-plain-text-password-file")))
              (.build))))
