...
public class LogEntrySender {
    ...
    public static final int DEFAULT_READER_QUEUE_SIZE = 20;
     ...
     */
    final static private long TIME_INCREMEMNT = 50;
        ...
        for (int i = 0; i < pendingEntries.list.size() && taskActive; i++) {
            ...
            if (entry.timeout(getCurrentTime(), msgTimer)) {
                if (errorOnMsgTimeout && entry.retry >= maxRetry) {
                    log.warn("Entry {} data {} has been resent max times {}.", entry, entry.data, maxRetry);
                    //todo use the new error code
    ...
    public void send(UUID logEntrySyncEventId) {
        ...
        // If there are pending entries, resend them. If resend has an error,
        // stop sending messages.
        if (!pendingEntries.list.isEmpty() && !resend()) {
            return;
        }
    ...
    public static class LogReplicationPendingEntry {
        ...
        boolean timeout(long time, long timer) {
            return  (time - this.time) > timer;
        }
